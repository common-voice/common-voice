...
passport.deserializeUser((sessionUser: any, done: Function) => done(null, sessionUser));
        ...
        ((
          {
            ...
            dev: 'https://falak.ksaa.gov.sa/voicewall',
...
router.get(
  ...
  async (request: Request, response: Response) => {
    ...
    if (!user) {
      ...
      const success = await UserClient.updateSSO(old_email, user.emails[0].value);
      if (!success) {
      ...
      response.redirect(
        redirect || `${basePath}login-success?challenge=${enrollment.challenge}&achievement=1`
      );
    } else {
...
export async function authMiddleware(request: Request, response: Response, next: NextFunction) {
  if (request.user) {
    const accountClientId = await UserClient.findClientId(request.user.emails[0].value);
    if (accountClientId) {
