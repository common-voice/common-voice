...
export default class Clip {
  ...
  saveClipVote = async (
    ...
    await this.model.database.saveVote(id, client_id, isValid)
    ...
    const ret = challengeTokens.includes(challenge)
      ? {
          ...
          challengeEnded: await this.model.database.hasChallengeEnded(
            challenge
          ),
        }
      : { glob }
  ...
  saveClip = async (request: Request, response: Response) => {
    ...
    const sentence = await this.model.database.findSentence(sentenceId)
    if (!sentence) {
      this.clipSaveError(
        headers,
        response,
    ...
    if (await this.model.database.clipExists(client_id, sentenceId)) {
      this.clipSaveError(
        headers,
        response,
        204,
  ...
  serveDailyCount = async (request: Request, response: Response) => {
    response.json(
      await this.model.database.getDailyClipsCount(request.params.locale)
    )
  }

  serveDailyVotesCount = async (request: Request, response: Response) => {
    response.json(
      await this.model.database.getDailyVotesCount(request.params.locale)
    )
  }
