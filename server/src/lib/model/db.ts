...
import { ChallengeToken, Sentence, TaxonomyToken, taxonomies, Language, Datasets } from 'common';
...
// dashbord functions
export async function getAllSentencesWithReactions(): Promise<any[]> {
  const query = `
    SELECT 
      s.id AS sentence_id,
      s.text AS sentence_text,
      COALESCE(SUM(CASE WHEN ur.action = 'like' THEN 1 ELSE 0 END), 0) AS likes,
      COALESCE(SUM(CASE WHEN ur.action = 'dislike' THEN 1 ELSE 0 END), 0) AS dislikes
    FROM sentences s
    LEFT JOIN user_reaction ur ON ur.sentencesid = s.id
    GROUP BY s.id
...
export async function getSentenceWithReactionsById(sentenceId: string): Promise<any> {
  const query = `
    SELECT 
      s.id AS sentence_id,
      s.text AS sentence_text,
  ...
  const [rows] = await this.mysql.query(query, [sentenceId]);

  // If no sentence was found, return null or handle it as needed
  if (rows.length === 0) {
    return null;
  }
...
export async function deleteSentenceById(sentenceId: string): Promise<void> {
  // Begin transaction for safety
  const connection = await this.mysql.getConnection();
  await connection.beginTransaction();

  try {
    // Delete user reactions associated with the sentence
    await connection.query('DELETE FROM user_reaction WHERE sentencesid = ?', [sentenceId]);

    // Delete the sentence itself
    await connection.query('DELETE FROM sentences WHERE id = ?', [sentenceId]);
...
export async function searchSentencesByTitle(title: string): Promise<any[]> {
  const query = `
    SELECT 
      s.id AS sentence_id,
      s.text AS sentence_text,
  ...
  const [rows] = await this.mysql.query(query, [`%${title}%`]);

  return rows; // Return the array of sentences that match the title
}
export async function duplicateSentenceWithCopyTitle(sentenceId: string): Promise<any> {
  // Begin transaction for safety
  const connection = await this.mysql.getConnection();
  await connection.beginTransaction();

  try {
    // Fetch the original sentence by ID
    const [rows] = await connection.query('SELECT * FROM sentences WHERE id = ?', [sentenceId]);

    if (rows.length === 0) {
      throw new Error(`Sentence with ID ${sentenceId} not found`);
    }

    const originalSentence = rows[0];

    // Prepare the new sentence data
    const newSentence = {
      ...originalSentence,
      title: `${originalSentence.title} copy`, // Append "copy" to the original title
      id: generateNewSentenceId(), // Generate a new unique ID for the new sentence
      created_at: new Date(), // Set the created_at field to the current time
    };

    // Insert the duplicated sentence into the sentences table
    await connection.query('INSERT INTO sentences SET ?', [newSentence]);
...
// Helper function to generate a new unique sentence ID (you can adjust this if needed)
function generateNewSentenceId(): string {
  return 'new_' + Math.random().toString(36).substr(2, 9); // A simple example of generating a new unique ID
}
export async function updateSentenceById(
  sentenceId: string,
  updateData: Partial<SentenceDto>
): Promise<any> {
  // Begin transaction for safety
  const connection = await this.mysql.getConnection();
  await connection.beginTransaction();
  ...
  try {
    // Prepare the update query dynamically based on the provided updateData
    ...
    const [result] = await connection.query(
      `UPDATE sentences SET ${updateColumns} WHERE id = ?`,
      [...updateValues, sentenceId] // Add the sentence ID at the end of the parameters
    );

    // Commit the transaction
    await connection.commit();

    // Check if the sentence was updated
    if (result.affectedRows === 0) {
      throw new Error(`Sentence with ID ${sentenceId} not found`);
    }

    // Return the updated sentence
    return { id: sentenceId, updatedFields: updateData };
  } catch (error) {
    // Rollback the transaction if an error occurs
    await connection.rollback();
    throw error;
...
// Define the type for Sentence to be updated (adjust as necessary)
interface SentenceDto {
  ...
  // Add other fields from the `sentences` table as needed
}
...
export default class DB {
   ...
   * @returns
   */
  async getClipsToBeValidated(languageId: number, limit: number): Promise<DBClip[]> {
    const [rows] = await this.mysql.query(
      `
        SELECT 
          c.id as id, 
  ...
  async findSentencesMatchingTaxonomy(
    ...
    const [rows] = await this.mysql.query(
      `
        ...
        LIMIT ?
      `,
      [await getTermIds(segments), locale_id, client_id, client_id, client_id, SHUFFLE_SIZE, count]
    );
  ...
  async findSentencesWithFewClipsCorpusId(
    ...
    const [rows] = await this.mysql.query(
      `
        ...
        LIMIT ?
      `,
      [corpus_id, locale_id, client_id, client_id, client_id, SHUFFLE_SIZE, count]
    );
    return (rows || []).map(({ id, text }: any) => ({ id, text }));
  }
  ...
  async findClipsNeedingValidation(
    ...
    Sentry.captureMessage(
      `There are ${regularSentences.length} regular sentences for ${locale} locale`,
      Sentry.Severity.Info
    );
    return taxonomySentences.concat(regularSentences);
  }

  async findClipsWithFewVotesWithoutTaxonomy(
    client_id: string,
    locale_id: number,
    count: number,
    exemptFromSSRL?: boolean
...
ORDER BY RAND()
LIMIT ?`,
      [locale_id, client_id, client_id, client_id, client_id, SHUFFLE_SIZE, count]
    );
  ...
  async findClipsWithFewVotesWithoutTaxonomyCorpusId(
    ...
    const cachedClips: DBClip[] = await lazyCache(
      ...
      async () => {
        return await this.getClipsToBeValidatedCorpusId(locale_id, corpus_id, 10000);
      },
      MINUTE
    )();
...
ORDER BY RAND()
LIMIT ?`,
      [locale_id, corpus_id, client_id, client_id, client_id, client_id, SHUFFLE_SIZE, count]
    );
    console.log('without taxonomy', clips);
    return clips as DBClip[];
  }
  ...
  async findSentence(id: string) {
    return (
      await this.mysql.query('SELECT locale_id, text FROM sentences WHERE id = ? LIMIT 1', [id])
    )[0][0];
  }
  ...
  async getUserClient(client_id: string) {
    const [[row]] = await this.mysql.query('SELECT * FROM user_clients WHERE client_id = ?', [
      client_id,
    ]);
    return row;
  }
  ...
  async getVariants(client_id: string, locale?: string, corpus_id?: string) {
    ...
    const [variants] = await this.mysql.query(
      ...
      ${actual_corpus_id ? (locale ? 'AND v.corpus_id = ?' : 'WHERE v.corpus_id = ?') : ''}
      `,
      [
  ...
  async insertDownloader(locale: string, email: string, dataset: string) {
    try {
      ...
      console.error(`Unable to insert downloader (error message: ${error.message})`);
    }
  }
  ...
  async markInvalid(id: string) {
    await this.mysql.query(`UPDATE clips SET is_valid = 0 WHERE id = ? LIMIT 1;`, [id]);
  }
  ...
  async findAllsentences() {
    return await this.mysql.query(
      `
        SELECT * FROM voiceweb.sentences
                LIMIT 10
