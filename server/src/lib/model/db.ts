...
export async function getSentenceWithReactionsById(
  sentenceId: string
): Promise<any> {
  const query = `
    SELECT 
...
export async function deleteSentenceById(sentenceId: string): Promise<void> {
  ...
  try {
    ...
    await connection.query('DELETE FROM user_reaction WHERE sentencesid = ?', [
      sentenceId,
    ])
...
export async function duplicateSentenceWithCopyTitle(
  sentenceId: string
): Promise<any> {
  // Begin transaction for safety
  const connection = await this.mysql.getConnection()
