...
export default class DB {
  ...
  // [TODO] similar functions could be refactored into one function
  async firstContributionBonus(challenge: string, client_id: string) {
    const achievement = { achievement: false };

    if (!challenge) return achievement;
  ...
  async firstInvitationBonus(client_id: string, challenge: string) {
    let earnedAchievement = false;
    const [[res]] = await this.mysql.query(
      `
      SELECT earn.id IS NOT NULL AS win_bonus,
          earn.client_id AS bonus_winner
      FROM challenges
      LEFT JOIN achievements ON achievements.name = ? AND achievements.challenge_id = challenges.id
      ...
      `,
      ['first_invitation', client_id, challenge]
    );
    const { win_bonus, bonus_winner } = res || {
      win_bonus: false,
      bonus_winner: '',
  ...
  async earnInviteContributeSameSessionBonus(client_id: string) {
    await this.mysql.query(
      `
        INSERT INTO earn (achievement_id, client_id) VALUES ((SELECT id FROM achievements WHERE name = ?), ?)
      `,
      ['invite_contribute_same_session', client_id]
    );
  }

  async earnedInviteContributeSameSessionBonus(
    client_id: string,
    challenge: string
  ) {
    const [[{ earned }]] = await this.mysql.query(
      `
      SELECT earn.id IS NOT NULL AS win_bonus,
          earn.client_id AS bonus_winner
      FROM challenges
