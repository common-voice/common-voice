...
const reduceLanguages = (user: any) => {
  ...
  return {
    ...user,
    languages: Object.keys(user.languages).map((key: string) => user.languages[key]),
  } as UserClientType;
};
 ...
 */
const getAccentsChanges = async (clientId: string, languages: UserLanguage[]) => {
  /*
...
async function updateLanguages(clientId: string, languages: UserLanguage[]) {
  const { accentsToUpdate, accentsToDelete } = await getAccentsChanges(clientId, languages);
...
async function updateVariants(clientId: string, languages: UserLanguage[]) {
  ...
  if (idsToBeRemoved.length > 0) {
    await db.query('DELETE FROM user_client_variants WHERE variant_id IN (?) and client_id = ?', [
      idsToBeRemoved,
      clientId,
    ]);
  }
  ...
  if (idsToBeAdded.length > 0) {
    ...
    const [validIds]: [{ variant_id: number; locale_name: string; locale_id: number }[]] =
      (await db.query(
...
const UserClient = {
      ...
      `
        ...
        CROSS JOIN
          (SELECT demographics.age_id, demographics.gender_id
            ...
            WHERE user_clients.${client_id ? `client_id = "${client_id}"` : `email = "${email}"`}
  ...
  async saveAnonymousAccountLanguages(client_id: string, languages: UserLanguage[]) {
    if (languages) {
      await Promise.all([
  ...
  async saveAccount(
    ...
    const updateDemographicsPromise = updateDemographics(clientId, data.age, data.gender);
    await Promise.all([
  ...
  async updateSSO(old_email: string, email: string): Promise<boolean> {
    const [[row]] = await db.query('SELECT 1 FROM user_clients WHERE email = ? AND has_login', [
      email,
    ]);
    ...
    await db.query('UPDATE user_clients SET email = ? WHERE email = ? AND has_login', [
      email,
      old_email,
    ]);
    ...
    await db.query(`DELETE FROM user_client_newsletter_prefs WHERE email = ?`, [old_email]);
  ...
  async updateAvatarURL(email: string, url: string) {
    const [[origAvatar]] = await db.query('SELECT avatar_url FROM user_clients WHERE email = ?', [
      email,
    ]);
  ...
  async hasSSO(client_id: string): Promise<boolean> {
    return Boolean(
      (
        await db.query('SELECT 1 FROM user_clients WHERE client_id = ? AND has_login', [client_id])
      )[0][0]
    );
  },
  ...
  async updateUserGender(clientId: string, gender: string) {
    await db.query(`UPDATE user_clients SET gender = ? WHERE client_id = ?`, [gender, clientId]);
  },
  async updateUserCountry(clientId: string, country: string) {
    await db.query(`UPDATE user_clients SET country = ? WHERE client_id = ?`, [country, clientId]);
  },
  async updateUserAge(clientId: string, age: string) {
    await db.query(`UPDATE user_clients SET age = ? WHERE client_id = ?`, [age, clientId]);
  },
};
