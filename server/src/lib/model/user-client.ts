...
const UserClient = {
  ...
  async updateUserGender(clientId: string, gender: string) {
    await db.query(`UPDATE user_clients SET gender = ? WHERE client_id = ?`, [
      gender,
      clientId,
    ])
  },
  async updateUserCountry(clientId: string, country: string) {
    await db.query(`UPDATE user_clients SET country = ? WHERE client_id = ?`, [
      country,
      clientId,
    ])
  },
  async updateUserAge(clientId: string, age: string) {
    await db.query(`UPDATE user_clients SET age = ? WHERE client_id = ?`, [
      age,
      clientId,
    ])
  },
