...
export default class Model {
  ...
  async findEligibleClips(
    ...
    return this.db.findClipsNeedingValidation(client_id, locale, Math.min(count, 50), corpus_id);
  }
  ...
  async findEligibleSentences(
    ...
    return this.db.findSentencesNeedingClips(client_id, locale, Math.min(count, 50), corpus_id);
  }
  ...
  getLanguageStats = lazyCache(
    ...
    async (): Promise<any> => {
      ...
      const [localizedPercentages, validClipsCounts, speakerCounts, allClipsCount] =
        await Promise.all([
          this.getLocalizedPercentages(), //translation %, no en
          this.db.getValidClipCount(allLanguageIds, '').then(data => statsReducer(data)),
          this.db.getTotalUniqueSpeakerCount(allLanguageIds, '').then(data => statsReducer(data)),
          this.db.getAllClipCount(allLanguageIds, '').then(data => statsReducer(data)),
        ]);

      // map over every lang in db
      const languageStats = languages.map(lang => {
        const totalSecDur = getAverageSecondsPerClip(lang.name) * (allClipsCount[lang.id] || 0);
        const validSecDur = getAverageSecondsPerClip(lang.name) * (validClipsCounts[lang.id] || 0);
