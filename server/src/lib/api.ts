...
export default class API {
  ...
  constructor(model: Model) {
    ...
    this.takeout = new Takeout(this.model.database.mysql, this.s3, this.bucket)
  }
  ...
  getRequestedLanguages = async (request: Request, response: Response) => {
    response.json(await this.model.database.getRequestedLanguages())
  }

  createLanguageRequest = async (request: Request, response: Response) => {
    await this.model.database.createLanguageRequest(
  ...
  createSkippedSentence = async (request: Request, response: Response) => {
    ...
    await this.model.database.createSkippedSentence(id, client_id)
    response.json({})
  }
  ...
  createSkippedClip = async (request: Request, response: Response) => {
    ...
    await this.model.database.createSkippedClip(id, client_id)
    response.json({})
  }
  ...
  getContributionActivity = async (
    ...
    response.json(
      await (query.from == 'you'
        ? this.model.database.getContributionStats(locale, client_id)
  ...
  insertDownloader = async ({ body }: Request, response: Response) => {
    await this.model.database.insertDownloader(
      body.locale,
      body.email,
      body.dataset
    )
  ...
  getAccents = async ({ client_id, params }: Request, response: Response) => {
    response.json(
      await this.model.database.getAccents(client_id, params?.locale || null)
    )
  }

  getVariants = async ({ client_id, params }: Request, response: Response) => {
    response.json(
      await this.model.database.getVariants(client_id, params?.locale || null)
    )
  }
