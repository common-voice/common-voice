...
export default class API {
  ...
  constructor(model: Model) {
    ...
    this.dashboard = new Dashboard(this.model);
  }
  ...
  getRouter(): Router {
    ...
    router.get('/datasets', validate({ query: datasetSchema }), this.getAllDatasets);
    ...
    router.use('/dashboard', this.dashboard.getRouter());
  ...
  // test = async (_request: Request, response: Response) => {
  //   response.json('{"hello"}')
  // }
  getRandomSentences = async (request: Request, response: Response) => {
    ...
    const sentences = await this.model.findEligibleSentences(client_id, locale, count, corpus_id);
  ...
  getAllLanguagesWithDatasets = async (_request: Request, response: Response) => {
  ...
  saveAnonymousAccountLanguages = async (request: Request, response: Response) => {
    const {
      client_id,
      body: { languages, age, gender, country },
    } = request;
    ...
    const result = await UserClient.saveAnonymousAccountLanguages(client_id, languages);
  ...
  subscribeToNewsletter = async (request: Request, response: Response, next: NextFunction) => {
  ...
  saveAvatar = async (
    ...
    if (imageUploadType === 'file') {
      ...
      const prefix = (new Date().getUTCMilliseconds() * Math.random()).toString(36).slice(-5);
      ...
      try {
        ...
        console.error(error);
        next(
          new APIError(error?.message || 'Image could not be uploaded.', StatusCodes.BAD_REQUEST)
        );
      }
      ...
      try {
        avatarURL = 'https://gravatar.com/avatar/' + MD5(user.emails[0].value).toString() + '.png';
    ...
    const oldAvatar = await UserClient.updateAvatarURL(user.emails[0].value, avatarURL);
  ...
  getTakeoutLinks = async (request: Request, response: Response) => {
    ...
    const links = await this.takeout.generateDownloadLinks(client_id, parseInt(id));
    response.json(links);
  };
  ...
  getGoals = async ({ client_id, params: { locale } }: Request, response: Response) => {
  ...
  seenAwards = async ({ client_id, query }: Request, response: Response) => {
    await Awards.seen(
      client_id,
      Object.prototype.hasOwnProperty.call(query, 'notification') ? 'notification' : 'award'
    );
    response.json({});
  };

  getPublicUrl = async ({ params: { bucket_type, path } }: Request, response: Response) => {
  ...
  getAccents = async ({ client_id, params }: Request, response: Response) => {
    response.json(
      await this.model.db.getAccents(client_id, params?.locale || null, params?.corpus_id || null)
    );
  };

  getVariants = async ({ client_id, params }: Request, response: Response) => {
    response.json(
      await this.model.db.getVariants(client_id, params?.locale || null, params?.corpus_id || null)
    );
  };

  sendLanguageRequest = async (request: Request, response: Response, next: NextFunction) => {
