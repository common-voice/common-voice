...
import { isValidatorLocale, ValidatorLocale, ValidatorRule, ValidatorRuleError } from '../types';
...
const runValidatorOnSentence =
  ...
  (sentence: string): E.Either<ValidatorRuleError, string> => {
    for (const rule of rules) {
      switch (rule.type) {
        case 'fn':
          if (rule.fn(sentence)) return E.left({ error: rule.error, errorType: rule.errorType });
          else continue;
        case 'regex':
...
export const validateSentence = (locale: string) => (sentence: string) => {
  return pipe(sentence, normalizeForLocale(locale), validateSentenceForLocale(locale));
};
