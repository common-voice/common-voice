...
describe('Sentence validation', () => {
  ...
  const shouldRejectSentence = ({
    ...
    lang: string;
    sentence: string;
    expectedErrorType: ValidatorRuleErrorType;
  }) => {
    const result = validateSentence(lang)(sentence);
  ...
  it.each(wrongSymbolDataProvider)(
    ...
    ({ sentence, error }) => {
      ...
      expect(E.getOrElse((err: ValidatorRuleError) => err.error)(result)).toBe(error);
    }
  );
  ...
  it('Should normalize sentence for the "ko" locale', () => {
    ...
    expect(E.getOrElse((err: ValidatorRuleError) => err.error)(result) === '콜').toBeFalsy();
  });
  ...
  it.each([
    ...
    'גם נִיקוד הולך',
  ])('Should validate Hebrew sentences with no errors', sentence =>
    shouldAcceptSentence({ lang: 'he', sentence })
  );

  it('Should invalidate overly long Hebrew sentence', () =>
  ...
  it('Should invalidate Hebrew sentence with cantillation marks', () =>
    shouldRejectSentence({
      lang: 'he',
      sentence: 'בְּרֵאשִׁ֖ית בָּרָ֣א אֱלֹהִ֑ים אֵ֥ת הַשָּׁמַ֖יִם וְאֵ֥ת הָאָֽרֶץ',
      expectedErrorType: ERR_NO_SYMBOLS,
    }));

  it('Should invalidate Hebrew sentence with invalid symbols that are also invalid for English', () =>
    shouldRejectSentence({
      lang: 'he',
      sentence: 'הסימון כרוכית @ משמש לעיתים בדוא"ל',
      expectedErrorType: ERR_NO_SYMBOLS,
    }));

  it('Should invalidate Hebrew sentence with invalid symbols that are not invalid for English', () =>
    shouldRejectSentence({
      lang: 'he',
      sentence: 'הסימן שווה = משמש גם כיחס שקילות',
      expectedErrorType: ERR_NO_SYMBOLS,
    }));

  it('Should invalidate Hebrew sentence with non-Hebrew letters', () =>
    shouldRejectSentence({
      lang: 'he',
      sentence: 'לפעמים בעברית אומרים okay למרות שזו מילה באנגלית',
      expectedErrorType: ERR_NO_FOREIGN_SCRIPT,
    }));
  ...
  ])('Should invalidate Hebrew sentence with abbreviations', sentence =>
    shouldRejectSentence({
      lang: 'he',
      sentence: sentence,
      expectedErrorType: ERR_NO_ABBREVIATIONS,
    })
  );
});
