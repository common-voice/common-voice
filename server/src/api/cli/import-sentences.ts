...
const importSentences = async (args: any, options: any) => {
  ...
  const languagesResponse = await fetch(`http://localhost:${config.SERVER_PORT}/api/v1/languages`);
...
program
  ...
  .requiredOption('-f, --filepath <filepath>', 'path to the tsv file containing the sentences')
