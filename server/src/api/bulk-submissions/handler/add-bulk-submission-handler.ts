...
export const handler =
  (cmdHandler: (cmd: AddBulkSubmissionCommand) => TE.TaskEither<ApplicationError, boolean>) =>
  async (req: Request, res: Response) => {
    ...
    if (!client_id) return res.status(StatusCodes.BAD_REQUEST).json({ message: 'no client id' });
