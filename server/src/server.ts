...
export default class Server {
  ...
  public setupAppRouting = async () => {
    ...
    if (PROD) {
      this.app.use(this.ensureSSL)
    }

    if (getConfig().MAINTENANCE_MODE + '' === 'true') {
      this.print('Application starting in maintenance mode')

      this.app.use(express.static(MAINTENANCE_PATH, staticOptions))
      ...
      this.app.use(express.static(FULL_CLIENT_PATH, staticOptions))

      this.setupCrossLocaleRoute()

      this.setupPrivacyAndTermsRoutes()

      this.app.use(await setupAuthRouter())
