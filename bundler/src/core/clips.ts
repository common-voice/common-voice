...
import { either as E, readerTaskEither as RTE, task as T, taskEither as TE } from 'fp-ts';
...
const getTmpClipsPath = (locale: string) => path.join(getTmpDir(), `${locale}_clips.tsv`);
...
const writeFileStreamToTsv = (locale: string, releaseDirPath: string) => {
  const writeStream = fs.createWriteStream(path.join(releaseDirPath, locale, 'clips.tsv'), {
    encoding: 'utf-8',
  });
...
const checkClipForExistence = (releaseDirPath: string) => {
  return new Transform({
    transform(chunk: ClipRow, encoding, callback) {
      const clipFilename = createClipFilename(chunk.locale, chunk.id);
      if (fs.existsSync(path.join(releaseDirPath, chunk.locale, 'clips', clipFilename))) {
        callback();
      } else {
        pipe(
...
const filterMissingClips = (releaseDirPath: string) =>
  new Transform({
    transform(chunk: ClipRow, encoding, callback) {
      const filename = createClipFilename(chunk.locale, chunk.id);
      const currentReleaseClipPath = path.join(releaseDirPath, chunk.locale, 'clips', filename);
...
const streamQueryResultToFile = (
  ...
  TE.tryCatch(async () => {
    ...
    await pipeline(stream, stringify({ header: true, delimiter: '\t' }), writeStream);

    console.log(`Query result for ${locale} finished streaming. Closing DB connection.`);
    const endConnection = () =>
...
const copyExistingClipsFromPrevRelease = (
  ...
  TE.tryCatch(async () => {
    ...
    for (const clip of clips) {
      ...
      const currentReleaseClipPath = path.join(releaseDirPath, clip.locale, 'clips', filename);
...
const extractClipsFromPreviousRelease = (locale: string, prevReleaseName: string) => {
...
export const fetchAllClipsPipeline = (
  ...
  return pipe(
    ...
    TE.chainFirst(({ clipsTmpPath }) =>
      streamQueryResultToFile(clipsTmpPath, includeClipsFrom, includeClipsUntil, locale)
    ),
    TE.chainFirst(() => TE.fromIO(prepareDir(clipsDirPath))),
    TE.chainFirst(({ clipsTmpPath }) =>
      previousReleaseName
        ? copyExistingClipsFromPrevRelease(clipsTmpPath, releaseDirPath, previousReleaseName)
        : TE.right(constVoid())
    ),
    TE.chainFirst(({ clipsTmpPath }) =>
...
export const runFetchAllClipsForLocale = (
  ...
  return pipe(
    ...
    RTE.chainTaskEitherK(
      ({ type, locale, from, until, clipsDirPath, releaseDirPath, previousReleaseName }) =>
