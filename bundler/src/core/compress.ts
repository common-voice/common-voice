...
const pathsFilter =
  ...
  (filepath: string): boolean => {
    ...
    const excludeList =
      releaseType === 'full' ? clipsTsv : [...clipsTsv, ...CORPORA_CREATOR_SPLIT_FILES];
...
const compressPipeline = (
  ...
  return pipe(
    ...
    TE.let('paths', getPathsToAddToTarball(locale, releaseName, releaseDirPath, releaseType)),
    TE.chainFirst(() => TE.fromIO(prepareDir(releaseTarballDir))),
    TE.chainFirst(({ tarballFilepath, paths }) => compress(paths)(tarballFilepath)),
...
export const runCompress = (): RTE.ReaderTaskEither<AppEnv, Error, string> =>
  pipe(
    RTE.ask<AppEnv>(),
    RTE.chainTaskEitherK(({ locale, releaseName, releaseDirPath, releaseTarballsDirPath, type }) =>
      compressPipeline(locale, releaseName, releaseDirPath, releaseTarballsDirPath, type)
    )
  );
