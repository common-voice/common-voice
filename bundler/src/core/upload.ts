...
const uploadDataset = (
  ...
  pipe(
    ...
    TE.chainFirst(({ readStream, uploadPath }) => uploadDatasetToPath(uploadPath)(readStream)),
...
export const runUpload = (
  ...
  pipe(
    RTE.ask<AppEnv>(),
    RTE.chainTaskEitherK(({ releaseName }) => uploadDataset(datasetTarballFilepath, releaseName))
  );
