...
import { AppEnv, REPORTED_SENTENCES_COLUMNS, ReportedSentencesRow } from '../types';
...
const writeFileStreamToTsv = (locale: string, releaseDirPath: string) => {
  const writeStream = fs.createWriteStream(path.join(releaseDirPath, locale, 'reported.tsv'), {
    encoding: 'utf-8',
  });
      ...
      () =>
        new Promise<void>((resolve, reject) => {
          const { conn, stream } = streamingQuery(
            fs.readFileSync(path.join(getQueriesDir(), 'getReportedSentencesLocale.sql'), {
              encoding: 'utf-8',
            }),
...
export const runReportedSentences = (): RTE.ReaderTaskEither<AppEnv, Error, void> =>
  pipe(
    RTE.ask<AppEnv>(),
