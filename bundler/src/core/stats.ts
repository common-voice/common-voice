...
import { CORPORA_CREATOR_FILES, isCorporaCreatorFile } from '../infrastructure/corporaCreator';
...
const getAllRelevantFilepaths = (locale: string, releaseDirPath: string): string[] => {
  const ccFiles = CORPORA_CREATOR_FILES.map(entry => path.join(releaseDirPath, locale, entry));
  const reportedSentencesFilepath = path.join(releaseDirPath, locale, 'reported.tsv');
  return [...ccFiles, reportedSentencesFilepath];
};

const mapLineCountsToStats = (obj: LineCounts): { buckets: Buckets; reportedSentences: number } => {
  ...
  TE.tryCatch(
    () =>
      new Promise<Stats>(resolve => {
        const fileStream = fs.createReadStream(path.join(releaseDirPath, locale, 'clips.tsv'));
...
const unitToHours = (duration: number, unit: 'ms' | 's' | 'min', sigDig: number) => {
  let perHr = 1;
...
export const statsPipeline = (
  ...
  pipe(
    ...
    TE.bindTo('stats'),
    TE.chainFirst(({ stats }) =>
      uploadToBucket(getDatasetBundlerBucketName())(`${releaseName}/stats/stats_${locale}.json`)(
...
export const runStats = (
  ...
  pipe(
    ...
    RTE.chainTaskEitherK(({ locale, releaseDirPath, releaseName }) =>
      statsPipeline(locale, totalDurationInMs, tarFilepath, releaseDirPath, releaseName)
    )
  );
