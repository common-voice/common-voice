...
describe('The Write Page - Single Submission', () => {
  ...
  it('requires citation to add a sentence', () => {
    ...
    cy.get('[data-testid=citation-input]').should('have.css', 'border-color', 'rgb(255, 79, 94)');

    // assert that the no citation rule shows the alert icon
    cy.get('[data-testid=error-no-citation]').should('have.css', 'list-style-image');
  });
  ...
  it('prevents adding a sentence if it is too long', () => {
    ...
    cy.get('[data-testid=error-too-long]').should('have.css', 'list-style-image');
  });

  it('prevents adding a sentence with numbers', () => {
    cy.visit('/write');

    const alphanumericSentence = `${faker.lorem.sentence(10)} ${faker.number.int()}`;
    ...
    cy.get('[data-testid=error-no-numbers-no-symbols]').should('have.css', 'list-style-image');
  });
  ...
  it('prevents adding a sentence with foreign script', () => {
    ...
    cy.get('[data-testid=error-no-foreign-script]').should('have.css', 'list-style-image');
  });
});
