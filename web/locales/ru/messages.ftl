...
dataset-description =
    Каждая запись в наборе данных состоит из уникального MP3 и соответствующего ему текстового файла. Большинство <b>{ $hours }</b> записей в этом наборе данных также включают в себя демографические метаданные, такие как возраст, пол и акцент, которые могут помочь увеличить точность обучения движков распознавания речи.
    
    В настоящее время набор данных состоит из <b>{ $hours }</b> проверенных часов на <b>{ $languageCount }</b> языках, но мы постоянно добавляем новые голоса и языки. Загляните на нашу <languagesLink>страницу языков</languagesLink>, чтобы запросить язык или принять участие.
...
common-voice-info = Есть вопросы о Common Voice? Присоединяйтесь к нам на нашем <discourseLink>форуме Discourse</discourseLink>.
