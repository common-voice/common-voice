speak = Ƒo nu
speak-now = Ƒo nu fifia
speak-subtitle = Tsɔ wò gbe na
record-cta = Dze nuŋɔŋlɔ gɔme
record-platform-not-supported = Míeɖe kuku, womele asi kpem ɖe wò gbea ŋu fifia o.
record-platform-webview = Ðewohĩ èɖo afisia ne èzi kadodo aɖe dzi le social mdia . Èle in-app browser zãm si mewɔa odio ɖe odzi o.
record-platform-not-supported-webview = Le mobile devices dzi la, àte ŋu akpe asi ɖe eŋu to wò system browser zã me...
record-platform-not-supported-desktop = Le desktop devices dzi la, àte ŋu akpe asi ɖe eŋu to eƒe downloading eme...
record-platform-not-supported-ios-non-safari = Le iOS dzi la, taflatse yi edzi kple Safari be nàte ŋu alé nyawo ɖe mɔ̃ dzi...
record-must-allow-microphone = Ele be nàɖe mɔ na nuƒomɔ̃ ƒe gege ɖe eme.
record-no-mic-found = Womekpɔ nuƒomɔ̃ aɖeke o.
record-error-too-short = Nuƒoƒo si wolé ɖe mɔ̃ dzi la nɔ kpuie akpa.
record-error-too-long = Nuƒoƒo si wolé ɖe mɔ̃ dzi la didi akpa.
record-error-too-quiet = Nuƒoƒo si wolé ɖe mɔ̃ dzi la nɔ anyi kpoo akpa.
record-error-empty-recording = Womelé odio aɖeke o. Taflatse lé ŋku ɖe wò nuƒomɔ̃a ŋu eye nàgadze agbagba ake.
record-error-unknown-format = Mete ŋu nya odio ƒe nɔnɔme o. Taflatse dze agbagba nàzã web-browser bubu.
record-error-uploaded-clip-corrupted =
    Edze abe wò odio la gblẽ. Esia ate ŋu adzɔ le:
    • Nya siwo ku ɖe web-browser ƒe ɖekawɔwɔ ŋu (te kpɔ nàzã browser bubu) .
    • Nuƒomɔ̃ alo audio driver ƒe kuxiwo
    • Network ƒe kuxiwo le wò nuƒoƒo me
    Taflatse dze agbagba nàga recordi.
record-cancel = Te fli ɖe re-recording ame
record-instruction = { $actionType }<recordIcon></recordIcon>emegbe nàxlẽ nyagbea sesĩe
record-stop-instruction = { $actionType }<stopIcon></stopIcon> ne ewɔe vɔ
record-three-more-instruction = Esusɔ etɔ̃ nawɔ!
record-again-instruction = Edze agbagba!<recordIcon></recordIcon> Record wò clip si kplɔe ɖo
record-again-instruction2 = Yi edzi, nà record ake <recordIcon></recordIcon>
record-last-instruction = <recordIcon></recordIcon>Mlɔetɔ!
review-tooltip = To klipiwoa me eye nàgbugbɔ a record
share-clip = Do wò klipi la
share-common-voice = Do Common Voice
review-instruction = To  wò klipiwo ame  nàgbugbɔ a record ne ehiã
record-submit-tooltip = { $actionType } tsɔe ɖo ɖa ne èle klalo
clips-uploaded = Clips Uploaded
record-abort-title = Wu nuƒoƒo nu gbãa?
record-abort-text = Dzodzo fifia ana be nàbu wò ŋgɔyiyi
record-abort-submit = Do wò klipiwo ɖa
record-abort-continue = Wu recordi anu
record-abort-delete = Do go le klipiwo ame eye nàtutu wo ɖa
review-aborted = Eɖe asi le upload ŋu. Ðe nèdi be yeatutu yeƒe recording woa?
review-submit-title = To eme & Tsɔe ɖo ɖa
review-submit-msg = Akpe na wò ɖe wò  recording ta!<lineBreak></lineBreak>Azɔ lé ŋku ɖe wò  klipiwo ŋu eye nàɖo wo ɖe ete.
review-recording = Dzro eme
review-rerecord = Re-record
review-cancel = Te fli ɖe edɔa me
review-keep-recordings = Dzra nya siwo wolé ɖe mɔ̃ dzi la ɖo
review-delete-recordings = Tutu nye recording
review-terms = Ne èzã Common Voice la, efia be èlɔ̃ ɖe míaƒe <termsLink>Terms</termsLink> kple <privacyLink>Privacy Notice</privacyLink>
terms-agree = Melɔ̃ ɖe edzi
terms-disagree = Nyemelɔ̃ ɖe edzi o
# FIRST POST SUBMISSION CTA
first-cta-header-text = Akpe be mietsɔ miaƒe voice clips na mi!
first-cta-subtitle-text = Ne ègblɔ ɖokuiwò ŋu nya na mí la, àkpe ɖe dɔwɔlawo ŋu woatu mɔ̃ɖaŋununya si axɔ amewo katã ɖe eme wu
why-donate = Nukatae nèle biabiam?
why-donate-explanation-1 = Nyatakaka sia nye tiatia be yeana - ne èse le ɖokuiwò me be yeƒe dzi dze eme ko hafi wòle be nàtsɔe akpe ɖe eŋu. Ne míeɖe nyatakakadzraɖoƒea ɖe go la, míeɖea ameŋunyatakakawo katã ɖe vovo, eye míekpɔa egbɔ be nyatakakawo abe wò ŋkɔ kple e-mail ene medo ƒome kple amewo ƒe xexlẽme ŋuti nyatakakawo abe ƒexɔxɔ alo gbɔdɔdɔ ene o.
why-donate-explanation-2 = Be mɔ̃ɖaŋununya nawɔ dɔ nyuie na amesiame la, ele be nyatakaka siwo kpena ɖe mɔ̃ɖaŋudɔwɔlawo ŋu be woatu mɔ̃ɖaŋununya ɖo la naɖe ame vovovowo afia. Le kpɔɖeŋu me, ne míetsɔ nuƒomɔ̃ aɖeke si ƒe gbeɖiɖi le koŋ de eme o la, ekema mɔ̃ɖaŋununya la mawɔ dɔ nyuie na amesiwo ƒe gbeɖiɖi le nenema o. <learnMore>Srɔ̃ nu geɖe tso nusita metadata le vevie ŋu.</learnMore>
add-information-button = Tsɔ nyatakakawo kpee
continue-speaking-button = Ao akpe, yi edzi nàƒo nu
create-profile-text = Àdi be yeadzra onyatakakawo ɖoa? <createProfile>Wɔ nɔnɔmetata</createProfile><createProfile>
thanks-for-voice-toast = Akpe na wò le wò nuƒo
thanks-for-voice-toast-error = Vodada aɖe dzɔ le wò gbe ŋuti nyatakakawo ɖoɖo ɖa me
first-cta-gender-select-default-option = Tia nya aɖe
first-cta-gender-select-help-text =
    .label = nukae nye wò ŋutsu enyea alo nyɔnu?
# SECOND POST SUBMISSION CTA
second-cta-header-text = Akpe na wò be nètsɔ wò gbe!
second-cta-subtitle-text = Ne èzã profile aɖe la, àte ŋu akpɔ wò dɔwɔna eye nàdo ka kple gbeɖiɖi ƒe habɔbɔ aɖe.
already-have-an-account = Account bubu le asiwò xoa? <login>Ge ɖe eme</login>
create-profile-button = Wɔ wò profile aɖe
# IMAGE TAGS
img-alt-success-checkmark = dzidzedzekpɔkpɔ ƒe dzesi
# menu item
read-sentences = Xlẽ Nyagbewo
# text wrapped in <small></small> will be shown in a smaller font size
read-sentences-coming-soon = Xlẽ Nyagbewo <small>(Ele vava ge)</small>

## MENU ITEM TOOLTIPS

speak-contribute-menu-tooltip = Xlẽ Nyagbewo, Ðo Nyabiasewo Ŋu
read-sentences-menu-item-tooltip = Lé wò ŋutɔ ɖokuiwò ne èle nyagbewo xlẽm le wò gbe me
answer-questions-menu-item-tooltip = Lé ɖokuiwò ɖe nyabiasewo kple nusiwo wobia tso asiwò ŋu le wò gbe me

## MENU ITEM ARIA LABELS

speak-contribute-menu-aria-label = Nuƒoƒo ƒe tatiawɔblɔɖe ƒe menu
read-sentences-menu-item-aria-label = Xlẽ nyagbe siwo nutoa me tɔwo tsɔ ɖo ɖa le gbegbɔgblɔ vovovowo me
answer-questions-menu-item-aria-label = Miɖo babia yeye aɖe ɖa be nutoa me tɔwo naxlẽ ahaɖo eŋu
