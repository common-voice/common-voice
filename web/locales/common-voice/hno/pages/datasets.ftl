## Datasets Page

datasets-heading = ڈیٹا سیٹس
datasets-headline = اسیں آوازاں دا ہک کھلا ماخذ، کثیر لسانی ڈیٹا سیٹ بنڑا رے ہاں جِڑا ہرکوئی سپیچ-انیبلڈ ایپاں دی ٹریننگ واسطے استمعال کر سکدا ہے۔
datasets-positioning =
    اساں کو یقین ہے کہ بڑے، عوامی سطح تے دستیاب آواز دے ڈیٹا سیٹس مشین لرننگ تے مبنی تقریری ٹکنالوجی بچ جدت تے صحت مند تجارتی مقابلہ کو فروغ دیسن۔
    کامن وائس دا کثیر لسانی ڈیٹا سیٹ اپنڑی نوعیت دا ساریاں تو بڑا عوامی طور تے دستیاب آواز دا ڈیٹا سیٹ ہے، لیکن اے اکیلا نیں۔ اس صفحے کو دوسرے اوپن سورس وائس ڈیٹا سیٹس دے حوالے دے مرکز دی حیثیت نال دکھو، ہور کامن وائس بچ اضافہ ہوندا رہندا ہے، اے جگہ ہے اسدی نئی معلومات دینڑا دی۔
language = زبان
download-dataset-header = ڈیٹا سیٹ کو ڈاؤنلوڈ کرو
download-delta-explainer = اساں کوئی تبدیلیاں کیتاں دیاں ہن۔ ڈیلٹا سیگمنٹس بچ صرف حالیہ کلپس ہن جدوں تو آخری ریلیز ہوئی۔ <deltaLink>اس کم دے بارے ہور پڑھو</deltaLink>۔
download-dataset-tag = مطلوبہ زبان دا ڈیٹا سیٹ چنڑو ہور انتخاب کرو جڑا ورژن تسیں ڈاؤن لوڈ کرنا چہندے ہو۔
# File size in gigabytes
size = سائز
validated-hr-total = ایچ آر ٹوٹل تصدیق کیتا دا
overall-hr-total = ایچ آر ٹوٹل مجموعی طور تے
cv-license = لائسنس
audio-format = آڈیو فارمیٹ
dataset-splits = تقسیم (عمر تے جنس)
number-of-voices = آوازاں دی تعداد
splits = ٹکڑے
email-to-download = ڈاؤنلوڈ واسطے ای میل باؤ
why-email = <b>ای میل کِیّاں؟</b>اسیں تساں کو مستقبل بچ ڈیٹا سیٹ بچ تبدیلیاں بارے رابطہ کرنے دی ضرورت پڑھ ہکدی ہے، ای میل اساں کو  دیندی ہے رابطہ دا ہک رستہ۔
why-donate-datasets-page = موزیلا کامن وائس دُنیا دا سب تو متنوع کراوڈ سورس اوپن سپیچ ڈیٹا سیٹ ہے- تے اسیں پوری طرح عطیات اتے چلدے ہاں۔ڈیٹا سیٹس دی میزبانی کرنا اَستے ہور اس پلیٹ فارم کو 100 تو زیادہ  زبان دی کمیونیٹیاں جڑیاں استے اُتا بھروسہ کردیاں ہن واسطے اچھا بنڑانڑا واسطے تقریباََ ہک ملین ڈالر سالانہ خرچ آندا ہے۔ اگر تُسین اوپن، جامع ڈیٹا دی قدر کردے ہو تے- <b>اَج ہی عطیہ کرو!</b>
confirm-size = تسیں <b>{ $size }</b>کو ڈاؤن لوڈ شروع کرنا واسطے تیار ہو
size-gigabyte = جی بی
size-megabyte = ایم بی
confirm-no-identify = <b>تسیں متفق ہو</b>کہ تسیں کامن وائس ڈیٹا سیٹ بچ بولنا والیاں دی شناخت دسنڑاں دی کوشش نہ کر
confirm-join-mailing-list = تسیں کامن وائس دی ای میل والی لسٹ نال جڑنا چہندے ہو۔
download-language = ڈاؤن لوڈ
validated-hours = تصدیق شدہ گھنٹے
recorded-hours = ریکاڈ کیتے دے گھنٹے
whats-inside = کامن وائس ڈیٹا سیٹ دے اندر کے ہے؟
dataset-description-hours = ڈیٹا سیٹ بچ ہر انداج ہک منفرد MP3 ہور اس دے متعلق ہک ٹیکسٹ فائل تے مشتمل ہوندا ہے۔ ڈیٹا سیٹ بچ ریکاڈ کیتے دے مُچ سارے<b>{ $total }</b>گھنٹیاں بچ ڈیموگرافک میٹا ڈیٹا جسطرح عمر، جنس، تے لہجہ بھی شامل ہوندا ہے جَڑھا سپیچ ریکگنیشن انجناں دی درستگی نال ٹرین کرنا بچ مدد کر سکدا ہے۔ فل حال ڈیٹا سیٹ<b>{ $valid }</b>تصدیق شدہ گھنٹیاں بچ<b>{ $languages }</b>زباناں تے مشتمل ہے، لیکن اسیں ہمیشاں مزید آوازاں تے زباناں شامل کردے ہاں۔ اسدے<languagesLink>زباناں دا صفحہ</languagesLink>تے نظر کرو زبان واسطے درخواست یا حصہ بینڑا واسطے۔
want-dataset-update = اپڈیٹس چہندے ہو جدوں اسیں کامن وائس ڈیٹا سیٹ دا نیا ورژن کرساں؟ اسدا نیوز لیڑ سبسکرائب کرو۔
subscribe = سبسکرائب
get-started-speech = سپیچ ریکگنیشن نال شروع کرو
other-datasets = دوسرے آواز دے ڈیٹا سیٹ
feedback-q = رائے دیسو؟
# This indicates that there is no data to display
no-information = کوئی معلومات نیں
resource-nemo-info = <githubLink>NVIDIA NeMo</githubLink>™ ہک <docsLink>اوپن سورس ٹول کٹ </docsLink> ہے اُنہاں تحقیق کرنا والیاں واسطے جیڑے سٹیٹ-آف-دی-آرٹ AI ماڈلز تیار کررے ہن۔
resource-deepspeech-info =
    موزیلا دے اوپن سورس وائس ریکگنیشن انجن ڈیپ سپیچ کو سپیچ ریکگنیشن ایپلی کیشنز بنڑا واسطے استعمال کر ہکدے ہاں۔ اسدا <githubLink>Github 
     جائزہ </githubLink> پڑھو یا شروع کرنا دا طریقہ سیکھنڑا کیتے <discourseLink>DeepSpeech ڈسکورس </discourseLink> بچ شامل ہوو۔
resource-coqui-info = <coquiLink> Coqui </coquiLink> تقریر دی ٹیکنالوجی کو کھولنڑ واسطے وقف ہے۔ ان دے منصوبیاں بچ ڈیپ لرننگ  تے مبنی STT تے TTS انجن شامل ہن۔
community-playbook = کمیونٹی پلے بک
resource-playbook-info = کامن وائس دے پورے سفر تے، لوکلائزیشن تو گِِھن کے ڈیٹاسیٹ دے استعمال تک، نال اسدی کمیونٹی نال جُڑنا دے طریقے تے <playbookLink> مددگار رہنمائی گِِھنو </playbookLink>۔
data-other-librispeech-description = لائبرسپیچ  تقریبََا 1000 گھنٹیاں دا 16Khz ریڈ انگلش سپیچ تے مشتمل ہک کارپس ہے جڑا لائبری ووکس پراجیکٹ دی ریڈ آڈیو بکس تو حاصل کیتا گیا۔
data-other-ted-name = TED-LIUM کارپس
data-other-ted-description = TED-LIUM کارپس آڈیو ٹاکس نال بنڑایا دا آسا تے اس دی ٹرانسکرپشنز TED ویب سائٹ تے دستیاب ہے۔
data-other-voxforge-description = VoxForge کو ٹرانسکرائبڈ سپیچ جمع کرنا واسطے بنڑایا گیا جس (ٹرانسکرائبڈ سپیچ) کو مفت تے کھلا ماخذ سپیچ ریکگنیشن انجنز نال استعمال ہوسی۔
data-other-tatoeba-description = Tatoeba جملیاں، ترجمے ہور سپکوکن آڈیو کو زبان سیکھنڑا بچ استعمال کرنے دا ہک بڑا ڈیٹا بیس ہے۔
your-feedback = تساں کول کوئی تجویز ہے کہ کسطرح اسیں کامن وائس ڈیٹا کو اچھا بڑا سکدے ہاں؟ اساں کو ڈسکورس تے دسو
go-discourse = ڈسکورس تے جُلو
missing-language = کہ تساں کو اپنڑی زبان دی عکاسی ڈیٹا سیٹ بچ نیں نظر آ ری؟ زبان دی درخواست واسطے اسدے زبان دے صفحے تے آؤ۔
go-languages-page = زبان دے صفحے تے جُلو
ready-to-validate = جملیاں دی توثیق واسطے مدد کرنا کو تیار ہو؟
more = مزید
close = بند
download = ڈاؤنلوڈ
dataset-version = ورژن
sha256-checksum-copied = SHA256 چیکسم کاپی کر چُھڑیا
sha256-checksum-copied-error = SHA256 چیکسم کو کاپی نیں کر ہکیا
clipboard-not-supported = کلپ بورڈ کو سہارا نیں دیندا
no-information-available = کوئی معلومات دستیاب نیں۔
dataset-metadata-sex = جنس
# dataset metadata - age of contributor
dataset-metadata-age = عمر
donate-modal-message = تُسدا ڈیٹا سیٹ ڈاؤنلوڈ ہو رہیا ہے!
dataset-donate-modal-heading = تساں کو علم آسا۔۔۔
donate-modal-explanation-1 = ڈیٹا سیٹس دی میزبانی کرنا اَستے ہور اس پلیٹ فارم کو 100 تو زیادہ  زبان دی کمیونیٹیاں جڑیاں استے اُتا بھروسہ کردیاں ہن واسطے اچھا بنڑانڑا واسطے تقریباََ ہک ملین ڈالر سالانہ خرچ آندا ہے۔
# Text in <bold></bold> will shown in bold
donate-modal-explanation-2 = <bold>اگر تسیں اوپن، جامع ڈیٹا دی قدر کردے ہو- تے اج ہی عطیہ دو!</bold>
data-download-singleword-title = ہک لفظ والا ہدف حصہ ڈاؤنلوڈ کرو
data-download-singleword-callout-v2 = ایہ ہک یوز کیس ڈریون حصہ ہے جس بچ سپوکن ڈجّٹ ریککنیشن ہور ہاں /ناں دا پتہ لینڑا دا ڈیٹا ہے۔
data-download-button = کامن وائس ڈیٹا ڈاؤنلوڈ کرو
data-download-yes = ہاں
data-download-deny = ناں
data-download-license = لائسنس: <licenseLink>CC-0</licenseLink>
data-download-modal = تسیں <size>{ $size }GB</size>دا ڈاؤنلوڈ شروع کرن لگے ہو، اگاں جُلاں؟
data-subtitle = اسیں آوازاں دا ہک کھلا ہور عوامی طور تے دستیاب ڈیٹا سیٹ بنڑا رے ہاں جِڑا ہرکوئی سپیچ-انیبلڈ ایپاں دی ٹریننگ واسطے استمعال کر سکدا ہے۔
data-explanatory-text = اساں کو یقین ہے کہ بڑے ہور عوامی سطح تے دستیاب آواز دے ڈیٹا سیٹس مشین لرننگ تے مبنی تقریری ٹکنالوجی بچ جدت تے صحت مند تجارتی مقابلے کو فروغ دیسن۔ ایہ عالمی سطح دی ہک کوشش ہے ہور اسیں ہر ہکا کو اس بچ شامل ہونڑا دی دعوت دیندے ہاں۔ اسدا مقصد ہے کہ آواز تے مبنی ٹکنالوجی مزید جامع ہوے، دنیا بھر تو مختلف آوازاں دی عکاسی کرا۔
data-get-started = <speechBlogLink>سپیچ ریکگنیشن نال شروع کرو</speechBlogLink>
data-other-title = دوسرے آواز دے ڈیٹا سیٹ۔۔۔
data-other-goto = { $name }تے جُلو
data-other-download = ڈیٹا ڈاؤنلوڈ کرو
data-bundle-button = ڈیٹا سیٹ دا بنڈل ڈاؤنلوڈ کرو
data-bundle-description = کامن وائس ڈیٹا جمع اُتلے دوئے سارے وائس ڈیٹاسیٹ۔
release-version = ورژن
dataset-date = تریخ
license = لسانس:<licenseLink>{ $license }</licenseLink>
license-mixed = مکس کیتادا
# MENU ITEM TOOLTIPS
download-contribute-menu-tooltip = اسدے آواز دے ڈیٹا سیٹس کو ڈاؤنلوڈ کرو
# MENU ITEM ARIA LABELS
download-contribute-menu-aria-label = کامن وائس ڈیٹا پہنچنڑا آستے ڈاؤنلوڈ دے آپشنز
