# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Afar
aat = Arvanitika
ab = Abcaso
abb = Bankon
abq = Afar
ace = Acehnese
ady = Adigue
af = Afrikaans
aii = Suret
ajg = Adja
aln = Gheg albanese
am = Amharico
an = Aragonese
ar = Arabe
arn = Mapudungun
as = Assamese
ast = Asturiano
az = Azerbaijano
azz = Nahuatl del montanias de Puebla
ba = Bashkir
bag = Tuki
bal = Balochi
bas = Basaa
bax = Bamun
bba = Baatonum
bbj = Ghomala
bbl = Tush
bce = Mengambo
bci = Baoule
be = Bielorusso
beb = Bamvele
bew = Betawi
bfd = Bafut
bft = Balti
bg = Bulgaro
bgp = Beluci oriental
bin = Edo
bkh = Bakoko
bkm = Kom
bm = Bambara
bn = Bengali
bnm = Batanga
bnn = Bunun
bo = Tibetano
br = Breton
brh = Brahui
bri = Mokpwe
brx = Bodo
bs = Bosniano
bsh = Kateviri
bsk = Brushaski
bsy = Sabah Bisaya
btv = Bateri
bum = Bulu
bxk = Bukusu
bxr = Buriato
byv = Medumba
ca = Catalano
cak = Kaqchikel
cdo = Min oriental
ce = Tchetcheno
cgg = Chiga
cjk = Chokwe
ckb = Kurdo central
cnh = Hakha Chin
co = Corso
cpx = Heng Hua
cpy = Asheninka Sud Ucayali
crh = tatar crimean
cs = Checo
cut = Teutila Cuicatec
cux = Tepeuxila Cuicatec
cv = Chuvasio
cy = Gallese
da = Danese
dag = Dagbani
dar = Dargwa
dav = Kidaw'ida
de = Germano
din = Dinka
dmk = Dawoodi
dml = Dameli
dru = Rukai
dsb = Sorbio, basse
dua = Duala
dv = Maldiviano
dyu = Dioula
ebr = Ebrie
ee = Ewe
efk = Efik
eko = Ekoti
el = Greco
en = Anglese
eo = Esperanto
es = Espaniol
esu = Yupʼik del Alaska central
et = Estoniano
eto = Eton
eu = Basco
ewo = Ewondo
fa = Persiano
fan = Fang
ff = Fula
fi = Finnese
fmp = Feʼefeʼe
fo = Faroese
fr = Francese
fub = Fulfulde del Adamawa
fue = Fulfulde del Borgu
fuf = Pular Guinea
fy-NL = Frison
ga-IE = Irlandese
gaa = Ga
gcf = Creolo del Martinica
gej = Mina
ggg = Gurgula
gid = Guidar
gig = Goaria
giz = Guiziga
gjk = Kachhi
gju = Gujari
gl = Galiciano
glk = Gilaki
gn = Guarani
gom = Goan Konkani
gos = Gronings
gsw = Alsatian
gu-IN = Gujarati
guc = Wayuunaiki
gv = Manx
gwc = Gawri
gwt = Gawarbaiti
gya = Gbaya de Nord-West
ha = Hausa
hac = Gorani
hak = Hakka
haz = Hazargi
hch = Wixárika
he = Hebreo
hem = Kihemba
hi = Hindi
hil = Hiligaynon
hno = Hindko septentrional
hr = Croata
hrx = Hunsrik
hsb = Sorbio, alte
ht = Haitiano
hu = Hungaro
hux = Nüpode Huitoto
hy-AM = Armenio
ia = Interlingua
iba = Iban
ibb = Ibibio
id = Indonesiano
ie = Interlingue
ig = Igbo
ipk = Iñupiaq
is = Islandese
it = Italiano
izh = Azoriano
ja = Japonese
jam = Patois Jamaican
jbo = Lojban
jgo = Ngomba
jqr = Jaqaru
jv = Javanese
ka = Georgiano
kaa = Karakalpak
kab = Cabilo
kam = Kikamba
kbd = Kabardo
kcn = Nubi
kdh = Kotokoli
khw = Khowar
ki = Kikuyu
kjh = Khakas
kk = Kazakh
kln = Kalendjin
kls = Kalasha
km = Khmer
kmr = Kurdo Kurmanxhi
kn = Kannada
knn = Konkani (Devanagari)
ko = Coreano
koo = Konzo
kpv = Komi-Zyriano
krc = Karachay-Balkar
ks = Kashmiri
ksf = Bafia
kvx = Parkari Koli
kw = Cornico
kxp = Wadiyara Koli
ky = Kirghiso
kzi = Kelabit
laj = Lango
lb = Luxemburgese
led = Lendu
leu = Kara
lg = Luganda
lij = Ligure
lke = Kenyi
lld = Ladin
ln = Lingala
lo = Lao
lrk = Loarki
lrl = Achomi
lss = Lassi
lt = Lituano
ltg = Latgaliano
lth = Thur
lua = Tschiluba
luo = Dholuo
lv = Latviano
lzz = Laz
mai = Maithili
mau = Huautla Mazatec
mbf = Baba Malay
mbo = Mbo
mcf = Matses
mcn = Massa
mcx = Mpiemo
mdd = Mbum
mdf = Moksha
meh = Mixteco Yucuhiti
mel = Melanau
mfe = Morisyen Kreol
mg = Malagasy
mgg = Kunabembe
mhk = Mungaka
mhr = Mari oriental
mk = Macedone
mki = Dhatki
ml = Malayalam
mmc = Michoacán Mazahua
mn = Mongol
mni = Meetei Lon
mnw = Mon
mos = Mossi
mqh = Mixteco tlazoyaltepec
mr = Marathi
mrh = Mara
mrj = Mari occidental
ms = Malay
mse = Moussey
msi = Sabah Malay
mt = Maltese
mua = Mundang
mug = Musgum
mve = Marwari pachistano
mvy = Indus Kohistani
mxu = Mada
my = Birmano
myv = Erzya
nan-tw = Taiwanese (Minnan)
nb-NO = Norvegiano Bokmål
ncx = Puebla Nahuatl Central
nd = IsiNdebele (nord)
ne-NP = Nepalese
new = Newari
nhe = Huasteca Nahuatl oriental
nhi = Westelijk Sierra Puebla Nahuatl
nia = Nias
nl = Nederlandese
nla = Ngombale
nlv = Orizaba Nahuatl
nmg = Kwasio
nmz = Losso
nn-NO = Norvegiano Nynorsk
nnh = Ngiembon
nqo = NKo
nr = IsiNdebele (sud)
nso = sotho del nord
ny = Chinyanja
nyn = Nyankole
nyu = Nyungwe
oc = Occitano
odk = Oadki
om = oromo
or = Odia
oru = Ormuri
os = Osseta
pa-IN = Punjabi
pap = Papiamento
pap-AW = Papiamento (Aruba)
pau = Palauano
pcd = Picard
pcm = Pidgin nigerian anglese
pez = Penan oriental
phl = Palula
phr = Pahari-Pothwari
pl = Polonese
plk = Kohistani Shina
pne = Penan occidental
prq = Asheninka Perene
ps = Pashto
pt = Portugese
pua = Purepecha del Highlands occidental
pwn = Paiwan
quc = K'iche'
qug = Quichua Chimborazo Highland
qup = Pastaza Quechua Meridional
qur = Quechua Yanahuanca
qus = Quechua Santiago del Estero
qux = Quechua Yauyos
quy = Quechua Chanka
qva = Quechua Ambo-Pasco
qvi = Kichwa
qvj = Loja Highland Kichwa
qvl = Quechua Cajatambo
qwa = Quechua Corongo Ancash
qws = Quechua Sihuas Ancash
qxa = Quechua Chiquián
qxp = Quechua de Puno
qxq = Qashqai
qxt = Quechua Pasco Santa Ana de Tusi
qxu = Quechua Arequipa-La Unión
qxw = Quechua Jauja Wanka
rif = Tarifit
rki = Rakhine
rm-sursilv = Romansh Sursilvan
rm-vallader = Romancio vallader
rn = Kirundi
ro = Romaniano
rof = Kirombo
ru = Russo
ruc = Ruuli
rup = Aromanian
rw = Ruanda
rwm = Amba
sah = Sakha
sat = Santali (Ol Chiki)
sba = Ngambay
sbn = Sindhi Bhili
sc = Sardo
scl = Shina
scn = Siciliano
sco = scotese
sd = sindhi
sdh = Kurdo meridional
sdo = Serian Bidayuh
seh = Sena
sei = Seri
sg = Sango
shi = Shilha
shn = Shan
si = Sinhala
sk = Slovaco
skr = Saraiki
sl = Sloveno
sn = Shona
snk = Soninke
snv = Sa’ban
so = Somalo
sq = Albanese
sr = Serbo
ss = Siswati
ssi = Sansi
st = sesotho
su = Sundanese
sv-SE = Svedese
sva = Svan
sw = Swahili
swb = Shimaore
syr = Syriac
szy = Sakizaya
ta = Tamil
tar = Tarahumara Central
tay = Atayal
te = Telugu
teg = Teke
tg = Tajik
th = Thai
ti = Tigrinya
tig = Tigre
tiv = Tiv
tk = turkmeno
tl = Tagalog
tli = Tlingit
tn = Setswana
tob = Toba Qom
tok = Toki Pona
top = Papantla Totonac
tr = Turco
trv = Seediq
trw = Torwali
ts = xitsonga
tt = Tataro
ttj = Rutoro
tui = Tupuri
tum = Tumbuka
tuq = Tedaga
tvu = Tunen
tw = Twi
ty = Tahitiano
tyv = Tuvano
uby = Ubykh
udl = Ouldémé
udm = Udmurte
ug = Uyghur
uk = Ukrainiano
ukv = Kuku
ur = Urdu
ush = Ushojo
usp = Uspanteko
uz = Uzbek
var = Huarijio
ve = tshivenda
vec = Venetian
vi = Vietnamese
vmw = Makua
vot = Votico
wbl = Wakhi
wep = Vestfalian
wes = Pidgin del Camerun
wo = Wolof
xdq = Kaitag
xh = Xhosa
xhe = Ketrani
xka = Kalkoti
xkl = Kenyah
xmf = Mingrelian
xsm = Kasem
yaq = Yaqui
yav = Nuasue
ydg = Yadgha
yi = Yiddish
yo = Yoruba
yue = Cantonese
zgh = Berbero
zh-CN = Chinese (China)
zh-HK = Chinese (Hong Kong)
zh-TW = Chinese (Taiwan)
zoc = Copainalá Zoque
zu = Zulu
zza = Zaza

# [/]


## Layout

profile = Profilo
help = Adjuta
contact = Contacto
privacy = Confidentialitate
terms = Terminos
cookies = Cookies
faq = Folio a questiones
documentation = Documentation
content-license-text = Contento disponibile sub <licenseLink>licentia Creative Commons</licenseLink>
share-title = Adjuta nos a trovar alteres pro donar lor voce!
share-text = Adjuta a inseniar a machinas como personas real parla! Dona tu voce sur { $link }
link-copied = Ligamine copiate
back-top = Retornar al initio
logout = Clauder session
donate = Donar
stayintouch = Nos a Mozilla construe un communitate de interessatos in technologias de voce. Nos volerea remaner in contacto pro inviar te actualitates, informar te de nove fontes de datos e saper plus sur como tu usa iste datos.
privacy-info = Nos promitte de gerer tu informationes con cura. Lege plus in nostre <privacyLink>aviso de confidentialitate</privacyLink>.
return-to-cv = Retornar a Common Voice
email-input =
    .label = Email
loading = Cargamento…
email-opt-in-info = Io volerea reciper emails como rememorationes de objectivos, actualisationes de mi progresso e bulletines de novas sur Common Voice.
email-opt-in-info-title = Inscribe te al lista de diffusion de Common Voice
email-opt-in-info-sub-with-challenge = Recipe emails como rememorationes de defias e objectivos, actualisationes de progresso e bulletines de novas sur Common Voice.
email-opt-in-privacy-v2 = Si tu opta pro reciper emails, tu accepta que Mozilla usa iste informationes como explicate in su <privacyLink>politica de confidentialitate</privacyLink>.
indicates-required = * indica un campo obligatori
not-available-abbreviation = N/D
# Text shown on a button that can be clicked to copy text to the clipboard
copy = Copiar
# Text shown on a button that can be clicked to close a modal or panel
close = Clauder
# Text shown on a button that can be clicked to cancel an action
cancel = Cancellar
# Text shown on a button that can be clicked to delete an item
delete = Deler

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>Nos besonia</mark><br/><mark>tu adjuta!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = Il costa quasi un million de dollars per anno le allogiamento del collectiones de datos e le melioration del platteforma pro le plus que 100 communitates linguistic que conta sur illo que nos face. <strong>Si vos apprecia le datos gratuite e inclusive: dona hodie!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>Usa vos CV pro</mark><br/><mark>vostre recerca?</mark>
languages-donate-banner-cta-explanation = Common Voice es financiate per donationes e subventiones! Nos ama collaborar con academicos, recercatores de societate civil e industria. Le uso de Common Voice es gratuite, ma contribuer al platteforma e al costos de allogiamento proponente subventiones es realmente utile.
localization-select =
    .label = Eliger lingua/localisation

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = Le <strong>nove collectiones de datos de Common Voice</strong> es ora disponibile pro discargar exclusivemente per nostre platteforma soror, Mozilla Datos Collective. Junge te pro acceder a plus que 300 collectiones mundial de datos, producite per e pro le communitate.
# Button text for MDC announcement
announcement-mdc-button-text = Junge te a Mozilla Datos Collective
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = Se aperi in un nove scheda

## Release Delay Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-delay = Car communitate, nos actualmente labora sur le version. Nos presume que illo sera implementate in partes in le proxime dies sur <mdcLink>Mozilla Data Collective</mdcLink>.

## Release Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release = Car communitate, le nove version es ora disponibile sur <mdcLink>Mozilla Data Collective</mdcLink>. Inscribe te pro acceder a plus que 500 collectiones mundial de datos producite per e pro le communitate.

## Pre-Release Announcement (dates TBD)

# Text wrapped in <cvDatasheetsLink></cvDatasheetsLink> will be rendered as an external link to the cv-datasheets GitHub repo
announcement-pre-release = Car communitate, le tempore del nove version se approxima. Va validar e transcriber, e alsi visitar le pagina <cvDatasheetsLink>cv-datasheets</cvDatasheetsLink> pro actualisar tu folio de datos.

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = Initia a scriber pro cercar
searchbox-default-placeholder = Cercante...
searchbox-default-aria-label = Cercar
dropdown-no-results = Nulle correspondentia trovate
# Common Voice Dataset=Language search box
dataset-searchbox-label = Cambia sia le lingua de visualisation que lo del collection de datos
dataset-searchbox-placeholder = Cercar un lingua...
dataset-searchbox-aria-label = Cercar un lingua
dataset-search-no-results = Nulle lingua concordante trovate.
request-new-language = Requirer nove lingua.
