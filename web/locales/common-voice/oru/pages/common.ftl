# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = افار
aat = اروانیتیکا
ab = ابخاز
abb = بېنکن
abq = ابازا
ace = اچنېسی
ady = ادېغې
af = افریقاز
aii = صورېت
ajg = ادجا
aln = غېګ البانئے
am = امهارک
an = اراګونس
ar = عربي
arn = ماپوډنګن
as = اسامی
ast = استورئن
az = آزربائجانی
azz = هائلېنډ پیوبلا ناهوتل
ba = باشکر
bag = تُکی
bal = بلوچی
bas = بسا
bax = بامُون
bba = باتونم
bbj = غومالا
bbl = تُش
bce = مېنګامبو
bci = باؤلې
be = بېلاروسی
beb = بارموېلے
bew = بېټاوئے
bfd = بافوت
bft = بلتی
bg = بلغارئے
bgp = ځېستر بلوچی
bkh = بکُوکُو
bkm = کوم
bm = بمباره
bn = بنګاليے
bnm = بتنګا
bnn = بُونُون
bo = تیبتي
br = برېظونئے
brh = براهوی
bri = موکپؤئے
brx = بوډو
bs = بوسنئے
bsh = کاټېوېری
bsk = بروشُسکی
bsy = صبا بِسایا
btv = بټېری
bum = بُلُو
bxk = بکُوسُو
bxr = بریاظ
byv = مېدمبا
ca = کتلانئے
cak = ککچییکلئے
cdo = مشرقي مِن
ce = چېچن
cgg = چیګا
cjk = چوکوی
ckb = منځینئے کُردي
cnh = هکا چنئے
co = کورسیکنئے
cpx = هېنګ هُوا
cpy = اشېنِنکا ساؤتهـ اُسایالی
crh = کریمنئے تاتري
cs = ځېک
cut = ټیئوټیلا سیئوسېټِک
cux = ټېپېئوکسیلا سیئوسېټېک
cv = چُواشي
cy = وېلشئے
da = دانِشئے
dag = دغبادئے
dar = درګاوئے
dav = کِردایدئے
de = جرمنئے
din = دِنکا
dmk = داؤدی/ډوماکی
dml = دامېلی
dru = رُوکائی
dsb = ځېم سربئے
dua = ډیوالئے
dv = دیواهئے
dyu = دیوولئے
ebr = عبرئے
ee = اېوئے
efk = ایفېک
eko = ایکوټی
el = یونانئے
en = انګرېزي
eo = اِسپرانتيے
es = سپېنئے
esu = سنټرل الاسکن یُوپِک
et = استونئے
eto = ایتونئے
eu = باسقئے
ewo = اېواندُو
fa = فارسي
fan = فنګئے
ff = فُولاح
fi = فنلېنډئے
fmp = فېفې
fo = فاروسئے
fr = فرانسئے
fub = آدماؤ فلفلډئے
fue = برغو فلفلډئے
fuf = پُولرګنئے
fy-NL = فریزنئے
ga-IE = آئرېشي
gaa = ګا
gej = مینه
ggg = ګُرګُولا
gid = ګویدر
gig = ګوآریا
giz = ګُزیګا
gjk = کچی
gju = ګُوجری
gl = ګلشینئے
glk = ګیلاکی
gn = ګُورانئے
gom = ګون کنکئے
gos = ګروننګئے
gsw = السېشئن
gu-IN = ګُجراتئے
guc = ویوناکئے
gv = مانخ
gwc = ګاوري
gwt = ګواربتی
gya = خورېنڅه بېشترګوبئے
ha = هاؤسئے
hac = ګورانی
hak = هکا
haz = هزارګی
hch = ویخاریکا
he = عبراني
hem = کیهېمبا
hi = هندي
hil = هیلیګائنون
hno = شمالی هندکو
hr = کروشانئے
hrx = هُنسرِک
hsb = بېژ سربنئے
ht = هېټانئے
hu = هنګرئے
hux = نیوپوډے نیئوټوټو
hy-AM = ارمانئے
ia = اِنترلینګئے
iba = اِبان
ibb = ایبوبئے
id = انډونېشئے
ie = انټرلینګئے
ig = ایګبو
ipk = اینوپیاق
is = آئس لېنډئے
it = اظالوي
izh = اِݫورنئے
ja = جاپانيے
jam = جمېکن پاټوئز
jbo = لوجبانئے
jgo = ګومبا
jqr = جقارو
jv = جاوانئے
ka = جارجانئے
kaa = کراکلپاکئے
kab = کربلانئے
kam = کیکامبا
kbd = کبردانئے
kcn = نُوبی
kdh = کوټوکولی
khw = کهوار
ki = کیکُونئے
kjh = خکاس
kk = کزاخئے
kln = کالینجانئے
kls = کلاشه
km = خمیرئے
kmr = کُرمانجی کُردئے
kn = کاناډئے
knn = کونکانئے
ko = کورئے
koo = کونزو
kpv = کومی زائرینئے
krc = کراچے بالکر
ks = کشمیری
ksf = بافئے
kvx = پرکاری کولی
kw = کورنیشئے
kxp = وادیارا کولی
ky = کَرغیزئے
kzi = کېلابِت
laj = لانګو
lb = لګزمبرګئے
led = لېنډو
leu = کارا
lg = لوګنډئے
lij = لیګُرنئے
lke = کېنئی
lld = لادین
ln = لِنګالئے
lo = لاؤئے
lrk = لووارکی
lrl = اچومی
lss = لاسی
lt = لیتونئے
ltg = لتګالئے
lth = تُهر
lua = تشیبُولا
luo = دولو
lv = لټونئے
lzz = لازئے
mai = میتیلئے
mau = هُیایُوتلا
mbf = بابا مَلے
mbo = موبئے
mcf = متسېس
mcn = ماسا
mcx = ممپئیمو
mdd = مبُوم
mdf = موکشئے
meh = مِکسټېکو یُوکُوهیټی
mel = مېلاناؤ
mfe = کرول موریسئن
mg = مالاګاسئے
mgg = کُنابېمبے
mhk = مُنګکا
mhr = میډومارئے
mk = مېکیډونئے
mki = ډټکی
ml = ملیالمئے
mmc = مِچواکان مزاهُوا
mn = منګولئے
mni = مېتیلونئے
mnw = مون
mos = موݭئے
mqh = تلازویالپېک مِکسفېک
mr = مراټئے
mrh = مارا
mrj = هِلمارئے
ms = ملائی
mse = موسی
msi = صباح مَلے
mt = مالټئے
mua = مُنډانګ
mug = مُسګم
mve = مرواړی
mvy = اِنډس کوهستانی
mxu = مادے
my = برمئے
myv = اِرضیا
nan-tw = تائوانئے
nb-NO = ناروېژئے بُکمال
ncx = سنټرل پیوبلا ناهوواتی
nd = اِسین دېبلئې
ne-NP = نېپالئے
new = نېواری
nhe = ځېسترهوسټیکا ناهوتلئے
nhi = بېشترسیاره پیوبلا ناهوتئې
nia = نیاسئے
nl = هالېډئے
nla = نګومبالے
nlv = اوریزبا ناهوتل
nmg = کواسیو
nmz = لوسو
nn-NO = ناروېژي نینورکئے
nnh = ګمبئے
nqo = نکئے
nr = هِسی نبولئے
nso = خورېنڅه نوتئے
ny = چینیانجئے
nyn = رنیانکئے
nyu = نیونګوی
oc = اوسیټانئے
odk = اودکی
om = افان اورومئے
or = اودئے
oru = اُرمړي
os = اوسېشنئے
pa-IN = پنجاپئے
pap = پاپیامېنتو
pap-AW = پپیامېنتو
pau = پلاؤان
pcd = پیکارډ
pcm = نائجېرئے پِډګِن انګرېزي
pez = ځېستر پېنان
phl = پالُولا
phr = پهاړی/پوټواری
pl = پولیشئے
plk = کوهستانی شینا
pne = بېشتر پېنان
prq = اشینِنکا پېرین
ps = پشتو
pt = پُرتګالئے
pua = بېشتر غرېئیځی پیورېپېچا
pwn = پائوان
quc = کِچئے
qug = قویچاؤ چِمبورازو هائی لېنډ
qup = سدرن پاستازه قیوچُوا
qur = قویچاؤ یاناهنسا
qus = قویچاؤ سانتیاګو ډېل اېسټیرو
qux = قیوچُوا یاؤیوس
quy = قیچاؤ چانکئے
qva = قیوچُوا یویوس
qvi = کِچوئے
qvj = لوجا هائی لېنډ کیچوا
qvl = قیوچُوا کاجاتمبو
qwa = قیوچُوا کورونګو انکاش
qws = قیوچُوا سیهواس انکاش
qxa = قیوچُوا چیکوئین
qxp = پونو کِچوئے
qxq = قشکائی
qxt = قیوچُوا پاسکو سانتا آنا دی تُوسی
qxu = قیوچُوا ارېقویپا لا یونئن
qxw = قیوچُوا جاؤجا وانکا
rif = ظاری فِتئے
rki = راخائن
rm-sursilv = رومانش سرسِلونئے
rm-vallader = رومانش ولادرئے
rn = کیرُنډدی
ro = رومانئے
rof = کیرومبو
ru = روسئے
ruc = رُولی
rup = اررومانئن
rw = کنیاروندئے
rwm = امبا
sah = سخے
sat = سنتالئے
sba = ګمبایٛ
sbn = سندهی بِل
sc = سردینئے
scl = شینا
scn = سیسیلنئے
sco = سکاڅئے
sd = سندهی
sdh = جيېله کُردئے
sdo = سېرئن بیدایُوح
seh = سېنا
sei = سېري
shi = شِلهالئے
shn = شانئے
si = سِنهالئے
sk = سکووېکئے
skr = سرائکي
sl = سکوېنئے
sn = شونا
snk = سونِنکئے
snv = سابن
so = صومالئے
sq = البانئے
sr = سربئے
ss = سِسواتئے
ssi = سانسی
st = چيېله سوتئے
su = سندانِس
sv-SE = سویدنئے
sva = سوان
sw = صواهېلئے
syr = شامئے
szy = سکیزایا
ta = تامېلئے
tar = مرکزي تارا هُمارا
tay = اتائل
te = تیلوګئے
teg = ټېکئے
tg = تاجېکئے
th = تهالئے
ti = ټِګرنئے
tig = ټیګرئے
tk = ترکمنئے
tl = تګالګئے
tli = تلِنګِت
tn = سېتسوانئے
tob = ټوبا قوم
tok = ټوقی پُنئے
top = پاپانتلا ټوټونېک
tr = تُرکئے
trv = صیدق
trw = تورولی
ts = کیڅونګئے
tt = تانرئے
ttj = روټورو
tui = تُوپُرئے
tum = تُمبُوکا
tuq = تېداګا
tvu = تونِنئے
tw = توئے
ty = تاهیتنئے
tyv = توونئے
uby = اوبېخئے
udl = اُلدېمې
udm = اودمرتئے
ug = یوغُرئے
uk = یوکرنئے
ukv = کُوکُو
ur = اردو
ush = اوشوجو
usp = اُسپانتیکو
uz = اُزبک
var = هُواریجو
ve = تاشوندئے
vec = وېنېشئے
vi = وېټنامئے
vmw = ایماخوئے
vot = ووټیشئے
wbl = واخی
wep = وېسټفلنئے
wes = کیمېرون پِډګِنئے
wo = وولوفئے
xdq = کېټېګ
xh = کوسئے
xhe = کهېترانی
xka = کلکوټی
xkl = کېنیاح
xmf = مِنګرېلئن
xsm = کاسېم
yaq = یُوقی
yav = نواسو
ydg = یدغا
yi = ییدِشئے
yo = یاروبئے
yue = کېنټونیشئے
zgh = تمازیټئے
zh-CN = چینئے
zh-HK = هنګ کنګ چینئے
zh-TW = تهائیوانئے چینئے
zoc = کوپېنالا زوقُو
zu = زُولُو
zza = زازئے

# [/]


## Layout

profile = پروفِائل
help = مدد
contact = رابظه
privacy = ذاتي
terms = ݭړظی
cookies = کُکیز
faq = فق
documentation = کاغذ کېک
content-license-text = مواد دی موجود هۀ تخلیقي مشترک لائسنس ګیرډ
share-title = ترماخ مدد کوئ که ته خلق ا آواز دُشيېن
share-text = هر مشین کی وه زدۀ کوئ که واقعي ا خلق بُو څا ژغا
link-copied = ا لِنکن کوپی دوک
back-top = واپس بېژ کی
logout = بهر کی لاګ آوټ سُن
donate = پېش دی کَۀ
stayintouch = ماخ يې بُو موزیلا نر ته آواز کمیونېټی جوړېن۔ ماخ بُو آ زېن که ترماخ يې سۀ بی ګیرډ رابطه بَۀ۔ ته نیوه اپډېټ، ته نیووه ذریعی باره نر که آ ګه تیوس لاسته اميېن که آ ډېټا بو څا او ته څۀ پاره اتعمالَوَک سَۀ
privacy-info = ماخ يې بو آ ګه وعده کيېن که ترتیوس ا معلومات په سره انډلېن۔ مزیده جیر کَۀ ترماخ <privacyLink> لِنک نر
return-to-cv = مشترک آواز کی ر واپس سُن
email-input =
    .label = ای مېل
loading = لوډنګ
email-opt-in-info = از بُو آ زېم که ترتیوس  ای مېل اِر دې څوا ته مقصد ته یاد دهانی باره نر، ترمُن ته مُخ په تک او ته خبري باره نر
email-opt-in-info-title = ته مشترک آواز ته ای مېلنګ لسټ نر تُو ګه شامل سُن
email-opt-in-info-sub-with-challenge = سا ا ای مېلز دی وصولېون سکه چېلنجی، او ته مقصد یاد دهاني، ته اپډېټ ا په مختګ او ته مشترک آواز باره نر خبري
email-opt-in-privacy-v2 = ته پا وُرغېوي ته ڒیوڅن ا مطلب يې او ݭیوک که تُو ته موزیلا ته ډېټا ا سا قسم انډلؤ ګیرډ متفق هې سکه څا ته موزیلا ای پۀ لنک نر موجود هۀ<privacyLink> پرائوېسی پالېسي </privacyLink>
indicates-required = فیلډ ته ستُرک * ای نشان ګیرډ يې ضروري هۀ
not-available-abbreviation = ن/ا
# Text shown on a button that can be clicked to copy text to the clipboard
copy = کوپی
# Text shown on a button that can be clicked to close a modal or panel
close = بند
# Text shown on a button that can be clicked to cancel an action
cancel = کېنسل
# Text shown on a button that can be clicked to delete an item
delete = ډیلیټ

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark> ماخ کی يې ترتیوس ته امداد ضرورت هۀ <br/> </mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = ته ډېټا سېټ ته ساتؤ او ته پۀ پلېټ فارم ته ترقي پاره يې ماخ کی ته څان اندازاً ته دس لکهـ ډالر ضرورت هۀ، کيې که سُو 100 لاسته ته ݫات زبني ته غوېڅن خلق ګیرډ کوک بُو که پۀ زر انحصار کَوِن، فئ ګیرډ يې ترماخ تعلق هۀ۔ ماخ بُو که څه کيېن <strong> او که تُو ګیرډ يې ته پا ډېټا قدِر بَۀ، نو مهرباني نيې سُو بَۀ که ماخ ګیرډ يې امداد ضرور کوئ۔ </strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = ته CV </mark> <br/> <mark> ترتُو ا ریسرچ </mark>
languages-donate-banner-cta-explanation = مشترک آواز بُو په چندي او په پېشکشي چلا۔  ترماخ يې بُو تعلیمیافته خلق، عام خلق، صنعت کارنی، ریسرچرنی او کسبدري ګیرډ تعلق بَۀ۔ ته مشترک آواز ا اِستعمال يې په لیۀ هۀ، خه پلېټ فارم کی يې ته چنده ا ڒیوک او ته پۀ ته ساتؤ پناره ته امداد ضرورت هۀ
localization-select =
    .label = ته زبان ا خؤشؤ/ مقامی کؤ

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = نیوکامن ډېټاسېټس پېری ته ډاؤن لوډ کؤ پاره ته سِسټر پلېټ فارم موزیلا ډډېټا کولېکټېو په ذریعه موجود هۀ۔ ڒی سوه لاسته ݫات ډېټاسېټس کی ا ځان رسېون که ته کمیونېټی ا خلق يې ته رئ خوئ پاره جوړ داک هۀ
# Button text for MDC announcement
announcement-mdc-button-text = موزیلا ډېټا سېټس کلېکټېو ګېرډ مل سُن
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = نیوو ټېب نره وه میوز

## Release Delay Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-delay = خوېڒی ته کمیونېټی واله وئ، ایزیره بُو ماخ ریلیزکؤ زر کر کيېن۔ ترماخ يې یمېند هۀ که پئ څُون رݫی نره سُو ماخ پَلَۀ پسے ریلیز کيېن Mozilla Data Collective کی۔

## Release Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release = خوېڒی ته کمیونیټی واله وئ، ا نیوو ریلیز پېری پُورۀ ”آن” هۀ Mozilla Data Collective نر۔ پېری ل بُو پېنځ سوه لاسته ݫات ګلوبل ډېټاسېټس کی ګډ اېنڅئ Mozilla Data Collectiv که ا کمیونېټی يې ته کمیونېټی پاره جوړ دوک هۀ

## Pre-Release Announcement (dates TBD)

# Text wrapped in <cvDatasheetsLink></cvDatasheetsLink> will be rendered as an external link to the cv-datasheets GitHub repo
announcement-pre-release = خوېڒی ته کمیونېټی واله وئ، ته ریلیز ا وخت بویٛ هۀ۔ ته توثیق کؤ او ته خوئ زبان کی وه ته پلټؤ ا کر شُوري کوئ، او ا cv-datasheets ګۀ جیر کوئ که ا ډېټاشیټن اپډېټ سَۀ

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = ته تلاش پاره ا ټائنګ شوري کَۀ
searchbox-default-placeholder = تلاش وه کَۀ
searchbox-default-aria-label = دُشؤ
dropdown-no-results = مېچ وې نک ووک ݭیوک
# Common Voice Dataset=Language search box
dataset-searchbox-label = ډِسپلے او ډېټاسېټ زبان ا دُوګډ بُو بدلَوی
dataset-searchbox-placeholder = ته زبان ا تلاش
dataset-searchbox-aria-label = تلاش ته زبان پاره
dataset-search-no-results = مېچ وې نک ووک ݭیوک
request-new-language = ته نیوو زبان پاره درخواست
