# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Afar
aat = Arvanitique
ab = Abkhaze
abb = Bankon
abq = Abaza
ace = Aceh
ady = Adyguéen
af = Afrikaans
aii = Soureth
ajg = Aja
aln = Guègue
am = Amharique
an = Aragonais
apc = Arabe levantin
ar = Arabe
arn = Mapuche
as = Assamais
ast = Asturien
az = Azerbaïdjanais
azz = Nahuatl des montagnes du Puebla
ba = Bashkir
bag = Tuki
bal = Baloutchi
bas = Basaa
bax = Bamoun
bba = Bariba
bbj = Ghomalaʼ
bbl = Bats
bce = Mengambo
bci = Baoulé
be = Biélorusse
beb = Bebele
bew = Betawi
bfd = Bafut
bfq = Badaga
bft = Balti
bg = Bulgare
bgp = Baloutchi oriental
bin = Édo
bkh = Bakoko
bkm = Kom
blk = Pa’O
bm = Bambara
bn = Bengali
bnm = Batanga
bnn = Bunun
bo = Tibétain
br = Breton
brh = Brahoui
bri = Bakweri
brx = Bodo
bs = Bosniaque
bsh = Kateviri
bsk = Bourouchaski
bsy = Sabah Bisaya
btv = Bateri
bum = Boulou
bxk = Bukusu
bxr = Bouriate
byv = Medumba
ca = Catalan
cak = Cakchiquel
cdo = Mindong
ce = Tchétchène
cgg = Kiga
cjk = Tchokwé
ckb = Kurde central
cnh = Hakha Chin
co = Corse
cpx = Puxian
cpy = Ashéninka du sud d’Ucayali
crh = Tatar de Crimée
cs = Tchèque
cut = Cuicatèque de Teutila
cux = Cuicatèque de Tepeuxila
cv = Tchouvache
cy = Gallois
da = Danois
dag = { "" }
dar = Dargwa
dav = Taita
de = Allemand
din = Dinka
dmk = Domaaki
dml = Dameli
dru = Rukai
dsb = Bas-sorabe
dua = Duala
dv = Maldivien
dyu = Dioula
dz = Dzongkha
ebr = Tchaman
ee = Éwé
efk = Efik
eko = Ekoti
el = Grec
en = Anglais
eo = Espéranto
es = Espagnol
esu = Yupik de l’Alaska central
et = Estonien
eto = Eton
eu = Basque
ewo = Ewondo
fa = Persan
fan = Fang
ff = Peul
fi = Finnois
fmp = Feʼefeʼe
fo = Féroïen
fr = Français
fub = Peul de l’Adamaoua
fue = Peul de Borgou
fuf = Pular de Guinée
fy-NL = Frison
ga-IE = Irlandais
gaa = Ga
gcf = Créole martiniquais
gej = Mina
ggg = Gurgula
gid = Guidar
gig = Goaria
giz = Guiziga
gjk = Kachi
gju = Gojri
gl = Galicien
glk = Guilaki
gn = Guarani
gom = Goan Konkani
gos = Groningois
gsw = Alsacien
gu-IN = Gujarati
guc = Wayuu
gv = Mannois
gwc = Gaouri
gwt = Gawar-bati
gya = Gbaya du Nord-Ouest
ha = Haoussa
hac = Gurani
hak = Hakka
haz = Hazaragi
hch = Huichol
he = Hébreu
hem = Hemba
hi = Hindi
hil = Hiligaïnon
hno = Hindko du Nord
hr = Croate
hrx = Hunsrik
hsb = Haut-sorabe
ht = Haïtien
hu = Hongrois
hux = Witoto nipode
hy-AM = Arménien
ia = Interlingua
iba = Iban
ibb = Ibibio
id = Indonésien
ie = Interlingue
ig = Igbo
ipk = Inupiaq
is = Islandais
it = Italien
izh = Ingrien
ja = Japonais
jam = Créole jamaïcain
jbo = Lojban
jgo = Ngomba
jqr = Jaqaru
jv = Javanais
ka = Géorgien
kaa = Karakalpak
kab = Kabyle
kam = Kamba
kbd = Kabarde
kcn = Nubi
kdh = Kotokoli
khw = Khowar
ki = Kikuyu
kjh = Khakasse
kk = Kazakh
kl = Groenlandais
kln = Kalendjin
kls = Kalasha
km = Khmer
kmr = Kurmandji kurde
kn = Kannada
knn = Konkani (devanagari)
ko = Coréen
koo = Konjo
kpv = Komi-zyriène
krc = Karatchaï balkar
ks = Cachemiri
ksf = Bafia
kvx = Parkari
kw = Cornique
kxp = Wadiyari
ky = Kirghize
kzi = Kelabit
laj = Leblango
lb = Luxembourgeois
led = Lendu
leu = Kara
lg = Luganda
lij = Ligure
lke = Kenyi
lld = Ladin
ln = Lingala
lo = Lao
lrk = Loarki
lrl = Achomi
lss = Lassi
lt = Lituanien
ltg = Latgalien
lth = Thur
lua = Tshiluba
luo = Luo
lv = Letton
lzz = Laze
mai = Maïthili
mau = Mazatèque de Huautla
mbf = Malais baba
mbo = Mbo
mcf = Matses
mcn = Massa
mcx = Mpiemo
mdd = Mboum
mdf = Mokcha
meh = Mixtèque de Yucuhiti
mel = Melanau
mfe = Créole mauricien
mg = Malgache
mgg = Konabembe
mhk = Mungaka
mhr = Mari des prairies
mk = Macédonien
mki = Dhatki
ml = Malayalam
mmc = Mazahua occidental
mn = Mongol
mni = Meitei Lon
mnw = Môn
mos = Moré
mqh = Mixtèque de Tlazoyaltepec
mr = Marathe
mrh = Mara
mrj = Mari des montagnes
ms = Malais
mse = Moussey
msi = Malais du Sabah
mt = Maltais
mua = Moundang
mug = Muskum
mui = Musi
mve = Marwari pakistanais
mvy = Kohistani de l’Indus
mxu = Mada
my = Birman
myv = Erzya
mzn = Mazandarani
nan-tw = Taïwanais (Minnan)
nb-NO = Norvégien bokmål
ncx = Nahuatl central de Puebla
nd = Ndébélé du Nord
ne-NP = Népalais
new = Newari
nhe = Nahuatl de la Huasteca oriental
nhi = Nahuatl de la Sierra occidentale de Puebla
nia = Nias
nl = Néerlandais
nla = Ngombale
nlv = Nahuatl d’Orizaba
nmg = Kwasio
nmz = Losso
nn-NO = Norvégien nynorsk
nnh = Ngiemboon
nqo = N’ko
nr = Ndébélé du Sud
nso = Sotho du Nord
ny = Chewa
nyn = Nkore
nyu = Nyungwe
oc = Occitan
odk = Od
om = Afaan Ormoo
or = Odia
oru = Ormuri
os = Ossète
pa-IN = Pendjabi
pap = Papiamento
pap-AW = Papiamento (Aruba)
pau = Palau
pcd = Picard
pcm = Pidgin nigérian
pez = Punan oriental
phl = Phalura
phr = Pahari
pl = Polonais
plk = Shina du Kohistani
pne = Punan occidental
prq = Ashéninka perené
ps = Pachto
pt = Portugais
pua = Purépecha des montagnes de l’Ouest
pwn = Paiwan
quc = K'iche'
qug = Quechua de la région montagneuse du Chimborazo
qup = Quechua du sud du Pastaza
qur = Quechua de Yanahuanca
qus = Quechua de Santiago del Estero
qux = Quechua de Yauyos
quy = Quechua Chanka
qva = Quechua d’Ambo-Pasco
qvi = Kichwa
qvj = Kichwa des montagnes de Loja
qvl = Quechua de Cajatambo
qwa = Quechua de Corongo en Áncash
qws = Quechua de Sihuas en Áncash
qxa = Quechua de Chiquián
qxp = Quechua de Puno
qxq = Kachkaï
qxt = Quechua de Santa Ana de Tusi du Pasco
qxu = Quechua d’Arequipa-La Unión
qxw = Quechua de Jauja Wanka
rif = Rifain
rki = Arakanais
rm-sursilv = Romanche sursilvan
rm-vallader = Bas-engadinois
rn = Kirundi
ro = Roumain
rof = Kirombo
ru = Russe
ruc = Ruuli-nyala
rup = Aroumain
rw = Kinyarwanda
rwm = Amba
sah = Iakoute
sat = Santali (Ol Chiki)
sba = Ngambay
sbn = Sindhi Bhili
sc = Sarde
scl = Shina
scn = Sicilien
sco = Écossais
sd = Sindhi
sdh = Kurde du Sud
sdo = Serian Bidayuh
seh = Sena
sei = Seri
sg = Sango
shi = Tachelhit/chleuh
shn = Shan
si = Sinhala
sk = Slovaque
skr = Saraiki
sl = Slovène
sn = Shona
snk = Soninké
snv = Sa'ban
so = Somali
sq = Albanais
sr = Serbe
ss = Swati
ssi = Sansi
st = Sotho du Sud
su = Soundanais
sv-SE = Suédois
sva = Svane
sw = Swahili
swb = Mahorais
syr = Syriaque
szy = Sakizaya
ta = Tamoul
tar = Tarahumara central
tay = Atayal
te = Télougou
teg = Téké
tg = Tadjik
th = Thaï
ti = Tigrigna
tig = Tigré
tiv = Tiv
tk = Turkmène
tl = Tagalog
tli = Tlingit
tn = Tswana
tob = Toba
tok = Toki Pona
top = Totonaque de Papantla
tr = Turc
trv = Seediq
trw = Torwali
ts = Tsonga
tt = Tatar
ttj = Rutoro
tui = Toupouri
tum = Tumbuka
tuq = Tedaga
tvu = Tunen
tw = Twi
ty = Tahitien
tyv = Touvain
uby = Oubykh
udl = Ouldémé
udm = Oudmourte
ug = Ouïghour
uk = Ukrainien
ukv = Kuku
ur = Ourdou
ush = Ushojo
usp = Uspanteko
uz = Ouzbek
var = Guarijio
ve = Venda
vec = Vénitien
vi = Vietnamien
vls = Flamand occidental
vmw = Makua
vot = Vote
wbl = Wakhi
wep = Westphalien
wes = Pidgin du Cameroun
wo = Wolof
xdq = Kaïtag
xh = Xhosa
xhe = Khetrani
xka = Kalkoti
xkl = Kenyah
xmf = Mingrélien
xsm = Kassem
yaq = Yaqui
yav = Nuasue
ydg = Yidgha
yi = Yiddish
yo = Yoruba
yue = Cantonais
zgh = Amazighe
zh-CN = Chinois (Chine)
zh-HK = Chinois (Hong-Kong)
zh-TW = Chinois (Taïwan)
zoc = Zoque du Chiapas
zu = Zoulou
zza = { "" }

## Layout

profile = Profil
help = Aide
contact = Nous contacter
privacy = Vie privée
terms = Conditions
cookies = Cookies
faq = Questions fréquentes
documentation = Documentation
content-license-text = Contenu disponible sous licence <licenseLink>Creative Commons</licenseLink>
share-title = Aidez-nous à collecter davantage de voix !
share-text = Aidez les machines à apprendre à parler comme de vraies personnes en donnant votre voix sur { $link }
link-copied = Lien copié
back-top = Haut de la page
logout = Déconnexion
donate = Faire un don
stayintouch = Chez Mozilla, nous rassemblons une communauté autour des technologies vocales. Nous aimerions rester en contact avec vous pour vous tenir au courant des nouveautés, des nouvelles sources de données et aussi pour découvrir comment vous utilisez ces données.
privacy-info = Nous promettons de prendre soin de vos données. Apprenez-en davantage en consultant notre <privacyLink>Politique de confidentialité</privacyLink>.
return-to-cv = Retourner vers Common Voice
email-input =
    .label = Adresse e-mail
loading = Chargement…
email-opt-in-info = J’aimerais recevoir des e-mails tels que des rappels d’objectifs, des rapports sur mes progrès et les lettres d’information de Common Voice.
email-opt-in-info-title = Rejoignez la liste de diffusion Common Voice
email-opt-in-info-sub-with-challenge = Recevez des e-mails tels que des défis, des rappels d’objectifs, des rapports d’avancement et des lettres d’information sur Common Voice.
email-opt-in-privacy-v2 = En acceptant de recevoir des e-mails, vous indiquez accepter que Mozilla gère ces informations, conformément à la <privacyLink>politique de confidentialité</privacyLink> de Mozilla.
indicates-required = * indique un champ obligatoire
not-available-abbreviation = Non disponible
# Text shown on a button that can be clicked to copy text to the clipboard
copy = Copier
# Text shown on a button that can be clicked to close a modal or panel
close = Fermer
# Text shown on a button that can be clicked to cancel an action
cancel = Annuler
# Text shown on a button that can be clicked to delete an item
delete = Supprimer

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>Nous avons besoin</mark><br/><mark>de votre aide !</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = Héberger les jeux de données et améliorer la plateforme pour les plus de 100 communautés linguistiques qui font confiance à notre service coûte près d’un million de dollars par an. <strong>Si vous appréciez les données ouvertes et inclusives, faites un don aujourd’hui !</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>Vous utilisez CV pour</mark><br/><mark>vos recherches ?</mark>
languages-donate-banner-cta-explanation = Common Voice est financé par des dons et des subventions ! Nous aimons collaborer avec des universitaires et des chercheur·euse·s issu·e·s de la société civile ou de l’industrie. Common Voice est gratuit, mais contribuer à la plateforme et aux coûts d’hébergement en proposant des subventions est vraiment utile.
localization-select =
    .label = Choisissez la langue/la localisation

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = <strong>De nouveaux jeux de données Common Voice</strong> sont disponibles au téléchargement exclusivement via notre plateforme jumelle, Mozilla Data Collective. Rejoignez-nous pour accéder à plus de 300 jeux de données mondiaux, créés par et pour la communauté.
# Button text for MDC announcement
announcement-mdc-button-text = Rejoindre le Mozilla Data Collective
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = S’ouvre dans un nouvel onglet

## Release Delay Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-delay = Chère communauté, nous travaillons actuellement sur la prochaine version. Nous prévoyons qu’elle sera déployée progressivement dans les prochains jours sur <mdcLink>Mozilla Data Collective</mdcLink>.

## Release Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release = Chère communauté, la nouvelle version est maintenant entièrement disponible sur <mdcLink>Mozilla Data Collective</mdcLink>. Inscrivez-vous pour accéder à plus de 500 jeux de données mondiaux, créés par et pour la communauté.

## Pre-Release Announcement (dates TBD)

# Text wrapped in <cvDatasheetsLink></cvDatasheetsLink> will be rendered as an external link to the cv-datasheets GitHub repo
announcement-pre-release = Chère communauté, la prochaine version est en approche. Merci de valider et de retranscrire, et pensez également à consulter les <cvDatasheetsLink>cv-datasheets</cvDatasheetsLink> afin de mettre à jour votre fiche technique.

## Release Commencing

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-commencing = Chère communauté, nous avons atteint la date limite pour la publication et nous sommes en train de regrouper les jeux de données. La nouvelle version sera disponible sur <mdcLink>Mozilla Data Collective</mdcLink> d’ici quelques jours.

## Downtime Announcements

# Generic scheduled downtime for next Sunday early hours
announcement-scheduled-downtime = Chère communauté, afin d’optimiser nos systèmes, nous prévoyons de brèves interruptions de service, qui ne durent généralement que quelques heures. La prochaine aura lieu tôt ce dimanche matin. Contactez-nous sur Matrix en cas de problème.

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = Commencez à écrire pour rechercher
searchbox-default-placeholder = Rechercher…
searchbox-default-aria-label = Rechercher
dropdown-no-results = Aucune correspondance
# Common Voice Dataset=Language search box
dataset-searchbox-label = Changer la langue d’affichage et la langue du jeu de données
dataset-searchbox-placeholder = Rechercher une langue…
dataset-searchbox-aria-label = Rechercher une langue
dataset-search-no-results = Aucune langue correspondante n’a été trouvée
request-new-language = Proposer une nouvelle langue
