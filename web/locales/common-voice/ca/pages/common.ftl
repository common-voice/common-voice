# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = àfar
aat = arvanitika
ab = abkhaz
abb = bankon
abq = abaza
ace = acehnès
ady = adigué
af = afrikaans
aii = suret
ajg = adja
aln = gheg albanès
am = amhàric
an = aragonès
ar = àrab
arn = mapudungun
as = assamès
ast = asturià
az = àzeri
azz = nàhuatl de les Terres Altes de Puebla
ba = baixkir
bag = tuki
bal = balochi
bas = basaa
bax = bamun
bba = baatonum
bbj = ghomala
bbl = tush
bce = mengambo
bci = baoule
be = bielorús
beb = bamvele
bew = betawi
bfd = bafut
bft = balti
bg = búlgar
bgp = balochi oriental
bkh = bakoko
bkm = kom
bm = bambara
bn = bengalí
bnm = batanga
bnn = bunun
bo = tibetà
br = bretó
brh = brahui
bri = mokpwe
brx = bodo
bs = bosnià
bsh = kateviri
bsk = brushaski
bsy = sabah Bisaya
btv = bateri
bum = bulu
bxk = bukusu
bxr = buriat
byv = medumba
ca = català
cak = kaqtxikel
cdo = min oriental
cjk = chokwe
ckb = kurd central
cnh = hakha chin
co = cors
cpx = heng hua
cpy = asheninka del sud d'Ucayali
crh = tàtar de Crimea
cs = txec
cut = cuicateca de Teutila
cux = cuicateca de Tepeuxila
cv = txuvaix
cy = gal·lès
da = danès
dag = dagbani
dar = dargwa
dav = kidaw'ida
de = alemany
din = dinka
dmk = dawoodi
dml = dameli
dru = rukai
dsb = baix sòrab
dua = duala
dv = divehi
dyu = julà
ebr = ebri
ee = ewe
eko = ekoti
el = grec
en = anglès
eo = esperanto
es = espanyol
esu = yupikik central d'Alaska
et = estonià
eto = eton
eu = basc
ewo = ewondo
fa = persa
fan = fang
ff = ful
fi = finès
fmp = bafang
fo = feroès
fr = francès
fuf = ful de Guinea
fy-NL = frisó
ga-IE = irlandès
gaa = ga
gej = mina
ggg = gurgula
gid = guidar
gig = goària
giz = guiziga
gjk = cutchi
gju = gojari
gl = gallec
glk = guilequi
gn = guaraní
gom = konkani
gos = gronings
gsw = alsacià
gu-IN = gujarati
guc = wayuunaiki
gv = manx
gwc = gawri
gwt = gawarbaiti
gya = gbaya del nord-oest
ha = haussa
hac = gorani
hak = hakka
haz = hazargi
hch = huichol
he = hebreu
hem = zimba
hi = hindi
hil = híligaynon
hno = hindko septentrional
hr = croat
hrx = hunsrückisch de Rio Grande do Sul
hsb = alt sòrab
ht = haitià
hu = hongarès
hy-AM = armeni
ia = interlingua
iba = iban
ibb = ibibio
id = indonesi
ie = interlingue
ig = igbo
ipk = inupiatun
is = islandès
it = italià
izh = ingrià
ja = japonès
jam = Patois jamaicà
jbo = lojban
jgo = Ngomba
jqr = jaqaru
jv = javanès
ka = georgià
kaa = karakalpak
kab = cabilenc
kam = Kikamba
kbd = kabardí
kcn = nubi
kdh = Kotokoli
ki = kikuiu
kk = kazakh
kln = kalenjin
km = khmer
kmr = kurd del nord
kn = kannada
knn = konkani (devanagari)
ko = coreà
koo = Konjo
kpv = komi
krc = Karachái-bálkar
ks = Caixmirià
ksf = bafia
kvx = Parkari Koli
kw = còrnic
kxp = Wadiyara Koli
ky = kirguís
kzi = Kelabit
laj = lango
lb = luxemburguès
led = Lendu
leu = Kara
lg = ganda
lij = lígur
lke = Kenyi
lld = Ladí
ln = lingala
lo = laosià
lrk = Loarki
lrl = Achomi
lss = Lassi
lt = lituà
ltg = latgalià
lth = Thur
lua = Luba-Kasai
luo = luo
lv = letó
lzz = laz
mai = maithili
mau = Mazatec de Huautla
mbf = Baba Malay
mbo = mbo
mcf = Matsès
mcn = Massa
mcx = Mpiemo
mdd = Mbum
mdf = mokxa
meh = Mixteca de Yucuhiti
mel = Melanau
mfe = Crioll Morisien
mg = malgaix
mgg = Kunabembe
mhk = Mungaka
mhr = mari oriental
mk = macedònic
mki = Dhatti
ml = malaiàlam
mmc = Mazahua de Michoacán
mn = mongol
mni = meitei
mos = mossi
mqh = Mixteca de Tlazoyaltepec
mr = marathi
mrh = mara
mrj = mari occidental
ms = malai
mse = moussey
msi = malai de Sabah
mt = maltès
mua = mundang
mve = marwari de Pakistan
mvy = maiya
mxu = mada
my = birmà
myv = erzya
nan-tw = taiwanès (min nan)
nb-NO = Bokmål noruec
ncx = nàhuatl del centre de Puebla
nd = IsiNdebele septentrional
ne-NP = nepalès
new = newari
nhe = Nàhuatl de La Huasteca oriental
nhi = nàhuatl de la Sierra Occidental de Puebla
nia = nias
nl = neerlandès
nla = ngombale
nlv = nàhuatl d'Orizaba
nmg = kwasio
nmz = losso
nn-NO = noruec nynorsk
nnh = Ngiembon
nqo = N'Ko
nr = IsiNdebele meridional
nso = sotho del nord
ny = chinyanja
nyn = nyankole
nyu = nyungwe
oc = occità
om = oromo
or = oriya
oru = Ormuri
os = ossetià
pa-IN = panjabi
pap = Papiamento
pap-AW = papiament (Aruba)
pau = palauà
pcd = picard
pcm = Pidgin anglès nigerià
pez = penan oriental
phl = Palula
pl = polonès
plk = Shina de Kohistan
pne = penan occidental
ps = paixtú
pt = portuguès
pua = Purèpecha de la Serra Occidental
pwn = Paiwan
quc = quitxé
qug = quítxua del Chimborazo
qup = Quítxua del sud de Pastaza
qur = quítxua Yanahuanca
qus = quítxua de Santiago del Estero
qux = Quítxua Yauyos
quy = quítxua chanka
qva = Quítxua d'Ambo-Pasco
qvi = quítxua septentrional
qvj = quítxua de Loja
qvl = Quítxua de Cajatambo
qwa = Quítxua de Corongo Ancash
qws = Quítxua de Sihuas Ancash
qxa = quítxua Chiquián
qxp = Quítxua de Puno
qxq = qashqai
qxt = quítxua de Pasco
qxu = quítxua d'Arequipa-La Unión
qxw = quítxua Jauja Wanka
rif = rifeny
rm-sursilv = romanx (sobreselvà)
rm-vallader = romanx engiadinès
rn = krundi
ro = romanès
rof = kirombo
ru = rus
ruc = ruuli
rup = aromànic
rw = ruandès
rwm = amba
sah = iacut
sat = santali (ol chiki)
sc = sard
scl = shina
scn = sicilià
sco = escocès
sd = sindhi
sdh = kurd del sud
seh = sena
shi = chelha
shn = shan
si = singalès
sk = eslovac
skr = seraiki
sl = eslovè
sn = shona
snk = soninké
snv = saʼban
so = somali
sq = albanès
sr = serbi
ss = siswati
ssi = sansi
st = sesotho
su = sudanès
sv-SE = suec
sva = esvano
sw = suahili
syr = siríac
szy = sakizaya
ta = tamil
tar = tarahumara central
tay = atayal
te = telugu
teg = Teke
tg = tadjik
th = tailandès
ti = tigrinya
tig = tigre
tk = turcman
tl = tagal
tli = tlingit
tn = setswana
tok = toki pona
top = totonac de Papantla
tr = turc
trv = seediq
trw = torwali
ts = xitsonga
tt = tàtar
ttj = rutooro
tui = tupuri
tum = tumbuka
tvu = Tunen
tw = twi
ty = tahitià
tyv = tuvinià
uby = ubikh
udl = ouldémé
udm = udmurt
ug = uigur
uk = ucraïnès
ukv = kuku
ur = urdú
ush = ushojo
uz = uzbek
var = guarijío
ve = tshivenda
vec = vènet
vi = vietnamita
vmw = makhuwa
vot = vòtic
wbl = wají
wep = westfalià
wes = pidgin del Camerun
wo = wòlof
xdq = kaytak
xh = xhosa
xhe = ketrani
xka = kalkoti
xkl = kenyah
xmf = megrelià
xsm = kasem
yaq = yaqui
yav = nukalonge
ydg = yadgha
yi = ídix
yo = ioruba
yue = cantonès
zgh = amazic
zh-CN = xinès (Xina)
zh-HK = xinès (Hong Kong)
zh-TW = xinès (Taiwan)
zoc = zoque de Copainalá
zu = zulu
zza = zaza

# [/]


## Layout

profile = Perfil
help = Ajuda
contact = Contacte
privacy = Privadesa
terms = Condicions d'ús
cookies = Galetes
faq = PMF
documentation = Documentació
content-license-text = Contingut disponible en els termes de la <licenseLink>llicència Creative Commons</licenseLink>
share-title = Ajudeu-nos a trobar més persones que donin la veu!
share-text = Ensenyeu a les màquines com parlem la gent de debò. Col·laboreu amb la vostra veu a { $link }
link-copied = S'ha copiat l'enllaç
back-top = Torna a dalt
logout = Finalitza la sessió
donate = Feu un donatiu
stayintouch = A Mozilla estem construint una comunitat al voltant de les tecnologies de la veu. Voldríem estar al dia de les novetats, de noves fonts de dades i saber com useu aquestes dades.
privacy-info = Ens comprometem a gestionar la vostra informació amb cura. Per a més informació, vegeu l'<privacyLink>avís de privadesa</privacyLink>.
return-to-cv = Torna al Common Voice
email-input =
    .label = Adreça electrònica
loading = S'està carregant…
email-opt-in-info = M'agradaria rebre correus electrònics com ara recordatoris d’objectius, actualitzacions del meu progrés i butlletins de notícies sobre el Common Voice.
email-opt-in-info-title = Uniu-vos a la llista de correu del Common Voice
email-opt-in-info-sub-with-challenge = Rebreu correus com ara recordatoris d'objectius i reptes, actualitzacions del progrés i butlletins sobre el Common voice.
email-opt-in-privacy-v2 = En optar per rebre correus electrònics, accepteu que Mozilla gestioni aquesta informació en els termes descrits en la seva <privacyLink>política de privadesa</privacyLink>.
indicates-required = * Indica un camp necessari
not-available-abbreviation = N/D
# Text shown on a button that can be clicked to copy text to the clipboard
copy = Copia
# Text shown on a button that can be clicked to cancel an action
cancel = Cancel·la
# Text shown on a button that can be clicked to delete an item
delete = Suprimeix

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>Necessitem</mark><br/><mark>que ens ajudeu!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = Allotjar els conjunts de dades i millorar la plataforma per a les més de 100 comunitats lingüístiques que depenen del que fem costa gairebé un milió de dòlars l'any. <strong>Si valoreu les dades obertes i inclusives, feu un donatiu avui mateix!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>Feu servir CV per</mark><br/><mark>a la recerca?</mark>
languages-donate-banner-cta-explanation = El Common Voice es finança amb donatius i subvencions! Ens encanta col·laborar amb investigadors acadèmics, de la societat civil i de la indústria. El Common Voice és d'ús gratuït, però ens serà molt útil si podeu ajudar amb els costos de la plataforma i de l'allotjament mitjançant propostes de subvencions.
localization-select =
    .label = Trieu una llengua/localització

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = Ja estan disponibles <strong>nous conjunts de dades de Common Voice</strong> per a baixar, de manera exclusiva, a través de la nostra plataforma germana, Mozilla Data Collective. Uniu-vos-hi per a accedir a més de 300 conjunts de dades globals, creats per i per a la comunitat.
# Button text for MDC announcement
announcement-mdc-button-text = Uneix-te a Mozilla Data Collective
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = S'obre en una pestanya nova
