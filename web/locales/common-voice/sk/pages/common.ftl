# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Afarčina
aat = Arvanitika
ab = Abcházčina
abb = Bankón
abq = Abaza
ace = Acehčina
ady = Adygejčina
af = Afrikánčina
aii = Suret
ajg = Adja
aln = Gheg albánsky
am = Amharčina
an = Aragónčina
ar = Arabčina
arn = Mapudungun
as = Ásámčina
ast = Astúrčina
az = Azerbajdžančina
azz = Highland Puebla Nahuatl
ba = Baškirčina
bag = Tuki
bal = Balúččina
bas = Basaa
bax = Bamun
bba = Baatonum
bbj = Ghomala
bbl = Tush
bce = Mengambo
bci = Baoule
be = Bieloruština
beb = Bamvele
bew = Betawi
bfd = Bafut
bft = Balti
bg = Bulharčina
bgp = Východné Balúdže
bin = Edo
bkh = Bakoko
bkm = Kom
bm = Bambarčina
bn = Bengálčina
bnm = Batanga
bnn = Bunun
bo = Tibetčina
br = Bretónčina
brh = Brahui
bri = Mokpwe
brx = Bodo
bs = Bosniančina
bsh = Kateviri
bsk = Brushaski
bsy = Sabah Bisaya
btv = Bateri
bum = Bulu
bxk = Bukusu
bxr = Burjatčina
byv = Medumba
ca = Katalánčina
cak = Kaqchikel
cdo = Východný Min
ce = Čečenčina
cgg = Čiga
cjk = Čokwe
ckb = Centrálna kurdčina
cnh = Hakha Chin
co = Korzičtina
cpx = Heng Hua
cpy = Asheninka Južné Ucayali
crh = Krymská tatárčina
cs = Čeština
cut = Teutila Cuicatec
cux = Tepeuxila Cuicatec
cv = Čuvaščina
cy = Waleština
da = Dánčina
dag = Dagbani
dar = Dargwa
dav = Kidaw'ida
de = Nemčina
din = Dinka
dmk = Dawoodi
dml = Dameli
dru = Rukai
dsb = Lužická srbčina, dolná
dua = Duala
dv = Dhivehi
dyu = Dioula
ebr = Ebrie
ee = Ewe
efk = Efik
eko = Ekoti
el = Gréčtina
en = Angličtina
eo = Esperanto
es = Španielčina
esu = Stredoaljašský Yupʼik
et = Estónčina
eto = Eton
eu = Baskičtina
ewo = Ewondo
fa = Perzština
fan = Fang
ff = Fulah
fi = Fínčina
fmp = Feʼefeʼe
fo = Faerčina
fr = Francúzština
fub = Adamawa Fulfulde
fue = Borgu Fulfulde
fuf = Pularčina, Guinea
fy-NL = Frízčina
ga-IE = Írčina
gaa = Ga
gcf = Martinická kreolčina
gej = Mina
ggg = Gurgula
gid = Guidar
gig = Goaria
giz = Guiziga
gjk = Kachhi
gju = Gudžarátčina
gl = Galícijčina
glk = Gilaki
gn = Guarančina
gom = Konkáni (rómčina)
gos = Gronings
gsw = Alsasčina
gu-IN = Gudžarátčina
guc = Wayuunaiki
gv = Manština
gwc = Gawri
gwt = Gawarbaiti
gya = Severozápadná Gbaya
ha = Hausa
hac = Gorani
hak = Hakka
haz = Hazargi
hch = Wixárika
he = Hebrejčina
hem = Kihemba
hi = Hindčina
hil = Hiligaynon
hno = Severný Hindko
hr = Chorvátčina
hrx = Hunsrik
hsb = Lužická srbčina, horná
ht = Haitská kreolčina
hu = Maďarčina
hux = Nüpode Huitoto
hy-AM = Arménčina
ia = Interlingua
iba = Iban
ibb = Ibibio
id = Indonézština
ie = Interlingue
ig = Igbčina
ipk = Inupiak
is = Islandčina
it = Taliančina
izh = Ižorian
ja = Japončina
jam = Jamajský patois
jbo = Lojban
jgo = Ngomba
jqr = Džakáru
jv = Jávčina
ka = Gruzínčina
kaa = Karakalpak
kab = Kabyle
kam = Kikamba
kbd = Kabardčina
kcn = Núbi
kdh = Kotokoli
khw = Khowar
ki = Kikujčina
kjh = Chakasi
kk = Kazaština
kln = Kalenjin
kls = Kalaša
km = Khmérčina
kmr = Kurdčina
kn = Kannadčina
knn = Konkánčina (dévanágarí)
ko = Kórejčina
koo = Konzo
kpv = Komi-Zyrianska
krc = Karachay-Balkar
ks = Kašmírčina
ksf = Bafia
kvx = Parkari Koli
kw = Cornwallčina
kxp = Wadiyara Koli
ky = Kirgizčina
kzi = Kelabit
laj = Lango
lb = Luxemburčina
led = Lendu
leu = Kara
lg = Lugandčina
lij = Ligúrčina
lke = Kenyi
lld = Ladinčina
ln = Lingalčina
lo = Laoština
lrk = Loarki
lrl = Achomi
lss = Lassi
lt = Litovčina
ltg = Latgalčina
lth = Thur
lua = Čiluba
luo = Dholuo
lv = Lotyština
lzz = Laz
mai = Maithilčina
mau = Huautla Mazatec
mbf = Baba Malaj
mbo = Mbo
mcf = Matses
mcn = Massa
mcx = Mpiemo
mdd = Mbum
mdf = Mókša
meh = Mixteco Yucuhiti
mel = Melanau
mfe = Kreolský Morisien
mg = Madagaskarčina
mgg = Kunabembe
mhk = Mungaka
mhr = Lúčna Mari
mk = Macedónčina
mki = Dhatki
ml = Malajálamčina
mmc = Michoacán Mazahua
mn = Mongolčina
mni = Meetei Lon
mnw = Mon
mos = Mossi
mqh = Tlazoyaltepec Mixtec
mr = Maráthčina
mrh = Mara
mrj = Hill Mari
ms = Malajčina
mse = Moussey
msi = Sabah Malajčina
mt = Maltčina
mua = Mundang
mug = Musgum
mve = Pakistanská Marwari
mvy = Indus Kohistani
mxu = Mada
my = Barmčina
myv = Erzja
nan-tw = Taiwančina (Minnan)
nb-NO = Nórčina (Bokmål)
ncx = Nahuatl v centrálnej Pueble
nd = IsiNdebele (sever)
ne-NP = Nepálčina
new = Newari
nhe = Východná huastecká nahuatlčina
nhi = Západná Sierra Puebla Nahuatl
nia = Nias
nl = Holandčina
nla = Ngombale
nlv = orizaba nahuatl
nmg = kwasio
nmz = losso
nn-NO = Nórčina (Nynorsk)
nnh = Ngiembon
nqo = NKo
nr = IsiNdebele (juh)
nso = Severné Sotho
ny = Činjanja
nyn = Runyankole
nyu = nyungwe
oc = Okcitánčina
odk = oadki
om = Afaan Oromo
or = Odia
oru = Ormuri
os = Osetčina
pa-IN = Pandžábčina
pap = Papiamento
pap-AW = Papiamento (Aruba)
pau = Palauánčina
pcd = Picard
pcm = Nigérijská lámaná angličtina
pez = Východný Penan
phl = Palula
phr = Pahari-Pothwari
pl = Poľština
plk = Kohistiánska Šina
pne = Západný Penan
prq = Ašeninka Perene
ps = Paštčina
pt = Portugalčina
pua = Západná vysočina Purepecha
pwn = Paiwan
quc = K'iche'
qug = Quichua Chimborazo Highlands
qup = Južná kečuánska pastaza
qur = Kečuánska Yanahuanca
qus = Kečuánčina v Santiagu del Estero
qux = Kečujskí Yauyos
quy = Kečuánska čanka
qva = Kečuánska Ambo-Pasco
qvi = Kičva
qvj = Loja Highland Kichwa
qvl = Kečuánska Cajatambo
qwa = Kečuánska Corongo Ancash
qws = Kečuánska Sihua Ancash
qxa = Kečuánska čiquiánska
qxp = Puno Quechua
qxq = Qashqai
qxt = Quechua Pasco Santa Ana de Tusi
qxu = Kečuánska Arequipa-La Unión
qxw = Kečuánska Jauja Wanka
rif = Tarifit
rki = Rakhine
rm-sursilv = Rétorománčina (sursilvan)
rm-vallader = Rétorománčina (vallader)
rn = Kirundi
ro = Rumunčina
rof = Kirombo
ru = Ruština
ruc = Ruuli
rup = Arómčina
rw = Kinyarwanda
rwm = Amba
sah = Sakha
sat = Santali (Ol Chiki)
sba = Ngambay
sbn = Sindhi Bhili
sc = Sardínčina
scl = Šina
scn = Sicílčina
sco = Škótčina
sd = Sindhčina
sdh = Južná kurdčina
sdo = Serian Bidayuh
seh = Sena
sei = Seri
sg = Sangro
shi = Šilha
shn = Šan
si = Sinhálčina
sk = Slovenčina
skr = Saraiki
sl = Slovinčina
sn = Šónčina
snk = Soninke
snv = Sa'ban
so = Somálčina
sq = Albánčina
sr = Srbčina
ss = Siswati
ssi = Sansi
st = Južné Sotho
su = Sudánčina
sv-SE = Švédčina
sva = svan
sw = Svahilčina
swb = Shimaore
syr = Sýrčina
szy = Sakizaya
ta = Tamilčina
tar = Centrálna Tarahumara
tay = Atayal
te = Telugčina
teg = Teke
tg = Tadžičtina
th = Thajčina
ti = Tigriňa
tig = Tigre
tiv = Tiv
tk = Turkménčina
tl = Tagalčina
tli = Tlingit
tn = Setswana
tob = Toba Qom
tok = Toki Pona
top = Papantla Totonac
tr = Turečtina
trv = Seediq
trw = Torwali
ts = Xitsonga
tt = Tatárčina
ttj = Rutoro
tui = Tupuri
tum = Tumbuka
tuq = Tedaga
tvu = Tunen
tw = Twi
ty = Tahitčina
tyv = Tuvinčina
uby = Ubych
udl = Ouldémé
udm = Udmurtčina
ug = Ujgurčina
uk = Ukrajinčina
ukv = Kuku
ur = Urdčina
ush = Ushojo
usp = Uspanteko
uz = Uzbečtina
var = Huarijio
ve = Čivenda
vec = Benátčina
vi = Vietnamčina
vmw = Emakhuwa
vot = Votic
wbl = Wakhi
wep = Vestfálčina
wes = Kamerunská lámaná čínština
wo = Volofčina
xdq = Kaitag
xh = Xhoština
xhe = Ketrani
xka = Kalkoti
xkl = Keňa
xmf = Mingrelian
xsm = Kásem
yaq = Yaqui
yav = Nuasue
ydg = Jadgha
yi = Jidiš
yo = Jorubčina
yue = Kantončina
zgh = Tamazight
zh-CN = Čínština (Čína)
zh-HK = Čínština (Hongkong)
zh-TW = Čínština (Taiwan)
zoc = Copainalá Zoque
zu = Zulu
zza = Zaza

# [/]


## Layout

profile = Profil
help = Pomocník
contact = Kontakt
privacy = Súkromie
terms = Podmienky
cookies = Cookies
faq = FAQ
documentation = Dokumentácia
content-license-text = Obsah je dostupný v rámci licencie <licenseLink>Creative Commons</licenseLink>.
share-title = Pomôžte nám nájsť ďalších, ktorí prispejú svojím hlasom!
share-text = Pomôžte strojom pochopiť ľudskú reč. Prispejte svojím hlasom na { $link }
link-copied = Odkaz bol skopírovaný
back-top = Návrat hore
logout = Odhlásiť sa
donate = Prispieť
stayintouch = V Mozille budujeme komunitu okolo technológií rozpoznávania reči. Radi by sme s vami zostali v kontakte prostredníctvom noviniek, napríklad o nových zdrojoch údajov. Radi by sme sa taktiež dozvedeli, ako tieto údaje používate.
privacy-info = Sľubujeme, že sa o vaše informácie budeme príkladne starať. Ďalšie informácie sa dočítate v našom <privacyLink>Vyhlásení o ochrane osobných údajov</privacyLink>.
return-to-cv = Návrat do Common Voice
email-input =
    .label = E‑mailová adresa
loading = Načítava sa…
email-opt-in-info = Chcem dostávať e‑maily, ako sú upozornenia na ciele, môj pokrok a novinky o službe Common Voice.
email-opt-in-info-title = Pripojte sa do e‑mailovej skupiny Common Voice
email-opt-in-info-sub-with-challenge = Dostávajte e‑maily, ako sú upozornenia na ciele či výzvy, informácie o pokroku a novinky o službe Common Voice.
email-opt-in-privacy-v2 = Prihlásením sa na odber e‑mailov vyjadrujete, že súhlasíte s tým, že Mozilla narába s týmito informáciami tak, ako je vysvetlené v <privacyLink>Zásadách ochrany osobných údajov</privacyLink> spoločnosti Mozilla.
indicates-required = * označuje povinné pole
not-available-abbreviation = N/A
# Text shown on a button that can be clicked to copy text to the clipboard
copy = Kopírovať
# Text shown on a button that can be clicked to close a modal or panel
close = Zavrieť
# Text shown on a button that can be clicked to cancel an action
cancel = Zrušiť
# Text shown on a button that can be clicked to delete an item
delete = Odstrániť

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>Potrebujeme</mark><br/><mark>vašu pomoc!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = Hosťovanie súborov údajov a zlepšenie platformy pre viac ako 100 jazykových komunít, ktoré sa spoliehajú na to, čo robíme, stojí takmer milión dolárov ročne. <strong>Ak si ceníte otvorené, inkluzívne dáta – pošlite dar ešte dnes!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>Používate Common Voice</mark><br/><mark>na svoj výskum?</mark>
languages-donate-banner-cta-explanation = Common Voice je financovaný z darov a grantov! Radi spolupracujeme s akademikmi, občianskou spoločnosťou a priemyselnými výskumníkmi. Používanie Common Voice je bezplatné, ale prispievanie na náklady na platformu a hosťovanie prostredníctvom návrhov grantov je skutočne užitočné.
localization-select =
    .label = Vyberte jazyk/lokalizáciu

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = <strong>Nové kolekcie údajov Common Voice</strong> sú teraz k dispozícii na stiahnutie exkluzívne prostredníctvom našej sesterskej platformy Mozilla Data Collective. Pripojte sa a získajte prístup k viac ako 300 globálnym kolekciám údajov, ktoré vytvorila komunita pre komunitu.
# Button text for MDC announcement
announcement-mdc-button-text = Pridajte sa k Mozilla Data Collective
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = Otvorí sa na novej karte

## Release Delay Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-delay = Vážení členovia komunity, momentálne pracujeme na vydaní tejto aktualizácie. Očakávame, že bude v najbližších dňoch postupne sprístupňovaná na <mdcLink>Mozilla Data Collective</mdcLink>.

## Release Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release = Vážení členovia komunity, nová verzia je teraz plne dostupná na platforme <mdcLink>Mozilla Data Collective</mdcLink>. Pridajte sa a získajte prístup k viac ako 500 globálnym kolekciách údajov, ktoré vytvorila komunita pre komunitu.

## Pre-Release Announcement (dates TBD)

# Text wrapped in <cvDatasheetsLink></cvDatasheetsLink> will be rendered as an external link to the cv-datasheets GitHub repo
announcement-pre-release = Vážení členovia komunity, čas vydania aktualizácie sa blíži. Prosím, dokončite overovanie a prepisy a tiež navštívte <cvDatasheetsLink>cv-datasheets</cvDatasheetsLink> a aktualizujte si dátový list.

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = Začať vyhľadávanie
searchbox-default-placeholder = Hľadať...
searchbox-default-aria-label = Hľadať
dropdown-no-results = Nenašla sa žiadna zhoda
# Common Voice Dataset=Language search box
dataset-searchbox-label = Zmení jazyk zobrazenia aj kolekcie údajov
dataset-searchbox-placeholder = Hľadať jazyk...
dataset-searchbox-aria-label = Vyhľadať jazyk
dataset-search-no-results = Hľadanému výrazu nezodpovedá žiadny jazyk
request-new-language = Požiadať o nový jazyk
