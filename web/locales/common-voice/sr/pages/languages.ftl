## Languages Overview

language-section-in-progress = У току
language-section-in-progress-new-description = Ови језици су тренутно у развоју заједнице. Траке напретка показују докле је сваки језик стигао у процесима <localizationGlossaryLink>локализације странице</localizationGlossaryLink> и <sentenceCollectionGlossaryLink>прикупљању реченица</sentenceCollectionGlossaryLink>.
language-section-launched = Покренут
language-section-launched-description = За ове језике веб страница је успешно <localizationGlossaryLink>локализована</localizationGlossaryLink>, и има довољно <sentenceCollectionGlossaryLink>прикупљених реченица</sentenceCollectionGlossaryLink> да би се омогућио стални допринос <speakLink>говора</speakLink> и <listenLink>слушања</listenLink> .
# lastUpdatedTimeStamp is a timestamp that indicates when the language stats was last updated
language-section-last-updated = Последње ажурирање: { $lastUpdatedTimeStamp }
language-communities = Размислите о додавању канала ваше заједнице у <communitiesLink>COMMUNITIES.md документ на GitHub-у</communitiesLink>.
languages-show-more = Погледајте више
languages-show-less = Погледајте мање
language-meter-in-progress = Напредак
language-total-progress = Укупно
language-search-input =
    .placeholder = Претрага
language-speakers = Говорници
localized = Локализоване
sentences = Реченице
language-validation-hours = Сати
language-validation-progress = Напредак потврђивања
contribute = Допринеси
get-involved-button = Укључите се
get-involved-title = Допринесите језику { $lang }
get-involved-text = Хвала вам што желите да допринесете језику { $lang }. Трудимо се да припремимо сваки језик пре изласка и шаљемо новости тимовима преко е-поште. Уколико желите да допринесете, унесите вашу е-адресу испод.
get-involved-form-title = Претплатите се на новости у вези језика { $lang }:
get-involved-email =
    .label = Е-адреса
get-involved-opt-in = Да, шаљите ми е-поруке. Желим да останем обавештен о напретку овог језика у пројекту Common Voice.
get-involved-submit = Пошаљи
get-involved-stayintouch = У Mozilla-и градимо заједницу око гласовне технологије. Желимо да останемо у контакту тако што ћемо вам слати вести, нове изворе података и тако што ћемо чути како користите ове податке.
get-involved-privacy-info = Обећавамо вам да ћемо пажљиво баратати са вашим подацима. Прочитајте о томе више у нашем <privacyLink>обавештењу о приватности</privacyLink>.
get-involved-success-title = Успешно сте се пријавили за доприношење језику { $language }. Хвала вам.
get-involved-success-text = Остаћемо у контакту тако што ћемо вам послати још података након што пристигну.
get-involved-return-to-languages = Назад на језике
