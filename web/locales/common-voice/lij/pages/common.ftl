# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Afar
aat = Arvanitika
ab = Abkhazo
abb = Bankon
abq = Abaza
ace = Aceh
ady = Adighè
af = Afrikaans
aii = Suret (neo-aramaico)
ajg = Adja
aln = Albaneise Gheg
am = Amarico
an = Aragoneise
ar = Arabo
arn = Mapudungun
as = Assameise
ast = Asturian
az = Azerbaigian
azz = Nahuatl di monti do Puebla
ba = Bashkir
bag = Tuki
bal = Baloci
bas = Basaa
bax = Bamun
bba = Bariba
bbj = Ghomala'
bbl = Tush
bce = Mengambo
bci = Boulé
be = Ruscio gianco
beb = Bebele
bew = Betawi
bfd = Bafut
bfq = Badaga
bft = Balti
bg = Burgao
bgp = Balochi de levante
bin = Edo
bkh = Bakoko
bkm = Kom
bm = Bambara
bn = Bengaleise
bnm = Batanga
bnn = Bunun
bo = Tibetan
br = Breton
brh = Brahui
bri = Mokpwe
brx = Bodo
bs = Bosniaco
bsh = Kateviri
bsk = Brushaski
bsy = Sabah Bisaya
btv = Bateri
bum = Bulu
bxk = Bukusu
bxr = Buriato
byv = Medumba
ca = Catalan
cak = Kaqchikel
cdo = Min de levante
ce = Ceceno
cgg = Chiga
cjk = Chokwe
ckb = Kurdo do mezo
cnh = Hakha Chin
co = Còrso
cpx = Heng Hua
cpy = Asheninka South Ucayali
crh = Tataro da Crimea
cs = Ceco
cut = Cuicateco de Teutila
cux = Cuicateco de Tepeuxila
cv = Ciuvascio
cy = Galleise
da = Daneise
dag = Dagbani
dar = Dargwa
dav = Kidaw’ida
de = Todesco
din = Dinka
dmk = Dawoodi
dml = Dameli
dru = Rukai
dsb = Sòrabo basso
dua = Duala
dv = Maldivian
dyu = Dioula
ebr = Ebrie
ee = Ewe
efk = Efik
eko = Ekoti
el = Grego
en = Ingleise
eo = Esperanto
es = Spagnòllo
esu = Yupʼik de l’Alaska do mezo
et = Estone
eto = Eton
eu = Basco
ewo = Ewondo
fa = Perscian
fan = Fang
ff = Fulah
fi = Finlandeise
fmp = Feʼefeʼe
fo = Faroeise
fr = Franseise
fub = Fulfulde de l’Adamawa
fue = Fulfulde do Borgu
fuf = Pular da Guinea
fy-NL = Frixan
ga-IE = Irlandeise
gaa = Ga
gcf = Creolo da Martinicca
gej = Mina
ggg = Gurgula
gid = Guidar
gig = Goaria
giz = Guiziga
gjk = Kachhi
gju = Gujari
gl = Galiçian
glk = Gilaki
gn = Guaraní
gom = Goan Konkani
gos = Gronings
gsw = Alsaçian
gu-IN = Gujarati
guc = Wayuunaiki
gv = Manx
gwc = Gawri
gwt = Gawarbaiti
gya = Gbaya do nòrd-òvest
ha = Hausa
hac = Gorani
hak = Hakka
haz = Hazargi
hch = Wixárika
he = Ebraico
hem = Kihemba
hi = Hindi
hil = Hiligaynon
hno = Hindko do settentrion
hr = Croato
hrx = Hunsrik
hsb = Sòrabo erto
ht = Haitian
hu = Ongareise
hux = Nüpode Huitoto
hy-AM = Ermeno
ia = Interlingua
iba = Iban
ibb = Ibibio
id = Indonexan
ie = Interlingue
ig = Igbo
ipk = Iñupiaq
is = Islandeise
it = Italian
izh = Isorian
ja = Giapponeise
jam = Jamaican Patois
jbo = Lojban
jgo = Ngomba
jqr = Jaqaru
jv = Giavaneise
ka = Zorzan
kaa = Karakalpak
kab = Cabilo
kam = Kikamba
kbd = Cabardo
kcn = Nubi
kdh = Kotokoli
khw = Khowar
ki = Kikuyu
kjh = Khakas
kk = Kazako
kln = Kalenjin
kls = Kalasha
km = Khmer
kmr = Curdo Kurmanji
kn = Kannada
knn = Konkani (Devanagari)
ko = Corean
koo = Konzo
kpv = Komi-Zyrian
krc = Karachay-Balkar
ks = Kashmiri
ksf = Bafia
kvx = Parkari Koli
kw = Còrneco
kxp = Wadiyara Koli
ky = Kirghizo
kzi = Kelabit
laj = Lango
lb = Luxemburgheise
led = Lendu
leu = Kara
lg = Luganda
lij = Ligure
lke = Kenyi
lld = Ladin
ln = Lingala
lo = Lao
lrk = Loarki
lrl = Achomi
lss = Lassi
lt = Lituan
ltg = Latgalian
lth = Thur
lua = Tshiluba
luo = Dholuo
lv = Lettone
lzz = Laz
mai = Maithili
mau = Mazateco de Huautla
mbf = Baba Malay
mbo = Mbo
mcf = Matses
mcn = Massa
mcx = Mpiemo
mdd = Mbum
mdf = Mokša
meh = Mixteco Yucuhiti
mel = Melanau
mfe = Kreol Morisien
mg = Malgascio
mgg = Kunabembe
mhk = Mungaka
mhr = Mari de levante
mk = Maçedone
mki = Dhatki
ml = Malayalam
mmc = Mazahua de Michoacán
mn = Mongolo
mni = Meitei Lon
mnw = Lun
mos = More
mqh = Mixteco de Tlazoyaltepec
mr = Marathi
mrh = Mara
mrj = Mari de ponente
ms = Maleise
mse = Moussey
msi = Sabah maleize
mt = Marteise
mua = Mundang
mug = Musgum
mve = Marwari
mvy = Indus Kohistani
mxu = Mada
my = Burmeise
myv = Erzya
nan-tw = Taiwaneise (Minnan)
nb-NO = Norvegin Bokmål
ncx = Nahuatls de Puebla Central
nd = IsiNdebele do settentrion
ne-NP = Nepaleise
new = Newari
nhe = Huasteca Nahuatl de levante
nhi = Nahuatl de la Sierra de ponente de Puebla
nia = Nias
nl = Olandeise
nla = Ngombale
nlv = Orizaba Nahuatl
nmg = Kwasio
nmz = Losso
nn-NO = Norvegin Nynorsk
nnh = Ngiembon
nqo = NKo
nr = IsiNdebele do meridion
nso = Sotho do settentrion
ny = Chinyanja
nyn = Runyankole
nyu = Nyungwe
oc = Occitan
odk = Oadki
om = Afaan Ormoo
or = Odia
oru = Ormuri
os = Ossetico
pa-IN = Punjabi
pap = Papiamento
pap-AW = Papiamento (Aruba)
pau = Palauan
pcd = Picard
pcm = Pidgin nigerian
pez = Penan òrientale
phl = Palula
phr = Pahari-Pothwari
pl = Polacco
plk = Kohistani Shina
pne = Penan de ponente
prq = Asheninka Perene
ps = Pashto
pt = Portogheise
pua = Purepecha de tære erte de ponente
pwn = Paiwan
quc = K'iche'
qug = Quichua Chimborazo Highland
qup = Quechua do Pastaza do meridion
qur = Quechua Yanahuanca
qus = Quechua Santiago del Estero
qux = Quechua Yauyos
quy = Quechua Chanka
qva = Quechua Ambo-Pasco
qvi = Kichwa
qvj = Loja Highland Kichwa
qvl = Quechua Cajatambo
qwa = Quechua Corongo Ancash
qws = Quechua Sihuas Ancash
qxa = Quechua de Chiquián
qxp = Quechua de Puno
qxq = Qashqai
qxt = Quechua de Pasco Santa Ana de Tusi
qxu = { "" }
qxw = Quechua de Jauja Wanka
rif = Tarifit
rki = Rakhine
rm-sursilv = Romancio sottascirvan
rm-vallader = Romancio Vallader
rn = Kirundi
ro = Romen
rof = Kirombo
ru = Ruscio
ruc = Ruuli
rup = Aromanian
rw = Kinyarwanda
rwm = Amba
sah = Sakha
sat = Santali (Ol Chiki)
sba = Ngambay
sbn = Sindhi Bhil
sc = Sardo
scl = Shina
scn = Siçilian
sco = Scoçeise
sd = Sindhi
sdh = Curdo do meridion
sdo = Serian Bidayuh
seh = Sena
sei = Seri
sg = Sango
shi = Shilha
shn = Shan
si = Sinhala
sk = Slovacco
skr = Saraiki
sl = Sloveno
sn = Shona
snk = Soninke
snv = Saʼban
so = Somalo
sq = Albaneise
sr = Serbo
ss = Siswati
ssi = Sansi
st = Sotho do meridion
su = Sundaneize
sv-SE = Svedeise
sva = Svan
sw = Swahili
swb = Shimaore
syr = Siriaco
szy = Sakizaya
ta = Tamil
tar = Central Tarahumara
tay = Atayal
te = Telugu
teg = Teke
tg = Tagico
th = Tailandeise
ti = Tigrinya
tig = Tigre
tiv = Tiv
tk = Tukmen
tl = Tagalog
tli = Tlingit
tn = Setswana
tob = Toba Qom
tok = Toki Pona
top = Papantla Totonac
tr = Turco
trv = Seediq
trw = Torwali
ts = Xitsonga
tt = Tataro
ttj = Rutoro
tui = Tupuri
tum = Tumbuka
tuq = Tedaga
tvu = Tunen
tw = Twi
ty = Tahitian
tyv = Tuvan
uby = Ubykh
udl = Ouldémé
udm = Udmurto
ug = Uiguro
uk = Ucrain
ukv = Kuku
ur = Urdu
ush = Ushojo
usp = Uspanteko
uz = Uzbeco
var = Huarijio
ve = Tshivenda
vec = Veneçian
vi = Vietnamitto
vmw = Emakhuwa
vot = Vòtico
wbl = Wakhi
wep = Vestfalian
wes = Pidgin do Cameron
wo = Wolof
xdq = Kaitag
xh = Xhosa
xhe = Ketrani
xka = Kalkoti
xkl = Kenyah
xmf = Mingrelian
xsm = Kasem
yaq = Yaqui
yav = Nuasue
ydg = Yadgha
yi = Yiddish
yo = Yoruba
yue = Cantoneise
zgh = Tamazight
zh-CN = Cineise (Ciña)
zh-HK = Cineise (Hong Kong)
zh-TW = Cineise (Taiwan)
zoc = Zoque de Copainalá
zu = Zulu
zza = Zaza

# [/]


## Layout

profile = Profî
help = Aggiutto
contact = Contatti
privacy = Privacy
terms = Condiçioin
cookies = Cookies
faq = Domande frequente
documentation = Documentaçion
content-license-text = Contegnuo disponibile sotta <licenseLink>liçensa Creative Commons</licenseLink>
share-title = Aggiuttine à trovâ di atri donatoî de voxe!
share-text = Agiutta à mostrâ a-e machine comme parla e persoñe: doña a teu voxe in sce { $link }
link-copied = Ligamme copiou
back-top = Vanni in çimma
logout = Sciòrti
donate = Fanni unna donaçion
stayintouch = Mozilla a l’é apreuvo à mette sciù unna communitæ de persoñe interessæ a-e tecnologie vocale. Ne saieiva cao restâ in contatto con ti con notiçie, neuve vivagne de dæti, e pe saveine de ciù in sce comme t'addeuvi i dæti de Common Voice.
privacy-info = Promettemmo de gestî e teu informaçioin personæ con attençion. Lezi de ciù inta nòstra <privacyLink>informativa in sciâ privacy</privacyLink>.
return-to-cv = Vanni inderê à Common Voice
email-input =
    .label = Adresso de pòsta elettrònica
loading = Carregamento...
email-opt-in-info = Veuggio reçeive di messaggi pe pòsta elettrònica pe-i arregòrdi di obiettivi, pe-e neuve in sciô mæ progresso, e pe-e newsletter in sce Common Voice.
email-opt-in-info-title = Scrivite a-a mailing list de Common Voice
email-opt-in-info-sub-with-challenge = Reçeivi di messaggi de pòsta elettrònica comme de desfie, di arregòrdi in scî obiettivi, de neuve in scî progressi e e newsletter de Common Voice.
email-opt-in-privacy-v2 = Con deçidde de reçeive a pòsta elettrònica, ti açetti che Mozilla a l’addeuvie ste informaçioin in conformitæ à l’<privacyLink>informativa in sciâ privacy</privacyLink>.
indicates-required = * ìndica 'n cànpo òbligatöio
not-available-abbreviation = n.d.
# Text shown on a button that can be clicked to copy text to the clipboard
copy = Còpia
# Text shown on a button that can be clicked to close a modal or panel
close = Særa
# Text shown on a button that can be clicked to cancel an action
cancel = Anulla
# Text shown on a button that can be clicked to delete an item
delete = Scancella

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>Emmo beseugno do teu</mark><br/><mark>aggiutto!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = Ospitâ i dataset e amegioâ a ciattaforma pe e ciù de 100 communitæ linguistiche che contan in sce de niatri o costa squæxi un milion de dòllai à l’anno. <strong>Se t’apprexi a poscibilitæ d’accede à di dæti averti e incluxivi, fanni unna donaçion ancheu!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>T’addeuvi CV pe </mark><br/><mark>a teu reçerca?</mark>
languages-donate-banner-cta-explanation = Common Voice o l’é finançiou da donaçioin e sovvençioin. N’é cao conlaborâ con academichi, soçietæ çivile e reçercatoî do settô. Common Voice o l’é de badda, ma sostegnî parte di costi da ciattaforma e de l’hosting pe mezo de propòste de sovvençion o l’é utile pe in davei.
localization-select =
    .label = Çerni a lengua/localizzaçion

## MDC Announcement

# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = Arve inte 'n neuvo feuggio

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = Comensa a scrive pe çercâ
searchbox-default-placeholder = Çerca...
searchbox-default-aria-label = Çerca
dropdown-no-results = Nisciunn-a atrovâ
dataset-searchbox-placeholder = Çerca 'na lengua...
dataset-searchbox-aria-label = Çerca 'na lengua
dataset-search-no-results = Nisciunn-a lengoa corispondente
request-new-language = Domanda 'na neuva lengua
