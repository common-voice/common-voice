# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Αφαρικά
aat = Αρβανίτικα
ab = Αμπχαζικά
abb = Μπανκόν
abq = Αμπάζα
ace = Ατσενικά
ady = Αντύγκε
af = Αφρικάανς
aii = Σούριθ
ajg = Άτζα
aln = Αλβανικά (Γκέγκικα)
am = Αμχαρικά
an = Αραγωνικά
apc = Λεβαντίνικα Αραβικά
ar = Αραβικά
arn = Αραουκάνια
as = Ασαμικά
ast = Αστουριανά
az = Αζερικά
azz = Ναχουάτλ (Υψίπεδα Πουέμπλα)
ba = Μπασκίρ
bag = Τούκι
bal = Βαλουχικά
bas = Μπάσα
bax = Μπαμούν
bba = Μπαατόνουμ
bbj = Γκομάλα
bbl = Μπατς
bce = Μπαμενγιάμ
bci = Μπαουλέ
be = Λευκορωσικά
beb = Μπεμπέλε
bew = Μπετάβι
bfd = Μπαφούτ
bfq = Μπαντάγκα
bft = Μπάλτι
bg = Βουλγαρικά
bgp = Βαλουχικά (Ανατολικά)
bin = Μπίνι
bkh = Μπακόκο
bkm = Κομ
blk = Πα'Ο
bm = Μπαμπάρα
bn = Βεγγαλικά
bnm = Μπατάνγκα
bnn = Μπουνούν
bo = Θιβετιανά
br = Βρετονικά
brh = Μπραχούι
bri = Μοκπούε
brx = Μπόντο
bs = Βοσνιακά
bsh = Κάτι
bsk = Μπουρουσάσκι
bsy = Μπισάγια (Σαμπά)
btv = Μπατέρι
bum = Μπούλου
bxk = Μπουκούσου
bxr = Μπουριάτ
byv = Μεντούμπα
ca = Καταλανικά
cak = Κακτσικουέλ
cdo = Kινεζικά (Μιν Ντονγκ)
ce = Τσετσενικά
cgg = Τσίγκα
cjk = Τσόκβε
ckb = Κουρδικά (Κεντρικά)
cnh = Τσιν (Λάι)
co = Κορσικά
cpx = Κινεζικά Που-Ξιάν
cpy = Ασένινκα (Νότια Ουκαγιάλι)
crh = Ταταρικά (Κριμαία)
cs = Τσεχικά
cut = Κουικατέκ (Τεουτίλα)
cux = Κουικατέκ (Τεπεουξίλα)
cv = Τσουβασικά
cy = Ουαλικά
da = Δανικά
dag = Νταγκμπάνι
dar = Νταργκούα
dav = Τάιτα
de = Γερμανικά
din = Ντίνκα
dmk = Ντομαάκι
dml = Νταμέλι
dru = Ρουκάι
dsb = Σορβιανά (Κάτω)
dua = Ντουάλα
dv = Ντιβέχι
dyu = Ντιούλα
dz = Τζόνγκα
ebr = Εμπριέ
ee = Έουε
efk = Εφίκ
eko = Κότι
el = Ελληνικά
en = Αγγλικά
eo = Εσπεράντο
es = Ισπανικά
esu = Γιουπίκ (Κεντρικά)
et = Εσθονικά
eto = Ετόν
eu = Βασκικά
ewo = Εουόντο
fa = Περσικά
fan = Φανγκ
ff = Φουλά
fi = Φιλανδικά
fmp = Φεʼφεʼε
fo = Φεροϊκά
fr = Γαλλικά
fub = Φουλφούλντε (Ανταμάουα)
fue = Φουλφούλντε (Μπόργκου)
fuf = Πουλάρ (Γουινέα)
fy-NL = Φριζικά
ga-IE = Ιρλανδικά
gaa = Γκα
gcf = Μαρτινικανά Κρεολικά
gej = Γκεν
ggg = Γκουργκούλα
gid = Γκιντάρ
gig = Γκοάρια
giz = Γκιζίγκα
gjk = Κάτσι
gju = Γκουτζάρι
gl = Γαλικικά
glk = Γκιλάκι
gn = Γκουαρανί
gom = Κονκάνι (Γκοάν)
gos = Γκρόνινγκς
gsw = Αλεμανικά (Αλσατία)
gu-IN = Γκουτζαράτι
guc = Ουαγιούου
gv = Μανξ
gwc = Καλάμι
gwt = Γκαβάρ Μπάτι
gya = Γκμπάγια (Βορειοδυτικά)
ha = Χάουσα
hac = Γκουράνι
hak = Χάκκα
haz = Χαζαράγκι
hch = Χουιτσόλ
he = Εβραϊκά
hem = Χέμπα
hi = Χίντι
hil = Χιλιγκάυνον
hno = Χίντκο (Βόρεια)
hr = Κροατικά
hrx = Χούνσρικ
hsb = Σορβιανά (Άνω)
ht = Αϊτινά
hu = Ουγγρικά
hux = Χουιτότο (Νουπόντε)
hy-AM = Αρμενικά
ia = Ιντερλίνγκουα
iba = Ιμπάν
ibb = Ιμπίμπιο
id = Ινδονησιακά
ie = Ιντερλίνγκουε
ig = Ίγκμπο
ipk = Ινουπιάκ
is = Ισλανδικά
it = Ιταλικά
izh = Ινγκρικά
ja = Ιαπωνικά
jam = Πατουά (Τζαμάικα)
jbo = Λοτζμπάν
jgo = Νγκόμπα
jqr = Τζακάρου
jv = Ιαβαϊκά
ka = Γεωργιανά
kaa = Καρακαλπάκ
kab = Καμπύλ
kam = Κάμπα
kbd = Καμπαρντιάν
kcn = Νούμπι
kdh = Τεμ
khw = Χοβάρ
ki = Κικούγιου
kjh = Χάκας
kk = Καζακικά
kl = Γροιλανδικά
kln = Καλεντζίν
kls = Καλάσα
km = Χμερ
kmr = Κουρδικά (Βόρεια)
kn = Κανάντα
knn = Κονκάνι (Ντεβανάγκαρι)
ko = Κορεατικά
koo = Κόντζο
kpv = Κόμι-Ζυριανικά
krc = Καρατσάυ-Μπαλκάρ
ks = Κασμίρι
ksf = Μπάφια
kvx = Κόλι (Παρκάρι)
kw = Κορνουαλικά
kxp = Κόλι (Βαντιγιάρα)
ky = Κιργιζικά
kzi = Κελαμπίτ
laj = Λάνγκο
lb = Λουξεμβουργιανά
led = Λέντου
leu = Κάρα
lg = Λουγκάντα
lij = Λιγουριανά
lke = Κένυι
lld = Λαντίν
ln = Λινγκάλα
lo = Λαοτινά
lrk = Λοάρκι
lrl = Λάρι
lss = Λάσι
lt = Λιθουανικά
ltg = Λατγκαλιανά
lth = Θουρ
lua = Λούμπα-Λούλουα
luo = Λούο
lv = Λετονικά
lzz = Λαζικά
mai = Μαϊθίλι
mau = Μαζατέκ (Χουαούτλα)
mbf = Μαλαϊκά (Μπάμπα)
mbo = Μπο
mcf = Ματσές
mcn = Μασάνα
mcx = Μπιέμο
mdd = Μπουμ
mdf = Μόξχα
meh = Μίξτεκ (Νοτιοδυτικά Τλαξιάκο)
mel = Μελανάου
mfe = Μορισυέν
mg = Μαλαγασικά
mgg = Μπόνγκμπονγκ
mhk = Μουνγκάκα
mhr = Μάρι (Ανατολικά)
mk = Σλαβομακεδονικά
mki = Ντχάτκι
ml = Μαλαγιάλαμ
mmc = Μαζάχουα (Μιτσοακάν)
mn = Μογγολικά
mni = Μέιτεϊ
mnw = Μον
mos = Μόσσι
mqh = Μίξτεκ (Τλαζογιαλτεπέκ)
mr = Μαράτι
mrh = Μαρά
mrj = Μάρι (Δυτικά)
ms = Μαλαισιακά
mse = Μουσέυ
msi = Μαλαϊκά (Σαμπά)
mt = Μαλτεζικά
mua = Μουντάνγκ
mug = Μούσγκου
mui = Μούσι
mve = Μαρβάρι (Πακιστάν)
mvy = Κοχιστάνι (Ινδός Ποταμός)
mxu = Μάντα
my = Βιρμανικά
myv = Ερζύα
mzn = Μαζανταρανί
nan-tw = Ταϊβανικά (Μιν Ναν)
nb-NO = Νορβηγικά (Bokmål)
ncx = Ναχουάτλ (Κεντρική Πουέμπλα)
nd = Ντεμπέλε (Βόρεια)
ne-NP = Νεπαλικά
new = Νεβάρι
nhe = Ναχουάτλ (Ανατολικό Χουαστέκα)
nhi = Ναχουάτλ (Τενάνγκο)
nia = Νιάς
nl = Ολλανδικά
nla = Νγκομπάλε
nlv = Ναχουάτλ (Οριζάμπα)
nmg = Νγκούμπα
nmz = Ναούντμ
nn-NO = Νορβηγικά (Nynorsk)
nnh = Νγκιεμπούν
nqo = Ν'Κο
nr = Ντεμπέλε (Νότια)
nso = Βόρεια Σόθο
ny = Νιάντζα
nyn = Νυανκόλε
nyu = Νυούνγκβε
oc = Οξιτανικά
odk = Οντ
om = Αφάν Ορόμο
or = Οντία
oru = Ορμούρι
os = Οσσετικά
pa-IN = Παντζάμπι
pap = Παπιαμέντο
pap-AW = Παπιαμέντο (Αρούμπα)
pau = Παλάου
pcd = Πικάρντ
pcm = Αγγλικά Pidgin (Νιγηρία)
pez = Πενάν (Ανατολικά)
phl = Παλούλα
phr = Παχάρι-Ποτβάρι
pl = Πολωνικά
plk = Σίνα (Κοχιστάνι)
pne = Πενάν (Δυτικά)
prq = Ασένινκα Περενέ
ps = Παστού
pt = Πορτογαλικά
pua = Πουρεπέτσα (Δυτικά Υψίπεδα)
pwn = Παϊβάν
quc = Κουισέ
qug = Κουιτσούα (Υψίπεδα Τσιμποράζο)
qup = Κουετσούα (Νότια Παστάζα)
qur = Κουετσούα (Γιαναχουάνκα Πάσκο)
qus = Κουιτσούα (Σαντιάγκο ντελ Εστέρο)
qux = Κουετσούα (Γιαούγιος)
quy = Κουετσούα (Αγιακούτσο)
qva = Κουετσούα (Άμπο-Πάσκο)
qvi = Κουιτσούα
qvj = Κουιτσούα (Υψίπεδα Λόχα)
qvl = Κουετσούα (Καταχάμπο)
qwa = Κουετσούα (Κορόνγκο Ανκάς)
qws = Κουετσούα (Σιχούας Ανκάς)
qxa = Κουετσούα (Τσικουιάν)
qxp = Κουετσούα (Πούνο)
qxq = Κασκάι
qxt = Κουετσούα (Σάντα Άνα ντε Τούσι Πάσκο)
qxu = Κουετσούα (Αρεκουίπα-Λα Γιουνιόν)
qxw = Κουετσούα (Τζαουτζά Ουάνκα)
rif = Ταριφίτ
rki = Ραχίνε
rm-sursilv = Ρομανσικά (Σουρσέλβα)
rm-vallader = Ρομανσικά (Βαλάντερ)
rn = Κιρούντι
ro = Ρουμανικά
rof = Ρόμπο
ru = Ρώσικα
ruc = Ρούλι
rup = Βλάχικα
rw = Κινιαρουάντα
rwm = Άμπα
sah = Σάκχα
sat = Σαντάλι (Ολ Τσίκι)
sba = Νγκμπέι
sbn = Σίντι Μπιλ
sc = Σαρδηνιακά
scl = Σίνα
scn = Σικελικά
sco = Σκωτς
sd = Σίντι
sdh = Κουρδικά (Νότια)
sdo = Μπουκάρ Σαντόνγκ
seh = Σένα
sei = Σέρι
sg = Σάνγκο
shi = Τατσελχίτ
shn = Σαν
si = Σιναλεζικά
sk = Σλοβακικά
skr = Σαραΐκι
sl = Σλοβενικά
sn = Σόνα
snk = Σονινκέ
snv = Σαʼμπαν
so = Σομαλικά
sq = Αλβανικά
sr = Σερβικά
ss = Σουάτι
ssi = Σάνσι
st = Νότια Σόθο
su = Σουνδανικά
sv-SE = Σουηδικά
sva = Σβαν
sw = Σουαχίλι
swb = Κομοριανά
syr = Συριακά
szy = Σακιζάγια
ta = Ταμίλ
tar = Ταραχουμάρα (Κεντρικά)
tay = Αταγιάλ
te = Τελούγκου
teg = Τέκε
tg = Τατζικικά
th = Ταϊλανδικά
ti = Τιγκρινιακά
tig = Τίγκρε
tiv = Τιβ
tk = Τουρκμενικά
tl = Τάγκαλογκ
tli = Τλινγκίτ
tn = Τσουάνα
tob = Τόμπα
tok = Τόκι Πόνα
top = Τοτονάκ (Παπάντλα)
tr = Τουρκικά
trv = Σεεντίκ
trw = Τορβάλι
ts = Ξιτσόνγκα
tt = Ταταρικά
ttj = Τοόρο
tui = Τουπούρι
tum = Τουμπούκα
tuq = Τεντάνγκα
tvu = Τουνέν
tw = Τουί
ty = Ταϊτινά
tyv = Τουβινιανά
uby = Ουμπίκχ
udl = Βουσλάμ
udm = Ουντμούρτ
ug = Ουιγκούρ
uk = Ουκρανικά
ukv = Κούκου
ur = Ούρντου
ush = Ουσότζο
usp = Ουσπαντέκο
uz = Ουζμπεκικά
var = Χουαρίχιο
ve = Βέντα
vec = Βενετικά
vi = Βιετναμέζικα
vls = Δυτικά Φλαμανδικά
vmw = Μακχούα
vot = Βοτίκ
wbl = Ουάκχι
wep = Βεστφαλικά
wes = Γλώσσα Pidgin (Καμερούν)
wo = Γουόλοφ
xdq = Κάιταγκ
xh = Κόσα
xhe = Κχετράνι
xka = Καλκότι
xkl = Κένυα (Κελινγάου)
xmf = Μινγκρελιανά
xsm = Κασέμ
yaq = Γιακούι
yav = Γιανγκμπέν
ydg = Γίντχα
yi = Γίντις
yo = Γιορούμπα
yue = Καντονέζικα
zgh = Ταμαζίγκχτ
zh-CN = Κινεζικά (Κίνα)
zh-HK = Κινεζικά (Χονγκ Κονγκ)
zh-TW = Κινεζικά (Ταϊβάν)
zoc = Ζόκουε (Κοπαϊναλά)
zu = Ζουλού
zza = Ζάζα

## Layout

profile = Προφίλ
help = Βοήθεια
contact = Επικοινωνία
privacy = Απόρρητο
terms = Όροι
cookies = Cookie
faq = Συχνές ερωτήσεις
documentation = Τεκμηρίωση
content-license-text = Το περιεχόμενο διατίθεται υπό την <licenseLink>άδεια Creative Commons</licenseLink>
share-title = Βοηθήστε μας να βρούμε κι άλλα άτομα που θα δωρίσουν τη φωνή τους!
share-text = Βοηθήστε τις μηχανές να κατανοήσουν πώς μιλάνε οι πραγματικοί άνθρωποι, δωρίζοντας τη φωνή σας στο { $link }
link-copied = Ο σύνδεσμος αντιγράφηκε
back-top = Επιστροφή στην κορυφή
logout = Αποσύνδεση
donate = Δωρεά
stayintouch = Στη Mozilla, χτίζουμε μια κοινότητα γύρω από τη φωνητική τεχνολογία. Θα θέλαμε να επικοινωνούμε μαζί σας για ενημερώσεις, νέες πηγές δεδομένων και για να μαθαίνουμε πώς χρησιμοποιείτε αυτά τα δεδομένα.
privacy-info = Υποσχόμαστε να χειριστούμε τις πληροφορίες σας με προσοχή. Διαβάστε περισσότερα στη <privacyLink>Δήλωση απορρήτου</privacyLink> μας.
return-to-cv = Επιστροφή στο Common Voice
email-input =
    .label = Email
loading = Φόρτωση…
email-opt-in-info = Θα ήθελα να λαμβάνω email, όπως υπενθυμίσεις στόχων, ενημερώσεις προόδου και ενημερωτικά δελτία για το Common Voice.
email-opt-in-info-title = Εγγραφή στη λίστα αλληλογραφίας Common Voice
email-opt-in-info-sub-with-challenge = Λάβετε email, όπως υπενθυμίσεις στόχων και προκλήσεων, ενημερώσεις προόδου και ενημερωτικά δελτία για το Common Voice.
email-opt-in-privacy-v2 = Επιλέγοντας να λαμβάνετε email, δηλώνετε ότι συμφωνείτε με τον χειρισμό αυτών των δεδομένων από τη Mozilla, όπως εξηγεί η <privacyLink>Πολιτική απορρήτου</privacyLink>.
indicates-required = * Υποχρεωτικό πεδίο
not-available-abbreviation = Μ/Δ
# Text shown on a button that can be clicked to copy text to the clipboard
copy = Αντιγραφή
# Text shown on a button that can be clicked to close a modal or panel
close = Κλείσιμο
# Text shown on a button that can be clicked to cancel an action
cancel = Ακύρωση
# Text shown on a button that can be clicked to delete an item
delete = Διαγραφή

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>Χρειαζόμαστε</mark><br/><mark>τη βοήθειά σας!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = Η φιλοξενία του συνόλων των δεδομένων και η βελτίωση της πλατφόρμας για τις 100+ διαφορετικές γλωσσικά κοινότητες που στηρίζονται σε εμάς, κοστίζει σχεδόν ένα εκατομμύριο δολάρια ετησίως. <strong>Αν εκτιμάτε τα ανοικτά και χωρίς αποκλεισμούς δεδομένα, κάντε δωρεά σήμερα!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>Χρησιμοποιείτε το βιογραφικό σας</mark><br/><mark>για την έρευνά σας;</mark>
languages-donate-banner-cta-explanation = Το Common Voice χρηματοδοτείται από δωρεές και επιχορηγήσεις! Μας αρέσει να συνεργαζόμαστε με ακαδημαϊκούς, την κοινωνία των πολιτών και ερευνητές του κλάδου. Η χρήση του Common Voice είναι δωρεάν, αλλά η συνεισφορά στο κόστος της πλατφόρμας και της φιλοξενίας μέσω επιχορηγήσεων είναι πολύ χρήσιμη.
localization-select =
    .label = Επιλογή γλώσσας/μετάφρασης

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = <strong>Νέα σύνολα δεδομένων του Common Voice</strong> είναι πλέον διαθέσιμα για λήψη αποκλειστικά μέσω της πλατφόρμας Mozilla Data Collective. Εγγραφείτε για να αποκτήσετε πρόσβαση σε 300+ καθολικά σύνολα δεδομένων, που αναπτύσσονται από και για την κοινότητα.
# Button text for MDC announcement
announcement-mdc-button-text = Συμμετοχή στο Mozilla Data Collective
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = Ανοίγει σε νέα καρτέλα

## Release Delay Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-delay = Αγαπητή κοινότητα, αυτή τη στιγμή εργαζόμαστε πάνω σε αυτή την έκδοση. Αναμένουμε να κυκλοφορήσει τμηματικά τις επόμενες ημέρες στο <mdcLink>Mozilla Data Collective</mdcLink>.

## Release Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release = Αγαπητή κοινότητα, η νέα έκδοση διατίθεται πλήρως στο <mdcLink>Mozilla Data Collective</mdcLink>. Εγγραφείτε για πρόσβαση σε 500+ σύνολα δεδομένων, που αναπτύσσονται από και για την κοινότητα.

## Pre-Release Announcement (dates TBD)

# Text wrapped in <cvDatasheetsLink></cvDatasheetsLink> will be rendered as an external link to the cv-datasheets GitHub repo
announcement-pre-release = Αγαπητή κοινότητα, ο χρόνος κυκλοφορίας πλησιάζει. Παρακαλούμε προχωρήστε στην επικύρωση και μεταγραφή και επισκεφθείτε επίσης τα <cvDatasheetsLink>cv-datasheets</cvDatasheetsLink> για να ενημερώσετε το φύλλο δεδομένων σας.

## Release Commencing

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-commencing = Αγαπητή κοινότητα, έχουμε φτάσει στην καταληκτική ημερομηνία για την κυκλοφορία και ομαδοποιούμε τα σύνολα δεδομένων. Η νέα έκδοση θα είναι διαθέσιμη στο <mdcLink>Mozilla Data Collective</mdcLink> σε μερικές ημέρες.

## Downtime Announcements

# Generic scheduled downtime for next Sunday early hours
announcement-scheduled-downtime = Αγαπητή κοινότητα, για τη βελτιστοποίηση των συστημάτων μας προγραμματίζουμε σύντομες διακοπές λειτουργίας, οι οποίες συνήθως διαρκούν λίγες ώρες. Η επόμενη προγραμματισμένη διακοπή θα πραγματοποιηθεί τις πρώτες πρωινές ώρες αυτής της Κυριακής. Εάν αντιμετωπίσετε κάποιο πρόβλημα, επικοινωνήστε μαζί μας μέσω του Matrix.

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = Ξεκινήστε να πληκτρολογείτε για αναζήτηση
searchbox-default-placeholder = Αναζήτηση...
searchbox-default-aria-label = Αναζήτηση
dropdown-no-results = Δεν βρέθηκε αντιστοιχία
# Common Voice Dataset=Language search box
dataset-searchbox-label = Αλλάζει τη γλώσσα προβολής και του συνόλου δεδομένων
dataset-searchbox-placeholder = Αναζήτηση για γλώσσα...
dataset-searchbox-aria-label = Αναζήτηση για γλώσσα
dataset-search-no-results = Δεν βρέθηκε αντιστοιχία γλώσσας
request-new-language = Αίτημα για νέα γλώσσα
