# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Afar
aat = Arvanitika
ab = Abecásio
abb = Bankon
ace = Achém
ady = Adigue
af = Africâner
aln = Gheg Albanês
am = Amárico
an = Aragonês
ar = Árabe
arn = Mapuche
as = Assamês
ast = Asturiano
az = Azerbaidjano
azz = Asteca (náuatle) das montanhas de Puebla
ba = Basquir
bal = Balúchi
bas = Basa
bax = Bamum
bba = Bariba
bbj = Ghomala
bbl = Tush
bci = Baúle
be = Bielorrusso
beb = Bebele
bew = Malaio de Jacarta
bfd = Bafut
bft = Balti
bg = Búlgaro
bgp = Balúchi Oriental
bkm = Kom
bm = Bambara
bn = Bengalês
bnm = Batanga
bnn = Bunun
bo = Tibetano
br = Bretão
bri = Mokpwe
bs = Bósnio
bsh = Kateviri
bsk = Brushaski
bum = Bulu
bxr = Buriato
byv = Medumba
ca = Catalão
cak = Caqchiquel
cdo = Min Oriental
ckb = Curdo central
cnh = Hakha Chin
co = Corso
crh = Tártaro da Crimeia
cs = Tcheco
cut = Teutila Cuicatec
cux = Tepeuxila Cuicatec
cv = Tchuvache
cy = Galês
da = Dinamarquês
dag = Dagbani
dar = Dargwa
dav = Kidaw'ida
de = Alemão
dmk = Dawoodi
dml = Dameli
dru = Rukai
dsb = Baixo Sorábio
dua = Duala
dv = Divehi
dyu = Dioula
ebr = Ebrié
ee = Euê
el = Grego
en = Inglês
eo = Esperanto
es = Espanhol
esu = Yup'ik do Alasca Central
et = Estoniano
eto = Eton
eu = Basco
ewo = Euondo
fa = Persa
fan = Fang
ff = Fula
fi = Finlandês
fo = Feroês
fr = Francês
fub = Fula de Adamaua
fue = Fula de Borgu
fuf = Pular da Guiné
fy-NL = Frisão
ga-IE = Irlandês
gju = Gujari
gl = Galego
gn = Guarani
gom = Konkani de Goa
gos = Gronings
gsw = Alsaciano
gu-IN = Guzerate
guc = Wayuu
gv = Manês
gwc = Gawri
gwt = Gawarbaiti
gya = Gbaya do Noroeste
ha = Hauçá
hac = Gorani
hch = Wixárika
he = Hebraico
hi = Hindi
hil = Hiligaynon
hno = Hindko do Norte
hr = Croata
hsb = Alto Sorábio
ht = Crioulo haitiano
hu = Húngaro
hy-AM = Armênio
ia = Interlíngua
ibb = Ibibio
id = Indonésio
ie = Interlíngua
ig = Igbo
ipk = Inupiaque
is = Islandês
it = Italiano
izh = Ijoriano
ja = Japonês
jbo = Lojban
jqr = Jacaré
jv = Javanês
ka = Georgiano
kaa = Caracalpaque
kab = Kabyle
kbd = Kabardiano
kcn = Núbio
khw = Khowar
ki = Kikuyu
kk = Cazaque
kln = Kalenjin
kls = Kalasha
km = Khmer
kmr = Curdo do norte
kn = Canarês
knn = Konkani (Devanagari)
ko = Coreano
kpv = Komi-Zyrian
ksf = Bafia
kw = Córnico
ky = Quirguiz
lb = Luxemburguês
leu = Cara
lg = Luganda
lij = Liguriano
lke = Queniano
ln = Lingala
lo = Lao
lss = Lassi
lt = Lituano
ltg = Latgaliano
lth = Thur
luo = Dholuo
lv = Letão
lzz = Laz
mai = Maitili
mau = Mazateca de Huautla
mbo = Mbo
mdf = Moksha
meh = Mixteco de Yucuhiti
mg = Malgaxe
mhr = Meadow Mari
mk = Macedônio
ml = Malaiala
mmc = Michoacán Mazahua
mn = Mongol
mni = Meitei-lon
mos = Moré
mqh = Mixtec de Tlazoyaltepec
mr = Marata
mrj = Hill Mari
ms = Malaio
mt = Maltês
mvy = Kohistani do Indo
mxu = Mada
my = Birmanês
myv = Erzya
nan-tw = Taiwanês (Minnan)
nb-NO = Norueguês (Bokmål)
ncx = Nahuatl de Puebla Central
nd = IsiNdebele (Norete)
ne-NP = Nepalês
nhe = Huasteca Nahuatl Oriental
nhi = Nahuatl de Sierra Puebla Ocidental
nia = Nias
nl = Holandês
nn-NO = Novo norueguês
nnh = Ngiemboon
nqo = NKo
nr = IsiNdebele (Sul)
nso = Soto do norte
ny = Chinyanja
nyn = Ancolé
oc = Occitânico
om = Afaan Ormoo
or = Odia
oru = Ormuri
os = Osseto
pa-IN = Punjabi
pap = Papiamento
pap-AW = Papiamento (Aruba)
pcm = Naijá (pidgin inglês nigeriano)
phl = Pálula
pl = Polaco
plk = Shina Kohistani
ps = Pachto
pt = Português
pua = Purepecha das Terras Altas do Oeste
pwn = Paiwan
quc = K'iche'
qup = Quechua do Sul de Pastaza
qux = Quechua de Yauyos
quy = Quechua Chanka
qva = Quechua de Ambo-Pasco
qvi = Kichwa
qvl = Quechua de Cajatambo
qwa = Quechua de Corongo Ancash
qws = Quechua de Sihuas Ancash
qxa = Quechua Chiquián
qxp = Puno Quechua
qxt = Quechua Pasco Santa Ana de Tusi
qxu = Quechua de Arequipa-La Unión
qxw = Quechua Jauja Wanka
rif = Rifenho
rm-sursilv = Romanche (Sursilvan)
rm-vallader = Romanche (Vallader)
ro = Romeno
ru = Russo
ruc = Ruuli
rw = Kinyarwanda
sah = Sakha
sat = Santali (Ol Chiki)
sc = Sardo
scl = Shina
scn = Siciliano
sco = Ânglico Escocês (Scots)
sd = Sindi
sdh = Curdo do Sul
sei = Seri
shi = Tachelhit
shn = Xã
si = Cingalês
sk = Eslovaco
skr = Saraiki
sl = Esloveno
snk = Soninquê
so = Somali
sq = Albanês
sr = Sérvio
ss = Suáti
st = Sesoto
sv-SE = Sueco
sva = Suano
sw = Suahíli
syr = Siríaco
szy = Sakizaya
ta = Tâmil
tay = Atayal
te = Telugo
teg = Teke
tg = Tajique
th = Tailandês
ti = Tigrínia
tig = Tigré
tk = Turcomano
tl = Filipino
tn = Tswana
tob = Toba Qom
tok = Toki Pona
top = Papantla Totonac
tr = Turco
trv = Seediq
trw = Torwali
ts = Xitsonga
tt = Tártaro
tui = Tupuri
tvu = Tunen
tw = Twi
ty = Taitiano
tyv = Tuvan
uby = Ubykh
udm = Udmurte
ug = Uigur
uk = Ucraniano
ukv = Kuku
ur = Urdu
ush = Ushojo
uz = Usbeque
var = Huarijio
ve = Tshivenda
vec = Veneziano
vi = Vietnamita
vmw = Emacua
vot = Vótico
wbl = Wakhi
wep = Westfaliano
wes = Pidgin de Camarões
wo = Wolof
xh = Xhosa
xhe = Ketrani
xka = Calcutá
xmf = Mingreliano
yaq = Yaqui
yi = Iídiche
yo = Iorubá
yue = Cantonês
zgh = Tamazigue
zh-CN = Chinês (China)
zh-HK = Chinês (Hong Kong)
zh-TW = Chinês (Taiwan)
zoc = Zoque de Copainalá
zu = Zulu
zza = Zaza

# [/]


## Layout

profile = Perfil
help = Ajuda
contact = Contato
privacy = Privacidade
terms = Termos
cookies = Cookies
faq = Perguntas frequentes
documentation = Documentação
content-license-text = Conteúdo disponível sob licença <licenseLink>Creative Commons</licenseLink>
share-title = Ajude-nos a encontrar outros doadores de voz!
share-text = Ajude a ensinar às máquinas como uma pessoal fala, doando sua voz em { $link }
link-copied = Link copiado
back-top = Voltar para o início
logout = Sair
donate = Faça uma doação
stayintouch = Nós da Mozilla estamos construindo uma comunidade em torno de tecnologias de voz. Queremos nos manter em contato com atualizações, novas fontes de dados e ouvir mais sobre como você está usando esses dados.
privacy-info = Prometemos lidar com suas informações com cuidado. Saiba mais em nosso <privacyLink>Aviso de privacidade</privacyLink>.
return-to-cv = Voltar para o Common Voice
email-input =
    .label = Email
loading = A carregar…
email-opt-in-info = Quero receber emails, tais como lembretes de metas, atualizações sobre meu progresso e boletins informativos sobre o Common Voice.
email-opt-in-info-title = Participe na lista de discussão do Common Voice
email-opt-in-info-sub-with-challenge = Receba emails, tais como lembretes de objetivos e desafios, atualizações de progresso e boletins informativos sobre o Common Voice.
email-opt-in-privacy-v2 = Ao optar por receber emails, você declara que concorda com a Mozilla lidar com essas informações, conforme explicado na <privacyLink>Política de privacidade</privacyLink> da Mozilla.
indicates-required = * Indica campo obrigatório
not-available-abbreviation = Não disponível

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>Precisamos</mark><br/><mark>da sua ajuda!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = Custa quase um milhão de dólares por ano hospedar os conjuntos de dados e melhorar a plataforma para as mais de 100 comunidades de idiomas que dependem do que fazemos. <strong>Se você valoriza dados abertos e inclusivos, faça uma doação!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>Usa o Common Voice para</mark><br/><mark>sua pesquisa?</mark>
languages-donate-banner-cta-explanation = Common Voice é financiado por doações e subsídios! Adoramos colaborar com pesquisadores acadêmicos, da sociedade civil e da indústria. O uso do Common Voice é gratuito, mas contribuir para os custos de plataforma e hospedagem por meio de propostas de subsídios é realmente útil.
localization-select =
    .label = Escolher idioma/localização
