## Languages Overview

language-section-in-progress = en elavuraziun
language-section-in-progress-new-description = Quels lungatgs vegnan actualmein sviluppai dalla communitad. Las travs da progress indicheschan per mintga lungatg con lunsch che la <localizationGlossaryLink>translaziun dalla pagina-web</localizationGlossaryLink> e la <sentenceCollectionGlossaryLink>collecziun da construcziuns</sentenceCollectionGlossaryLink> ei vegnida realisada.
language-section-launched = lantschau
language-section-launched-description =
    Per ils suandonts lungatgs activs ei la pagina-web vegnida
    <localizationGlossaryLink>translatada</localizationGlossaryLink> cumpleinamein ed ei dat ina
    <sentenceCollectionGlossaryLink>collecziun da construcziuns</sentenceCollectionGlossaryLink> ch'ei gronda avunda per cuntinuar cun <speakLink>discuorer</speakLink> e <listenLink>tedlar</listenLink> contribuziuns.
languages-show-more = mussar dapli
languages-show-less = mussar pli pauc
language-meter-in-progress = Progress
language-total-progress = Total
language-search-input =
    .placeholder = encurir
language-speakers = Plidader/plidadra
localized = translatau
sentences = Construcziuns
contribute = contribuir
get-involved-button = Separticipar
get-involved-title = Contribuir a { $lang }
get-involved-text = Engraziel per tiu interess da contribuir a { $lang }. Nus dein tutta breigia da preparar mintga lungatg per saver lantschar el e da tener sil current ils teams per e-mail. Sche ti vul contribuir, sche indichescha tia adressa dad e-mail cheusut.
get-involved-form-title = S'annunziar per novitads da { $lang }.
get-involved-email =
    .label = E-mail
get-involved-opt-in = Gie, tarmettei a mi e-mails. Jeu lessel star sil current davart il progress da quei lungatg sin Common Voice.
get-involved-submit = Tarmetter
get-involved-stayintouch = Nus da Mozilla essan vid crear ina communitad per tecnologia vocala. Nus lessen restar en contact cun parter novitads, novas fontaunas da datas e cun intervegnir per tgei che ti drovas quellas datas.
get-involved-privacy-info = Nus empermettein da tractar tias informaziuns precautamein. Leger dapli en nossas <privacyLink>Informaziuns davart la protecziun da datas</privacyLink>.
get-involved-success-title = Ti eis s'annunziaus cun success per contribuir a { $language }. Engraziel fetg.
get-involved-success-text = Nus restein en contact cun ulteriuras informaziuns, sco che quellas stattan a disposiziun.
get-involved-return-to-languages = Turnar a Lungatgs
