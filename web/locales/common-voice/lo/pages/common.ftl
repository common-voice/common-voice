# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = ພາສາອາຟາຣ
aat = ອາວະນິທິກາ
ab = ພາສາອັບ​ກາດ​ຊ
abb = Bankon
abq = ອາບາຊາ
ace = ພາສາອາເຊນ
ady = ອາດີເກ
af = ພາສາແອຟຣິຄານສ
aii = ສຸເຣັດ
ajg = Adja
aln = Gheg Albanian
am = ພາ​ສາ​ອຳ​ຮາ​ຣິກ
an = ພາ​ສາ​ອາ​ຣາ​ໂກນ
ar = ພາສາອາຣັບ
arn = Mapudungun
as = ພາ​ສາ​ອາ​ຊຳ​ມີ​ດ
ast = ພາສາອັສຕູເຣຍ
az = ພາສາອາເຊີໄບຈັນ
azz = Highland Puebla Nahuatl
ba = ພາສາບາສເກີ
bag = Tuki
bal = Balochi
bas = ພາສາບາຊາ
bax = Bamun
bba = Baatonum
bbj = Ghomala
bbl = Tush
bce = Mengambo
bci = Baoule
be = ພາ​ສາເບ​ລາ​ຣຸດ​
beb = Bamvele
bew = Betawi
bfd = Bafut
bfq = ບາດາກາ
bft = Balti
bg = ພາ​ສາ​ບຸນ​ກາ​ຣີ
bgp = Balochi ຕາ​ເວັນ​ອອກ​
bin = ເອໂດ
bkh = Bakoko
bkm = Kom
bm = ພາສາບຳບາລາ
bn = ພາສາເບັງກາລີ
bnm = Batanga
bnn = Bunun
bo = ພາສາຕິເບດ
br = ພາສາເບຣຕອນ
brh = Brahui
bri = Mokpwe
brx = Bodo
bs = ພາສາບອສເນຍ
bsh = Kateviri
bsk = Brushaski
bsy = Sabah Bisaya
btv = Bateri
bum = ບູລູ
bxk = Bukusu
bxr = Buryat
byv = Medumba
ca = ພາສາ​ກາ​ຕາ​ລັນ
cak = Kaqchikel
cdo = Eastern Min
ce = ພາສາເຊເຊັນ
cgg = Chiga
cjk = Chokwe
ckb = ຊາວເຄີດກາງ
cnh = Hakha Chin
co = ພາສາໂຄຊິແກນ
cpx = Heng Hua
cpy = Asheninka South Ucayali
crh = Crimean Tatar
cs = ພາສາເຊັກ
cut = Teutila Cuicatec
cux = Tepeuxila Cuicatec
cv = ພາ​ສາ​ຊູ​ວາດ​ສ
cy = ພາສາເວລສ
da = ພາ​ສາເດນ​ມາກ
dag = Dagbani
dar = Dargwa
dav = Kidaw'ida
de = ພາ​ສາ​ເຢຍ​ລະ​ມັນ
din = Dinka
dmk = Dawoodi
dml = Dameli
dru = Rukai
dsb = ພາສາຊໍບຽນຕອນລຸ່ມ
dua = Duala
dv = ພາສາດິເວຮິ
dyu = ພາສາ Dioula
ebr = Ebrie
ee = Ewe
efk = ເອຟິກ
eko = Ekoti
el = ພາສາເກ​ຣັກ
en = ພາສາອັງກິດ
eo = ພາສາເອັດ​ສ​ເປີ​ຣານ​ໂຕ
es = ພາສາສະເປນ
esu = Central Alaskan Yupʼik
et = ພາສາເອສໂຕນຽນ
eto = Eton
eu = ພາສາບາສເກະ
ewo = ອີວອນໂດ
fa = ພາສາເປີເຊຍ
fan = Fang
ff = ພາສາ​ຟູ​ລາ
fi = ພາສາຟິນແລນ
fmp = Feʼefeʼe
fo = ພາ​ສາ​ຟາ​ໂຣ
fr = ພາ​ສາ​ຝ​ຣັ່ງ
fub = Adamawa Fulfulde
fue = Borgu Fulfulde
fuf = Pular Guinea
fy-NL = ພາສາ​ຟ​ຣີ​ຊຽນ
ga-IE = ພາສາໄອຣິສ
gaa = Ga
gcf = ມາຕິນິກັນ ເຄຣໂອລ
gej = Mina
ggg = Gurgula
gid = Guidar
gig = Goaria
giz = Guiziga
gjk = Kachhi
gju = Gujari
gl = ພາສາກາລິຊຽນ
glk = ກີລາກີ
gn = ພາ​ສາກົວ​ຣາ​ນີ
gom = Goan Konkani
gos = Gronings
gsw = Alsatian
gu-IN = ພາສາກູຈາຣາຕີ
guc = Wayuunaiki
gv = Manx
gwc = Gawri
gwt = Gawarbaiti
gya = ຕາເວັນຕົກສຽງເໜືອ Gbaya
ha = ພາສາໂຮຊາ
hac = Gorani
hak = ແຮກກາ
haz = Hazargi
hch = Wixárika
he = ພາສາຢິວ
hem = Kihemba
hi = ພາສາຮີນດູ
hil = ພາ​ສາຮິ​ລິ​ເກນອນ
hno = ພາກເຫນືອ Hindko
hr = ພາສາໂຄຣເອເຊຍ
hrx = Hunsrik
hsb = Sorbian, ເທິງ
ht = ພາສາໄຮຕິ
hu = ພາສາຮົງກາລີ
hux = Nüpode Huitoto
hy-AM = ພາສາອາເມເນຍ
ia = ພາ​ສາ​ອິນ​ເຕີ​ລິງ​ກົວ
iba = Iban
ibb = Ibibio
id = ພາ​ສາອິນ​ໂດ​ເນ​ເຊຍ
ie = ພາ​ສາ​ອິນ​ເຕີ​ລິງ​
ig = ພາສາອິກ​ໂບ
ipk = Iñupiaq
is = ພາສາໄອສແລນ
it = ພາສາອີຕາລີ
izh = Izhorian
ja = ພາສາຍີ່ປຸ່ນ
jam = Jamaican Patois
jbo = Lojban
jgo = Ngomba
jqr = Jaqaru
jv = ພາສາຈາວານີສ
ka = ພາສາຈໍເຈຍ
kaa = Karakalpak
kab = ພາສາກາໄບເລ
kam = Kikamba
kbd = Kabardian
kcn = Nubi
kdh = Kotokoli
khw = Khowar
ki = ພາສາຄິຄູຢຸ
kjh = ຄາກາສ
kk = ພາ​ສາ​ຄາ​ຊັກ
kln = Kalenjin
kls = Kalasha
km = ພາສາກຳປູເຈຍ
kmr = Kurmanji Kurdish
kn = ພາສາກັນນາດາ
knn = ຄອນຄານີ (ເທວະນາການຣີ)
ko = ພາສາເກົາຫຼີ
koo = Konzo
kpv = ໂຄມິ-ຊີຣຽນ
krc = Karachay-Balkar
ks = Kashmiri
ksf = Bafia
kvx = Parkari Koli
kw = ພາ​ສາ​ຄໍ​ນິດ
kxp = Wadiyara Koli
ky = ພາສາກິຣກິສ
kzi = Kelabit
laj = ລາງໂກ
lb = ພາ​ສາ​ລຸກ​ຊຳ​ບວກ
led = Lendu
leu = Kara
lg = Luganda
lij = ພາ​ສາ​ລິ​ກູ​ຣຽນ
lke = Kenyi
lld = Ladin
ln = ພາສາລິງກາລາ
lo = ພາສາລາວ
lrk = Loarki
lrl = Achomi
lss = Lassi
lt = ພາສາລີທົວເນຍ
ltg = Latgalian
lth = Thur
lua = Tshiluba
luo = Dholuo
lv = ພາສາລັດເວຍ
lzz = Laz
mai = ພາ​ສາ​ໄມ​ທິ​ລິ
mau = Huautla Mazatec
mbf = Baba Malay
mbo = Mbo
mcf = Matses
mcn = Massa
mcx = Mpiemo
mdd = Mbum
mdf = ພາສາໂມກສາ
meh = Mixteco Yucuhiti
mel = Melanau
mfe = Kreol Morisien
mg = ພາ​ສາມາ​ລາ​ກາ​ຊີ
mgg = Kunabembe
mhk = Mungaka
mhr = Meadow Mari
mk = ພາສາມາເຊໂດເນຍ
mki = Dhatki
ml = ພາ​ສາ​ມາ​ລາ​ຢາ​ລຳ
mmc = Michoacán Mazahua
mn = ພາສາມົງໂກລີ
mni = Meetei Lon
mnw = ມອນ
mos = ພາ​ສາ​ມອດ​ຊີ
mqh = Tlazoyaltepec Mixtec
mr = ພາສາມາຣາຕີ
mrh = Mara
mrj = Hill Mari
ms = ພາສາມາເລ
mse = Moussey
msi = Sabah Malay
mt = ພາສາມອນຕາ
mua = Mundang
mug = Musgum
mve = Pakistani Marwari
mvy = Indus Kohistani
mxu = Mada
my = ພາ​ສາພະ​ມ້​າ
myv = Erzya
nan-tw = ໄຕ້ຫວັນ (ມິນນານ)
nb-NO = ພາສານໍເວບັອກມວລ
ncx = Central Puebla Nahuatl
nd = IsiNdebele (ເໜືອ)
ne-NP = ພາສາເນປານ
new = Newari
nhe = ຕາເວັນອອກ Huasteca Nahuatl
nhi = ຕາເວັນຕົກ Sierra Puebla Nahuatl
nia = Nias
nl = ພາສາເນເທີແລນ
nla = Ngombale
nlv = Orizaba Nahuatl
nmg = Kwasio
nmz = Losso
nn-NO = ພາສານໍເວນີນຣອກ
nnh = Ngiembon
nqo = NKo
nr = IsiNdebele (ໃຕ້)
nso = ໂຊໂທເໜືອ
ny = Chinyanja
nyn = Runyankole
nyu = Nyungwe
oc = ພາ​ສາ​ອອກ​ຊີ​ຕັນ
odk = Oadki
om = Afaan Ormoo
or = ພາສາໂອເດຍ
oru = Ormuri
os = ພາສາໂອສເຊຊຽນ
pa-IN = ພາສາປັນຈາບ
pap = Papiamento
pap-AW = Papiamento (Aruba)
pau = ປາລາວ
pcd = Picard
pcm = ໄນຈີເຣຍ Pidgin ພາສາອັງກິດ
pez = ເປນານຕາເວັນອອກ
phl = Palula
phr = Pahari-Pothwari
pl = ພາສາໂປແລນ
plk = Kohistani Shina
pne = ຕາເວັນຕົກ Penan
prq = Asheninka Perene
ps = ພາສາປາສໂທ
pt = ພາສາໂປຣຕຸເກສ
pua = Western Highland Purepecha
pwn = Paiwan
quc = K'iche'
qug = ເນີນສູງ Quichua Chimborazo
qup = Southern Pastaza Quechua
qur = Quechua Yanahuanca
qus = Quechua Santiago del Estero
qux = Quechua Yauyos
quy = Quechua Chanka
qva = Quechua Ambo-Pasco
qvi = Kichwa
qvj = Loja Highland Kichwa
qvl = Quechua Cajatambo
qwa = Quechua Corongo Ancash
qws = Quechua Sihuas Ancash
qxa = Quechua Chiquián
qxp = Puno Quechua
qxq = Qashqai
qxt = Quechua Pasco Santa Ana de Tusi
qxu = Quechua Arequipa-La Unión
qxw = Quechua Jauja Wanka
rif = Tarifit
rki = ຍະໄຂ່
rm-sursilv = Romansh Sursilva
rm-vallader = Romansh Vallader
rn = Kirundi
ro = ພາສາໂລມາເນຍ
rof = Kirombo
ru = ພາສາລັດເຊຍ
ruc = Ruuli
rup = Aromanian
rw = ພາສາຄິນຢາວັນດາ
rwm = Amba
sah = ພາສາຊາກາ
sat = Santali (Ol Chiki)
sba = ງຳເບ
sbn = Sindhi Bhili
sc = ພາ​ສາ​ຊາ​ດິ​ນຽນ
scl = Shina
scn = ພາ​ສາ​ຊິ​ຊິ​ລຽນ
sco = ສະກັອດ
sd = ພາສາ Sindhi
sdh = ຊາວເຄີດໃຕ້
sdo = Serian Bidayuh
seh = Sena
sei = Seri
sg = ຊັງໂກ
shi = ພາສາຊິລຮາ
shn = Shan
si = ພາສາຊິນຮາລາ
sk = ພາ​ສາສະ​ໂລ​ວັກ
skr = ພາສາຊາຣາກິ
sl = ພາສາສະໂລເວເນຍ
sn = Shona
snk = Soninke
snv = Saʼban
so = ພາສາໂຊ​ມາ​ລີ
sq = ພາສາອານບາເນຍ
sr = ພາສາເຊີເບຍ
ss = ພາສາ Siswati
ssi = Sansi
st = Sotho ພາກໃຕ້
su = ພາ​ສາ​ຊຸນ​ດາ
sv-SE = ພາສາສະວີເດັນ
sva = Svan
sw = ພາສາສະວາຮີລິ
swb = ຊິມາໂອເຣ
syr = ພາສາຊີຣິອາກ
szy = Sakizaya
ta = ພາສາທະມິນ
tar = Central Tarahumara
tay = Atayal
te = ພາ​ສາ​ເຕ​ລູ​ກູ
teg = Teke
tg = ພາສາທາຈິກ
th = ພາສາໄທ
ti = ພາສາທິກຣິນຢາ
tig = ພາສາໄທເກຣ
tiv = Tiv
tk = ພາສາເຕຣິກເມນິສຖານ
tl = ພາ​ສາ​ຕາ​ກາ​ລັອກ
tli = Tlingit
tn = Setswana
tob = Toba Qom
tok = Toki Pona
top = Papantla Totonac
tr = ພາສາຕຸຣະກີ
trv = Seediq
trw = Torwali
ts = Xitsonga
tt = ພາສາຕາຕ່າ
ttj = Rutoro
tui = Tupuri
tum = ຕຳບູກາ
tuq = ທູດາກາ
tvu = Tunen
tw = ພາສາຕະວີ
ty = ພາສາຕາຮິຕຽນ
tyv = ຕູວານ
uby = Ubykh
udl = Ouldémé
udm = ພາສາຢູດເມີດ
ug = ພາສາອຸຍເກີກ
uk = ພາສາຢູເຄຣນ
ukv = Kuku
ur = ພາສາອູຣດູ
ush = Ushojo
usp = ອຸດສະປັນເຕໂກ
uz = ພາສາອຸສເບກິສຖານ
var = Huarijio
ve = Tshevenda
vec = ພາ​ສາ​ເວ​ເນ​ຕຽນ
vi = ພາສາຫວຽດນາມ
vmw = Emakhuwa
vot = Votic
wbl = Wakhi
wep = Westphalian
wes = Cameroon Pidgin
wo = ພາສາໂວລັອບ
xdq = Kaitag
xh = ພາສາໂຊຊາ
xhe = Ketrani
xka = Kalkoti
xkl = Kenyah
xmf = Mingrelian
xsm = Kasem
yaq = Yaqui
yav = Nuasue
ydg = Yadgha
yi = ພາສາຢີວ
yo = ພາສາໂຢຣູບາ
yue = ພາສາກວາງຕຸ້ງ
zgh = Tamazight
zh-CN = ພາສາຈີນ (ຈີນ)
zh-HK = ພາສາຈີນ (ຮົງກົງ)
zh-TW = ພາສາຈີນ (ໄຕ້ຫວັນ)
zoc = Copainalá Zoque
zu = ພາສາຊູລູ
zza = Zaza

# [/]


## Layout

profile = ໂປ​ຣ​ໄຟ​ລ
help = ຊ່ວຍເຫລືອ
contact = ຕິດຕໍ່
privacy = ຄວາມເປັນສ່ວນຕົວ
terms = ຂໍ້ກຳນົດ
cookies = ຄຸກກີ້
faq = ຄຳຖາມທີພົບເປັນປະຈຳ
documentation = ເອກະສານ
content-license-text = ເນື້ອຫາທີ່ມີຢູ່ພາຍໃຕ້ <licenseLink>ໃບອະນຸຍາດ Creative Commons</licenseLink>
share-title = ຊ່ວຍພວກເຮົາຊອກຫາຄົນອື່ນເພື່ອບໍລິຈາກສຽງຂອງເຂົາເຈົ້າ!
share-text = ຊ່ວຍສອນເຄື່ອງຈັກໃຫ້ຄົນເວົ້າແທ້ໆ, ບໍລິຈາກສຽງຂອງເຈົ້າໄດ້ທີ່ { $link }
link-copied = ໄດ້ສຳເນົາລີ້ງໄວ້ແລ້ວ
back-top = ກັບຂື້ນໄປເທິງ
logout = ອອກຈາກລະບົບ
donate = ບໍລິຈາກ
stayintouch = ພວກເຮົາທຸກຄົນທີ່ Mozilla ກໍາລັງສ້າງຊຸມຊົນກ່ຽວກັບເຕັກໂນໂລຢີດ້ານສຽງ. ພວກເຮົາຕ້ອງການຕິດຕາມຂໍ້ມູນໃຫມ່ໆ, ແຫຼ່ງຂໍ້ມູນໃຫມ່ຕ່າງໆ ແລະ ເພື່ອເຮັດໃຫ້ຮູ້ຫຼາຍຂື້ນວ່າທ່ານໃຊ້ຂໍ້ມູນນີ້ແບບໃດ.
privacy-info = ພວກເຮົາສັນຍາວ່າຈະຈັດການຂໍ້ມູນຂອງທ່ານດ້ວຍຄວາມລະມັດລະວັງ. ອ່ານເພີ່ມເຕີມໃນ <privacyLink> ແຈ້ງການກ່ຽວກັບຄວາມເປັນສ່ວນຕົວ </privacyLink> ຂອງພວກເຮົາ.
return-to-cv = ກັບໄປທີ່ Common Voice
email-input =
    .label = ອີເມລ
loading = ກຳລັງໂຫລດ…
email-opt-in-info = ຂ້ອຍຕ້ອງການຮັບອີເມວເຊັ່ນ: ການແຈ້ງເຕືອນເປົ້າໝາຍ, ອັບເດດຄວາມຄືບໜ້າຂອງຂ້ອຍ ແລະຈົດໝາຍຂ່າວກ່ຽວກັບ Common Voice.
email-opt-in-info-title = ເຂົ້າຮ່ວມບັນຊີລາຍຊື່ທາງໄປສະນີຂອງ Voice ທົ່ວໄປ
email-opt-in-info-sub-with-challenge = ຮັບອີເມວເຊັ່ນ: ການທ້າທາຍ ແລະການແຈ້ງເຕືອນເປົ້າໝາຍ, ການອັບເດດຄວາມຄືບໜ້າ, ແລະຈົດໝາຍຂ່າວກ່ຽວກັບ Common Voice.
email-opt-in-privacy-v2 = ໂດຍການເລືອກຮັບອີເມວແມ່ນທ່ານລະບຸວ່າທ່ານບໍ່ເປັນຫຍັງທີ່ Mozilla ຈັດການຂໍ້ມູນນີ້ຕາມທີ່ອະທິບາຍໄວ້ໃນ <privacyLink>ນະໂຍບາຍຄວາມເປັນສ່ວນຕົວ</privacyLink> ຂອງ Mozilla.
indicates-required = ຊ່ອງຂໍ້ມູນທີ່ມີເຄື່ອງໝາຍດາວ (*) ແມ່ນຕ້ອງການ.
not-available-abbreviation = N/A
# Text shown on a button that can be clicked to copy text to the clipboard
copy = ສຳເນົາ
# Text shown on a button that can be clicked to close a modal or panel
close = ປິດ
# Text shown on a button that can be clicked to cancel an action
cancel = ຍົກເລີກ
# Text shown on a button that can be clicked to delete an item
delete = ລຶບ

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>ພວກເຮົາຕ້ອງການ</mark><br/><mark>ການຊ່ວຍເຫຼືອຂອງທ່ານ!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = ມັນມີມູນຄ່າເກືອບຫນຶ່ງລ້ານໂດລາຕໍ່ປີເພື່ອເກັບຊຸດຂໍ້ມູນ ແລະ ປັບປຸງເວທີສໍາລັບຊຸມຊົນພາສາ 100+ ຜູ້ທີ່ອີງໃສ່ສິ່ງທີ່ພວກເຮົາເຮັດ. <strong>ຖ້າທ່ານໃຫ້ຄ່າເປີດ, ລວມຂໍ້ມູນ - ບໍລິຈາກມື້ນີ້!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>ໃຊ້ CV ສໍາລັບ</mark><br/><mark>ການຄົ້ນຄວ້າຂອງທ່ານບໍ?</mark>
languages-donate-banner-cta-explanation = Common Voice ໄດ້ຮັບທຶນຈາກການບໍລິຈາກ ແລະ ການຊ່ວຍເຫຼືອລ້າ! ພວກເຮົາມັກຮ່ວມມືກັບນັກວິຊາການ, ສັງຄົມພົນລະເຮືອນ ແລະ ນັກຄົ້ນຄວ້າອຸດສາຫະກໍາ. Common Voice ແມ່ນບໍ່ເສຍຄ່າເພື່ອໃຊ້, ແຕ່ການປະກອບສ່ວນໃນເວທີ ແລະ ຄ່າໃຊ້ຈ່າຍໃນການເກັບຊຸດຂໍ້ມູນໂດຍຜ່ານການສະເຫນີການຊ່ວຍເຫຼືອແມ່ນເປັນປະໂຫຍດຫຼາຍ.
localization-select =
    .label = ເລືອກພາສາ/ທ້ອງຖິ່ນ

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = <strong>ຊຸດຂໍ້ມູນ Common Voice ໃໝ່</strong> ຕອນນີ້ມີໃຫ້ດາວໂຫຼດສະເພາະຜ່ານແພລດຟອມເອື້ອຍຂອງພວກເຮົາ, Mozilla Data Collective. ເຂົ້າຮ່ວມເພື່ອເຂົ້າເຖິງ 300+ ຊຸດຂໍ້ມູນທົ່ວໂລກ, ສ້າງໂດຍ ແລະສໍາລັບຊຸມຊົນ.
# Button text for MDC announcement
announcement-mdc-button-text = ເຂົ້າຮ່ວມ Mozilla Data Collective
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = ເປີດໃນແຖບໃໝ່

## Release Delay Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-delay = ເຖິງຊຸມຊົນທີ່ຮັກແພງ, ພວກເຮົາກຳລັງເຮັດວຽກກ່ຽວກັບການປ່ອຍອອກມາ. ພວກເຮົາຄາດວ່າຈະມີການເປີດຕົວເປັນສ່ວນໆໃນອີກບໍ່ກີ່ມື້ຂ້າງໜ້າໃນ <mdcLink>Mozilla Data Collective</mdcLink>.

## Release Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release = ເຖິງຊຸມຊົນທີ່ຮັກແພງ, ລຸ້ນໃໝ່ນີ້ມີຢູ່ໃນ <mdcLink>Mozilla Data Collective</mdcLink> ຢ່າງຄົບຖ້ວນແລ້ວ. ເຂົ້າຮ່ວມເພື່ອເຂົ້າເຖິງຊຸດຂໍ້ມູນທົ່ວໂລກຫຼາຍກວ່າ 500 ຊຸດ, ສ້າງໂດຍ ແລະ ສຳລັບຊຸມຊົນ.

## Pre-Release Announcement (dates TBD)

# Text wrapped in <cvDatasheetsLink></cvDatasheetsLink> will be rendered as an external link to the cv-datasheets GitHub repo
announcement-pre-release = ເຖິງຊຸມຊົນທີ່ຮັກແພງ, ເວລາປ່ອຍຕົວໃກ້ຈະຮອດແລ້ວ. ກະລຸນາໄປກວດສອບຄວາມຖືກຕ້ອງ ແລະ ຖອດຂໍ້ຄວາມ, ແລະ ເຂົ້າໄປທີ່ <cvDatasheetsLink>cv-datasheets</cvDatasheetsLink> ເພື່ອອັບເດດແຜ່ນຂໍ້ມູນຂອງທ່ານ.

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = ເລີ່ມພິມເພື່ອຄົ້ນຫາ
searchbox-default-placeholder = ຄົ້ນຫາ…
searchbox-default-aria-label = ຄົ້ນຫາ
dropdown-no-results = ບໍ່ພົບລາຍການທີ່ກົງກັນ
# Common Voice Dataset=Language search box
dataset-searchbox-label = ຈະປ່ຽນທັງພາສາສຳລັບການສະແດງຜົນ ແລະ ຊຸດຂໍ້ມູນ
dataset-searchbox-placeholder = ຄົ້ນຫາພາສາ...
dataset-searchbox-aria-label = ຄົ້ນຫາພາສາ
dataset-search-no-results = ບໍ່ພົບພາສາທີ່ກົງກັນ
request-new-language = ຂໍພາສາໃໝ່
