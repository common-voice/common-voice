listen = Poslušaj
listen-subtitle = Pomagajte nam potrditi glasove
listen-instruction = { $actionType } <playIcon></playIcon> ali je bil stavek pravilno izgovorjen?
listen-again-instruction = Odlično opravljeno!<playIcon></playIcon> Poslušajte znova, ko boste pripravljeni.
listen-3rd-time-instruction = 2 sta pod streho, kar tako naprej! <playIcon></playIcon>
listen-last-time-instruction = <playIcon></playIcon> Še zadnji!
listen-empty-state = Zmanjkalo nam je posnetkov za potrditev v tem jeziku ...
listen-abort-title = Želite končati s preverjanjem posnetkov?
listen-abort-cancel = Nadaljujte s potrjevanjem
listen-abort-confirm = Dokončajte potrjevanje
# Menu item
validate-readings = Preverite izgovore
# text wrapped in <small></small> will be shown in a smaller font size
validate-readings-coming-soon = Preverite izgovore <small>(kmalu)</small>

## MENU ITEM TOOLTIPS

listen-contribute-menu-tooltip = Preverite izgovore, preglejte prepise
review-transcriptions-menu-item-tooltip = Preglejte prepise, ki so jih predložili uporabniki v vašem jeziku

## MENU ITEM ARIA LABELS

listen-contribute-menu-aria-label = Meni možnosti poslušanja
review-transcriptions-menu-item-aria-label = Preverite točnost prepisov posnetih stavkov
