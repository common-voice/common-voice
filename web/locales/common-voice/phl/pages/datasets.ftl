## Datasets Page

datasets-heading = ڈیٹا سیٹ
datasets-headline = بے آک اوپن سورس سمیانہ۔ بیڈی زبینیمی دپارہ ڈیٹاسیٹ سمیانہ کی ایڑا ہر آک تیݨی بیتی آک اپلکیشنی وےترتیب دئنی دپارہ استعمال تھائی بھوت۔
datasets-positioning =
    اسام تھے یقین ہِنی کی بُٹھیمی دپارہ دستیاب ہاویزی ڈیٹا سیٹ نیوی ایجادات او مشین ازدہ تھئنی بنیاد کی بیتی ٹیکنالوجی او تجارتی مقابلہ زیات بھی۔ 
    
    کامن وائس بیڈی زبینیمی واران ڈیٹاسیٹ آویلی نیگرا بُٹھیم دی گاڈو عام خلکیمی دپارہ ہاویزیمی ڈیٹا سیٹ مگم انو صرف آک نہ۔
    
    انو صفحہ دوئی اوپن سورس ڈیٹاسیٹی حوالئی دپارہ دڇھوئی او کامن وائس کی گاڈو بھئیم بھی وڑئی، اسی خبری دئنی دپارہ آک زھئی۔
datasets-speech-bubble-content = دلچسپ خبر! بے ساختہ تقریر جاری ہنی! اپ ڈیٹس دہارہ دڇھی ہونسوئیں او تازہ ترین معلوماتی دپارہ انو لنک  <discord Link>Discord</discord Link> چیک تھئینی یقینی جوݜوئی۔
language = زُبان
download-dataset-header = ڈیٹا سیٹ ڈونلوڈ تھہ
download-delta-explainer = اسم توش بدل تھیلو۔ پروݨہ کراممی بادڈٰیلٹا حصہ ناؤہ ہاواز چھوݨو <deltaLink> ۔ انی کرامی بارئی دوئی معلومات انیوے</deltaLink> زھئی وے ہِنی۔
download-dataset-tag = تیݨی خواہشی مطابقی زبینی ڈیٹآسیٹ منتخب تھہ۔ تو کھئیو ورژن  دوائینو ایتس ڈون لوڈ تھہ۔
# File size in gigabytes
size = سائز
validated-hr-total = مستند بھلہ واندہ  ایچ آر مکمل
overall-hr-total = تمام  ایچ آر مکمل
cv-license = لائسنس
audio-format = ہاویزی ساخت
dataset-splits = تقسیم(عمر او  جنس)
number-of-voices = ہاویزیمی تعداد
splits = تقسیم
email-to-download = ڈون لوڈ تھئنی دپارہ ای میل دہ
why-email = <b> ای میل کے دوائینو</b> بے تسام تھے یھاٹہ وختی ڈیٹا سیٹ بدل تھئنی دپارہ رابطہ تھینڈیو بھی۔ ای میل تُسام سنگی رابطہ تھئنی زریعہ میسر تھی۔
why-donate-datasets-page = موزیلا کامن وائس دُنیئی بُٹھیم دی مختلف خلکیمی ازدہ تھئینی سمِجیلو او کھولاؤ بیتیمی ڈیٹا سیٹ۔ بے مکمل طور کی عطیمی زریعہ کی کرام تھانہ۔ سالانا آک ملین ڈالر ڈیٹا سیٹ  انٹرنیٹی چھورئنی دپارہ او ڑیوے دوئی کرام تھئنی دپارہ ضرورت بھانہ۔ بے گُبا گلہ تھانہ ایڑا دی پان بھِشہ دی زیات زُبینیمی خلکہ ڑا دی فائدہ گِھنانہ۔ اگر تو ڑائی اہمیت جھانانو ہینتہ انو ڈیٹا کھولاؤ تھہ او آجے مالی مدد تھہ
confirm-size = تو اندوئی تھی<b>{ $size }</b> ڈاون لوڈ شروع تھائی بھوڑ۔
size-gigabyte = جی بی
size-megabyte = ایم بی
confirm-no-identify = <b> تو متفق کی</b>  تو  کامن وائس ڈیٹا سیٹی وے کھونڈاٹمی شناخت تھئنی کوشش نہ تھیڑ تھنی۔
confirm-join-mailing-list = تو  کامن وائسو رابطئی لیسٹی وے بئنی دوائیانو ہینتہ
download-language = ڈون لوڈ  بھی { $language }
validated-hours = سند بِھلو گھنٹہ
recorded-hours = ریکاڈ بِھلو گھنتہ
whats-inside = کامن وائسی ڈیٹآ سیٹی شیٹی گُبا ہِنو؟
dataset-description-hours =
    ڈیٹا سیٹی چھوݨہ بھاؤ ہر شئی ایم پی 3 او تس سنگی لیشی فائلیم سنگی۔ ڈیٹاسیٹی وے زیات <b>{ $total }</b> ریکاڈ بھلہ گھنٹے خاندانی معلومات مثلا عمر، جنس او لہجہ کی سے بات جھانیٹی مشینی صحی تربیت دئنی مجی مدد تھینی۔ 
    
    موجودہ وختی ڈیٹآ سیٹ<b>{ $languages }زبینیمی  <b>{ $valid }</b> سند بھلو بھاؤ گھنٹہ کی مشتمل ہنو۔ مگم بے ہر وختی دوئی ہاویزی او دوئی زبینی شامل تھانہ۔۔انی زھئی <languagesLink>  زبینیمی صفحہ دڇھہ کہ زبینی دپارہ  درخواست یا مدد تھئنی دوائیانو ہینتہ۔
want-dataset-update = تو آگاہ بھئنی دوائیانو ہینتہ  کی اسی کمن وائسی ڈیٹا سیٹی ناؤ ورژن کرے گڈِیہ تھنی؟ تھے اسپہ نیوز لیٹر سبسکرائب تھہ
subscribe = سبسکرائب
get-started-speech = بات جھاناٹہ جھلی کرام شروع تھہ
other-datasets = دوئی وائس ڈیٹا سیٹ
feedback-q = تبصرہ ہنوے؟
# This indicates that there is no data to display
no-information = معلومات نئنی
resource-nemo-info = <githubLink>NVIDIA NeMo</githubLink>™ آک <docsLink>اوپن سورس سامان </docsLink> محقیقینی دپارہ اعلی  AI مشقولگی آک نمونہ سمئنی دپارہ۔
resource-deepspeech-info = موزیلا اوپن سورس ہاواز جھانئنی ہ انجن "ڈیپ سپیچ" بیتی جھانئنی  اپلیکیشن سمئنی دپارہ استعمال بھانو ے۔  بے <githubLink> "گیتھب" دڇھی </githubLink> یا شامل بھیڑ<discourseLink> "ڈیپ سپیچ" مشقول بھہ </discourseLink> کنا تھے شروع تھوم تھنی ازدہ تھئنی دپارہ۔
resource-coqui-info = <coquiLink>کوقی </coquiLink> آزاد بات دئنی دپارہ ہنو۔ ٹیکنالوجی کرام ازدہ تھئنی بنیاد کی STT وا TTS انجن شامل ہنو۔
community-playbook = کمنیوٹی پلے بک
resource-playbook-info = <playbookLink> بُٹھے کامن وائسی بارئی رہنمائی انی زھئی</playbookLink>وے ہنہ۔ ایڑام مجی مقامی تھئنی گھنی ڈیٹاسیٹی استعمیلی تی او اسی کمیونٹی وے کنا بھے یھین تھنی معلومات ہنی۔
data-other-librispeech-description = لیبری بکس تقریبا 1000 گھنٹے 16Khzمناٹہ آک انگریزی تقریری مجموعہ  ایڑے لیبری وکس پروجکٹی  ہاویزی کتینی دی گڈی ہنہ۔
data-other-ted-name = ٹیڈ-لیئم ہاویزیمی مجموعہ
data-other-ted-description = ٹیڈ-لیئمی مجموعہ ہاویزی مشقولگی دی سمجی ہنہ او مشقولگی وے چونٹولو ٹیڈ ویپ سائٹہ وے دستیاب ہنہ۔
data-other-voxforge-description = وکس فورج چونٹیلی ویندی تقریری جمع تھئنی دپارہ قائم بھلو ہنو۔ انی وے مفت او اوپن سورس تقریری ہوش تھیی انجنی وے  استعمال بھیدے۔
data-other-tatoeba-description = تاتوئبا جملیمی، ترجمیمی، او منیتہ ہاویزیمی ڈیٹابیس کی ڑو زبان ازدہ تھئنی دپارہ ا استعمال بھانو۔ انیوےڈاون لوڈی وے کمیونیٹی ریکاڈ تھیلی بیتی  انگلیش کی ہنہ۔
your-feedback = تو دی گہ ائیڈیے ہنے کی بے کامن وائس کنا تھے مزید بہتر تھیہ؟ مشقوگی کی اسام سنگی کھونڈ۔
go-discourse = مشقولگی وے بہ
missing-language = تہ زبان کی ڈِیٹا سیٹی وے نئینی ہینتہ۔ تیݨی زبان شامل تھئنی دپارہ  زبینیمی صفحہ تھے بہ۔
go-languages-page = زُبینیمی  صفحہ تھے بہ
ready-to-validate = صحیح جملے مدد تھئنی دپارہ تیارے؟
more = دوئی بی
download = ڈون لوڈ
dataset-version = ورژن
sha256-checksum-copied = SHA256 چیکسم کاپی بھلی!
sha256-checksum-copied-error = SHA256 چیکسم کاپی نہ بھلی
clipboard-not-supported = کلپ بورڈ سپورٹ نہ تھانو
no-information-available = معلومات دستیاب نئنی
dataset-metadata-sex = جنس
# dataset metadata - age of contributor
dataset-metadata-age = عُمر
donate-modal-message = تھی ڈیٹا سیٹ ڈون لوڈ بھانو
dataset-donate-modal-heading = تو تھے معلومے؟
donate-modal-explanation-1 = سالان تقریبا آک میلین ڈالر دیٹابیس نیٹی وے چھورئنی او بہتر تھئنی دپارہ خرچ بھانو۔ بے کھئیو کرام تھانہ انی جھلی پانج بحےشہ دی زیات زبینیمی خلکہ بھروسہ تھانہ۔
# Text in <bold></bold> will shown in bold
donate-modal-explanation-2 = <bold> تو اگر اسی معلومیتمی قدر تھانو ہینتہ آجے اسام سنگی مالی تعاون تھہ
data-download-singleword-title = سنگل ورڈ ٹارگیٹ سیگمنٹ ڈونلوڈ بھلو
data-download-singleword-callout-v2 = انو استعمالی جھلی چلئیجاٹو حصہ  کی لیوے بات دئنی نمبری شناخت، او یا نہ پتہ لگئنی ڈیٹا ہانسانہ۔
data-download-button = کامن وائسی معلومات ڈونلوڈ بھلو
data-download-yes = او
data-download-deny = نہ
data-download-license = لائسینس: <licenseLink>{ $license }</licenseLink>
data-download-modal = تو اندوئی تھی<b>{ $size }</b> ڈاون لوڈ شروع تھائی بھوڑ۔
data-subtitle = بے آک اوپن سورس سمیانہ۔ بیڈی زبینیمی دپارہ ڈیٹاسیٹ سمیانہ کی ایڑا ہر آک تیݨی بیتی آک اپلکیشنی وےترتیب دئنی دپارہ استعمال تھائی بھوت۔
data-explanatory-text =
    اسام تھے یقین ہِنی کی بُٹھیمی دپارہ دستیاب ہاویزی ڈیٹا سیٹ نیوی ایجادات او مشین ازدہ تھئنی بنیاد کی بیتی ٹیکنالوجی او تجارتی مقابلہ زیات بھی۔ 
    
    کامن وائس بیڈی زبینیمی واران ڈیٹاسیٹ آویلی نیگرا بُٹھیم دی گاڈو عام خلکیمی دپارہ ہاویزیمی ڈیٹا سیٹ مگم انو صرف آک نہ۔
    
    انو صفحہ دوئی اوپن سورس ڈیٹاسیٹی حوالئی دپارہ دڇھوئی او کامن وائس کی گاڈو بھئیم بھی وڑئی، اسی خبری دئنی دپارہ آک زھئی۔
data-get-started = <speechBlogLink> تقریر ہوݰ تھاٹہ جھلی شروع تھہ</speechBlogLink>
data-other-title = دوئی وائس ڈیٹا سیٹ
data-other-goto = { $name } تھے بہ
data-other-download = ڈون لوڈ ڈیٹا
data-bundle-button = ڈیٹاسیٹی بنڈال ڈون لوڈ تھہ
data-bundle-description = کامن وائسی معلومات او تس جھلی بُٹھے دوئی ہاویزی ڈیٹاسیٹ
release-version = ورژن
dataset-date = تاریخ
license = لائسینس: <licenseLink>{ $license }</licenseLink>
license-mixed = مکس بھلو
# MENU ITEM TOOLTIPS
download-contribute-menu-tooltip = اسی وائس ڈیٹا سیٹ ڈون لوڈ تھہ
# MENU ITEM ARIA LABELS
download-contribute-menu-aria-label = کامن وائس ڈیٹا تی رسائی دپارہ اختیارات ڈاؤن لوڈ تھہ۔
