## Error pages

banner-error-slow-1 = Bóžko Common Voice pómału běžy. Źěkujomy se za waš zajm.
banner-error-slow-2 = Dostawamy wjele wobchada a pśepytujomy tuchylu problemy.
banner-error-slow-link = Statusowy bok
error-something-went-wrong = Hopla, něco njejo se raźiło
error-clip-upload = Nagrawanje toś togo klipa se pśecej zasej njeraźi, dalej wopytowaś?
error-clip-upload-server = Nagrawanje toś togo klipa se pśecej zasej na serwerje njeraźi. Zacytajśo bok znowego abo wopytajśo pózdźej hyšći raz.
error-clip-upload-too-large = Waša nagrawańska dataja jo pśewjelika za nagraśe. Wopytajśo pšosym krotšy klip nagraś.
error-clip-upload-server-error = Zmólka serwera pśi pśeźěłowanju wašogo klipa. Pšosym zacytajśo bok znowego abo wopytajśo pózdźej hyšći raz.
error-title-404 = Njejsmy mógli ten bok za was namakaś
error-content-404 = Snaź buźo naš <homepageLink>startowy bok</homepageLink> pomagaś? Aby pšašanje stajił, wobdělśo se pšosym na <matrixLink>chaśe Matrix zgromaźeństwa</matrixLink>, wobglědujśo sedłowe problemy pśez <githubLink>GitHub</githubLink> abo woglědajśo se k <discourseLink>našym forumam Discourse</discourseLink>.
error-title-429 =
    { $retryAfter ->
        [0] Sćo pśemalsny. Pšosym buźćo pómałčejšy a wopytajśo za wokognuśe hyšći raz.
        [one] Sćo pśemalsny. Pšosym wopytajśo za { NUMBER($retryAfter) } sekundu hyšći raz.
        [two] Sćo pśemalsny. Pšosym wopytajśo za { NUMBER($retryAfter) } sekunźe hyšći raz.
        [few] Sćo pśemalsny. Pšosym wopytajśo za { NUMBER($retryAfter) } sekundy hyšći raz.
       *[other] Sćo pśemalsny. Pšosym wopytajśo za { NUMBER($retryAfter) } sekundow hyšći raz.
    }
error-title-500 = Hopla, něco njejo se raźiło
error-content-500 = Njewócakana zmólka jo nastała. Pšosym wopytajśo pózdźej hyšći raz. Wobdělśo se pšosym za pomoc na <matrixLink>chaśe Matrix zgromaźeństwa</matrixLink>, wobglědujśo sedłowe problemy pśez <githubLink>GitHub</githubLink> abo woglědajśo se k <discourseLink>našym forumam Discourse</discourseLink>.
error-title-502 = Zwisk psétergnjony
error-content-502 = Njamóžośo stabilny zwisk z našymi serwerami direktnje zwězaś. Wobdělśo se pšosym za pomoc na <matrixLink>chaśe Matrix zgromaźeństwa</matrixLink>, wobglědujśo sedłowe problemy pśez <githubLink>GitHub</githubLink> abo woglědajśo se k <discourseLink>našym forumam Discourse</discourseLink>.
error-title-503 = Nazgónijomy njewótcakany cas stojanja
error-content-503 = Sedło buźo tak skóro ako móžno zasej k dispoziciji byś. Nejnowše informacije dostanjośo w  <matrixLink>chaśe Matrix zgromaźeństwa</matrixLink> abo woglědajśo se k <githubLink>GitHub</githubLink> abo <discourseLink>našym forumam Discourse</discourseLink>, aby problemy ze sedłom k wěsći dał abo wobglědował.
error-title-504 = Cas za napšašowanje pśekšocony
error-content-504 = Napšašowanje jo trało pśedłujko, aby se skóńcyło. To jo zwětšego nachylu. Pšosym wopytajśo hyšći raz. Wobdělśo se pšosym za pomoc na <matrixLink>chaśe Matrix zgromaźeństwa</matrixLink>, wobglědujśo sedłowe problemy pśez <githubLink>GitHub</githubLink> abo woglědajśo se k <discourseLink>našym forumam Discourse</discourseLink>.
error-code = Zmólka { $code }
# Warning message shown when none of the clips could be uploaded
error-duplicate-clips-all =
    { $total ->
        [one] Njejsmy mógli waš { $total } klip nagraś. Jo se južo nagrał do togo. Cyńmy z pśiducym štapjelom dalej!
        [two] Njejsmy mógli wašej { $total } klipa nagraś. Stej se južo nagrałej do togo. Cyńmy z pśiducym štapjelom dalej!
        [few] Njejsmy mógli waše { $total } klipy nagraś. Su se južo nagrali do togo. Cyńmy z pśiducym štapjelom dalej!
       *[other] Njejsmy mógli waše { $total } klipy nagraś. Su se južo nagrali do togo. Cyńmy z pśiducym štapjelom dalej!
    }
# Warning message shown when only some of the clips could be uploaded (uploaded count will be <5)
error-duplicate-clips-some = Smy nagrali { $uploaded } z wašych klipow – Zbytk jo se južo nagrał. Cyńmy z pśiducym štapjelom dalej!
