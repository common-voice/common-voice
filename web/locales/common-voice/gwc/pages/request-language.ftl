## Request Language Pages

request-language-heading = اٞ نیم جِباٞں باراٞ مئی تپوس کٞر
request-language-explanation-1 = اِیں بیر اُوں چھیں جِب <languagesPageLink>کامن وائیس آں جِباں پاݪ </languagesPageLink> رہ موجود تھی آ۔ لخہ نانت تے ایں فارم ڈاٞک کراٞ مہ تھاکہ اٞ <strong>ایمیل لاڑائے، تے تٞاس مئی تھاکہ پشائے اوں تھوں جِب مہ آس مئی کھیکی گھیاواں</strong>۔
request-language-form-email =
    .label = چھیں ایمیلاں پتہ
request-language-form-info =
    .label = جِباٞں باراٞ مئی مالومات
request-language-form-info-explanation = جِباٞں باٞراٞمئ کٞتیک مالموت جمع کٞروانش تے کٞر۔اٞن مئ شامل تھو؛
request-language-form-info-explanation-list-1 = چھیں جِباں نام
request-language-form-info-explanation-list-2 = ISO<isoCodeLink> کوڈ</isoCodeLink>، لخہ تھکہ مالوم آش تہ
request-language-form-info-explanation-list-3 = ویب سائیٹیں لنک، اُوں تن تھی ماکہ ایں جِباٞں باراٞ مئی پتہ ݭجئی
request-language-google-recaptcha-required = reCAPTCHA پکاٞر تھو لخہ تُو مِڄ کٞہ بچوگ دیانش تہ
request-language-google-recaptcha-error = اکثر مٞہ reCAPTCHA مئی خطا ہواٞں، ماربینی کاٞ دیگیراں کوشش کراٞ۔
# Success page
request-language-success-heading = ماربینی! تئی اٞ نیم جِبیں تحقیق لاڑیئے
request-language-success-list-1 = تھکہ شاردہ اٞ ایمیل یئی، ان مئی ایں مالومات ہوئے اوں تھوں جِب کامن وائیس مئی شامل کیر کٞہ کٞہ قداٞم راٞت کٞرجنت
request-language-success-list-2 = مُوں کمیونٹی آں منیجر تھا سہ رابطہ کٞرئی کہ چری تھاکہ دی مددیں ضرولاٞت ہی تہ، تے ایں مالومات پا کٞرئی اوں تھوں جِب مئی اٞ ڈلہ مِڄ گٞشی کار کٞرنت۔
request-language-success-list-3 = لخہ تئی کٞتیک دوس مئی ما ماں نہ بُد تے، ما سہ <emailLink>{ $email }</emailLink>رابطہ کراٞ تے ما تھا سہ دیگیر رابطہ کٞری۔
request-language-error = مہ کیدیشی اتھاں درخاس وصول نہ کیت۔ ماربینی کاٞ ماکہ اِیں دہ <emailLink></emailLink>ایمیل کٞرا تھوکو مہ تھا سہ رابطہ کٞری!
request-for-scripted-speech-toggle = مِیݭ گٞشی چُنڑاٞل جولاگیں کیر درخاس
request-for-spontaneous-speech-toggle = فوری طور دہ جولاگیں کیر درخاس
need-help-deciding-platform = جوݭوگ رہ فیصلہ کٞروگ مئ مٞدِدیں ضرولت
