...
spelling-and-pronunciation-explanation = 确保用字和语法正确。
length = 长度
length-explanation = 请提交只用几句话就能完成回答的问题。
...
offensive-content-sensitive-information-explanation-explanation-3 = 可能引导作出敏感回答，并可能由此引发不良后果的问题
...
transcribe-the-audio-subheader-1-explanation-example-4 = 遇到非标准语法和俚语时，应据实记录。不要更正或窜改他人发言。
...
transcribe-the-audio-subheader-5-explanation-1 = 即兴发言难免包含语法错误，转录时不应更正这些错误。转录文本应据实记录说话内容。
transcribe-the-audio-subheader-5-explanation-2 = 说话人可能会使用非正式的口语用法（例如“咱”“啥”等），这类用法需要据实转录，不应改为正式用语。
