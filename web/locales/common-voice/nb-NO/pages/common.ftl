# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Afar
aat = Arvanitika
ab = Abkhasisk
abb = Bankon
abq = Abaza
ace = Acehnesisk
ady = Adygeisk
af = Afrikaans
aii = Suret
ajg = Adja
aln = Gheg albansk
am = Amharisk
an = Aragonesisk
ar = Arabisk
arn = Mapudungun
as = Assamesisk
ast = Asturisk
az = Aserbajdsjansk
azz = Høylandet Puebla Nahuatl
ba = Basjkirsk
bag = Tuki
bal = Balochi
bas = Basaa
bax = Bamun
bba = Baatonum
bbj = Ghomala
bbl = Tush
bce = Mengambo
bci = Baoule
be = Hviterussisk
beb = Bamvele
bew = Betawi
bfd = Bafut
bft = Baltisk
bg = Bulgarsk
bgp = Østlige Balochi
bkh = Bakoko
bkm = Kom
bm = Bambara
bn = Bengalsk
bnm = Batanga
bnn = Bunun
bo = Tibetansk
br = Bretonsk
brh = Brahui
bri = Mokpwe
brx = Bodo
bs = Bosnisk
bsh = Kateviri
bsk = Brushaski
bsy = Sabah Bisaya
btv = Bateri
bum = Bulu
bxk = Bukusu
bxr = Burjatisk
byv = Medumba
ca = Katalansk
cak = Kakchikel
cdo = Østlig Min
ce = Tsjetsjensk
cgg = Chiga
cjk = Chokwe
ckb = Sentralkurdisk
cnh = Hakha Chin
co = Korsisk
cpx = Heng Hua
cpy = Asheninka Sør-Ucayali
crh = Krimtatarisk
cs = Tsjekkisk
cut = Teutila Cuicatec
cux = Tepeuxila Cuicatec
cv = Tsjuvasjisk
cy = Walisisk
da = Dansk
dag = Dagbani
dar = Dargwa
dav = Kidaw'ida
de = Tysk
din = Dinka
dmk = Dawoodi
dml = Dameli
dru = Rukai
dsb = Lavsorbisk
dua = Duala
dv = Dhivehi
dyu = Dioula
ebr = Ebrie
ee = Ewe
efk = Efik
eko = Ekoti
el = Gresk
en = Engelsk
eo = Esperanto
es = Spansk
esu = Sentralt Alaska Yup'ik
et = Estisk
eto = Eton
eu = Baskisk
ewo = Ewondo
fa = Persisk
fan = Fang
ff = Fulfulde
fi = Finsk
fmp = Feʼefeʼe
fo = Færøysk
fr = Fransk
fub = Adamawa Fulfulde
fue = Borgu Fulfulde
fuf = Fulfulde
fy-NL = Frisisk
ga-IE = Irsk
gaa = Ga
gej = Mina
ggg = Gurgula
gid = Guidar
gig = Goaria
giz = Guiziga
gjk = Kachhi
gju = Gujarati
gl = Galisisk
glk = Gileki
gn = Guarani
gom = Goan Konkani
gos = Gronings
gsw = Alsace
gu-IN = Gujarati
guc = Wayuunaiki
gv = Manx
gwc = Gawri
gwt = Gawarbaiti
gya = Nordvest Gbaya
ha = Hausa
hac = Gorani
hak = Hakka
haz = Hazargi
hch = Wixárika
he = Hebraisk
hem = Kihemba
hi = Hindi
hil = Hiligaynon
hno = Nordlige Hindko
hr = Kroatisk
hrx = Hunsrik
hsb = Høysorbisk
ht = Haitisk
hu = Ungarsk
hux = Nüpode Huitoto
hy-AM = Armensk
ia = Interlingua
iba = Iban
ibb = Ibibio
id = Indonesisk
ie = Interlingue
ig = Igbo
ipk = Inupiak
is = Islandsk
it = Italiensk
izh = Ingrisk
ja = Japansk
jam = Jamaican Patois
jbo = Lojban
jgo = Ngomba
jqr = Jaqaru
jv = Javanesisk
ka = Georgisk
kaa = Karakalpak
kab = Kabylsk
kam = Kikamba
kbd = Kabardisk
kcn = Nubi
kdh = Kotokoli
khw = Khowar
ki = Kikuyu
kjh = Khakassisk
kk = Kasakhisk
kln = Kalenjin
kls = Kalasha
km = Khmer
kmr = Nordkurdisk
kn = Kannada
knn = Konkani (Devanāgarī)
ko = Koreansk
koo = Konzo
kpv = Komi-syriansk
krc = Karatsjajbalkarsk
ks = Kasjmiri
ksf = Bafia
kvx = Parkari Koli
kw = Cornisk
kxp = Wadiyara Koli
ky = Kirgisisk
kzi = Kelabit
laj = Lango
lb = Luxembourgsk
led = Lendu
leu = Kara
lg = Luganda
lij = Ligurisk
lke = Kenyi
lld = Ladinsk
ln = Lingala
lo = Laotisk
lrk = Loarki
lrl = Achomi
lss = Lassi
lt = Litauisk
ltg = Lettgallisk
lth = Tor
lua = Tshiluba
luo = Dholuo
lv = Latvisk
lzz = Lasisk
mai = Maithili
mau = Huautla Mazatec
mbf = Baba Malay
mbo = Mbo
mcf = Matses
mcn = Massa
mcx = Mpiemo
mdd = Mbum
mdf = Moksja
meh = Mixteco Yucuhiti
mel = Melanau
mfe = Kreol Morisien
mg = Gassisk
mgg = Kunabembe
mhk = Mungaka
mhr = Østmarisk
mk = Makedonsk
mki = Dhatki
ml = Malayalam
mmc = Michoacán Mazahua
mn = Mongolsk
mni = Meitei-lon
mnw = Mon
mos = Mossi
mqh = Tlazoyaltepec Mixtec
mr = Marathi
mrh = Mara
mrj = Vestmarisk
ms = Malaysisk
mse = Moussey
msi = Sabah Malay
mt = Maltesisk
mua = Mundang
mug = Musgum
mve = Pakistansk marwari
mvy = Indus Kohistani
mxu = Mada
my = Burmesisk
myv = Erziamordvinsk
nan-tw = Taiwansk (Minnan)
nb-NO = Norsk (bokmål)
ncx = Sentrale Puebla Nahuatl
nd = Nordndebele
ne-NP = Nepalsk
new = Newari
nhe = Østlig la Huasteca-nahuatl
nhi = Vestlige Sierra Puebla Nahuatl
nia = Nias
nl = Nederlandsk
nla = Ngombale
nlv = Orizaba Nahuatl
nmg = Kwasio
nmz = Losso
nn-NO = Norsk (nynorsk)
nnh = Ngiembon
nqo = NKo
nr = Sørndebele
nso = Nordsotho
ny = Chinyanja
nyn = Runyankole
nyu = Nyungwe
oc = Oksitansk
odk = Oadki
om = Oromo
or = Oriya
oru = Ormuri
os = Ossetisk
pa-IN = Panjabi
pap = Papiamento
pap-AW = Papiamento (Aruba)
pau = Palauisk
pcd = Pikardisk
pcm = Nigeriansk Pidgin Engelsk
pez = Østlige Penan
phl = Palula
phr = Pahari-Pothwari
pl = Polsk
plk = Kohistani Shina
pne = Vestlige Penan
prq = Asheninka Perene
ps = Pashto
pt = Portugisisk
pua = Western Highland Purepecha
pwn = Paiwan
quc = K'iche'
qug = Quichua Chimborazo Highland
qup = Sørlige Pastaza Quechua
qur = Quechua Yanahuanca
qus = Quechua Santiago del Estero
qux = Quechua Yauyos
quy = Quechua Chanka
qva = Quechua Ambo-Pasco
qvi = Kichwa
qvj = Loja Highland Kichwa
qvl = Quechua Cajatambo
qwa = Quechua Corongo Ancash
qws = Quechua Sihuas Ancash
qxa = Quechua Chiquián
qxp = Puno Quechua
qxq = Qashqai
qxt = Quechua Pasco Santa Ana de Tusi
qxu = Quechua Arequipa-La Unión
qxw = Quechua Jauja Wanka
rif = Tarifit
rki = Rakhine
rm-sursilv = Sursilvan Retoromansk
rm-vallader = Vallader
rn = Kirundi
ro = Rumensk
rof = Kirombo
ru = Russisk
ruc = Ruuli
rup = Arumensk
rw = Kinyarwanda
rwm = Amba
sah = Jakutisk
sat = Santali (Ol Chiki)
sba = Ngambay
sbn = Sindhi Bhili
sc = Sardisk
scl = Shina
scn = Siciliansk
sco = Skotsk
sd = Sindhi
sdh = Sørkurdisk
sdo = Serian Bidayuh
seh = Sena
sei = Seri
shi = Shilha
shn = Shan
si = Singalesisk
sk = Slovakisk
skr = Saraiki
sl = Slovensk
sn = Shona
snk = Soninke
snv = Saʼban
so = Somali
sq = Albansk
sr = Serbisk
ss = Svati
ssi = Sansi
st = Sotho
su = Sundanesisk
sv-SE = Svensk
sva = Svan
sw = Swahili
swb = Shimaore
syr = Gammelsyrisk
szy = Sakizaya
ta = Tamilsk
tar = Central Tarahumara
tay = Atayal
te = Telugu
teg = Teke
tg = Tadsjikisk
th = Thai
ti = Tigrinja
tig = Tigre
tk = Turkmensk
tl = Tagalog
tli = Tlingit
tn = Setswana
tob = Toba Qom
tok = Toki Pona
top = Papantla Totonac
tr = Tyrkisk
trv = Seediq
trw = Torwali
ts = Tsonga
tt = Tatar
ttj = Rutoro
tui = Tupuri
tum = Tumbuka
tuq = Tedaga
tvu = Tunen
tw = Twi
ty = Tahitisk
tyv = Tuvinsk
uby = Ubykh
udl = Ouldémé
udm = Udmurtisk
ug = Uigurisk
uk = Ukrainsk
ukv = Kuku
ur = Urdu
ush = Ushojo
usp = Uspantansk
uz = Usbekisk
var = Huarigio
ve = Tshivenda
vec = Venetiansk
vi = Vietnamesisk
vmw = Makua
vot = Votisk
wbl = Wakhi
wep = Westfalsk
wes = Kamerun Pidgin
wo = Wolof
xdq = Kaitag
xh = Xhosa
xhe = Ketrani
xka = Kalkoti
xkl = Kenyah
xmf = Mingrelian
xsm = Kasem
yaq = Yaqui
yav = Nuasue
ydg = Yadgha
yi = Jiddisch
yo = Joruba
yue = Kantonesisk
zgh = Berbisk
zh-CN = Kinesisk (Kina)
zh-HK = Kinesisk (Hong Kong)
zh-TW = Kinesisk (Taiwan)
zoc = Copainalá Zoque
zu = Zulu
zza = Zazaisk

# [/]


## Layout

profile = Profil
help = Hjelp
contact = Kontakt
privacy = Personvern
terms = Vilkår
cookies = Infokapsler
faq = FAQ
documentation = Dokumentasjon
content-license-text = Innhold tilgjengelig under en <licenseLink>Creative Commons-lisens</licenseLink>
share-title = Hjelp oss med å finne andre som vil donere stemmen sin!
share-text = Hjelp til med å lære maskiner hvordan virkelige folk snakker, bidra med din stemme på { $link }
link-copied = Lenke kopiert
back-top = Tilbake til toppen
logout = Logg av
donate = Donere
stayintouch = Vi i Mozilla bygger et samfunn rundt taleteknologi. Vi vil gjerne holde deg oppdatert på endringer, nye datakilder og høre mer om hvordan du bruker disse dataene.
privacy-info = Vi lover å håndtere informasjonen din på en trygg måte. Les mer i vår <privacyLink>personvernerklæring</privacyLink>.
return-to-cv = Gå tilbake til Common Voice
email-input =
    .label = E-post
loading = Laster…
email-opt-in-info = Jeg vil gjerne motta e-post med påminnelser, oppdateringer om fremgang og nyheter om Common Voice.
email-opt-in-info-title = Abonner på Common Voice e-postlisten
email-opt-in-info-sub-with-challenge = Motta e-post om utfordrings- og målpåminnelser, oppdateringer om progresjon og nyhetsbrev om Common Voice.
email-opt-in-privacy-v2 = Ved å registrere deg for å motta e-post godtar du at Mozilla håndterer denne informasjonen som forklart i Mozillas <privacyLink>personvernpraksis</privacyLink>.
indicates-required = * Indikerer obligatorisk felt
not-available-abbreviation = ikke tilgjengelig
# Text shown on a button that can be clicked to copy text to the clipboard
copy = Kopier
# Text shown on a button that can be clicked to close a modal or panel
close = Lukk
# Text shown on a button that can be clicked to cancel an action
cancel = Avbryt
# Text shown on a button that can be clicked to delete an item
delete = Slett

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>Vi trenger</mark><br/><mark>din hjelp!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = Det koster nesten en million dollar i året å være vert for datasettene og forbedre plattformen for de over 100 språksamfunnene som er avhengige av det vi gjør. <strong>Hvis du verdsetter åpne, inkluderende data – doner i dag!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>Bruker du CV for</mark><br/><mark>forskningen din?</mark>
languages-donate-banner-cta-explanation = Common Voice er finansiert av donasjoner og tilskudd! Vi liker å samarbeide med akademikere, sivilsamfunn og industriforskere. Common Voice er gratis å bruke, men å bidra til plattform- og vertskostnader gjennom tilskuddsforslag er veldig nyttig.
localization-select =
    .label = Velg språk/lokalisering

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = <strong>Nye Common Voice-datasett</strong> er nå tilgjengelige for nedlasting eksklusivt gjennom vår søsterplattform, Mozilla Data Collective. Bli med for å få tilgang til over 300 globale datasett, bygget av og for fellesskapet.
# Button text for MDC announcement
announcement-mdc-button-text = Bli med i Mozilla Data Collective
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = Åpnes i en ny fane

## Release Delay Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-delay = Kjære fellesskap, vi jobber for tiden med lanseringen. Vi forventer at den rulles ut i etapper de kommende dagene på <mdcLink>Mozilla Data Collective</mdcLink>.

## Release Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release = Kjære fellesskap, den nye lanseringen er nå fullt tilgjengelig på <mdcLink>Mozilla Data Collective</mdcLink>. Bli med for tilgang til over 500 globale datasett, laget av og for fellesskapet.

## Pre-Release Announcement (dates TBD)

# Text wrapped in <cvDatasheetsLink></cvDatasheetsLink> will be rendered as an external link to the cv-datasheets GitHub repo
announcement-pre-release = Kjære fellesskap, lanseringstidspunktet nærmer seg. Gå og valider og transkriber, og besøk også <cvDatasheetsLink>cv-datasheets</cvDatasheetsLink> for å oppdatere ditt datablad.

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = Begynn å skrive for å søke
searchbox-default-placeholder = Søk…
searchbox-default-aria-label = Søk
dropdown-no-results = Ingen treff funnet
# Common Voice Dataset=Language search box
dataset-searchbox-label = Endrer både visnings- og datasettspråk
dataset-searchbox-placeholder = Søk etter et språk…
dataset-searchbox-aria-label = Søk etter et språk
dataset-search-no-results = Ingen samsvarende språk funnet
request-new-language = Be om nytt språk
