# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Afar
aat = Arvanitisch
ab = Abchasisch
abb = Bankon
abq = Abasinisch
ace = Acehnesisch
ady = Adygejisch
af = Afrikaans
aii = Suret
ajg = Aja
aln = Gegisches Albanisch
am = Amharisch
an = Aragonesisch
apc = Levantinisches Arabisch
ar = Arabisch
arn = Mapudungun
as = Assamesisch
ast = Asturisch
az = Aserbaidschanisch
azz = Hochland-Puebla-Nahuatl
ba = Baschkirisch
bag = Tuki
bal = Balochi
bas = Basaa
bax = { "" }
bba = Baatonum
bbj = Ghomala
bbl = Tuschetisch
bce = Mengambo
bci = Baoule
be = Weißrussisch
beb = Bamvele
bew = Betawi
bfd = Bafut
bfq = Badaga
bft = Balti
bg = Bulgarisch
bgp = Ost-Belutschi
bin = Edo
bkh = Bakoko
bkm = Kom
blk = Pa'O
bm = Bambara
bn = Bengali
bnm = Bananga
bnn = Bunun
bo = Tibetisch
br = Bretonisch
brh = Brahui
bri = Mokpwe
brx = Bodo
bs = Bosnisch
bsh = Kati
bsk = Brushaski
bsy = Sabah-Bisaya
btv = Bateri
bum = Bulu
bxk = Bukusu
bxr = Burjatisch
byv = Medumba
ca = Katalanisch
cak = Kaqchikel
cdo = Östliches Min
ce = Tschetschenisch
cgg = Chiga
cjk = Chokwe
ckb = Zentralkurdisch
cnh = Hakha Chin
co = Korsisch
cpx = Heng Hua
cpy = Ashéninka, Süd-Ucayali
crh = Krimtatarisch
cs = Tschechisch
cut = Cuicateco de Teutila
cux = Cuicateco de Tepeuxila
cv = Tschuwaschisch
cy = Walisisch
da = Dänisch
dag = Dagbani
dar = Dargwa
dav = Kidaw’ida
de = Deutsch
din = Dinka
dmk = Dawoodi
dml = Dameli
dru = Rukai
dsb = Niedersorbisch
dua = Duala
dv = Dhivehi
dyu = Dioula
dz = Dzongkha
ebr = Ebrie
ee = Ewe
efk = Efik
eko = Koti
el = Griechisch
en = Englisch
eo = Esperanto
es = Spanisch
esu = Zentralalaskanisches Yupik
et = Estnisch
eto = Eton
eu = Baskisch
ewo = Ewondo
fa = Persisch
fan = Fang
ff = Fulah
fi = Finnisch
fmp = Fe’fe’
fo = Färöisch
fr = Französisch
fub = Adamaua-Fulfulde
fue = Borgu-Fulfulde
fuf = Pular
fy-NL = Friesisch
ga-IE = Irisch
gaa = Ga
gcf = Martinique-Kreolisch
gej = Mina
ggg = Gurgula
gid = Gidar
gig = Goaria
giz = Giziga
gjk = Kachi
gju = Gujari
gl = Galizisch
glk = Gilaki
gn = Guarani
gom = Goanisches Konkani
gos = Gronings
gsw = Elsässisch
gu-IN = Gujarati
guc = Wayuunaiki
gv = Manx
gwc = Gawri
gwt = Gawarbaiti
gya = Nordwest-Gbaya
ha = Hausa
hac = Gorani
hak = Hakka
haz = Hazaragi
hch = Wixárika
he = Hebräisch
hem = Kihemba
hi = Hindi
hil = Hiligaynon
hno = Nord-Hindko
hr = Kroatisch
hrx = Hunsrückisch
hsb = Obersorbisch
ht = Haitianisch
hu = Ungarisch
hux = Huitoto, Nüpode
hy-AM = Armenisch
ia = Interlingua
iba = Iban
ibb = Ibibio
id = Indonesisch
ie = Interlingue
ig = Igbo
ipk = Iñupiaq
is = Isländisch
it = Italienisch
izh = Ischorisch
ja = Japanisch
jam = Jamaika-Patois
jbo = Lojban
jgo = Ngomba
jqr = Jaqaru
jv = Javanisch
ka = Georgisch
kaa = Karakalpak
kab = Kabylisch
kam = Kikamba
kbd = Kabardinisch
kcn = Nubi
kdh = Kotokoli
khw = Khowar
ki = Kikuyu
kjh = Khakas
kk = Kasachisch
kl = Kalaallisut
kln = Kalendjin
kls = Kalasha
km = Khmer
kmr = Kurmandschi Kurdisch
kn = Kannada
knn = Konkani (Devanagari)
ko = Koreanisch
koo = Konjo
kpv = Komi-Syrjänisch
krc = Karatschai-balkarisch
ks = Kaschmiri
ksf = Bafia
kvx = Koli, Parkari
kw = Cornish
kxp = Koli, Wadiyara
ky = Kirgisisch
kzi = Kelabit
laj = Lango
lb = Luxemburgisch
led = Lendu
leu = Kara
lg = Luganda
lij = Ligurisch
lke = Kenyi
lld = Ladinisch
ln = Lingala
lo = Laotisch
lrk = Loarki
lrl = Achomi
lss = Lassi
lt = Litauisch
ltg = Lettgallisch
lth = Thur
lua = Tschiluba
luo = Dholuo
lv = Lettisch
lzz = Lasisch
mai = Maithili
mau = Huautla-Mazatekisch
mbf = Baba-Malaysisch
mbo = Mbo
mcf = Matsés
mcn = Masana
mcx = Mpiemo
mdd = Mbum
mdf = Mokschanisch
meh = Mixteco-Yucuhiti
mel = Melanau
mfe = Morisyen, Kreol
mg = Madagassisch
mgg = Kunabembe
mhk = Mungaka
mhr = Wiesenmari
mk = Mazedonisch
mki = Dhatki
ml = Malayalam
mmc = Michoacán-Mazahua
mn = Mongolisch
mni = Meetei Lon
mnw = Mon
mos = Mossi
mqh = Tlazoyaltepec Mixtec
mr = Marathi
mrh = Mara
mrj = Bergmari
ms = Malaiisch
mse = Musey
msi = Sabah-Malaysisch
mt = Maltesisch
mua = Mundang
mug = Musgu
mui = Musi
mve = Marwari
mvy = Indus Kohistani
mxu = Mada
my = Birmanisch
myv = Ersjanisch
mzn = Mazandarani
nan-tw = Taiwanesisch (Minnan)
nb-NO = Norwegisches Bokmål
ncx = Zentral-Puebla-Nahuatl
nd = IsiNdebele (Nord)
ne-NP = Nepali
new = Newari
nhe = Ost-Huasteca-Nahuatl
nhi = Westliches Sierra Puebla Nahuatl
nia = Nias
nl = Niederländisch
nla = Ngombale
nlv = Nahuatl, Orizaba
nmg = Kwasio
nmz = Losso
nn-NO = Nynorsk
nnh = Ngiembon
nqo = NKo
nr = IsiNdebele (Süd)
nso = Nord-Sotho
ny = Chinyanja
nyn = Nyankole
nyu = Nyungwe
oc = Okzitanisch
odk = Od
om = Afaan Ormoo
or = Odia
oru = Ormuri
os = Ossetisch
pa-IN = Punjabi
pap = Papiamento
pap-AW = Papiamento (Aruba)
pau = Palauisch
pcd = Picardisch
pcm = Nigerianischers Pidgin-Englisch
pez = Östliches Penan
phl = Palula
phr = Pahari-Potwari
pl = Polnisch
plk = Kohistani Shina
pne = Westliches Penan
prq = Ashéninka Perené
ps = Paschtu
pt = Portugiesisch
pua = West-Hochland-Purepecha
pwn = Paiwan
quc = K’iche’
qug = Chimborazo-Hochland-Kichwa
qup = Süd-Pastaza-Quechua
qur = Chaupihuaranga-Yanahuanca
qus = Quechua von Santiago del Estero
qux = Quechua Yauyos
quy = Quechua Chanka
qva = Quechua Ambo-Pasco
qvi = Kichwa
qvj = Loja-Hochland-Kichwa
qvl = Quechua Cajatambo
qwa = Quechua Corongo Ancash
qws = Quechua Sihuas Ancash
qxa = Quechua Chiquián
qxp = Puno-Quechua
qxq = Qaschqai
qxt = Quechua Pasco Santa Ana de Tusi
qxu = Quechua Arequipa-La Unión
qxw = Quechua Jauja Wanka
rif = Tarifit
rki = Rakhine
rm-sursilv = Surselvisch
rm-vallader = Rätoromanisch Vallader
rn = Kirundi
ro = Rumänisch
rof = Rombo
ru = Russisch
ruc = Ruuli
rup = Aromunisch
rw = Kinyarwanda
rwm = Amba
sah = Sacha
sat = Santali (Ol Chiki)
sba = Ngambay
sbn = Sindhi Bhil
sc = Sardisch
scl = Shina
scn = Sizilianisch
sco = Scots
sd = Sindhi
sdh = Südkurdisch
sdo = Serian-Bidayuh
seh = Sena
sei = Seri
sg = Sango
shi = Taschelhit
shn = Shan
si = Singhalesisch
sk = Slowakisch
skr = Saraiki
sl = Slowenisch
sn = Shona
snk = Soninke
snv = Sa’ban
so = Somali
sq = Albanisch
sr = Serbisch
ss = Siswati
ssi = Sansi
st = Sesotho
su = Sundanesisch
sv-SE = Schwedisch
sva = Svan
sw = Swahili
swb = Shimaore
syr = Syrisch
szy = Sakizaya
ta = Tamil
tar = Zentral-Tarahumara
tay = Atayal
te = Telugu
teg = Teke
tg = Tadschikisch
th = Thailändisch
ti = Tigrinya
tig = Tigre
tiv = Tiv
tk = Turkmenisch
tl = Tagalog
tli = Tlingit
tn = Setswana
tob = Toba Qom
tok = Toki Pona
top = Papantla Totonac
tr = Türkisch
trv = Seediq
trw = Torwali
ts = Xitsonga
tt = Tatarisch
ttj = Tooro
tui = Tupuri
tum = Tumbuka
tuq = Tedaga
tvu = Tunen
tw = Twi
ty = Tahitisch
tyv = Tuwinisch
uby = Ubychisch
udl = Ouldémé
udm = Udmurtisch
ug = Uigurisch
uk = Ukrainisch
ukv = Kuku
ur = Urdu
ush = Ushoji
usp = Uspanteco
uz = Usbekisch
var = Huarijio
ve = Tshivenda
vec = Venezianisch
vi = Vietnamesisch
vls = Westflämisch
vmw = Makua
vot = Wotisch
wbl = Wakhi
wep = Westfälisch
wes = Kameruner Pidgin
wo = Wolof
xdq = Kaitag
xh = Xhosa
xhe = Ketrani
xka = Kalkoti
xkl = Kenyah
xmf = Mingrelian
xsm = Kasem
yaq = Yaqui
yav = Nuasue
ydg = Yidgha
yi = Jiddisch
yo = Yoruba
yue = Kantonesisch
zgh = Mazirisch
zh-CN = Chinesisch (China)
zh-HK = Chinesisch (Hong Kong)
zh-TW = Chinesisch (Taiwan)
zoc = Copainalá Zoque
zu = Zulu
zza = Zaza

## Layout

profile = Profil
help = Hilfe
contact = Kontakt
privacy = Datenschutz
terms = Nutzungsbedingungen
cookies = Cookies
faq = FAQ
documentation = Dokumentation
content-license-text = Inhalte stehen unter einer <licenseLink>Creative-Commons-Lizenz</licenseLink>
share-title = Überzeugen Sie auch andere, ihre Stimme zu spenden!
share-text = Zeigt Geräten, wie echte Menschen sprechen und spendet eure Stimme auf { $link }!
link-copied = Link kopiert
back-top = Zum Seitenanfang
logout = Abmelden
donate = Spenden
stayintouch = Wir bei Mozilla bauen eine Gemeinschaft rund um sprachbasierte Technologien auf. Wir würden gerne für Aktualisierungen und neue Datenquellen mit Ihnen in Kontakt bleiben und mehr darüber erfahren, wie Sie diese Daten nutzen.
privacy-info = Wir versichern Ihnen, mit Ihren Daten sorgfältig umzugehen. Lesen Sie mehr dazu in unserer <privacyLink>Datenschutzerklärung</privacyLink>.
return-to-cv = Zurück zu Common Voice
email-input =
    .label = E-Mail-Adresse
loading = Wird geladen…
email-opt-in-info = Ich möchte E-Mails wie Zielerinnerungen, Fortschrittsaktualisierungen und Newsletter zu Common Voice erhalten.
email-opt-in-info-title = Tragen Sie sich in die Mailingliste von Common Voice ein
email-opt-in-info-sub-with-challenge = Erhalten Sie E-Mails wie  Herausforderungs- und Zielerinnerungen, Fortschrittsaktualisierungen und Newsletter zu Common Voice.
email-opt-in-privacy-v2 = Mit Ihrer Zustimmung zum Empfang von E-Mails erklären Sie, dass Sie damit einverstanden sind, dass Mozilla diese Informationen gemäß der <privacyLink>Datenschutzerklärung</privacyLink> von Mozilla verarbeitet.
indicates-required = * Pflichtfeld
not-available-abbreviation = Nicht verfügbar
# Text shown on a button that can be clicked to copy text to the clipboard
copy = Kopieren
# Text shown on a button that can be clicked to close a modal or panel
close = Schließen
# Text shown on a button that can be clicked to cancel an action
cancel = Abbrechen
# Text shown on a button that can be clicked to delete an item
delete = Löschen

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>Wir brauchen</mark><br/><mark>Ihre Hilfe!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = Es kostet fast eine Million Dollar im Jahr, die Datensätze zu hosten und die Plattform für die über 100 Sprachgemeinschaften zu verbessern, die sich auf das verlassen, was wir tun. <strong>Wenn Sie Wert auf offene, integrative Daten legen – spenden Sie heute!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>Verwenden Sie Common Voice für</mark><br/><mark>Ihre Forschung?</mark>
languages-donate-banner-cta-explanation = Common Voice wird durch Spenden und Zuwendungen finanziert! Wir arbeiten gerne mit Akademikern, der Zivilgesellschaft und Forschern aus der Industrie zusammen. Common Voice ist kostenlos, aber es ist wirklich hilfreich, sich durch Zuwendungsvorschläge an den Plattform- und Hostingkosten zu beteiligen.
localization-select =
    .label = Sprache/Lokalisierung auswählen

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = <strong>Neue Common-Voice-Datensätze</strong> stehen jetzt ausschließlich über unsere Schwesterplattform Mozilla Data Collective zum Download zur Verfügung. Melden Sie sich an, um Zugriff auf über 300 globale Datensätze zu erhalten, die von und für die Gemeinschaft erstellt wurden.
# Button text for MDC announcement
announcement-mdc-button-text = Werden Sie Mitglied des Mozilla Data Collective
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = Wird in einem neuen Tab geöffnet

## Release Delay Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-delay = Liebe Gemeinschaft, wir arbeiten derzeit an der Veröffentlichung. Wir erwarten, dass sie in den kommenden Tagen in Teilen im <mdcLink>Mozilla Data Collective</mdcLink> eingeführt wird.

## Release Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release = Liebe Gemeinschaft, die neue Version befindet sich jetzt vollständig im <mdcLink>Mozilla Data Collective</mdcLink>. Machen Sie mit, um auf über 500 Datensätze aus aller Welt zuzugreifen, die von und für die Gemeinschaft erstellt wurden.

## Pre-Release Announcement (dates TBD)

# Text wrapped in <cvDatasheetsLink></cvDatasheetsLink> will be rendered as an external link to the cv-datasheets GitHub repo
announcement-pre-release = Liebe Gemeinschaft, die Zeit der Veröffentlichung nähert sich. Bitte validieren und transkribieren Sie und besuchen Sie auch die <cvDatasheetsLink>cv-Datenblätter</cvDatasheetsLink>, um Ihr Datenblatt zu aktualisieren.

## Release Commencing

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-commencing = Liebe Gemeinschaft, wir haben das Stichdatum für die Veröffentlichung erreicht und wir bündeln die Datensätze. Die neue Version wird in ein paar Tagen auf <mdcLink>Mozilla Data Collective</mdcLink> verfügbar sein.

## Downtime Announcements

# Generic scheduled downtime for next Sunday early hours
announcement-scheduled-downtime = Liebe Gemeinschaft, zur Optimierung unserer Systeme planen wir kurze Wartungsunterbrechungen ein, die in der Regel nur wenige Stunden dauern. Die nächste Unterbrechung ist für die frühen Morgenstunden dieses Sonntags vorgesehen. Falls Probleme auftreten, kontaktieren Sie uns bitte über Matrix.

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = Text eingeben, um Suche zu beginnen…
searchbox-default-placeholder = Suchen…
searchbox-default-aria-label = Suchen
dropdown-no-results = Keine Übereinstimmungen gefunden
# Common Voice Dataset=Language search box
dataset-searchbox-label = Ändert die Sprache der Anzeige und des Datensatzes
dataset-searchbox-placeholder = Nach einer Sprache suchen…
dataset-searchbox-aria-label = Nach einer Sprache suchen
dataset-search-no-results = Keine passende Sprache gefunden.
request-new-language = Neue Sprache vorschlagen
