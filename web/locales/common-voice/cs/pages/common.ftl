# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Afarština
aat = Arvanitika
ab = Abcházština
abb = Bankon
abq = Abaza
ace = Acehština
ady = Adygejština
af = Afrikánština
aii = Suret
ajg = Adja
aln = Gheg albánština
am = Amharština
an = Aragonština
ar = Arabština
arn = Mapudungunština
as = Ásámština
ast = Asturština
az = Ázerbájdžánština
azz = Highland Puebla Nahuatl
ba = Baškirština
bag = Tuki
bal = Balúčština
bas = Basarijština
bax = Bamun
bba = Baatonum
bbj = Ghomala
bbl = Tush
bce = Mengambo
bci = Baoule
be = Běloruština
beb = Bamvele
bew = Betawi
bfd = Bafut
bft = Balti
bg = Bulharština
bgp = Východní Balúčistán
bkh = Bakoko
bkm = Komijština
bm = Bambarština
bn = Bengálština
bnm = Batanga
bnn = Bunun
bo = Tibetština
br = Bretonština
brh = Brahui
bri = Mokpwe
brx = Bodo
bs = Bosenština
bsh = Katecheté
bsk = Brushaski
bsy = Sabah Bisaya
btv = Bateri
bum = Bulu
bxk = Bukusu
bxr = Burjatština
byv = Medumba
ca = Katalánština
cak = Kaqchikel
cdo = Eastern Min
ce = Čečenština
cgg = Chiga
cjk = Chokwe
ckb = Sorání kurdština
cnh = Hakha Chin
co = Korsičtina
cpx = Heng Hua
cpy = Asheninka Jižní Ucayali
crh = Krymská tatarština
cs = Čeština
cut = Teutila Cuicatec
cux = Tepeuxila Cuicatec
cv = Čuvaština
cy = Velština
da = Dánština
dag = Dagbani
dar = Dargwa
dav = Kidaw'ida
de = Němčina
din = Dinka
dmk = Dawoodi
dml = Dameli
dru = Rukai
dsb = Dolnolužická srbština
dua = Duala
dv = Dhivehi
dyu = Dioula
ebr = Ebrie
ee = Eweština
efk = Efik
eko = Ekoti
el = Řečtina
en = Angličtina
eo = Esperanto
es = Španělština
esu = Středoaljašský jupʼik
et = Estonština
eto = Eton
eu = Baskičtina
ewo = Ewondo
fa = Perština
fan = Fang
ff = Fulbština
fi = Finština
fmp = Feʼefeʼe
fo = Faerština
fr = Francouzština
fub = Adamawa Fulfulde
fue = Borgu Fulfulde
fuf = Pulární Guinea
fy-NL = Fríština
ga-IE = Irština
gaa = Ga
gej = Mina
ggg = Gurgula
gid = Guidar
gig = Goaria
giz = Guiziga
gjk = Kachhi
gju = Gujari
gl = Galicijština
glk = Gilaki
gn = Guaraní
gom = Konkánština
gos = Gronings
gsw = Alsasčina
gu-IN = Gudžarátština
guc = Wayuunaiki
gv = Manština
gwc = Gawri
gwt = Gawarbaiti
gya = Northwest Gbaya
ha = Hauština
hac = Gorani
hak = Hakka
haz = Hazargi
hch = Wixárika
he = Hebrejština
hem = Kihemba
hi = Hindština
hil = Hiligaynon
hno = Severní Hindko
hr = Chorvatština
hrx = Hunsrik
hsb = Hornosrbština
ht = Haitština
hu = Maďarština
hux = Nüpode Huitoto
hy-AM = Arménština
ia = Interlingua
iba = Iban
ibb = Ibibio
id = Indonéština
ie = Interlingue
ig = Igboština
ipk = Iñupiaq
is = Islandština
it = Italština
izh = Ižorština
ja = Japonština
jam = Jamaican Patois
jbo = Lojban
jgo = Ngomba
jqr = Jaqaru
jv = Jávština
ka = Gruzínština
kaa = Karakalpačtina
kab = Kabylština
kam = Kikamba
kbd = Kabardština
kcn = Nubi
kdh = Kotokoli
khw = Khowar
ki = Kikujština
kjh = Khakasština
kk = Kazaština
kln = Kalenjin
kls = Kalasha
km = Khmerština
kmr = Kurmándží kurdština
kn = Kannadština
knn = Kónkanština (dévanágarí)
ko = Korejština
koo = Konzo
kpv = Komi-zyrjanština
krc = Karachay-Balkar
ks = Kašmírština
ksf = Bafia
kvx = Parkari Koli
kw = Kornština
kxp = Wadiyara Koli
ky = Kyrgyzština
kzi = Kelabit
laj = Lango
lb = Lucemburština
led = Lendu
leu = Kara
lg = Lugandština
lij = Ligurština
lke = Kenyi
lld = Ladin
ln = Lingalština
lo = Laoština
lrk = Loarki
lrl = Achomi
lss = Lassi
lt = Litevština
ltg = Latgalština
lth = Thur
lua = Tshiluba
luo = Dholuo
lv = Lotyština
lzz = Laz
mai = Maithilština
mau = Huautla Mazatec
mbf = Baba Malay
mbo = Mbo
mcf = Matses
mcn = Massa
mcx = Mpiemo
mdd = Mbum
mdf = Mókša
meh = Mixteco Yucuhiti
mel = Melanau
mfe = Morisienská kreolština
mg = Malgaština
mgg = Kunabembe
mhk = Mungaka
mhr = Lužní Marijština
mk = Makedonština
mki = Dhatki
ml = Malajálamština
mmc = Michoacán Mazahua
mn = Mongolština
mni = Meetei Lon
mos = Mossijština
mqh = Tlazoyaltepec Mixtec
mr = Maráthština
mrh = Mara
mrj = Horská Marijština
ms = Malajština
mse = Moussey
msi = Sabah malajština
mt = Maltština
mua = Mundang
mug = Musgum
mve = Pákistánský marwari
mvy = Indus Kohistani
mxu = Mada
my = Barmština
myv = Erzja
nan-tw = Tchajwanština (Minnan)
nb-NO = Norština (Bokmål)
ncx = Střední Puebla Nahuatl
nd = Severní ndebelština
ne-NP = Nepálština
new = Newari
nhe = Východní Huasteca Nahuatl
nhi = Západní Sierra Puebla Nahuatl
nia = Niaština
nl = Nizozemština
nla = Ngombale
nlv = Orizaba Nahuatl
nmg = Kwasio
nmz = Losso
nn-NO = Nynorsk
nnh = Ngiembon
nqo = NKo
nr = Jižní ndebelština
nso = Severní Sotho
ny = Chinjanja
nyn = Runyankole
nyu = Nyungwe
oc = Okcitánština
odk = Oadki
om = Oromština
or = Udijština
oru = Ormuri
os = Osetština
pa-IN = Pandžábština
pap = Papiamento
pap-AW = Papiamento (Aruba)
pau = Palauanština
pcd = Picard
pcm = Nigerian Pidgin English
pez = Východní Penan
phl = Palula
phr = Pahari-Pothwari
pl = Polština
plk = Kohistani Shina
pne = Západní Penam
prq = Asheninka Perene
ps = Paštština
pt = Portugalština
pua = Western Highland Purepecha
pwn = Paiwan
quc = K'iche'
qug = Quichua Chimborazo Highland
qup = Jižní Pastaza Kečuánština
qur = Kečuánština Yanahuanca
qus = Quechua Santiago del Estero
qux = Kečuánské yauyos
quy = Kečuánština
qva = Kečuánština Ambo-Pasco
qvi = Kichwa
qvj = Loja Highland Kichwa
qvl = Kečuánština cajatambo
qwa = Kečuánština Corongo Ancash
qws = Kečuánština sihuas Ancash
qxa = Kečuánština Chiquián
qxp = Puno Quechua
qxq = Qashqai
qxt = Kečuánština Velikonoce Santa Ana de Tusi
qxu = Kečuánština (arequipa-La Unión)
qxw = Kečuánština Jauja Wanka
rif = Tarifit
rki = Rakhine
rm-sursilv = Rétorománština (sursilvan)
rm-vallader = Rétorománština (vallader)
rn = Kirundština
ro = Rumunština
rof = Kirombo
ru = Ruština
ruc = Ruuli
rup = Aromanian
rw = Rwandština
rwm = Amba
sah = Jakutština
sat = Santali (Ol Chiki)
sba = Ngambay
sbn = Sindhi Bhili
sc = Sardinština
scl = Shina
scn = Sicilština
sco = Skotština
sd = Sindhština
sdh = Jižní kurdština
sdo = Serian Bidayuh
seh = Sena
sei = Seri
shi = Shilha
shn = Shan
si = Sinhalština
sk = Slovenština
skr = Saraiki
sl = Slovinština
sn = Šonština
snk = Soninke
snv = Sa`ban
so = Somálština
sq = Albánština
sr = Srbština
ss = Swati
ssi = Sansi
st = Jižní Sotho
su = Sundánština
sv-SE = Švédština
sva = Swan
sw = Swahilština
syr = Syrština
szy = Sakizaya
ta = Tamilština
tar = Střední Tarahumara
tay = Atayal
te = Telugština
teg = Teke
tg = Tádžičtina
th = Thajština
ti = Tigriňa
tig = Tigre
tk = Turkmenština
tl = Tagalština
tli = Tlingit
tn = Tswanština
tob = Toba Qom
tok = Toki Pona
top = Papantla Totonac
tr = Turečtina
trv = Seediq
trw = Torwali
ts = Tsonga
tt = Tatarština
ttj = Rutoro
tui = Tupuri
tum = Tumbuka
tuq = Tedaga
tvu = Tunen
tw = Ťwiština
ty = Tahitština
tyv = Tuvinština
uby = Ubychština
udl = Ouldémé
udm = Udmurtština
ug = Ujgurština
uk = Ukrajinština
ukv = Kuku
ur = Urdština
ush = Ushojo
usp = Uspanteko
uz = Uzbečtina
var = Huarijio
ve = Tshivenda
vec = Benátština
vi = Vietnamština
vmw = Emakhuwa
vot = Votština
wbl = Wakhi
wep = Vestfálština
wes = Cameroon Pidgin
wo = Volofština
xdq = Kaitag
xh = Xhoština
xhe = Ketrani
xka = Kalkata
xkl = Kenyah
xmf = Mingrelian
xsm = Kasem
yaq = Yaqui
yav = Nuasue
ydg = Yadgha
yi = Jidiš
yo = Jorubština
yue = Kantonština
zgh = Berberština
zh-CN = Čínština (Čína)
zh-HK = Čínština (Hong Kong)
zh-TW = Čínština (Taiwan)
zoc = Copainalá Zoque
zu = Zuluština
zza = Zazaki

# [/]


## Layout

profile = Profil
help = Nápověda
contact = Kontakt
privacy = Soukromí
terms = Podmínky
cookies = Cookies
faq = FAQ
documentation = Dokumentace
content-license-text = Obsah je dostupný pod <licenseLink>licencí Creative Commons</licenseLink>
share-title = Pomozte nám najít další, kteří darují svůj hlas!
share-text = Pomozte strojům pochopit, jak mluví lidé, darujte svůj hlas na { $link }
link-copied = Odkaz byl zkopírován
back-top = Zpět nahoru
logout = Odhlásit
donate = Darovat
stayintouch = V Mozille budujeme komunitu okolo hlasových technologií. Rádi bychom zůstali v kontaktu s novinkami, novými zdroji dat a rádi bychom se dozvěděli, jak tato data používáte.
privacy-info = Slibujeme, že budeme vaše informace zpracovávat s péčí. Přečtěte si více v našich <privacyLink>zásadách ochrany osobních údajů</privacyLink>.
return-to-cv = Návrat do Common Voice
email-input =
    .label = E-mail
loading = Načítání…
email-opt-in-info = Chci dostávat e-maily jako jsou přimínky k nedosaženým cílům, informace o mém postupu nebo novinky o projektu Common Voice.
email-opt-in-info-title = Připojte se do elektronické konference Common Voice
email-opt-in-info-sub-with-challenge = Zasílání e-mailů jako jsou připomínky k nedosaženým cílům či výzvám, informace o postupu nebo novinky o projektu Common Voice.
email-opt-in-privacy-v2 = Rozhodnutím se přijímat e-maily potvrzujete svůj souhlas se zpracováním Vašich údajů dle <privacyLink>Zásad ochrany osobních údajů</privacyLink> společnosti Mozilla.
indicates-required = * označuje povinná pole
not-available-abbreviation = není k dispozici
# Text shown on a button that can be clicked to copy text to the clipboard
copy = Kopírovat
# Text shown on a button that can be clicked to close a modal or panel
close = Zavřít
# Text shown on a button that can be clicked to cancel an action
cancel = Zrušit
# Text shown on a button that can be clicked to delete an item
delete = Smazat

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>Potřebujeme</mark><br/><mark>vaši pomoc!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = Hostování datových sad a zlepšování platformy pro více než 100 jazykových komunit, které se spoléhají na naši práci, stojí téměř milion dolarů ročně. <strong>Jestliže si ceníte otevřených a inkluzivních dat - přispějte ještě dnes!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>Používáte Common Voice </mark><br/><mark>na svůj výzkum? </mark>
languages-donate-banner-cta-explanation = Common Voice je financován z darů a grantů! Rádi spolupracujeme s akademiky, výzkumníky z občanské společnosti a průmyslu. Používání služby Common Voice je zdarma, ale přispívat na náklady na platformu a hosting prostřednictvím návrhů na granty je opravdu užitečné.
localization-select =
    .label = Zvolte jazyk/lokalizaci

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = <strong>Nové datové sady Common Voice</strong> jsou nyní k dispozici ke stažení výhradně prostřednictvím naší sesterské platformy Mozilla Data Collective. Připojte se a získejte přístup k více než 300 globálním datovým sadám vytvořeným komunitou a pro komunitu.
# Button text for MDC announcement
announcement-mdc-button-text = Připojte se k Mozilla Data Collective
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = Otevře se v novém panelu

## Release Delay Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-delay = Vážená komunito, v současné době pracujeme na vydání této aktualizace. Očekáváme, že bude v následujících dnech postupně zaváděna na <mdcLink>Mozilla Data Collective</mdcLink>.

## Release Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release = Vážená komunito, nová verze je nyní plně k dispozici na <mdcLink>Mozilla Data Collective</mdcLink>. Připojte se a získejte přístup k více než 500 globálním datovým sadám, které vytvořila komunita pro komunitu.

## Pre-Release Announcement (dates TBD)

# Text wrapped in <cvDatasheetsLink></cvDatasheetsLink> will be rendered as an external link to the cv-datasheets GitHub repo
announcement-pre-release = Vážená komunito, datum vydání se blíží. Prosím, zkontrolujte a přepište údaje a navštivte také stránku <cvDatasheetsLink>cv-datasheets</cvDatasheetsLink>, abyste aktualizovali svůj datový list.

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = Pro vyhledávání začněte psát
searchbox-default-placeholder = Hledat…
searchbox-default-aria-label = Hledat
dropdown-no-results = Nic nebylo nalezeno
# Common Voice Dataset=Language search box
dataset-searchbox-label = Změní jak jazyk zobrazení, tak jazyk datové sady
dataset-searchbox-placeholder = Vyhledat jazyk...
dataset-searchbox-aria-label = Vyhledat jazyk
dataset-search-no-results = Žádný odpovídající jazyk nebyl nalezen
request-new-language = Požádat o nový jazyk
