# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Афарский
aat = Арванитика
ab = Абхазский
abb = Банкон
abq = Абазинский
ace = Ачехский
ady = Адыгейский
af = Африкаанс
aii = Сурет
ajg = Аджа
aln = Гегский Албанский
am = Амхарский
an = Арагонский
ar = Арабский
arn = Мапуче
as = Ассамский
ast = Астурийский
az = Азербайджанский
azz = Высокогорный пуэбла-науатль
ba = Башкирский
bag = Туки
bal = Белуджи
bas = Баса
bax = Бамун
bba = Баатонум
bbj = Гомала
bbl = Таш
bce = Меньгамбо
bci = Бауле
be = Белорусский
beb = Бамвеле
bew = Бетави
bfd = Бафут
bft = Балти
bg = Болгарский
bgp = Восточный белуджский
bin = эдо
bkh = Бакоко
bkm = Ком
bm = Бамана
bn = Бенгальский
bnm = Батанга
bnn = Бунун
bo = Тибетский
br = Бретонский
brh = Брауи
bri = Мокпве
brx = Бодо
bs = Боснийский
bsh = Катевири
bsk = Бурушаски
bsy = Сабах Бисайя
btv = Батери
bum = Булу
bxk = Букусу
bxr = Бурятский
byv = Медумба
ca = Каталанский
cak = Какчикельский
cdo = Восточный мин
ce = Чеченский
cgg = Чига
cjk = Чокве
ckb = Центральнокурдский (Сорани)
cnh = Чинский
co = Корсиканский
cpx = Хэн Хуа
cpy = Ашенинка Южный Укаяли
crh = Крымскотатарский
cs = Чешский
cut = Теутила-куикатекский
cux = Тепеуксильско-куикатекский
cv = Чувашский
cy = Валлийский
da = Датский
dag = Дагбани
dar = Даргинский
dav = Кидавида
de = Немецкий
din = Динка
dmk = Давуди
dml = Дамели
dru = Рукаи
dsb = Нижнелужицкий
dua = Дуала
dv = Мальдивский
dyu = Дьюла
ebr = Эбри
ee = Эве
efk = Эфик
eko = Экоти
el = Греческий
en = Английский
eo = Эсперанто
es = Испанский
esu = Центрально-аляскинский юпикский
et = Эстонский
eto = Итон
eu = Баскский
ewo = Эвондо
fa = Персидский
fan = Фанг
ff = Фула
fi = Финский
fmp = Феефее
fo = Фарерский
fr = Французский
fub = Аданава фулфулд
fue = Боргу фулфулд
fuf = Гвинейский пулар
fy-NL = Фризский
ga-IE = Ирландский
gaa = Га
gcf = мартиниканский креольский
gej = Мина
ggg = Гургула
gid = Гуидар
gig = Гоариа
giz = Гуизига
gjk = Качхи
gju = Гуджари
gl = Галисийский
glk = Гилаки
gn = Гуарани
gom = Гоанский конкани
gos = Гронингенский
gsw = Эльзасский
gu-IN = Гуджарати
guc = Ваюунаики
gv = Мэнский
gwc = Гаври
gwt = Гаварбайти
gya = Северо-Западный гбая
ha = Хауса
hac = Горанский
hak = Хакка
haz = Хазарги
hch = Вихарика
he = Иврит
hem = Кихемба
hi = Хинди
hil = Хилигайнон
hno = Северный хиндко
hr = Хорватский
hrx = Хунсрик
hsb = Верхнелужицкий
ht = Гаитянский
hu = Венгерский
hux = Нюподе уитото
hy-AM = Армянский
ia = Интерлингва
iba = Ибан
ibb = Ибибио
id = Индонезийский
ie = Интерлингве
ig = Игбо
ipk = Инупиакский
is = Исландский
it = Итальянский
izh = Ижорский
ja = Японский
jam = Ямайский патуа
jbo = Ложбан
jgo = Нгомба
jqr = Джакару
jv = Яванский
ka = Грузинский
kaa = Каракалпакский
kab = Кабильский
kam = Кикамба
kbd = Кабардино-черкесский
kcn = Нуби
kdh = Котоколи
khw = Ховар
ki = Кикуйю
kjh = Хакасский
kk = Казахский
kln = Каленин
kls = Калаша
km = Кхмерский
kmr = Севернокурдский (Курманджи)
kn = Каннада
knn = Конкани (Деванагари)
ko = Корейский
koo = Концо
kpv = Коми-зырянский
krc = Карачаево-Балкарский
ks = Кашмири
ksf = Бафия
kvx = Паркари Коли
kw = Корнский
kxp = Вадияра-коли
ky = Киргизский
kzi = Келабит
laj = Ланго
lb = Люксембургский
led = Ленду
leu = Кара
lg = Луганда
lij = Лигурский
lke = Кеньи
lld = Ладин
ln = Лингала
lo = Лаосский
lrk = Лоарки
lrl = Ачоми
lss = Ласси
lt = Литовский
ltg = Латгальский
lth = Тур
lua = Тшилуба
luo = Долуо
lv = Латышский
lzz = Лазский
mai = Майтхили
mau = Уаутла-мазатекский
mbf = Баба Малай
mbo = Мбо
mcf = Матсес
mcn = Масса
mcx = Мпиемо
mdd = Мбум
mdf = Мокшанский
meh = Юкуитский миштекский
mel = Меленау
mfe = Креольский Морисиен
mg = Малагасийский
mgg = Кунабембе
mhk = Мунгака
mhr = Лугово-восточный марийский
mk = Македонский
mki = Дхатки
ml = Малаялам
mmc = Мичоаканский масауа
mn = Монгольский
mni = Манипури
mnw = Мон
mos = Мооре
mqh = Тлазоялтепекский миштекский
mr = Маратхи
mrh = Мара
mrj = Горномарийский
ms = Малайский
mse = Моуссей
msi = Сабах Малай
mt = Мальтийский
mua = Мунданг
mug = Мусгум
mve = Пакистанский марвари
mvy = Индо-кохистанский
mxu = Мада
my = Бирманский
myv = Эрзянский
nan-tw = Тайваньский (миннань)
nb-NO = Норвежский Букмол
ncx = Центральнопуэбланский науатль
nd = Ндебеле Северный
ne-NP = Непальский
new = Невари
nhe = Восточно-уастеканский науатль
nhi = Западный сьерра-пуэбланский науатль
nia = Ниасский
nl = Голландский
nla = Нгомбале
nlv = Орисабаский науатль
nmg = Квасио
nmz = Лоссо
nn-NO = Норвежский Нюнорск
nnh = Нгембона
nqo = Нко
nr = Ндебеле Южный
nso = Северный сото
ny = Чиньянджа
nyn = Ньянколе
nyu = Нюнгве
oc = Окситанский
odk = Оадки
om = Оромо
or = Ория
oru = Ормури
os = Осетинский
pa-IN = Панджаби
pap = Папьяменто
pap-AW = Папьяменто (Аруба)
pau = Палауанский
pcd = Пикардийский
pcm = Нигерийский пигинский английский
pez = Восточный пенанский
phl = Палула
phr = Пахари-Потвари
pl = Польский
plk = Кохистанский шина
pne = Западный пенанский
prq = Ашенинка Перене
ps = Пушту
pt = Португальский
pua = Западнохайлендская пурепеча
pwn = Пайвань
quc = Киче
qug = Нагорье Кечуа-Чимборасо
qup = Южная пастаза кечуа
qur = Кечуа янауанка
qus = Кечуа Сантьяго-дель-Эстеро
qux = Кечуа яуйо
quy = Аякучанский кечуа
qva = Кечуа Амбо-Паско
qvi = Кичуа
qvj = Лоха нагорья Кичва
qvl = Кечуа-Кажатамбо
qwa = Кечуа коронго анкаш
qws = Кечуа сиуа анкаш
qxa = Кечуа-чикен
qxp = Пуно-кечуа
qxq = Кашкай
qxt = Кечуа Паско Санта-Ана-де-Туси
qxu = Кечуа Арекипа-Ла-Унион
qxw = Кечуа-хауджа-ванка
rif = Тарифит
rki = Рахайн
rm-sursilv = Романшский Сурсельвский
rm-vallader = Романшский Валладер
rn = Кирунди
ro = Румынский
rof = Киромбо
ru = Русский
ruc = Руули
rup = Арумынский
rw = Руанда
rwm = Амба
sah = Якутский
sat = Сантали (Ол Чики)
sba = Нгамбей
sbn = Синдхи Бхили
sc = Сардинский
scl = Шина
scn = Сицилийский
sco = Шотландский
sd = Синдхи
sdh = Южный Курдский
sdo = Серианский бидау
seh = Сена
sei = Сери
sg = санго
shi = Шильхский
shn = Шанский
si = Сингальский
sk = Словацкий
skr = Сирайки
sl = Словенский
sn = Шона
snk = Сонинке
snv = Сабан
so = Сомалийский
sq = Албанский
sr = Сербский
ss = Свати
ssi = Санси
st = Сесото
su = Сунданский
sv-SE = Шведский
sva = Сванский
sw = Суахили
swb = Симаоре
syr = Сирийский
szy = Сакизая
ta = Тамильский
tar = Центральный тараумара
tay = Атаял
te = Телугу
teg = Теке
tg = Таджикский
th = Тайский
ti = Тигринья
tig = Тигре
tiv = тив
tk = Туркменский
tl = Тагальский
tli = Тлингит
tn = Тсвана
tob = Тоба-Кум
tok = Токипона
top = Папантла тотонак
tr = Турецкий
trv = Сидик
trw = Торвали
ts = Тсонга
tt = Татарский
ttj = Руторо
tui = Тупури
tum = Тумбука
tuq = Тедага
tvu = Тунен
tw = Чви
ty = Таитянский
tyv = Тувинский
uby = Убыхский
udl = Улдеме
udm = Удмуртский
ug = Уйгурский
uk = Украинский
ukv = Куку
ur = Урду
ush = Ушодзе
usp = Успантеко
uz = Узбекский
var = Уариджио
ve = Венда
vec = Венетский
vi = Вьетнамский
vmw = Махува
vot = Водский
wbl = Вахи
wep = Вестфальский
wes = Камерунский пигин
wo = Волоф
xdq = Кайтаг
xh = Коса
xhe = Кетрани
xka = Калкоти
xkl = Кенийский
xmf = Мингрельский
xsm = Касем
yaq = Яки
yav = Нуасуе
ydg = Ядгха
yi = Идиш
yo = Йоруба
yue = Кантонский
zgh = Тамазигхт
zh-CN = Китайский (Китай)
zh-HK = Китайский (Гонконг)
zh-TW = Китайский (Тайвань)
zoc = Копайнала-Соке
zu = Зулу
zza = Зазаки

# [/]


## Layout

profile = Профиль
help = Справка
contact = Контакты
privacy = Приватность
terms = Условия
cookies = Куки
faq = ЧЗВ
documentation = Документация
content-license-text = Содержимое доступно под <licenseLink>лицензией Creative Commons</licenseLink>
share-title = Помогите нам, найдя других желающих записать свой голос!
share-text = Помогите машинам научиться понимать реальных людей, пожертвуйте свой голос на { $link }
link-copied = Ссылка скопирована
back-top = Наверх
logout = Выйти
donate = Пожертвовать
stayintouch = Мы в Mozilla создаём сообщество по языковым технологиям. Мы хотим, чтобы вы были в курсе всех новостей, новых источников данных, а также хотели бы знать больше о том, как вы используете такого рода данные.
privacy-info = Мы обещаем, что будем обрабатывать вашу информацию с осторожностью. Подробнее в нашем <privacyLink>Уведомлении о конфиденциальности</privacyLink>.
return-to-cv = Вернуться к Common Voice
email-input =
    .label = Эл. почта
loading = Загрузка…
email-opt-in-info = Я хочу получать такие письма, как напоминания о целях, уведомления о достижениях и новости о Common Voice.
email-opt-in-info-title = Присоединиться к рассылке Common Voice
email-opt-in-info-sub-with-challenge = Получать такие письма, как челленджи, напоминания о целях, уведомления о достижениях и новости о Common Voice.
email-opt-in-privacy-v2 = Согласившись получать письма, вы заявляете, что согласны с тем, как Mozilla обрабатывает эту информацию, согласно <privacyLink>Политике приватности</privacyLink> Mozilla.
indicates-required = * поля, обязательные для заполнения
not-available-abbreviation = Недоступно
# Text shown on a button that can be clicked to copy text to the clipboard
copy = Копировать
# Text shown on a button that can be clicked to close a modal or panel
close = Закрыть
# Text shown on a button that can be clicked to cancel an action
cancel = Отмена
# Text shown on a button that can be clicked to delete an item
delete = Удалить

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>Нам нужна</mark><br/><mark>ваша помощь!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = Размещение наборов данных и улучшение платформы для более чем 100 языковых сообществ обходится почти в миллион долларов в год. <strong>Если вы цените открытые, инклюзивные данные — сделайте пожертвование сегодня!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>Используете CV для</mark><br/><mark>вашего исследования?</mark>
languages-donate-banner-cta-explanation = Common Voice финансируется за счёт пожертвований и грантов! Нам нравится сотрудничать с учёными, гражданским обществом и отраслевыми исследователями. Common Voice бесплатен для использования, но участие в пожертвованиях на платформу и хостинг может быть действительно полезным.
localization-select =
    .label = Выберите язык/локализацию

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = <strong>Новые наборы данных Common Voice</strong> теперь доступны для скачивания эксклюзивно через нашу родственную платформу Mozilla Data Collective. Присоединитесь для получения доступа к более чем 300 глобальным наборам данных, созданным сообществом и для сообщества.
# Button text for MDC announcement
announcement-mdc-button-text = Присоединится к Mozilla Data Collective
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = Открывается в новой вкладке

## Release Delay Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-delay = Уважаемое сообщество, сейчас мы работаем над релизом. Мы ожидаем, что эта функция будет частично развернута в течение следующих дней на <mdcLink>Mozilla Datacollective</mdcLink>.

## Release Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release = Уважаемое сообщество, новый релиз теперь полностью размещен в <mdcLink>Mozilla Datacollective</mdcLink>. Присоединитесь для получения доступа к более чем 500 глобальным наборам данных, созданным сообществом и для сообщества.

## Pre-Release Announcement (dates TBD)

# Text wrapped in <cvDatasheetsLink></cvDatasheetsLink> will be rendered as an external link to the cv-datasheets GitHub repo
announcement-pre-release = Уважаемое сообщество, время релиза близится. Пожалуйста, валидируйте и транскрибируйте, а также посетите <cvDatasheetsLink>cv-таблицы</cvDatasheetsLink>, чтобы обновить свою таблицу.

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = Начните набирать для поиска
searchbox-default-placeholder = Поиск...
searchbox-default-aria-label = Поиск
dropdown-no-results = Совпадений не найдено
# Common Voice Dataset=Language search box
dataset-searchbox-label = Изменяет язык отображения и набора данных
dataset-searchbox-placeholder = Поиск языка...
dataset-searchbox-aria-label = Поиск языка
dataset-search-no-results = Подходящего языка не найдено
request-new-language = Запросить новый язык
