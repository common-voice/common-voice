# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Afar
aat = Arvanitisch
ab = Abchazisch
abb = Bankon
abq = Abaza
ace = Atjehs
ady = Adygees
af = Afrikaans
aii = Sureth
ajg = Adja
aln = Gegisch
am = Amhaars
an = Aragonees
ar = Arabisch
arn = Mapudungun
as = Assamees
ast = Asturisch
az = Azeri
azz = Hoogland-puebla-nahuatl
ba = Basjkiers
bag = Tuki
bal = Balochi
bas = Basaa
bax = Bamun
bba = Baatonum
bbj = Ghomala
bbl = Batsisch
bce = Mengambo
bci = Baoule
be = Wit-Russisch
beb = Bamvele
bew = Betawi
bfd = Bafut
bft = Balti
bg = Bulgaars
bgp = Oost-Beloetsji
bin = Edo
bkh = Bakoko
bkm = Kom
bm = Bambara
bn = Bengaals
bnm = Batanga
bnn = Bunun
bo = Tibetaans
br = Bretons
brh = Brahui
bri = Mokpwe
brx = Bodo
bs = Bosnisch
bsh = Katë
bsk = Brushaski
bsy = Sabah Bisaya
btv = Bateri
bum = Bulu
bxk = Bukusu
bxr = Boerjatisch
byv = Medumba
ca = Catalaans
cak = Kaqchikel
cdo = Oosterse Min
ce = Tsjetsjeens
cgg = Chiga
cjk = Chokwe
ckb = Centraal-Koerdisch
cnh = Hakha Chin
co = Corsicaans
cpx = Heng Hua
cpy = Asheninka Zuidelijk Ucayali
crh = Krim-Tataars
cs = Tsjechisch
cut = Teutila-Cuicatec
cux = Tepeuxila-Cuicatec
cv = Chuvash
cy = Welsh
da = Deens
dag = Dagbani
dar = Dargwa
dav = Kidaw'ida
de = Duits
din = Dinka
dmk = Dawoodi
dml = Dameli
dru = Rukai
dsb = Nedersorbisch
dua = Duala
dv = Divehi
dyu = Dioula
ebr = Ebrie
ee = Ewe
efk = Efik
eko = Ekoti
el = Grieks
en = Engels
eo = Esperanto
es = Spaans
esu = Centraal-Alaska Yup’ik
et = Estisch
eto = Eton
eu = Baskisch
ewo = Ewondo
fa = Perzisch
fan = Fang
ff = Fula
fi = Fins
fmp = Feʼefeʼe
fo = Faeröers
fr = Frans
fub = Adamawa Fulfulde
fue = Borgu Fulfulde
fuf = Guinee-Pular
fy-NL = Fries
ga-IE = Iers
gaa = Ga
gcf = Martinikaans Creools
gej = Mina
ggg = Gurgula
gid = Guidar
gig = Goaria
giz = Guiziga
gjk = Kachhi
gju = Gojiri
gl = Galicisch
glk = Gilaki
gn = Guaraní
gom = Goanees Konkani
gos = Gronings
gsw = Elzassisch
gu-IN = Gujarati
guc = Wayuunaiki
gv = Manx-Gaelisch
gwc = Gawri
gwt = Gawar-Bati
gya = Noordwest-GBaya
ha = Hausa
hac = Našinski
hak = Hakka
haz = Hazargi
hch = Huichol
he = Hebreeuws
hem = Kihemba
hi = Hindi
hil = Hiligaynon
hno = Noord-Hindko
hr = Kroatisch
hrx = Hunsrik
hsb = Oppersorbisch
ht = Haïtiaans
hu = Hongaars
hux = Nüpode Huitoto
hy-AM = Armeens
ia = Interlingua
iba = Iban
ibb = Ibibio
id = Indonesisch
ie = Interlingue
ig = Igbo
ipk = Inupiak
is = IJslands
it = Italiaans
izh = Izjorisch
ja = Japans
jam = Jamaicaans Patois
jbo = Lojban
jgo = Ngomba
jqr = Jaqaru
jv = Javaans
ka = Georgisch
kaa = Karakalpaks
kab = Kabylisch
kam = Kikamba
kbd = Kabardisch
kcn = Nubi
kdh = Kotokoli
khw = Khowar
ki = Kikuyu
kjh = Chakassisch
kk = Kazachs
kln = Kalenjin
kls = Kalasha
km = Khmer
kmr = Kurmanci
kn = Kannada
knn = Konkani (Devanagari)
ko = Koreaans
koo = Konzo
kpv = Zurjeens
krc = Karachay-Balkar
ks = Kashmiri
ksf = Bafia
kvx = Parkari Koli
kw = Cornish
kxp = Wadiyara Koli
ky = Kirgizisch
kzi = Kelabit
laj = Lango
lb = Luxemburgs
led = Lendu
leu = Kara
lg = Luganda
lij = Ligurisch
lke = Soga
lld = Ladin
ln = Lingala
lo = Laotiaans
lrk = Loarki
lrl = Achomi
lss = Lasi
lt = Litouws
ltg = Letgaals
lth = Thur
lua = Tshiluba
luo = Dholuo
lv = Lets
lzz = Laz
mai = Maithili
mau = Huautla-Mazateeks
mbf = Baba-Maleis
mbo = Mbo
mcf = Matses
mcn = Massa
mcx = Mpiemo
mdd = Mbum
mdf = Moksja
meh = Mixteco-Yucuhiti
mel = Melanau
mfe = Kreools Morisien
mg = Plateaumalagasi
mgg = Kunabembe
mhk = Mungaka
mhr = Weide-Mari
mk = Macedonisch
mki = Dhatki
ml = Malayalam
mmc = Michoacán-Mazahua
mn = Mongools
mni = Meetei Lono
mnw = Mon
mos = More
mqh = Tlazoyaltepec-Mixteeks
mr = Marathi
mrh = Mara
mrj = Berg-Mari
ms = Maleis
mse = Moussey
msi = Sabah-Maleis
mt = Maltees
mua = Mundang
mug = Musgum
mve = Pakistaans Marwari
mvy = Indus Kohistani
mxu = Mada
my = Burmees
myv = Erzja
nan-tw = Taiwanees (Minnan)
nb-NO = Noors Bokmål
ncx = Centraal-Puebla-Nahuatl
nd = IsiNdebele (Noordelijk)
ne-NP = Nepalees
new = Newari
nhe = Oostelijk Huasteca Nahuatl
nhi = Westelijk Sierra Puebla Nahuatl
nia = Nias
nl = Nederlands
nla = Ngombale
nlv = Orizaba Nahuatl
nmg = Kwasio
nmz = Losso
nn-NO = Noors Nynorsk
nnh = Ngiembon
nqo = NKo
nr = IsiNdebele (Zuidelijk)
nso = Noord-Sotho
ny = Chinyanja
nyn = Nyankole
nyu = Nyungwe
oc = Occitaans
odk = Oadki
om = Afaan Oromo
or = Odia
oru = Ormuri
os = Ossetisch
pa-IN = Punjabi
pap = Papiaments
pap-AW = Papiaments (Aruba)
pau = Palauan
pcd = Picard
pcm = Nigeriaans pidgin-Engels
pez = Oostelijk Penan
phl = Palula
phr = Pahari-Pothwari
pl = Pools
plk = Kohistani Shina
pne = Westelijk Penan
prq = Asheninka Perene
ps = Pashtu
pt = Portugees
pua = Purepecha van de Westelijke Hooglanden
pwn = Paiwaans
quc = K'iche'
qug = Quichua Chimborazo-Hoogland
qup = Zuid-Pastaza Quechua
qur = Quechua Yanahuanca
qus = Quechua Santiago del Estero
qux = Quechua-Yauyos
quy = Quechua Chanka
qva = Quechua Ambo-Pasco
qvi = Kichwa
qvj = Loja Hoogland-Kichwa
qvl = Quechua Cajatambo
qwa = Quechua Corongo Ancash
qws = Quechua Sihuas Ancash
qxa = Quechua Chiquián
qxp = Puno Quechua
qxq = Qashqai
qxt = Quechua Pasco Santa Ana de Tusi
qxu = Quechua Arequipa-La Unión
qxw = Quechua Jauja Wanka
rif = Tarifit
rki = Rakhine
rm-sursilv = Reto-Romaans Surselvisch
rm-vallader = Reto-Romaans Vallader
rn = Kirundi
ro = Roemeens
rof = Kirombo
ru = Russisch
ruc = Ruuli
rup = Aromanisch
rw = Kinyarwanda
rwm = Amba
sah = Jakoets
sat = Santali (Ol Chiki)
sba = Ngambay
sbn = Sindhi Bhili
sc = Sardijns
scl = Shina
scn = Siciliaans
sco = Schots
sd = Sindhi
sdh = Zuid-Koerdisch
sdo = Serian Bidayuh
seh = Sena
sei = Seri
sg = Sango
shi = Tashelhiyt (Shilha)
shn = Shan
si = Singalees
sk = Slowaaks
skr = Saraiki
sl = Sloveens
sn = Shona
snk = Soninke
snv = Saʼban
so = Somalisch
sq = Albanees
sr = Servisch
ss = Siswati
ssi = Sansi
st = Zuid-Sotho
su = Soendanees
sv-SE = Zweeds
sva = Svanetisch
sw = Swahili
swb = Shimaore
syr = Syrisch
szy = Sakizaya
ta = Tamil
tar = Centraal-Tarahumara
tay = Atayal
te = Telugu
teg = Teke
tg = Tadzjieks
th = Thai
ti = Tigrinya
tig = Tigre
tiv = Tiv
tk = Turkmeens
tl = Tagalog
tli = Tlingit
tn = Tswana
tob = Toba Qom
tok = Toki Pona
top = Papantla Totonac
tr = Turks
trv = Taroko
trw = Torwali
ts = Tsonga
tt = Tataars
ttj = Rutoro
tui = Tupuri
tum = Tumbuka
tuq = Tedaga
tvu = Tunen
tw = Twi
ty = Tahitiaans
tyv = Toevaans
uby = Oebychs
udl = Ouldémé
udm = Oedmoerts
ug = Oeigoers
uk = Oekraïens
ukv = Kuku
ur = Urdu
ush = Ushoji
usp = Uspanteeks
uz = Oezbeeks
var = Guarijio
ve = Venda
vec = Venetiaans
vi = Vietnamees
vmw = Emakhuwa
vot = Wotisch
wbl = Wachi
wep = Westfaals
wes = Kameroen Pidgin
wo = Wolof
xdq = Kaitag
xh = Xhosa
xhe = Khetrani
xka = Kalkoti
xkl = Keniaans
xmf = Mingreels
xsm = Kasem
yaq = Yaqui
yav = Nuasue
ydg = Yadgha
yi = Jiddisch
yo = Yoruba
yue = Kantonees
zgh = Tamazight
zh-CN = Chinees (China)
zh-HK = Chinees (Hongkong)
zh-TW = Chinees (Taiwan)
zoc = Copainalá Zoque
zu = Zoeloe
zza = Zazaki

# [/]


## Layout

profile = Profiel
help = Help
contact = Contact
privacy = Privacy
terms = Voorwaarden
cookies = Cookies
faq = FAQ
documentation = Documentatie
content-license-text = Inhoud is beschikbaar onder een <licenseLink>Creative Commons-licentie</licenseLink>
share-title = Help ons anderen te vinden die hun stem willen doneren!
share-text = Help machines te leren hoe echte mensen spreken, doneer uw stem op { $link }
link-copied = Koppeling gekopieerd
back-top = Naar boven
logout = Afmelden
donate = Doneren
stayintouch = Bij Mozilla bouwen we een gemeenschap rond spraaktechnologie. We blijven graag met u in contact met updates, nieuwe gegevensbronnen, en om te horen hoe u deze gegevens gebruikt.
privacy-info = We beloven uw gegevens met zorg te behandelen. Lees meer in onze <privacyLink>Privacyverklaring</privacyLink>.
return-to-cv = Terug naar Common Voice
email-input =
    .label = E-mailadres
loading = Laden…
email-opt-in-info = Ik wil graag e-mailberichten zoals herinneringen aan doelstellingen, mijn voortgangsupdates en nieuwsbrieven over Common Voice ontvangen.
email-opt-in-info-title = Meld u aan voor de Common Voice-mailinglijst
email-opt-in-info-sub-with-challenge = Ontvang e-mailberichten zoals herinneringen aan uitdagingen en doelstellingen, voortgangsupdates en nieuwsbrieven over Common Voice.
email-opt-in-privacy-v2 = Door u in te schrijven voor het ontvangen van e-mailberichten, verklaart u dat u akkoord gaat met het verwerken van deze informatie door Mozilla in overeenstemming met Mozilla’s <privacyLink>Privacybeleid</privacyLink>.
indicates-required = * Geeft verplicht veld aan
not-available-abbreviation = NB
# Text shown on a button that can be clicked to copy text to the clipboard
copy = Kopiëren
# Text shown on a button that can be clicked to close a modal or panel
close = Sluiten
# Text shown on a button that can be clicked to cancel an action
cancel = Annuleren
# Text shown on a button that can be clicked to delete an item
delete = Verwijderen

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>We hebben</mark><br/><mark>uw hulp nodig!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = Het kost bijna een miljoen dollar per jaar om de gegevenssets te hosten en het platform te verbeteren voor de meer dan 100 taalgemeenschappen die afhankelijk zijn van wat we doen. <strong>Als u open, inclusieve gegevens waardeert – doneer dan vandaag nog!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>Gebruikt u CV voor</mark><br/><mark>uw onderzoek?</mark>
languages-donate-banner-cta-explanation = Common Voice wordt gefinancierd door donaties en subsidies! We werken graag samen met academici, onderzoekers van het maatschappelijk middenveld en de industrie. Common Voice is gratis te gebruiken, maar bijdragen aan de platform- en hostingkosten via subsidievoorstellen helpt echt.
localization-select =
    .label = Kies uw taal/localisatie

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = <strong>Nieuwe Common Voice-gegevenssets</strong> kunnen nu exclusief worden gedownload via ons zusterplatform, Mozilla Data Collective. Sluit u aan voor toegang tot meer dan 300 wereldwijde gegevenssets, gebouwd door en voor de gemeenschap.
# Button text for MDC announcement
announcement-mdc-button-text = Aansluiten bij Mozilla Data Collective
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = Opent in een nieuw tabblad

## Release Delay Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-delay = Beste gemeenschap, we werken momenteel aan de uitgave. We verwachten dat dit de komende dagen in delen wordt uitgerold op <mdcLink>Mozilla Data Collective</mdcLink>.

## Release Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release = Beste gemeenschap, de nieuwe release is nu volledig beschikbaar op <mdcLink>Mozilla Data Collective</mdcLink>. Doe mee voor toegang tot meer dan 500 wereldwijde gegevenssets, gebouwd door en voor de gemeenschap.

## Pre-Release Announcement (dates TBD)

# Text wrapped in <cvDatasheetsLink></cvDatasheetsLink> will be rendered as an external link to the cv-datasheets GitHub repo
announcement-pre-release = Beste gemeenschap, de tijd van uitgave nadert. Ga valideren en transcriberen, en bezoek ook de <cvDatasheetsLink>cv-datasheets</cvDatasheetsLink> om uw datasheet bij te werken.

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = Typ hier om te zoeken
searchbox-default-placeholder = Zoeken…
searchbox-default-aria-label = Zoeken
dropdown-no-results = Geen overeenkomst gevonden
# Common Voice Dataset=Language search box
dataset-searchbox-label = Wijzigt zowel de weergave- als de gegevenssettaal
dataset-searchbox-placeholder = Zoeken naar een taal…
dataset-searchbox-aria-label = Zoeken naar een taal
dataset-search-no-results = Geen overeenkomende taal gevonden.
request-new-language = Nieuwe taal aanvragen
