action-click = ჟილითლე
action-tap = ლიგდური
contribute = ლიმურჯი
review =
    ქალისჷ
    
    ნიმუში #1
    ნიმუში #2
    უ̄
    უ̄ჟ
    უ̄თა
    უ̈
    გუ̈
    თხუ̈მ
    უ̄̈
    ლიწუ̄̈ლე
    ოთბუჭკუ̄̈რ
    უ̂
    მურყუ̂ამ
    ხეხუ̂
    ა̄
    მა̄რე
    ზურა̄ლ
    ა̈
    ბა̈ჩ
    ლა̈ირ
    ა̄̈
    აჯელუ̂ა̄̈ლი
    იდიარა̄̈ლ
    ო̄
    ხო̄გ
    ხო̄ზ
    ო̈
    ო̈რბ
    ტო̈ეტ
    ო̄̈
    იწო̄̈ნე
    ჭო̄̈ერ
    ჷ
    მჷტ
    ბჷგი
    ჷ̄
    ჷ̄ნძღა-ბჷ̄ნძღა
    ლიფტჷ̄რე
    ი̄
    ლი̄შგი
    ი̄შდნე
    ე̄
    ლიბე̄ლე
    ლიბე̄რგე
    ჴ
    ჴამშა
    ჴასგ
    ჲ
    ჲერხი
    ჲენკანობ
    
    გასახმოვანებელი წინადა
    დაწერეთ 3 წინადადება (ან დააკოპირეთ  საიდანმე). სასურველი 10-15 სიტყვა იყოს თითოეულ წინადადებაში. გაახმოვანეთ ეს წინადადებები, რომ მქონდეს ნიმუში. სიტყვებში იყოს სპეციფიკური ბგერები.
    
    1
    ხუ̂ა̈ჲნ, შუკუ̂ჟი ლახ ა̈რიხ შუ̂ანა̈რ ი ლიცი ლათრა მო̄მა ხუღუ̂ახ, ეჩქა ლუშნუ ფაყუ̂შუ̂ ითრეხ ლიცს. 
    2
    შუ̂ა̈ნს ქართობილს ხუ̂ა̈ჲს ალა̄̈შიხ ი მაჴა̈დი ჩი ზაუ̂ ეზა̈რ ჴედნი.
    3
    ქართობილს ცა̄̈დიხ კუ̂ეცანღა ი მინი ხუ̂ა̈ჲს იჴმა̈რიხ, გუნ ხალა̈ტხ ი ეჯღა აშხუ̂ სოფელს ლაგა̈ცდ “ქართობლაუს” ხატუ̄ლიხ.
    
    შესამოწმებელი წინადადებები
    დაწერეთ სხვა 3 წინადადება (ან დააკოპირეთ  საიდანმე). სასურველი 10-15 სიტყვა იყოს თითოეულ წინადადებაში. გაახმოვანეთ ეს წინადადებები. გახმოვანებისას, 2 წინადადებაში დაუშვით თითო შეცდომა. ეს შეცდომა სასურველია იყოს სპეციფიკური ასოების ბგერებზე. 
    
    
    
    ✅/❌
    წინადადება
    შეცდომის განმარტება
    1
    ❌
    მეთხუ̂ია̈რდ ი მიჩა აფხნეგდ ქა̄დშყუჟუ̄რეხ მჷჟირ ლერქუ̂ა̈ლ ი ლეფკუ̂ად ოთრეკხ.
    უნდა იყოს: ქა̄̈დშყუ̄ჟურეხ (“გაიხადეს”), უმლაუტის იგნორირებამ პირველ მარცვალში გამოიწვია სემანტიკური ცვლილება — სათავისო ქცევას ჩენაცვლა საარვისო (“გახადეს”).
    2
    ✅
    ამზაუ̂ ხოჩა დარა̈ლგარ ლა̄̈სუ̂ხ ი ქართუ̂ებლი გუნ ხოჩა მაჴა̈დ ანჴა̈დ.
    
    
    3
    ❌
    ალ დო̈შდი ლადა̈ღ ქა̄̈დგუ̂ალა̄̈ნ, ხოჩა და̄̈რ ანჴა̈დ ი ამ ნაგზაისგა ამჟიგარ ი̄რა.
    უნდა იყოს: და̈რ “ამინდი”; სიტყვაში არაგრძელი უმლაუტიანი ხმოვნის ჩანაცვლებამ გრძელით გამოიწვია სემანტიკური ცვლილება: და̈რ “ამინდი” — და̄̈რ “არავინ”.
skip = ქალიცუ̂რე
shortcuts = ჩქა̈რდლატუ̂ი̄ლნა
goal-help-recording = სი ქალოხმა̈რჯუ̂ გუშგუ̂ეჲ ჩილა̈დღია̈ </goalPercentage></goalPercentage> ლეჲრი̄ { $goalValue } ლასრულას!
goal-help-validation = სი ქალოხმურჯ Common Voice-ს <goalPercentage></goalPercentage>გუშგუ̂ეჲ ჩილა̈დღი{ $goalValue }ა̈  ლეჲრი̄ ლასრულას.
contribute-more =
    { $count ->
        [one] ხიმა ლჷმა̄რე { $count } ხოშილი ლასყე̄ნად?
       *[other] ხიშდმა ლჷმა̄რე { $count } ხოშილი ლასყე̄ნად?
    }
speak-empty-state = ალ ნჷნჟი ლეირი წინადადება̄̈ლ ჩუ̂ა̈გუ̂შდა̈ხ...
no-sentences-for-variants = ისგუ ნინი უ̂არიანტ შეიძლება წინადადება̄̈ლურდ ლესსუ̂! ხომურჯა ჰე̄მა ჯა̄რ, ჰე, ჟა̈ხცა̄დუ̂ინ პარამეტრა̄̈ლ, ერე ქახეწე იშგენ წინადადება̄̈ლს ისგუ̂ა ნინჟი.
speak-empty-state-cta = სგა̄თი̄რ წინადადება̄̈ლ
speak-loading-error =
    დეშ ხუ̂ითხედ წინადადება ისგოვდ. 
    ხოშილ გუ̂ია̄ნდ აჯაღ ათკუ̄ჰირ.
record-button-label = სგა̄თი̄ჲრ ისგუ ჰერ
share-title-new = <bold>ქალა̈ნმა̈რჯუ̂</bold>, ერე ხოფში̄რა ჰერა̈ლ ოთთჷმად
keep-track-profile = გუ̂ი ლა̄ხო ისგუ სგუ̂ებდ ნა̄ზუ̂ს ისგუ პროფილისგა
login-to-get-started = ლა̈ბნად ჲედ სგა̄დე, ჲედ ჩუ̂ა̈თრეგისტირა̈ნ
target-segment-first-card = სი ისგუ ნა̄̈თი (წვლილ) სგე̄სჷდ გუშგუ̂ეჲ მა̄̈ნკუ̂ი̄ სამიზნე სეგმენტთე̄სგა
target-segment-generic-card = სი ისგუ ნა̄̈თი (წვლილ) სგე̄სჷდ სამიზნე სეგმენტთე̄სგა
target-segment-first-banner = ქალოხმურჯ Common Voice-ს მა̄̈ნკუ̂ი̄ სამიზნე სეგმენტი ლასყენას { $locale }
target-segment-add-voice = ჟესაქმ ისგუ ჰერ
target-segment-learn-more = ა̈ცხირ ხოშილ!
change-preferences = ჟ'ა̄ხცადუნ რეჟიმ
login-signup = სგალი̄ზი/რეგისტრაცია
vote-yes = ა̄დუ
vote-no = დე̄სა/მა̄მა
datasets = მონაცემა̈რ ჟილუყუ̂ბე
languages = ნჷნა̈რ
about = ამღა/ამი̄ ბედჟი
partner = პარტნიორ
submit-form-action = ქალიზზი

## Reporting

report = ანგარიშ
report-title = ანგარიში ქალიზზი
report-ask = მა̈ჲ ლი პრობლემა ალ წინადადებაისგა?
report-offensive-language = შეურაცხმყოფელ ნინ/ამბუ̂ა̈რ
report-offensive-language-detail = ალ წინადადება ლი ლჷირ უშგდა ი შეურაცხმყოფელ ნინშუ̂.
report-grammar-or-spelling = გრამატიკულ / ორთოგრაფიულ შეცდომა
report-grammar-or-spelling-detail = ალ წინადადებაისგა გრამატიკულ ჲედ ორთოგრაფიულ შეცდომა ლი.
report-different-language = იშგენ ნინ
report-different-language-detail = ალა მი ერ გუ̂ა̈რგლი ეჯ ნინშუ̂ მა̄მ ლი ლჷი̄რ.
report-difficult-pronounce = ხოლა ლექუ̂ისგ ლი
report-difficult-pronounce-detail = ალას ხეჩდა ეჯკა̈ლი სიტყუ̂ა̄̈ლ ჲედ ფრაზა̄̈ლ, ხედა̄̈ ლიჭუ̂და̄ნე ჲედ ლიქუ̈ისგ გარჯ ლი.
report-offensive-speech = შეურაცხმყოფელ/ ლეგხო̄ლე გარგლა
report-offensive-speech-detail = კლიპ შეურაცხმყოფელ ი უშგდა ნინშუ̂ ლი.
report-other-comment =
    .placeholder = კომენტარ
success = სგუ̂ებდ ლი̄ზი/წარმატება!
continue = აჯად ათი̄რ
report-success = მა̈გ ხოჩამდ ლა̈სუ̂!

## Speak & Listen Shortcuts


## Speak Shortcuts

shortcut-record-toggle-label = ლი̄რი/ლიჩრაუ̂ა̄̈ლ
shortcut-rerecord-toggle-label = კლიპი მახა̄ნ ლი̄რი
shortcut-discard-ongoing-recording-label = ჟახაღრ ათხე̄ ნა̈ი̄რ
shortcut-submit = უ̂ეშდ ლიტეხ/ლიტხე
shortcut-submit-label = კლიპრე ქალიზზი
request-language-text = ჩიქე მა̄მ წუ̂აჲა/თერაჲა ისგუ ნინ Common Voice-ჟი?
request-language-button = ხიჰრ ისგუ ნინ

## Listen Shortcuts

shortcut-play-toggle-label = სგუ̂ებდ ლი̄ზი/ჩულიჩრაუ̂ა̄̈ლ

## Validation criteria

contribution-criteria-nav = კრიტერიუმა̈ლ
contribution-criteria-link = ჩუ̂ა̈ცხირ კრიტერიუმა̈ლ
contribution-criteria-page-title = წუ̂ლილი კრიტერიუმა̈ლ
contribution-criteria-page-description = ჟა̈ხმეჴერ იმ ხაკუ ხოშა გუ̂ა̈კუ̂ირ ჰერშუ̂ ნა̈ირი ლაჰუ̂ნა̈რჟი ი ისგუ ჰერშუ̂ ნა̈ირა̈ლი ჩუ̂ათსიკეთ!
contribution-for-example = მაგალითად
contribution-misreadings-title = უსწოროდ ლჷჭუ̂და̄ნე
contribution-misreadings-description = ლაჰუ̂ნა̈რჟი ბჷგიდ ფირთხილდ ათმოწმა̈ნ, ერე სგანა̈ი̄რუ̂ ზუსტდ ლი ლჷი̄რ; უ̂არ ლე̄ქა, ჰემა შეცდომა ლი, მა̄̈ჲ დო უმნიშუ̂ნელო ლესსუ̂. <br /> მაფში̄რე შეცდომა̄̈ლ ლიხ:
contribution-misreadings-description-extended-list-1 = ნა̈ირი ლა̈ბნას ხაკლი<strong> 'ა'</strong> მო̄დეი<strong></strong>
contribution-misreadings-description-extended-list-2 = ხაკლი <strong></strong> სიტყვა̄̈ ლა̈შთხამჷყ.
contribution-misreadings-description-extended-list-3 = მეკუ̂შდე ი ჯო̄დი ფორმალე ლიჭუ̂და̈ნი, ხედუ̂ა̈ჲ მა̄მ ლი მნიშვნელოვან, მაგალითად, "მა̄მგუ̂ეშ" მო̄დეჲ  "მამაგუ̂ეშ", ჲედ პირიქით.
contribution-misreadings-description-extended-list-4 = ქახაცუ̂ირ უ̂ეშგმეპილჲა̈ სიტყუ̂ა̄̈ ლა̈შთხა ნა̈ი̄რი სურუ ჩქა̈რდ ლისრულემი ბა̈დშუ̂.
contribution-misreadings-description-extended-list-5 = სიტყვა̄̈ ლიჭუ̂და̄̈ნია̈ ფიშრა̄მინ ლიკჰუ̄რე.
contribution-misreadings-example-1-title = Triassi-ი უ̂ითმათა̈რ დინოზაუ̂რ.
contribution-misreadings-example-2-explanation = ['დინოზაუ̂რა̈ლ' ხაკუ]
contribution-misreadings-example-3-title = ტრიასი ხოშე̄მიშთან დინოზაუ̂რა̄̈ლ-.
contribution-misreadings-example-3-explanation = [ნა̈ი̄რ ქუ̂ეცნი უ̂ეშგიმპილჲა̈ სიტყუ̂ა̄̈ ქალისრულა̄ლ]
contribution-misreadings-example-4-title = ტრიასი ხოშე̄რება დინოზაუ̂რა̈. ა̄დუ.
contribution-misreadings-example-4-explanation = [ტექსტს ერ ხაწჷხ, ეჩან ხოშა ლი ლჷი̄რ]
contribution-misreadings-example-5-title = ნა̈ჲ ყაუ̂ა̄̈ ლათრად ესხუ̂რიდ ქა.
contribution-misreadings-example-6-title = ნა̈ჲ ქ'ე̄სღუ̂რიდ ყაუ̂ა̄̈ ლათრათე.
contribution-misreadings-example-6-explanation = [ხაკუ "ნა̈ჲ ხუ̂იშდ"]
contribution-misreadings-example-7-title = ყავა̄̈ ლათრათე ესღუ̂რიდ.
contribution-misreadings-example-7-explanation = [ორიგინალისგა ქა ხაკლი ა̄̈]
contribution-misreadings-example-8-title = ბუმბერაზ ჟა̄̈ნჷნჩქარა̄̈ნ.
contribution-misreadings-example-8-explanation = მა̄დმა მასყა კონტენტ.
contribution-varying-pronunciations-title = მო̄შდ ლჷქუ̂ე
contribution-varying-pronunciations-description = ფირთხილდ ხარდა̈ს, უ̂ოდე ნა̄̈ქუ̂ისგს ჩუ̂ათწჷნუ̂ი ეჯ მიზეზშუ̂, ერე მუჭუ̂და̄̈ნიდ უსწოროდ რა̄̈ქუ̂ე სიტყუ̂ა, ინტონაცია იშგნა̄̈გ ჩუ̂ემინ, ჲედ კითხვის ნიშა̈ნ ჰა̈დურდ დე̄სა ა̈დჭოდა̄ნე. თელ სოფლიოისგა ხუ̄̈ჲ ნუკუ̂იშ გარგლა ა̈რი, ხედუ̂ა̄̈ჲ ჲერხი სი  ისგუ გირკიდ საზოგადოებაისგა შეიძლება მა̄მ ჯესმე̄ნს. ჰემა იმა̈რგ, ნა̈ბ ლახო ეჯია̈რს, ერუ̂ა̄̈ჲ შეიძლება ისგუ̂აშა̄ლ მომ გა̈რგლი.
contribution-varying-pronunciations-description-extended = მე̄რმათე̄ჲ, სი ჰე̄მა ჯაბჟა, ერე მუჭუ̂და̄̈ნის ალ სიტყვა ათხად დეშომა ხასმა ი მიჩო̄უ̂შ ლჷქადდ ა̈ნმეჴრე, ეჩქა  უ̂არ ლექა. ჰე დოშ ხი დარწმუნებულ, ეჩქა ესოთლ ქალიცუ̂რე̄ ღილიკს.
contribution-varying-pronunciations-example-1-title = ეჯას თხუმჟი ბერეტ ხაგდა.
contribution-varying-pronunciations-example-1-explanation = ['ბერეტ']-ს მახუ̂ილს მა̄̈ნკუ̂ი̄ მარცუ̂ალჟიჲ ჩუხოყა (ბრიტ.) ი მე̄რმაჟიჲ (აშშ)]
contribution-varying-pronunciations-example-2-title = ტუ̂ეტ ანკაჩე.
contribution-varying-pronunciations-example-2-explanation = [‘Raised’ ინგლისურისგა ჩიგარ ი̄ქუ̂ი იმუ̂ა̄̈ჟი ეშხუ მარცვალ, მა̄მა ჲო̄რი]
contribution-background-noise-title = აგრე̄უ̂ს სა̈ხმა̈ხ
contribution-background-noise-description = ნა̈ჲ ნაკუ, ერე მანქანურ ლითუ̂რი̄ ალგორითმა̈ლდ ჟანმჲე̄დახ მო̄შ-მო̄შ აგრე̄უ̂ა̄̈ სა̈ხმა̈ხი ჩულიმშიჲე ი ზჷმშუ̂ ხოკლათხა ჰერა̄̈ლსი შეიძლება ჩუხეყე̄ნს ეჯ პირობშუ̂, ერე ეჯა მა̄მ  ა̈ჯმა̈უ̂ინნე ტექსტი მთლიანობაისგა ლაჰუ̂ნა̈რს. წყჷნა̄̈რ ფონურ მუსიკას ჩუ ხაყა; მუსიკა ეჯზუმ კჷლთხი, ერე ჯიმა̈უ̂ინე̄დს სიტყუ̂ა̄ლე თითოლაჟი ლა̈ცხირს — მა̄მა.
contribution-background-noise-description-extended = ნა̈ჲრ ჟი ჰემა ხეღრენი, მო̄დეჲ შჷრშნი, უ̂არ ლე̄ქა, ტექსტი მთლია̈ნდ ლიჰუ̂ნა̈რი ჰემა მა̄მ იჴჷრჴი.
contribution-background-noise-example-1-fixed-title = <strong> [იჩჩხიელხ</strong>] ტრიასულ პერიოდიშ ხოშე̄მიშთა̄ნ დინოზავრა̈ლ <strong> [იჴუ̂შიე̄ლხ]
contribution-background-noise-example-2-fixed-title = ტრიასულ ხოშე̄მიშთან დინო <strong> [იჴუ̂შიე̄ლ].
contribution-background-noise-example-2-explanation = [ტექსტი ნა̈წილ მა̄მ ისმი]
contribution-background-noise-example-3-fixed-title = <strong> [შჷრშჷნ</strong>] ხოშე̄მიშთა̄ნ დინოზაუ̂რა̈ლ <strong> [შჷრშჷნ] </strong> -რიასულ.
contribution-background-voices-title = გირკიდი ჰერა̄̈ლ
contribution-background-voices-description = წყჷნა̄̈რ ფონ ხოჩა გარ ლი, მარე ნა̈ჲ მა̄მ გუ̂აკუ ზედმეტ ჰერა̄̈ლ,  იმშა ტა̈რშუ̂ შეიძლება მანქანურ ალგორითმს ეჯკა̈ლი სიტყუ̂ა̄̈ლ ლახწუ̂ა̄̈ნდე̄დს,ხედუ̂ა̄̈ჲ მა̄მ ა̈რიხ ლჷი̄რ ტექსტისგა. სი ტექსტი სიტყუ̂ა̄̈ლჷნქა მო̄შ სიტყუ̂ა̄̈ლ ჰემა ჯესმი, კლიპღა უ̂არ ხეკუ̂ეს რა̄̈ქუ̂ა. ხოშა მჷნწირდ, ალა ხეხუ̂ა, შომა̄̈ჲ ტელევზორ ლი ჩართულ მესდე ჲედ აგრეუ̂ს იმბუ̂ა̄̈ლხ.
contribution-background-voices-description-extended = ნა̈ჲრ ჰე̄მა ქუ̂ეცნი ჰედ შჷრშჷნ ხონჴრი, უარ რე̄ქა, ტექსტ მთლია̈ნდ ჰე მო̄მა ისმი.
contribution-background-voices-example-1-title = ტრიასი გიგანტურ დინოზავრა̄̈ლ. <strong> [ლუჭუ̂და̄ნე ლი აშხუ̂ ჰერშუ̂] </strong>
contribution-background-voices-example-1-explanation = სი ანღრიმა? <strong> [ლახტუ̂ილ იშგენდ] </strong>
contribution-volume-title = ჰერი ნაკლა̈თხი
contribution-volume-description = ი̄რახ ბუნებრიუ̂ განსხუ̂აუ̂ება̄̈ლ ჰერი ნაკლათხაჲსგა მუჭუ̂და̄ნრეშ. უ̂არ ეჩქა გარ რე̄ქა, ე,  ჰერ ეჯზუმ კჷლთხი ჰე̄მა ლი, ერე ნა̈ირ ქუ̂ეცნი, მო̄დეჲ (ხოფშირა̄მინ) ჰერ ეჯზუმ დამბლი ლი, ერე ლჷირ ტექსტურ დეშ ხა̈ცხრი, იმდო ტუ̄ლი.
contribution-reader-effects-title = ეფექტა̈რ მუჭუ̂და̂ნრეშდ
contribution-reader-effects-description = ჩანაწერლეშ ხოშა ნა̄̈თი ლიხ ხა̈ლხი გა̈რგლა მინე ბუნებრიუ̂ ჰერშუ̂. სი ჩუ ჯა̈მჲე̄და, ერე ა̈ხკჷდა არასტანდარტულ ჩანაწერ, ხედუ̂ა̄̈ჲ ი̄რა ლა̈ტუ̂ი̄ლია̄ლუშ, ლაფურსტინშუ̂ ჰედ ქალჷთნად 'დრამატულ' ჰერშუ̂. უ̂არ რე̄ქა ლაღრა̄ლოშ ნა̈ჲრ ჩანაწერა̈ლღა ი კომპიუტერი სინთეზირებულ ჰერს ერ იჴმა̈რიხ, ეჯჲა̈რღა.
contribution-just-unsure-title = უბრალოდ დარწმუნებულ მა̄მ ხიჲა?
contribution-just-unsure-description = ეჯკა̈ლი ჰე̄სამა აჯხუ̂იდე̄დს, ერე ალ სახელმძღვანელოჲსგა მა̄მ ხა̈ჲრა, ჰერ ლა̄ხო ისგუ̂ა ლაწეუ̂შუუ̂. ჰე გაცხა̈ჲდ დეშ ასკჷბა, ა̈ხჴჷმა̈რ ქალიცუ̂რე̄მი ღილაკ ი ქა̄̈ტხ ეჩუნღუ̂ე̄ ჩანაწერთე.
see-more = <chevron></chevron> ხეწდ ხოში̄ლს
see-less = <chevron></chevron> ხეწდ ხოხუ̂რა̄მ
