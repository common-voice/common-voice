## Error pages

banner-error-slow-1 = სამწუხაროდ, Common Voice შენელებულია. გმადლობთ ყურადღებისთვის.
banner-error-slow-2 = დიდი დატვირთვაა და მიმდინარეობს ამ საკითხის მოგვარებაზე მუშაობა.
banner-error-slow-link = მდგომარეობის გვერდი
error-something-went-wrong = სამწუხაროდ, რაღაც ხარვეზი წარმოიქმნა
error-clip-upload = ეს ჩანაწერი კვლავ ვერ აიტვირთა, განაგრძოს მცდელობა?
error-clip-upload-server = ჩანაწერის ატვირთვა ვერ ხერხდება, ახლიდან გახსენით გვერდი ან მოგვიანებით სცადეთ.
error-clip-upload-too-large = თქვენი ჩანაწერი ზედმეტად დიდია ასატვირთად. გთხოვთ უფრო მოკლედ ჩაწეროთ.
error-clip-upload-server-error = სერვერის შეცდომა ჩანაწერის დამუშავებისას. გთხოვთ განაახლოთ გვერდი ან სცადოთ მოგვიანებით.
error-title-404 = გვერდი ვერ მოიძებნა
error-content-404 = იქნებ ჩვენი <homepageLink>მთავარი გვერდი</homepageLink> დაგეხმაროთ? კითხვის დასასმელად, შემოუერთდით <matrixLink>Matrix-ის ერთობის სასაუბროს</matrixLink>, საიტის ხარვეზების შესახებ იხილეთ <githubLink>GitHub</githubLink> ან ეწვიეთ <discourseLink>ჩვენს Discourse-ფორუმებს</discourseLink>.
error-title-500 = სამწუხაროდ, რაღაც ხარვეზი წარმოიქმნა
error-content-500 = მოულოდნელი შეცდომა წარმოიშვა, სცადეთ მოგვიანებით. დახმარების მისაღებად შემოუერთდით <matrixLink>Matrix-ის ერთობის სასაუბროს</matrixLink>, საიტის ხარვეზების შესახებ იხილეთ <githubLink>GitHub</githubLink> ან ეწვიეთ <discourseLink>ჩვენს Discourse-ფორუმებს</discourseLink>.
error-title-502 = კავშირი გაწყდა
error-content-502 = ვერ ხერხდება მდგრადი კავშირის დამყარება ჩვენს სერვერებთან ამჟამად, სცადეთ მოგვიანებით. დახმარების მისაღებად შემოუერთდით <matrixLink>Matrix-ის ერთობის სასაუბროს</matrixLink>, საიტის ხარვეზების შესახებ იხილეთ <githubLink>GitHub</githubLink> ან ეწვიეთ <discourseLink>ჩვენს Discourse-ფორუმებს</discourseLink>.
error-title-503 = საიტის მუშაობა შეწყვეტილია დაუგეგმავად
error-content-503 = საიტი აღდგება უმოკლეს დროში. უახლესი ინფორმაციისთვის, გთხოვთ შემოუერთდეთ <matrixLink>Matrix-ის ერთობის სასაუბროს</matrixLink> ან იხილოთ <githubLink>GitHub</githubLink>, ხარვეზების მოსახსენებლად კი ეწვიეთ <discourseLink>ჩვენს Discourse-ფორუმებს</discourseLink>.
error-title-504 = მოთხოვნის დრო ამოიწურა
error-content-504 = მოთხოვნის შესრულებამ დიდ ხანს გასტანა. ეს, ჩვეულებრივ, დროებითია ხოლმე. კვლავ სცადეთ მოგვიანებით. დახმარების მისაღებად შემოუერთდით <matrixLink>Matrix-ის ერთობის სასაუბროს</matrixLink>, საიტის ხარვეზების შესახებ იხილეთ <githubLink>GitHub</githubLink> ან ეწვიეთ <discourseLink>ჩვენს Discourse-ფორუმებს</discourseLink>.
error-code = შეცდომა { $code }
# Warning message shown when none of the clips could be uploaded
error-duplicate-clips-all =
    { $total ->
        [one] თქვენი ჩანაწერის ატვირთვა ვერ მოხერხდა. უკვე აიტვირთა წინა ჯერზე. განაგრძეთ მომდევნო ჩანაწერების ერთიანად ატვირთვით!
       *[other] { $total } ჩანაწერის ატვირთვა ვერ მოხერხდა. უკვე აიტვირთა წინა ჯერზე. განაგრძეთ მომდევნო ჩანაწერების ერთიანად ატვირთვით!
    }
# Warning message shown when only some of the clips could be uploaded (uploaded count will be <5)
error-duplicate-clips-some = { $uploaded } ჩანაწერი ატვირთულია — უკვე აიტვირთა დარჩენილი ნაწილი. განაგრძეთ მომდევნო ჩანაწერების ერთიანად ატვირთვით!
