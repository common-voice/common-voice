# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = აფარული
aat = არვანიტიკა
ab = აფხაზური
abb = ბანკონი
abq = აბაზა
ace = აჩეჰი
ady = ადიღეური
af = აფრიკაანსი
ajg = აჯა
aln = გეგის ალბანური
am = ამჰარული
an = არაგონული
ar = არაბული
arn = მაპუდუნგუნი
as = ასამური
ast = ასტურიული
az = აზერბაიჯანული
azz = მაღალმთიანი პუებლა-ნაჰუატლი
ba = ბაშკირული
bag = ტუკი
bal = ბელუჯი
bas = ბასა
bax = ბამუნი
bba = ბაატონუმი
bbj = ღომალა
bbl = თუშური
bce = მენგამბო
bci = ბაულე
be = ბელარუსული
beb = ბამველე
bew = ბეტავი
bfd = ბაფუტი
bft = ბალტი
bg = ბულგარული
bgp = აღმოსავლური ბელუჯი
bkh = ბაკოკო
bkm = კომი
bm = ბამბარა
bn = ბენგალური
bnm = ბატანგა
bnn = ბუნუნი
bo = ტიბეტური
br = ბრეტონური
brh = ბრაჰუი
bri = მოკპვე
brx = ბოდო
bs = ბოსნიური
bsh = კატევირი
bsk = ბრუშასკი
bsy = საბაჰ-ბისაია
btv = ბატერი
bum = ბულუ
bxk = ბუკუსუ
bxr = ბურიატული
byv = მედუმბა
ca = კატალანური
cak = კაკჩიკელი
cdo = აღმოსავლური მინი
cgg = ჩიგა
cjk = ჩოკვე
ckb = ცენტრალურქურთული
cnh = ჰაკჰა ჩინი
co = კორსიკული
cpx = ჰენგ-ჰუა
cpy = აშენინკა სამხრეთ უკაიალი
crh = ყირიმული თათრული
cs = ჩეხური
cut = ტეუტილა კუიკატეკი
cux = ტეპეუხილა კუიკატეკი
cv = ჩუვაშური
cy = უელსური
da = დანიური
dag = დაგბანი
dar = დარგინი
dav = კიდავიდა
de = გერმანული
din = დინკა
dmk = დავუდი
dml = დამელი
dru = რუკაი
dsb = ქვემოსორბული
dua = დუალა
dv = დივეჰი
dyu = დიულა
ebr = ებრი
ee = ევე
eko = ეკოტი
el = ბერძნული
en = ინგლისური
eo = ესპერანტო
es = ესპანური
esu = ცენტრალურ ალასკური იუპიკი
et = ესტონური
eto = იტონი
eu = ბასკური
ewo = ევონდო
fa = სპარსული
fan = ფანგი
ff = ფულანი
fi = ფინური
fmp = ფეეფეე
fo = ფარერული
fr = ფრანგული
fub = ადამავა ფულფულდე
fue = ბორგუ ფულფულდე
fuf = გვინეის პულარი
fy-NL = ფრიზული
ga-IE = ირლანდიური
gaa = გა
gej = მინა
ggg = გურგულა
gid = გუიდარი
gig = გოარია
giz = გუიზიგა
gjk = კაჩჰი
gju = გუჯარი
gl = გალიციური
glk = გილაკი
gn = გუარანი
gom = გოას კონკანი
gos = გრონინგური
gsw = ელზასური
gu-IN = გუჯარათული
guc = ვაიუნაიკი
gv = მენური
gwc = გავრი
gwt = გავაირბაიტი
gya = ჩრდილო-დასავლეთი გბაია
ha = ჰაუსა
hac = გორანი
hak = ჰაკა
haz = ჰაზარგი
hch = ვირარიკა
he = ებრაული
hem = კიჰემბა
hi = ჰინდი
hil = ჰილიგაინონი
hno = ჩრდილო ჰინდკო
hr = ხორვატული
hrx = ჰუნსრიკი
hsb = ზემოსორბული
ht = ჰაიტური
hu = უნგრული
hux = ნიუპოდე უიტოტო
hy-AM = სომხური
ia = ინტერლინგვა
iba = იბანი
ibb = იბიბიო
id = ინდონეზიური
ie = ინტერლინგვა
ig = იგბო
ipk = ინუპიაკი
is = ისლანდიური
it = იტალიური
izh = იჟორული
ja = იაპონური
jam = იამაიკური პატუა
jbo = ლოჟბანი
jgo = ნგომბა
jqr = ხაქარუ
jv = იავური
ka = ქართული
kaa = ყარაყალაკი
kab = კაბილური
kam = კიკამბა
kbd = ყაბარდოული
kcn = კინუბი
kdh = კოტოკოლი
khw = კჰოვარი
ki = კიკუიუ
kk = ყაზახური
kln = კალენჯინი
kls = კალაშა
km = ქმერული
kmr = კურმანჯი
kn = კანადა
knn = კონკანი (დევანაგარი)
ko = კორეული
koo = კონზო
kpv = კომი
krc = ყარაჩაულ-ბალყარული
ks = ქაშმირი
ksf = ბაფია
kvx = პარკარი-კოლი
kw = კორნული
kxp = ვადიარა-კოლი
ky = ყირგიზული
kzi = კელაბიტი
lb = ლუქსემბურგული
led = ლენდუ
leu = კარა
lg = ლუგანდა
lij = ლიგურული
lke = კენი
lld = ლადინი
ln = ლინგალა
lo = ლაო
lrk = ლოარკი
lrl = აჩომი
lss = ლასი
lt = ლიტვური
ltg = ლატგალური
lth = თჰური
lua = ჩილუბა
luo = დოლუო
lv = ლატვიური
lzz = ლაზური
mai = მაიტჰილი
mau = ჰუაუტლა მაზატეკი
mbf = ბაბა-მალაური
mbo = მბო
mcf = მაცესი
mcn = მასანა
mcx = მპიემო
mdd = მბუმი
mdf = მოკშა
meh = მისტეკო იუკუჰიტი
mel = მელანაუ
mfe = კრეოლური მორისიენი
mg = მალაგასიური
mgg = კუნაბემბე
mhk = მუნგაკა
mhr = აღმოსავლეთ დაბლობის მარიული
mk = მაკედონური
mki = დატკი
ml = მალაიალამური
mmc = მიჩოაკან-მაზაჰუა
mn = მონღოლური
mni = მეიტეილონი
mos = მოსი
mqh = ტლაზოიალტეპეკი მისტეკი
mr = მარათული
mrh = მარა
mrj = მთის მარიული
ms = მალაიური
mse = მუსეი
msi = საბაჰ-მალაური
mt = მალტური
mua = მუნდანგი
mug = მუსგუმი
mve = პაკისტანური მარვარი
mvy = კოჰისტანი
mxu = მადა
my = ბირმული
myv = ერზიანული
nan-tw = ტაივანური (მინანი)
nb-NO = ნორვეგიული ბუკმოლი
ncx = ცენტრალურ პუებლას ნაუატლი
nd = სამხრეთული ნდებელე
ne-NP = ნეპალური
new = ნევარი
nhe = აღმოსავლური ჰუასტეკა-ნაჰუატლი
nhi = დასავლეთ სიერა-პუებლას ნაჰუატლი
nia = ნიასური
nl = ნიდერლანდური
nla = ნგომბალე
nlv = ორისაბული ნაუატლი
nmg = კვასიო
nmz = ლოსო
nn-NO = ნორვეგიული ნიუნორსკი
nnh = ნგიმბონი
nqo = ნკო
nr = ჩრდილოური ნდებელე
nso = ჩრდილოური სოთო
ny = ჩინიანჯა
nyn = ნიანკოლე
nyu = ნიუნგვე
oc = ოქსიტანური
odk = ოდკი
om = ორომო
or = ორია
oru = ორმური
os = ოსური
pa-IN = პენჯაბი
pap = პაპიამენტო
pap-AW = პაპიამენტო (არუბა)
pau = პალაუანური
pcd = პიკარდული
pcm = ნიგერიული პიჯინური ინგლისური
pez = აღმოსავლური პენანი
phl = პჰალულა
phr = პაჰარი-პოთვარი
pl = პოლონური
plk = კოჰისტანი შინა
pne = დასავლეთის პენანი
prq = აშენინკა პერენე
ps = პუშტუ
pt = პორტუგალიური
pua = დასავლეთის მაღალმთიანი პურეპეჩა
pwn = პაივანური
quc = კიჩე
qug = მთის კეჩუა-ჩიმბორასო
qup = სამხრეთული პასტაზა კეჩუა
qur = კეჩუა-იანაუანკა
qus = სანტიაგო-დელ-ესტეროს კეჩუა
qux = კეჩუა იაუიოსი
quy = კეჩუა ჩანკა
qva = კეჩუა ამბო-პასკო
qvi = კეჩუა
qvj = მთიანი ლოხას კიჩვა
qvl = კეჩუა კახატამბო
qwa = კეჩუა კორონგო ანკაში
qws = კეჩუა სიჰუას ანკაში
qxa = კეჩუა ჩიკუიანი
qxp = პუნო-კეჩუა
qxq = ქაშქაი
qxt = კეჩუა პასკო სანტა ანა დე ტუსი
qxu = კეჩუა არექიპა-ლა უნიონი
qxw = კეჩუა შაუშა ვანკა
rif = ტარიფიტი
rm-sursilv = რეტორომანული სურსილვანი
rm-vallader = რეტორომანული ვალადერი
rn = კირუნდი
ro = რუმინული
rof = კირომბო
ru = რუსული
ruc = რუული
rup = არუმინული
rw = კინიარუანდა
rwm = ამბა
sah = სახა
sat = სანტალი (ოლ-ჩიკი)
sbn = სინდი-ბილი
sc = სარდინიული
scl = შინა
scn = სიცილიური
sco = შოტლანდიური
sd = სინდჰი
sdh = სამხრეთქურთული
sdo = სერიანული ბიდაუ
seh = სენა
sei = სერი
shi = შილჰა
shn = შანური
si = სინჰალური
sk = სლოვაკური
skr = სარაიკი
sl = სლოვენური
sn = შონა
snk = სონინკე
snv = საბანი
so = სომალური
sq = ალბანური
sr = სერბული
ss = სისვატი
ssi = სანსი
st = სამხრეთული სოთო
su = სუნდანური
sv-SE = შვედური
sva = სვანური
sw = სუაჰილი
syr = სირიული
szy = საკიზაია
ta = ტამილური
tar = ცენტრალური ტარაუმარა
tay = ატაიალური
te = ტელუგუ
teg = ტეკე
tg = ტაჯიკური
th = ტაილანდური
ti = ტიგრინია
tig = ტიგრინია
tk = თურქმენული
tl = ტაგალური
tli = ტლინგიტი
tn = სეტსვანა
tob = ტობა-კომი
tok = ტოკიპონა
top = პაპანტლას ტოტონაკური
tr = თურქული
trv = სედიკი
trw = ტორვალი
ts = ცონგა
tt = თათრული
ttj = რუტორო
tui = ტუპური
tvu = ტუნენი
tw = ჩვი
ty = ტაიტური
tyv = ტუვა
uby = უბიხური
udl = ულდემე
udm = უდმურტული
ug = უიღურული
uk = უკრაინული
ukv = კუკუ
ur = ურდუ
ush = უშოჯი
uz = უზბეკური
var = ჰარიხიო
ve = შივენდა
vec = ვენეციური
vi = ვიეტნამური
vmw = მახუვა
vot = ვოტიკური
wbl = ვახი
wep = ვესტფალური
wes = კამერუნული პიჯინი
wo = ვოლოფი
xdq = კაიტაგი
xh = კოსა
xhe = კეტრანი
xka = ქალკოტი
xkl = კენიური
xmf = მეგრული
xsm = კასემი
yaq = იაკი
yav = ნუასუე
ydg = იადღა
yi = იდიში
yo = იორუბა
yue = კანტონური
zgh = ტამაზიგხტი
zh-CN = ჩინური (ჩინეთი)
zh-HK = ჩინური (ჰონგ-კონგი)
zh-TW = ჩინური (ტაივანი)
zoc = კოპაინალა სოკე
zu = ზულუ
zza = ზაზა

# [/]


## Layout

profile = პროფილი
help = დახმარება
contact = დაკავშირება
privacy = პირადულობა
terms = პირობები
cookies = ფუნთუშები
faq = ხ.დ.კ.
documentation = დოკუმენტაცია
content-license-text = მასალა ხელმისაწვდომია <licenseLink>Creative Commons ლიცენზიით</licenseLink>
share-title = დაგვეხმარეთ მოხალისეთა მოძიებაში, რომლებიც შემოგვწირავენ საკუთარ ხმებს!
share-text = დაგვეხმარეთ, რომ შევასწავლოთ მანქანებს, თუ როგორ საუბრობენ ადამიანები, მოგვაწოდეთ თქვენი ხმის ჩანაწერები შემოწირულობის სახით { $link }
link-copied = ბმულის ასლი აღებულია
back-top = დასაწყისში დაბრუნება
logout = გასვლა
donate = შეგვეწიეთ
stayintouch = ჩვენ, Mozilla-ში ვცდილობთ შევკრიბოთ დიდი ერთობა ხმოვანი ტექნოლოგიების გარშემო. გვსურს დავრჩეთ კავშირზე განახლებებით, მონაცემთა ახალი წყაროებით და მოვისმინოთ, თუ როგორ გადგებათ ეს მონაცემები.
privacy-info = ჩვენ გპირდებით, რომ თქვენს პირად ინფორმაციას სიფრთხილით მოვეკიდებით. ვრცლად იხილეთ ჩვენი <privacyLink>პირადი მონაცემების დაცვის განაცხადი</privacyLink>.
return-to-cv = დაუბრუნდით – Common Voice
email-input =
    .label = ელფოსტა
loading = იტვირთება...
email-opt-in-info = მსურს, მივიღო ელფოსტაზე შეხსენებები შესასრულებელი მიზნების, აგრეთვე სამუშაოს მიმდინარეობის შესახებ და მეცნობოს, რა სიახლეებითაა Common Voice.
email-opt-in-info-title = შემოუერთდით ელფოსტის ჯგუფს – Common Voice
email-opt-in-info-sub-with-challenge = მიიღეთ ელფოსტაზე შეხსენებები გამოწვევების, შესასრულებელი მიზნების, აგრეთვე სამუშაოს მიმდინარეობის შესახებ და ცნობები, თუ რა სიახლეებითაა Common Voice.
email-opt-in-privacy-v2 = ელფოსტით წერილების მიღებაზე თანხმობით აცხადებთ, რომ არ ხართ წინააღმდეგი, თუ Mozilla ამ მონაცემებს დაამუშავებს <privacyLink>პირადულობის დაცვის დებულების</privacyLink> შესაბამისად.
indicates-required = * აღნიშნავს აუცილებელ ველს
not-available-abbreviation = მიუწვდომელია
# Text shown on a button that can be clicked to copy text to the clipboard
copy = ასლი
# Text shown on a button that can be clicked to cancel an action
cancel = გაუქმება
# Text shown on a button that can be clicked to delete an item
delete = წაშლა

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>ჩვენ გვესაჭიროება</mark><br/><mark>თქვენი დახმარება!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = ყოველწლიურად მილიონ დოლარს აღწევს მონაცემთა კრებულების განთავსებისა და მომსახურების გასაუმჯობესებლად გაწეული ხარჯები 100-ზე მეტი ენისგან შემდგარი ერთობისთვის, რომელიც სრულადაა ჩვენზე დამოკიდებული. <strong>თუ თქვენთვის ღირებულია მონაცემთა გახსნილობა და მრავალფეროვნება – გაიღეთ შემოწირულობა ახლავე!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>CV-ს იყენებთ</mark><br/><mark>თქვენი კვლევისთვის?</mark>
languages-donate-banner-cta-explanation = Common Voice ფინანსდება შემოწირულობებითა და საქველმოქმედო სახსრებით! ჩვენთვის განსაკუთრებით სასიხარულოა სამეცნიერო, სამოქალაქო საზოგადოებებსა და დარგის მკვლევრებთან თანამშრომლობა. Common Voice უფასოა, მაგრამ მომსახურების გაუმჯობესებისა და მონაცემთა განთავსების ხარჯების დაფარვაში შემოშველება მეტად წაგვადგება.
localization-select =
    .label = აირჩიეთ ენა/არე

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = <strong>Common Voice ახალი კრებულების</strong> ჩამოსატვირთად ამიერიდან უკვე გთავაზობთ საგანგებოდ შექმნილ დობილ პლატფორმას, სახელწოდებით Mozilla Data Collective. შემოუერთდით წვდომისთვის 300+ საერთაშორისო კრებულთან, შექმნილი საზოგადოების მიერ საზოგადოებისთვის.
# Button text for MDC announcement
announcement-mdc-button-text = გვეწვიეთ – Mozilla Data Collective
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = გახსნის ახალ ჩანართში
