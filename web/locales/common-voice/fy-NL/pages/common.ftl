# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Afar
aat = Arvanitysk
ab = Abgazysk
abb = Bankon
abq = Abaza
ace = Atjeehsk
ady = Adygeesk
af = Afrikaansk
aii = Sureth
ajg = Adja
aln = Gegysk
am = Amhaarsk
an = Aragoneesk
ar = Arabysk
arn = Mapudungun
as = Assameesk
ast = Asturysk
az = Azerbeidzjaansk
azz = Heechlân-puebla-nahuatl
ba = Bashkyrsk
bag = Tuki
bal = Balochi
bas = Basaa
bax = Bamun
bba = Baatonum
bbj = Ghomala
bbl = Batsysk
bce = Mengambo
bci = Baoule
be = Wit-Russysk
beb = Bamvele
bew = Betawi
bfd = Bafut
bft = Balti
bg = Bulgaarsk
bgp = East-Beloetsji
bin = Edo
bkh = Bakoko
bkm = Kom
bm = Bambara
bn = Bingaalsk
bnm = Batanga
bnn = Bunun
bo = Tibetaansk
br = Bretonsk
brh = Brahui
bri = Mokpwe
brx = Bodo
bs = Bosnysk
bsh = Katë
bsk = Brushaski
bsy = Sabah Bisaya
btv = Bateri
bum = Bulu
bxk = Bukusu
bxr = Burjatysk
byv = Medumba
ca = Katalaansk
cak = Kaqchikel
cdo = Easterske Min
ce = Tsjetsjeensk
cgg = Chiga
cjk = Chokwe
ckb = Sintraal-Koerdysk
cnh = Hakha Chin
co = Korsikaansk
cpx = Heng Hua
cpy = Asheninka Súdlik Ucayali
crh = Krimean Tatar
cs = Tsjechysk
cut = Teutila-Cuicatec
cux = Tepeuxila-Cuicatec
cv = Tsjoevasjysk
cy = Welsk
da = Deensk
dag = Dagbani
dar = Dargwa
dav = Kidaw'ida
de = Dútsk
din = Dinka
dmk = Dawoodi
dml = Dameli
dru = Rukai
dsb = Leger Sorbysk
dua = Duala
dv = Divehi
dyu = Dioula
ebr = Ebrie
ee = Ewe
efk = Efik
eko = Ekoti
el = Gryksk
en = Ingelsk
eo = Esperanto
es = Spaansk
esu = Sintraal-Alaska Yup’ik
et = Estysk
eto = Eton
eu = Baskysk
ewo = Ewondo
fa = Perzysk
fan = Fang
ff = Fula
fi = Finsk
fmp = Feʼefeʼe
fo = Fariseesk
fr = Frânsk
fub = Adamawa Fulfulde
fue = Borgu Fulfulde
fuf = Guinee-Pular
fy-NL = Frysk
ga-IE = Iersk
gaa = Ga
gcf = Martinikaansk Kreoalsk
gej = Mina
ggg = Gurgula
gid = Guidar
gig = Goaria
giz = Guiziga
gjk = Kachhi
gju = Gojiri
gl = Galisysk
glk = Gilaki
gn = Guaraní
gom = Goaneesk Konkani
gos = Grinzers
gsw = Elzassysk
gu-IN = Gujarati
guc = Wayuunaiki
gv = Manks-Gaelysk
gwc = Gawri
gwt = Gawar-Bati
gya = Noardwest-GBaya
ha = Hausa
hac = Našinski
hak = Hakka
haz = Hazargi
hch = Huichol
he = Hebriuwsk
hem = Kihemba
hi = Hindy
hil = Hiligaynon
hno = Noard-Hindko
hr = Kroätysk
hrx = Hunsrik
hsb = Boppe-Sorbysk
ht = Haïtiaansk
hu = Hongaarsk
hux = Nüpode Huitoto
hy-AM = Armeensk
ia = Ynterlingua
iba = Iban
ibb = Ibibio
id = Yndonezysk
ie = Ynterlingua
ig = Igbo
ipk = Inupiak
is = Iislânsk
it = Italiaansk
izh = Izjorysk
ja = Japansk
jam = Jamaikaansk Patois
jbo = Lojban
jgo = Ngomba
jqr = Jaqaru
jv = Javaansk
ka = Georgysk
kaa = Karakalpaksk
kab = Kabylysk
kam = Kikamba
kbd = Kabardysk
kcn = Nubi
kdh = Kotokoli
khw = Khowar
ki = Kikuyu
kjh = Khakasysk
kk = Kazaksk
kln = Kalenjin
kls = Kalasha
km = Khmer
kmr = Kurmanci
kn = Kannada
knn = Konkani (Devanagari)
ko = Koreaansk
koo = Konzo
kpv = Kom-Zyrian
krc = Karachay-Balkar
ks = Kashmiri
ksf = Bafia
kvx = Parkari Koli
kw = Kornish
kxp = Wadiyara Koli
ky = Kirgizysk
kzi = Kelabit
laj = Lango
lb = Lúksemburgsk
led = Lendu
leu = Kara
lg = Luganda
lij = Ligurysk
lke = Soga
lld = Ladin
ln = Lingala
lo = Laotiaansk
lrk = Loarki
lrl = Achomi
lss = Lasi
lt = Litousk
ltg = Letgaalsk
lth = Thur
lua = Tshiluba
luo = Dholuo
lv = Letsk
lzz = Laz
mai = Maithili
mau = Huautla-Mazateeksk
mbf = Baba-Maleisk
mbo = Mbo
mcf = Matses
mcn = Massa
mcx = Mpiemo
mdd = Mbum
mdf = Moksja
meh = Miksteco-Yucuhiti
mel = Melanau
mfe = Kreoalsk Morisien
mg = Malagassysk
mgg = Kunabembe
mhk = Mungaka
mhr = Greide-Mari
mk = Masedoanysk
mki = Dhatki
ml = Malayalam
mmc = Michoacán Mazahua
mn = Mongoalsk
mni = Meetei Lono
mnw = Mon
mos = More
mqh = Tlazoyaltepec-Miksteeksk
mr = Marathy
mrh = Mara
mrj = Berch-Mari
ms = Maleizysk
mse = Moussey
msi = Sabah-Maleisk
mt = Malteesk
mua = Mundang
mug = Musgum
mve = Pakistaansk Marwari
mvy = Indus Kohistani
mxu = Mada
my = Burmeesk
myv = Erzja
nan-tw = Taiwaneesk (Minnan)
nb-NO = Noarsk Bokmål
ncx = Sintraal-Puebla-Nahuatl
nd = IsiNdebele (Noardlik)
ne-NP = Nepaleesk
new = Newari
nhe = Eastlik Huasteca Nahuatl
nhi = Westersk Sierra Puebla Nahuatl
nia = Nias
nl = Nederlânsk
nla = Ngombale
nlv = Orizaba Nahuatl
nmg = Kwasio
nmz = Losso
nn-NO = Norwegysk Nynorsk
nnh = Ngiembon
nqo = NKo
nr = IsiNdebele (Súdlik)
nso = Noard-Sotho
ny = Chinyanja
nyn = Nyankole
nyu = Nyungwe
oc = Okkitaansk
odk = Oadki
om = Afaan Oromo
or = Odia
oru = Ormuri
os = Ossetysk
pa-IN = Punjabi
pap = Papiamintsk
pap-AW = Papiamintsk (Aruba)
pau = Palauaansk
pcd = Picard
pcm = Nigeriaansk pidgin-Ingelsk
pez = Eastersk Penan
phl = Palula
phr = Pahari-Pothwari
pl = Poalsk
plk = Kohistani Shina
pne = Westlik Penan
prq = Asheninka Perene
ps = Pashtu
pt = Portugeesk
pua = Purepecha fan de Westlike Heechlannen
pwn = Paiwaansk
quc = K'iche'
qug = Quichua Chimborazo-Heechlân
qup = Súd-Pastaza Quechua
qur = Quechua Yanahuanca
qus = Quechua Santiago del Estero
qux = Quechua-Yauyos
quy = Quechua Chanka
qva = Quechua Ambo-Pasco
qvi = Kichwa
qvj = Loja Heechlân-Kichwa
qvl = Quechua Cajatambo
qwa = Quechua Corongo Ancash
qws = Quechua Sihuas Ancash
qxa = Quechua Chiquián
qxp = Puno Quechua
qxq = Qashqai
qxt = Quechua Pasco Santa Ana de Tusi
qxu = Quechua Arequipa-La Unión
qxw = Quechua Jauja Wanka
rif = Tarifit
rki = Rakhine
rm-sursilv = Reto-Romaansk Surselvysk
rm-vallader = Reto-Romaansk Fallader
rn = Kirundi
ro = Roemeensk
rof = Kirombo
ru = Russysk
ruc = Ruuli
rup = Aromanysk
rw = Kinyarwanda
rwm = Amba
sah = Sakha
sat = Santali (Ol Chiki)
sba = Ngambay
sbn = Sindhi Bhili
sc = Sardyn
scl = Shina
scn = Sisiliaansk
sco = Skotsk
sd = Sindhi
sdh = Súd-Koerdysk
sdo = Serian Bidayuh
seh = Sena
sei = Seri
sg = Sango
shi = Tashelhiyt (Shilha)
shn = Shan
si = Singaleesk
sk = Slowaaksk
skr = Saraiki
sl = Sloveensk
sn = Shona
snk = Sonynke
snv = Saʼban
so = Somalysk
sq = Albaneesk
sr = Servysk
ss = Siswaty
ssi = Sansi
st = Súd-Sotho
su = Soedaneesk
sv-SE = Sweedsk
sva = Svanetysk
sw = Swahili
swb = Shimaore
syr = Syrysk
szy = Sakizaya
ta = Tamilsk
tar = Sintraal-Tarahumara
tay = Atayal
te = Telugu
teg = Teke
tg = Tadzjyksk
th = Taisk
ti = Tigriniaansk
tig = Tigre
tiv = Tiv
tk = Turkmeensk
tl = Tagalog
tli = Tlingit
tn = Tswana
tob = Toba Qom
tok = Toki Pona
top = Papantla Totonac
tr = Turksk
trv = Taroko
trw = Torwali
ts = Tsonga
tt = Tatarsk
ttj = Rutoro
tui = Tupuri
tum = Tumbuka
tuq = Tedaga
tvu = Tunen
tw = Twi
ty = Tahitiaansk
tyv = Toevaansk
uby = Oebyksk
udl = Ouldeemee
udm = Oedmoertsk
ug = Oeigoersk
uk = Oekraynsk
ukv = Kuku
ur = Urdu
ush = Ushoji
usp = Uspanteeksk
uz = Oezbeeksk
var = Guarijio
ve = Fenda
vec = Fenetiaansk
vi = Fjetnameesk
vmw = Emakhuwa
vot = Wotysk
wbl = Wachi
wep = Westfaalsk
wes = Kameroen Pidgin
wo = Wolof
xdq = Kaitag
xh = Xhosa
xhe = Khetrani
xka = Kalkoti
xkl = Keniaansk
xmf = Mingreelsk
xsm = Kasem
yaq = Yaqui
yav = Nuasue
ydg = Yadgha
yi = Jiddysk
yo = Yoruba
yue = Kantoneesk
zgh = Tamazight
zh-CN = Sineesk (Sina)
zh-HK = Sineesk (Hong Kong)
zh-TW = Sineesk (Taiwan)
zoc = Copainalá Zoque
zu = Zoeloe
zza = Zazaki

# [/]


## Layout

profile = Profyl
help = Help
contact = Kontakt
privacy = Privacy
terms = Betingsten
cookies = Cookies
faq = FAQ
documentation = Dokumintaasje
content-license-text = Ynhâld beskikber ûnder in <licenseLink>Creative Commons-lisinsje</licenseLink>
share-title = Help ús oaren te finen dy’t harren stim donearje wolle!
share-text = Help mei masinen te learen hoe’t echte minsken prate, donearje jo stim op { $link }
link-copied = Keppeling kopiearre
back-top = Nei boppe
logout = Ofmelde
donate = Donearje
stayintouch = By Mozilla meitsje wy in mienskip rûnom spraaktechnology. Wy wolle graach yn kontakt bliuwe mei fernijingen, nije gegevensboarnen en om mear te hearren oer hoe’t jo de gegevens brûke.
privacy-info = Wy sizze jo ta dat wy jo ynformaasje mei omtinken behannelje. Mear ynfo dêr oer yn ús <privacyLink>Privacybelied</privacyLink>.
return-to-cv = Werom nei Common Voice
email-input =
    .label = E-mailadres
loading = Lade…
email-opt-in-info = Ik wol graach e-mailberjochten lykas oantinkens oan doelstellingen, myn foartgong en nijsbrieven oer Common Voice ûntfange.
email-opt-in-info-title = Meld jo oan foar de Common Voice-mailinglist
email-opt-in-info-sub-with-challenge = Untfang e-mailberjochten lykas oantinkens oan útdagingen en doelstellingen, foartgong en nijsbrieven oer Common Voice.
email-opt-in-privacy-v2 = Troch jo yn te skriuwen foar it ûntfangen fan e-mailberjochten, ferklearje jo dat jo akkoard gean mei it ferwurkjen fan dizze ynformaasje troch Mozilla yn oerienstimming mei Mozilla’s <privacyLink>Privacybelied</privacyLink>.
indicates-required = * jout fereaske fjild oan
not-available-abbreviation = NB
# Text shown on a button that can be clicked to copy text to the clipboard
copy = Kopiearje
# Text shown on a button that can be clicked to close a modal or panel
close = Slute
# Text shown on a button that can be clicked to cancel an action
cancel = Annulearje
# Text shown on a button that can be clicked to delete an item
delete = Fuortsmite

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>Wy hawwe</mark><br/><mark>jo help nedich!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = It kostet hast in miljoen dollar yn it jier om de gegevenssets te hosten en it platfoarm te ferbetterjen foar de mear as 100 taalmienskippen dy’t fertrouwe op wat wy dogge. <strong>As jo iepen, ynklusive gegevens wurdearje – donearje dan hjoed!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>Brûke jo CV foar</mark><br/><mark>jo ûndersyk?</mark>
languages-donate-banner-cta-explanation = Common Voice wurdt finansiere troch donaasjes en subsydzjes! Wy hâlde fan gearwurkjen mei akademisy, boargerlike maatskippij en yndustryûndersikers. Common Voice is fergees te brûken, mar bydrage oan platfoarm- en hostingkosten fia subsydzjefoarstellen helpt echt.
localization-select =
    .label = Kies jo taal/lokalisaasje

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = <strong>Nije Common Voice-datasets</strong> binne no eksklusyf te downloaden fia ús susterplatfoarm, Mozilla Data Collective. Doch mei foar tagong ta mear as 300 wrâldwide gegevenssets, boud troch en foar de mienskip.
# Button text for MDC announcement
announcement-mdc-button-text = Doch mei oan Mozilla Data Collective
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = Iepenet yn in nij ljepblêd

## Release Delay Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-delay = Beste mienskip, wy wurkje op dit stuit oan de útjeft. Wy ferwachtsje dat it yn de kommende dagen yn dielen útrôle wurdt op <mdcLink>Mozilla Data Collective</mdcLink>.

## Release Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release = Beste mienskip, de nije útjefte is no folslein beskikber op <mdcLink>Mozilla Data Collective</mdcLink>. Doch mei foar tagong ta mear as 500 wrâldwide gegevenssets, boud troch en foar de mienskip.

## Pre-Release Announcement (dates TBD)

# Text wrapped in <cvDatasheetsLink></cvDatasheetsLink> will be rendered as an external link to the cv-datasheets GitHub repo
announcement-pre-release = Beste mienskip, de tiid fan útjefte komt tichteby. Gean asjebleaft nei falidearjen en transkribearjen, en besykje ek de <cvDatasheetsLink>cv-datasheets</cvDatasheetsLink> om jo datasheet by te wurkjen.

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = Typ hjir om te sykjen
searchbox-default-placeholder = Sykje…
searchbox-default-aria-label = Sykje
dropdown-no-results = Gjin oerienkomst fûn
# Common Voice Dataset=Language search box
dataset-searchbox-label = Wiziget sawol de werjefte- as de gegevenssettaal
dataset-searchbox-placeholder = Sykje nei in taal…
dataset-searchbox-aria-label = Sykje nei in taal
dataset-search-no-results = Gjin oerienkommende taal fûn.
request-new-language = Nije taal oanfreegje
