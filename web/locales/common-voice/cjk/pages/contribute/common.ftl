action-click = Shindakenya ha
action-tap = Kupwola ha
contribute = Kuchingilako
review = Kuhengulula
skip = Kupalika
shortcuts = Kapatu
clips-with-count-pluralized =
    { $count ->
        [one] Tshibindji
       *[other] Ipindji
    }
goal-help-recording = Chihuthu Cha yenwe, Common voice ubalikilo wa kuhandjeka heta <goalPercentage></goalPercentage> kuchikuma cha shimbu lia{ $goalValue } kwandjikisa !
goal-help-validation = Mumu wa yenu, Common voice kaheta <goalPercentage></goalPercentage>kuchikuma cha shimbu lia { $goalValue } Kukwata !
contribute-more = Unataiza kulinga { $count } nawa ?
speak-empty-state = Tushi nyi uhandjeko uko wa kusoneka mu ma limi…
no-sentences-for-variants = Limi lia kuswipa malimi muchibwa nyi iphindji ya isoneko ! Nyi munubwa kanawa munuswipa ubalikilo wa kusolola ihandjeka ikwo ya limi.
speak-empty-state-cta = Nehenu ishinginyeka ikhwo ya isoneko
speak-loading-error = Twahashile kubwa nyi handjeka ikwo. Eseka shimbu likwo.
record-button-label = Kwata liji lie
share-title-new = <bold>Tukwaseko </bold> kuwana liji lia ha
keep-track-profile = Alula kya kulutwe kulinga unyingiko wa jila
login-to-get-started = Liadjikise mumu wa wakuputuka
target-segment-first-card = Nwa tukwasa kuchama tshi tangu tshize tunafupa
target-segment-generic-card = Nwa kwasana ku segment cible
target-segment-first-banner = Kwasenu kuputuka ize muikwasa
target-segment-add-voice = Hadjikenu nyi maliji jenu
target-segment-learn-more = Kunyingika tshindji
change-preferences = Chendjenu ize nuna nyonga
login-signup = Kuliadjikisa  / Kuliadjikisa Kha
vote-yes = Enga
vote-no = Kha
datasets = Hiheho ya hunyingikiso
languages = Limi
about = Hakutuala
partner = Kulikunda
submit-form-action = Kutuma

## Reporting

report = Kutoweza
report-title = Kutuma
report-ask = Ukalu uka nwa wananga nyi ahu ?
report-offensive-language = Maliji katelele
report-offensive-language-detail = Manyonga amwe kakwetele vumbi chipwe matuka.
report-grammar-or-spelling = Kuhona kwadjika  / chize chinatela
report-grammar-or-spelling-detail = Manyonga amwe kasonewene kulita chize chatamba kusoneka.
report-different-language = Limi liko
report-different-language-detail = Manyonga kasonekene mulimi linalisa  nyi lisena kuhandjika.
report-difficult-pronounce = Yikalukuhandjika
report-difficult-pronounce-detail = Yuma yiko inakaliwa kuitanga chipwe kuitongola
report-offensive-speech = Maliji akulemeka
report-offensive-speech-detail = Yuma ize tuakwija ili yakuhona vumbi hadji ya matuka
report-other-comment =
    .placeholder = Kuwezela
success = Ize watambula
continue = Kuya lume
report-success = Mujimbu kautuma hakeshi kampalia

## Speak & Listen Shortcuts

# Must be one letter that appears in the translated { skip } string.
shortcut-skip = a

## Speak Shortcuts

# Must be one letter that appears in the translated record-string inside of { shortcut-record-toggle-label }.
# Must be different from { shortcut-skip }, { shortcut-vote-yes } and { shortcut-vote-no }
shortcut-record-toggle = e
shortcut-record-toggle-label = Kusoneka/kimika
shortcut-rerecord-toggle = [1-5]
shortcut-rerecord-toggle-label = Lisonekesu nawa
shortcut-discard-ongoing-recording = Kwehuka
shortcut-discard-ongoing-recording-label = Kukatala kundjisa mutshipidji
shortcut-submit = Ndjila tumenu ize nwa kwata tumenu ize nwa soneka
shortcut-submit-label = Tumenu ize nwa kwata tumenu ize nwa soneka
request-language-text = Liji lieye kanda lichipwa ?
request-language-button = Kuchakula limi

## Listen Shortcuts

# Must be one letter that appears in the translated play-string inside of { shortcut-play-toggle-label }.
# Must be different from { shortcut-skip }, { shortcut-vote-yes } and { shortcut-vote-no }
shortcut-play-toggle = p
shortcut-play-toggle-label = Kutanga/Mana
# Must be one letter that appears in the { vote-yes } string.
# Must be different from { shortcut-skip }, { shortcut-vote-no } and { shortcut-play-toggle }
shortcut-vote-yes = o
# Must be one letter that appears in the { vote-no } string.
# Must be different from { shortcut-skip }, { shortcut-vote-yes } and { shortcut-play-toggle }
shortcut-vote-no = n

## Validation criteria

contribution-criteria-nav = Chize chatamba kupwa
contribution-criteria-link = Kunyingika chize chatamba kukunga
contribution-criteria-page-title = Ili tshize chatamba mba nukunge
contribution-criteria-page-description = Evililenu kanawa chimbu nunakwata maliji numbate ize ili ya ulemu !
contribution-for-example = Kwesekesa
contribution-misreadings-title = Hupi wa kutanga
contribution-misreadings-description = Chimbu unevilila talenu nyi kashishi nyi maliji nuakwatanga kanalite nyi hize nuadjikanga  kanda nusako tupalia hakehe. Talenu tupalia haola yeswe :
contribution-misreadings-description-extended-list-1 = Kahona ubunthuki <strong> «Un/Une» </strong> lo <strong>« Le/La »</strong> wa kukwata cha
contribution-misreadings-description-extended-list-2 = Kunahona chimuwikha kusongo<strong>«s»</strong> kusongo lia jina
contribution-misreadings-description-extended-list-3 = Kwecha cha kuliwana cha keshi kubwako kanatshitanga « chochene ».
contribution-misreadings-description-extended-list-4 = Kusongo lis isoneko ikwo ya kuteta, kuyikwata cha fuji fuji.
contribution-misreadings-description-extended-list-5 = Kwabwanga uehnge ulema paa kutanga isoneko ya majina
contribution-misreadings-example-1-title = Kushithu Dinosaures ayikomokeso anene atwama mu Trias
contribution-misreadings-example-2-title = Kushithu Dinosaures wakukomokeso munene atwama mu Trias
contribution-misreadings-example-2-explanation = [Kwatwama « di » dinaosaures]
contribution-misreadings-example-3-title = Kushithu Dinosaures ayikomokeso anene atwama mu Tria-.
contribution-misreadings-example-3-explanation = [Kukwata cha kuteta kuletwe lia songo lia nyima lia isoneko ya majina]
contribution-misreadings-example-4-title = Kushithu Dinosaures ayikomokeso anene atwama mu Trias. Chochene.
contribution-misreadings-example-4-explanation = [kulutwe lia isoneko ize hna kwata]
contribution-misreadings-example-5-title = Chochene, ili ibema lia kulu kuhiana.
contribution-misreadings-example-6-title = Chohene, chili chibema.
contribution-misreadings-example-6-explanation = [Yabwile ibema « Chochene »]
contribution-misreadings-example-7-title = Chochene, myibwa ibema.
contribution-misreadings-example-7-explanation = [Kushi lusona « y » mu isoneko y a ma jina ya mwenemwene]
contribution-misreadings-example-8-title = Pwuka kaya
contribution-misreadings-example-8-explanation = Udji wa
contribution-varying-pronunciations-title = Kutanga cha kulisasa
contribution-varying-pronunciations-description = Lihumikize unda kumbunge kulutwe lia kumbila kwecha Chinyingi cheswe welo wa kutanga isoneko mukubwa ulema ubi wa kuhadjika isoneko ya majina sona Jitwo ishi toba ya chihulo. Kwatwama unene wa muchili kutongola kuhadjika muno muze mu ihanda ikwo ize kanda nuiva mu ndanda. Acha chili alioze lia ngelenu kusa cheseko tshibema cha athu waze ma hadjika kulisasa.
contribution-varying-pronunciations-description-extended = Hachize nyi munu myonga ngo akwakutanga inoneko malunga chibwe mapwo kanda awama majina isoneko mahasa kutanga kanawa thuneta kuecha. Kukhakhajana lingilenu ya kubalika.
contribution-varying-pronunciations-example-1-title = Mashiwa temba mulimbata udji wa mauwo ya iteli.
contribution-varying-pronunciations-example-1-explanation = [Kutongola cha zuwo lia « iteli » ulemu ize nu nuvu mu limi lia ungeji]
contribution-varying-pronunciations-example-2-title = Kulia Sandwich cha Jambo beurre
contribution-varying-pronunciations-example-2-explanation = [« Sandwich » mu limi lia ungeji ka malitongola ngwe sona, Jina hi yaliko]
contribution-background-noise-title = Zungo lia kwihi lia kulu
contribution-background-noise-description = Chili chi bema alongeshi abwe nyi kaliawashi. Chibwe zungo kuli lia thachi mulibwa lia kutaiza welo wa kukinda ko kuiva ulemu wa ya djiko. Mwaso wa ukulungunga kakuntaiza alioze mwaso wathachi kuli yenwe mukinda kunbadjika hita sona jina lize ko.
contribution-background-noise-description-extended = Nyi kukwata muchibwa cha kulihicheka kuchimbila ku usuku wa isoneko
contribution-background-noise-example-1-fixed-title = <strong>[Reniflement]</strong>Thushithu ayikomokeso anene atwama mu <strong>[toux]</strong> Trias.
contribution-background-noise-example-2-fixed-title = Thushithu ayikomokeso anene <strong>[toux] </strong> mu Trias
contribution-background-noise-example-2-explanation = [Chihanda cha isoneko yatambile kwivakana kulihicheka]
contribution-background-noise-example-3-fixed-title = <strong>[craquement]</strong>Dinosaures anene mu<strong>[Craquement]</strong> - rias.
contribution-background-voices-title = Lili lia kunyima lia kulu
contribution-background-voices-description = Kuzeya cha zungo lia hashi lia khulu kakulitaiza alioze chatambile kubwa nyi liji lia kuwezela lize mulinehe kubemesa chitangililo cha  kusolola sona ja majina jize ja bwileko mu isoneko. Nyi munuhasa kuiva isoneko ya kulisasa ya kwadjika cha sona ja isoneko kuikwata muibwa ya kumbila. Acho youma idji muze chilauli cha katalilo kachi hichika cha kukwachisa shimbu kuhadjeka a chi hela cha kusesa.
contribution-background-voices-description-extended = Nyi kukwata cha kangana muchibwa nyi kuli hicheka mbilenu hamuwika nyi isoneko.
contribution-background-voices-example-1-title = Dinosaures anene atwama mu Trias. <strong>[tanga ku liji likhwo]
contribution-background-voices-example-1-explanation = Uneza ? <strong>[hadjika lia kulu nyi liji]</strong>.
contribution-volume-title = Kukololoka
contribution-volume-description = Kuli ku chedja cha mwenemwene cha kukololoka haka chi ka kwakutanga. Kumbila cha kukololoka cha helu kukwata mu chibwa ngwe hashi chize ushikwiva ize anambe kukanla sona ja isoneko.
contribution-reader-effects-title = Kuthungumuka kulingiwa cha maliji
contribution-reader-effects-description = Mu kachi ka kukwata maliji jia athu anahadjika nyi liji lio lia chisemwa. Munuhasa Kutaiza shimbu likwo kukwata chakeshi kukololoka kulinyinguna cha kashishi cha utame ku hadjika nyi liji lia maheho. Munatamba kumbila kukwata kwimba mawa waze anatongila liji lia kuemesa ku mashinyi.
contribution-just-unsure-title = Una kakajana ?
contribution-just-unsure-description = Nyi wa wana mulonga kulisela cho chahashile kafuka munu tonda kulinga mutambi ubema kuli yema. Nyi wahashile chochene kulinga cha lufuma lingila balika kukwata chikwo.
see-more = <chevron></chevron>Solola ishimo ikwo
see-less = <chevron></chevron> Solola ishimo ikepe
