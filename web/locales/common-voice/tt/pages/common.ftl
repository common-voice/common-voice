# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Афар
aat = Арванитика
ab = Абхаз
abb = Банкон
abq = Абазин
ace = Ачех
ady = Адыгей
af = Африкаанс
aii = Сурет
ajg = Аҗа
aln = Гег Албан
am = Амхар
an = Арагон
apc = Гарәп (Шәм диалекты)
ar = Гарәп
arn = Мадупунгун
as = Ассам
ast = Астур
az = Азәри
azz = Таулы Пуэбла Наһуатл
ba = Башкорт
bag = Туки
bal = Бәлүҗ
bas = Басаа
bax = Бамун
bba = Баатонум
bbj = Гомала
bbl = Таш
bce = Меңгамбо
bci = Бауле
be = Беларус
beb = Бамвеле
bew = Бетави
bfd = Бафут
bfq = Бадага
bft = Балти
bg = Болгар
bgp = Көнчыгыш бәлүҗ
bin = Эдо
bkh = Бакоко
bkm = Ком
blk = Пао
bm = Бамбара
bn = Бенгал
bnm = Батанга
bnn = Бунун
bo = Тибет
br = Бретон
brh = Брауи
bri = Мокпве
brx = Бодо
bs = Босния
bsh = Катевири
bsk = Бурушаски
bsy = Сабаһ Бисайя
btv = Батери
bum = Булу
bxk = Букусу
bxr = Бурят
byv = Медумба
ca = Каталан
cak = Какчикел
cdo = Көнчыгыш Мин
ce = Чечен
cgg = Чига
cjk = Чокве
ckb = Сорани көрд
cnh = Хакһа-чин
co = Корсикан
cpx = Хэн Хуа
cpy = Ашенинка Көньяк Укаяли
crh = Кырымтатар
cs = Чех
cut = Теутила Куикатек
cux = Тепеуксила Куикатек
cv = Чуваш
cy = Уэльс
da = Дан
dag = Дагбани
dar = Даргин
dav = Кидавида
de = Алман
din = Динка
dmk = Давуди
dml = Дамели
dru = Рукаи
dsb = Түбән сорб
dua = Дуала
dv = Мальдив
dyu = Дьюла
dz = Дзоңха
ebr = Эбри
ee = Эве
efk = Эфик
eko = Экоти
el = Грек
en = Инглиз
eo = Эсперанто
es = Испан
esu = Үзәк Алясканың Юпики
et = Эстон
eto = Итон
eu = Баск
ewo = Эвондо
fa = Фарсы
fan = Фанг
ff = Фула
fi = Фин
fmp = Фефе
fo = Фарер
fr = Француз
fub = Аданава фулфулд
fue = Боргу фулфулд
fuf = Пулар (Гвинея)
fy-NL = Фриз
ga-IE = Ирланд
gaa = Га
gcf = Мартиниканың креол
gej = Мина
ggg = Гургула
gid = Гуидар
gig = Гоариа
giz = Гуизига
gjk = Качхи
gju = Гуҗари
gl = Галисия
glk = Гиляки
gn = Гуарани
gom = Конкани (Гоа)
gos = Гронинген
gsw = Эльзас
gu-IN = Гуджарати
guc = Ваюунаики
gv = Мэнс
gwc = Гаври
gwt = Гаварбайти
gya = Төньяк-Көнбатыш Гбая
ha = Хауса
hac = Горани
hak = Хакка
haz = Хазарги
hch = Вихарика
he = Яһүд
hem = Кихемба
hi = Һинди
hil = Хилигайнон
hno = Төньяк Һиндко
hr = Хорват
hrx = Хунсрик
hsb = Югары сорб
ht = Гаити креол
hu = Маҗар
hux = Нюподе уитото
hy-AM = Әрмән
ia = Интерлингва
iba = Ибан
ibb = Ибибио
id = Индонез
ie = Интерлингве
ig = Игбо
ipk = Инупиак
is = Исланд
it = Итальян
izh = Ижор
ja = Япон
jam = Ямайка Патуасы
jbo = Ложбан
jgo = Нгомба
jqr = Җакару
jv = Ява
ka = Грузин
kaa = Каракалпак
kab = Кабил
kam = Кикамба
kbd = Кабардин
kcn = Нуби
kdh = Котоколи
khw = Ховар
ki = Кикуйу
kjh = Хакас
kk = Казакъ
kl = Калаалисут
kln = Каленин
kls = Калаша
km = Кхмер
kmr = Курманҗи көрд
kn = Каннада
knn = Конкани (Деванагари)
ko = Корей
koo = Конзо
kpv = Коми-зырян
krc = Карачай-балкар
ks = Кашмири
ksf = Бафия
kvx = Паркари Коли
kw = Корн
kxp = Вадияра-коли
ky = Кыргыз
kzi = Келабит
laj = Ланго
lb = Люксембург
led = Ленду
leu = Кара
lg = Луганда
lij = Лигур
lke = Кеньи
lld = Ладин
ln = Лингала
lo = Лао
lrk = Лоарки
lrl = Ачоми
lss = Ласси
lt = Литва
ltg = Латгаль
lth = Тур
lua = Тшилуба
luo = Долуо
lv = Латыш
lzz = Лаз
mai = Майтһили
mau = Уаутла Масатек
mbf = Баба Малай
mbo = Мбо
mcf = Матсес
mcn = Масса
mcx = Мпиемо
mdd = Мбум
mdf = Мукшы
meh = Миштеко Юкуһити
mel = Меленау
mfe = Маврикий Креолы
mg = Малагаси
mgg = Кунабембе
mhk = Мунгака
mhr = Болын мари
mk = Македон
mki = Дһатки
ml = Малаялам
mmc = Мичоакан масауа
mn = Монгол
mni = Манипури
mnw = Мон
mos = Мосси
mqh = Тлазоялтепек Миштек
mr = Маратһи
mrh = Мара
mrj = Таулы мари
ms = Малай
mse = Моуссей
msi = Сабаһ Малай
mt = Мальта
mua = Мунданг
mug = Мусгум
mui = Муси
mve = Пакистани Марвари
mvy = Индус Коһистани
mxu = Мада
my = Бирма
myv = Эрзя
mzn = Мазендерани
nan-tw = Тайван (Миннан)
nb-NO = Норвег (Букмол)
ncx = Үзәк Пуэбла Наһуатл
nd = Ндебеле (Төньяк)
ne-NP = Непал
new = Невари
nhe = Көнчыгыш Һуастека Наһуатл
nhi = Көнбатыш Сьерра Пуэбла Наһуатл
nia = Ниас
nl = Һолланд
nla = Нгомбале
nlv = Орисаба наһуатл
nmg = Квасио
nmz = Лоссо
nn-NO = Яңа норвег
nnh = Нгембон
nqo = Нко
nr = Ндебеле (Көньяк)
nso = Төньяк сото
ny = Чиньянҗа
nyn = Ньянколе
nyu = Нюнгве
oc = Окситан
odk = Оадки
om = Оромо
or = Одия
oru = Ормури
os = Осетин
pa-IN = Пунжаб
pap = Папьяменто
pap-AW = Папиаменто (Аруба)
pau = Палау
pcd = Пикард
pcm = Нигерия Инглиз Пиджины
pez = Көнчыгыш пенан
phl = Палула
phr = Пахари-потвари
pl = Поляк
plk = Кохистани шина
pne = Көнбатыш пенан
prq = Ашенинка перене
ps = Пушту
pt = Португал
pua = Көнбатыш Таулы Пурепеча
pwn = Пайвань
quc = Киче
qug = Таулы Чимборасоның Кичуасы
qup = Көньяк Пастаза Кечуасы
qur = Кечуа янауанка
qus = Кечуа Сантьяго-дель-Эстеро
qux = Кечуа яуйоc
quy = Кечуа Чанка
qva = Кечуа Амбо-Паско
qvi = Кичуа
qvj = Таулы Лоханың кичуасы
qvl = Кечуа-Кажатамбо
qwa = Кечуа коронго анкаш
qws = Кечуа сиуаc анкаш
qxa = Кечуа-чикен
qxp = Пуно-кечуа
qxq = Кашкай
qxt = Кечуа Паско Санта-Ана-де-Туси
qxu = Кечуа Арекипа-Ла-Унион
qxw = Кечуа-хауджа-ванка
rif = Тарифит
rki = Рахайн
rm-sursilv = Романш сурсельв
rm-vallader = Романш Валладер
rn = Кирунди
ro = Румын
rof = Киромбо
ru = Рус
ruc = Руули
rup = Арумын
rw = Кинйаруанда
rwm = Амба
sah = Саха
sat = Сантали (Ол-чики)
sba = Нгамбай
sbn = Синдхи бхили
sc = Сардин
scl = Шина
scn = Сицилия
sco = Скотс
sd = Синдһи
sdh = Көньяк көрд
sdo = Сериан бидау
seh = Сена
sei = Сери
sg = Санго
shi = Шилһа
shn = Шан
si = Синһала
sk = Словак
skr = Сирайки
sl = Словен
sn = Шона
snk = Сонинке
snv = Сабан
so = Сомали
sq = Албан
sr = Серб
ss = Свати
ssi = Санси
st = Сесото
su = Сунда
sv-SE = Швед
sva = Сван
sw = Суахили
swb = Шимаоре
syr = Сүрия
szy = Сакизая
ta = Тамил
tar = Үзәк Тараһумара
tay = Атаял
te = Телугу
teg = Теке
tg = Таҗик
th = Тай
ti = Тигринья
tig = Тигре
tiv = Тив
tk = Төрекмән
tl = Тагалог
tli = Тлингит
tn = Сетсвана
tob = Тоба-Кум
tok = Токи Пона
top = Папантла тотонак
tr = Төрек
trv = Сидик
trw = Торвали
ts = Тсонга
tt = Татар
ttj = Руторо
tui = Тупури
tum = Тумбука
tuq = Тедага
tvu = Тунен
tw = Тви
ty = Таити
tyv = Тыва
uby = Убых
udl = Улдеме
udm = Удмурт
ug = Уйгыр
uk = Украин
ukv = Куку
ur = Урду
ush = Ушодзе
usp = Успантеко
uz = Үзбәк
var = Уариджио
ve = Венда
vec = Венеция
vi = Вьетнам
vls = Көнбатыш фламанд
vmw = Махува
vot = Вот
wbl = Вахи
wep = Вестфаль
wes = Камерун пиҗин
wo = Волоф
xdq = Кайтаг
xh = Коса
xhe = Кетрани
xka = Калкоти
xkl = Кения
xmf = Мингрел
xsm = Касем
yaq = Яки
yav = Нуасуе
ydg = Ядгха
yi = Идиш
yo = Йоруба
yue = Кантон
zgh = Тамазигхт
zh-CN = Кытай (Кытай)
zh-HK = Кытай (Гонконг)
zh-TW = Кытай (Тайвань)
zoc = Копайнала-Соке
zu = Зулу
zza = Заза

## Layout

profile = Профиль
help = Ярдәм
contact = Элемтә
privacy = Хосусыйлык
terms = Шартлар
cookies = Кукилар
faq = Сорау-Җавап
documentation = Документация
content-license-text = Эчтәлек <licenseLink>Creative Commons лицензиясе</licenseLink> шартлары белән нәшер ителә
share-title = Күбрәк сөйләм яздырылган саен яхшырак. Бу проект хакында дусларыгызга сөйләгез!
share-text = Машиналарга кешеләрнең ничек сөйләшүен өйрәтергә ярдәм итегез — { $link } сәхифәсендә тавышыгызны иганә итегез
link-copied = Сылтама копияләнде
back-top = Сәхифә башына
logout = Чыгу
donate = Иганә итү
stayintouch = Mozilla'да без сөйләм технологияләре тирәсендә бер җәмгыять туплау белән мәшгуль. Безнең хәбәрләр, яңа мәгълүмат чыганаклары, һәм Сезнең бу мәгълүматны ничек куллануыгыз хакында аралашып торасыбыз килә.
privacy-info = Без Сезнең мәгълүмат белән нәзәкатьле эш итәргә вәгъдә бирәбез. Моның турында күбрәк белер өчен безнең <privacyLink>Хосусыйлык аңлатмасын</privacyLink> укыгыз.
return-to-cv = Common Voice'ка кире кайту
email-input =
    .label = Эл. почта
loading = Йөкләнә...
email-opt-in-info = Куелган максатлар турындагы искәртүләр, ирешкән уңышларым турындагы хәбәрләр һәм Common Voice яңалыклары кебек электрон хатлар килеп торсын.
email-opt-in-info-title = Common Voice электрон хатларына язылу
email-opt-in-info-sub-with-challenge = Куелган максатлар турындагы искәртүләр, ирешкән уңышларым турындагы хәбәрләр һәм Common Voice яңалыклары кебек электрон хатлар килеп торсын.
email-opt-in-privacy-v2 = Эл. хатларга язылу аша сез Mozilla'ның әлеге мәгълүматны <privacyLink>Хосусыйлык Сәясәтендә</privacyLink> аңлатылганча эшкәртүе белән килешәсез.
indicates-required = * Тиешле юлны күрсәтә
not-available-abbreviation = Бирелмәгән
# Text shown on a button that can be clicked to copy text to the clipboard
copy = Күчереп алу
# Text shown on a button that can be clicked to close a modal or panel
close = Ябу
# Text shown on a button that can be clicked to cancel an action
cancel = Баш тарту
# Text shown on a button that can be clicked to delete an item
delete = Бетерү

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>Безгә</mark><br/><mark>ярдәмегез кирәк!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = 100-дән артык тел активистлары берләшмәләре файдалана алырлык мәгълүматлар җыелмаларын урнаштыру һәм платформаны яхшырту елына миллион доллар чамасы акча сарыф итә. <strong>Ачык, инклюзив мәгълүматларны мөһим саныйсыз икән — акчалата иганәгез һәрвакытта да кирәкле һәм әһәмиятле булачак!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = Common Voice-ны <mark>тикшеренүләрегездә</mark> <mark>кулланасызмы?</mark>
languages-donate-banner-cta-explanation = Common Voice акчалата иганәләр һәм грантлар хисабына эшли! Безгә галимнәр, гражданлык җәмгыяте һәм аерым өлкәләрдә тикшеренүчеләр белән хезмәттәшлек итү ошый. Common Voice проектын куллану ирекле һәм бушлай. Ләкин платформа һәм хостинг өчен акчалата иганә итү нык булыша ала.
localization-select =
    .label = Телне / локализацияне сайлагыз

## MDC Announcement

# Main text for MDC announcement. Text wrapped in <strong></strong> will be rendered with bold font
announcement-mdc-text = <strong>Common Voice проектының яңа мәгълүмат җыелмаларын</strong> хәзер тик Mozilla Data Collective исемле тугандаш платформабыздан гына иңдереп була. Гади кешеләр тарафыннан һәм гади кешеләр өчен төзелгән 300-дән артык глобаль мәгъдүмат җыелмаларыннан файдалана башлау өчен, ул проектыбызга да кушылыгыз.
# Button text for MDC announcement
announcement-mdc-button-text = Mozilla Data Collective проектына кушылу
# Aria text for button which opens MDC on a new page
announcement-mdc-button-aria-text = Яңа табта ачыла

## Release Delay Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release-delay = Хөрмәтле җәмгыять, без әле релиз ясау белән мәшгуль. Без релизның киләсе берничә көн эчендә, өлешләп-өлешләп, <mdcLink>Mozilla Data Collective</mdcLink> платформасында тәмамланыр дип өметләнәбез.

## Release Announcement

# Text wrapped in <mdcLink></mdcLink> will be rendered as an external link to Mozilla Data Collective
announcement-release = Хөрмәтле җәмгыять, яңа чыгарылыш хәзер тулысынча <mdcLink>Mozilla Data Collective</mdcLink> эчендә урнаштырылган. Кушылыгыз һәм җәмгыять тарафыннан һәм җәмгыять өчен булдырылган 500-дән артык глобаль мәгълүматлар җыелмасын куллана башлагыз.

## Pre-Release Announcement (dates TBD)

# Text wrapped in <cvDatasheetsLink></cvDatasheetsLink> will be rendered as an external link to the cv-datasheets GitHub repo
announcement-pre-release = Хәрмәтле җәмгыять, чыгарылыш вакыты якынлаша. Зинһар, аудиоларны раслагыз һәм транскрипцияләгез. Һәм, шулай ук, үзегезнең datasheet җәдвәлен яңарту өчен <cvDatasheetsLink>cv-җәдвәлләрне</cvDatasheetsLink> дә карагыз.

## Common Language/Dataset Selector & SearchBox Related

# Generic search box defaults
searchbox-default-label = Эзләү өчен яза башлагыз
searchbox-default-placeholder = Эзләү…
searchbox-default-aria-label = Эзләү
dropdown-no-results = Тәңгәл килүләр табылмады
# Common Voice Dataset=Language search box
dataset-searchbox-label = Күренеш телен дә, мәгълүмат җыелмаларының телен дә үзгәртә
dataset-searchbox-placeholder = Телне эзләү…
dataset-searchbox-aria-label = Телне эзләү
dataset-search-no-results = Туры килгән тел табылмады
request-new-language = Яңа тел өстәүне сора
