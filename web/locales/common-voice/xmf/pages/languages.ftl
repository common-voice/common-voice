## Languages Overview

language-section-in-progress = პროგრესი
language-section-launched = გოტებულიე
language-section-launched-description = მოჩამილ ნინეფშა ვებსაიტიქ წუმოძინელო<localizationGlossaryLink>გეგნითანგუ</localizationGlossaryLink> დო დასაბაღი უღუ<sentenceCollectionGlossaryLink>ნაშაყარა ზიტყვასქვილეფი</sentenceCollectionGlossaryLink> შიორებერქ იჸუას ნაშაყარა ხონარეფიშ <speakLink>ჩიება</speakLink>დო<listenLink>მორჩქილაში</listenLink> გაგძელებაქ.
# lastUpdatedTimeStamp is a timestamp that indicates when the language stats was last updated
language-section-last-updated = ბოლო გაახალება{ $lastUpdatedTimeStamp }
languages-show-more = ქოძირი უმოსი
languages-show-less = ქოძირი ნაკლები
language-meter-in-progress = პროგრესი
language-total-progress = გვალო
language-search-input =
    .placeholder = გორუა
language-speakers = მაჩიებე
localized = ლოკალიზებური
sentences = ზიტყვასქვილეფი
language-validation-hours = საათეფი
language-validation-progress = წიმე ულა დადასურებას
contribute = წილიშ მიშაღალა
get-involved-button = ქუმაკათით
get-involved-title = წილი გემშეღით { $lang }
get-involved-email =
    .label = ელფოსტა
get-involved-opt-in = ქო, ქუმომჯღონით ნაჭარეფი ელფოსტაშა. მოკო, ირიათო მიჩქუდას თე ნინაში პროგრესიშ გურშენი Common Voice-ის.
get-involved-submit = ჯღონა
get-involved-stayintouch = ჩქი, Mozilla-ს, ვოცადუთ აკოფშაყარუათ დიდი ართობა ხონარუან ტექნოლოგიეფიშ მუკი-მუკი. მოკონა, ქუდოფსკიდათ კავშირს გაახალებული ამბეფით, მუნაჩემეფიშ ახალ წყუეფით დო ქიმიფრჩქილუათ, მუჭო გიმმირინუანთ თე მუნაჩემეფს.
get-involved-success-title = თქვა წუმოძინელო ქაკათით { $language } ნინაშო წილიშ მიშაღალაშენი. მარდი.
get-involved-success-text = ჩქი ქიდგეკავშირებუთ უმოსი ინფორმაციაშ მიოღებელო,ხეშმიოჭირინაფალი მუჭოთ ქიჸუაფუნი.
get-involved-return-to-languages = ნინეფშა დორთინა
