# PARTNERSHIPS PAGE
partnerships-header = Partnerstva
partnerships-header-text = Za ostvarenje projekta Common Voice potrebno je mnogo toga! Mi to ne radimo sami. Želiš li s nama surađivati? Bit će nam drago ako nam se javiš.
partnerships-get-in-touch = Kontaktiraj nas
partnerships-become-a-partner = Postani Common Voice partner
partnerships-community-header = Zajednica, kreativci i civilno društvo
partnerships-community-description = Zajednica je srce Common Voicea – osobe dolaze iz cijelog svijeta, jezični su aktivisti, znanstvenici i umjetnici. Saznaj više o tome kako se pridružiti zajednici na našim stranicama <about>Informacije</about> i <community>Priručnik zajednice</community>. <emailFragment>Kontaktiraj nas</emailFragment> ako želiš dublje organizacijsko partnerstvo,
partnerships-foundations-header = Zaklade
partnerships-governments-header = Vlade
partnerships-academia-header = Sveučilišta, akademičari i istraživači
partnerships-academia-description = Otvorena inovacija je osnova onoga što radimo. <emailFragment>Kontaktiraj nas</emailFragment> za istraživačku suradnju, sakupljanje podataka ili intervjua.
partnerships-small-business-header = Male tvrtke, uključujući nova poduzeća u području tehnologije i umjetne inteligencije
partnerships-small-business-description = Uvijek nas zanimaju informacije od korisnika podataka o njihovim potrebama i kontekstu – od suradnje na platformi za sakupljanje podataka do pružanja uvida u stanje podataka, želimo <emailFragment>čuti tvoje mišljenje.</emailFragment>
partnerships-corporates-header = Velike korporacije i platformske tvrtke
partnerships-corporates-description = Ako želiš razgovarati o skupu podataka, investirati u planirana proširenja platforme ili ako želiš surađivati, <emailFragment>kontaktiraj nas.</emailFragment>
partnerships-our-partners = Naši partneri
# menu item
partners = Partneri
