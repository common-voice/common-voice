## Error pages

banner-error-slow-1 = Ҳаҭамыз, Common Voice имыццакӡакәа аус ауеит. Иҭабуп аинтерес ахьшәымоу азы.
banner-error-slow-2 = Ҳара алшареидара ду ҳамоуп, уажәазы ​​ари апроблема аҭҵаара ҳаҿуп.
banner-error-slow-link = Аҭагылазаашьа адаҟьа
error-something-went-wrong = Акы ишаҭахыз акәымкәа иҟалеит
error-clip-upload = Ари аклип аҭагалара амуит. Даҽазнык шәҽазышәшәома?
error-clip-upload-server = Ари аклип асервер ахь аҭыгара аҽазышәара знызаҵәыкгьы иамуит. Адаҟьа еиҭаҿашәк, мамзаргьы ԥыҭрак ашьҭахь даҽазнык шәҽазышәшәа.
error-clip-upload-too-large = Шәҭаҩымҭа афаил аҭыгаразы даара идуцәоуп. Ҳшәыҳәоит, еиҳа икьаҿу аклип аҭаҩра шәҽазышәшәа.
error-clip-upload-server-error = Асервер аҿы агха ҟалеит шәыклип аус адулараан. Ҳшәыҳәоит, адаҟьа шәырҿыц ма ԥыҭрак ашьҭахь еиҭа шәҽазышәшәа.
error-title-404 = Ари адаҟьа ҳзымԥшааит
error-content-404 = Иҟалап <homepageLink>ҳаҩнытә даҟьа</homepageLink> ацхыраара ҟанаҵар? Азҵаара аҭаразы, шәалахәха <matrixLink>аилазаара ачат Matrix</matrixLink> аҿы, <githubLink>GitHub</githubLink> ала асаит апроблемақәа шәрыцклаԥшла, мамзаргьы <discourseLink>ҳ-Discourse  форумқәа</discourseLink> шәаҭаа.
error-title-500 = Ҳаи, акы ишаҭахыз акәымкәа иҟалеит
error-content-500 = Ҳазқәымгәыӷуаз агха ҟалеит. Ԥыҭраамҭак шәыԥшы, нас еиҭа шәҽазышәшәа. Ҳшәыҳәоит, ацхыраара аиуразы <matrixLink>Matrix аилазаара ачат</matrixLink> шәалал, ауадаҩрақәа асаит аҿы шәрышьклаԥшла <githubLink>GitHub</githubLink>ала, мамзаргьы <discourseLink>ҳ-Discourse-форумқәа шәырҭаа</discourseLink>.
error-title-502 = Аимадара еиԥырҟьоуп
error-content-502 = Уажәы иҭышәынтәаланы ҳсерверқәа  рыҽрымадара шәзышьақәыргылом. Ԥыҭраамҭак шәааԥшы, нас даҽазнык шәҽазышәшәа. Ацхыраара аиуразы, ҳшәыҳәоит, Matrix</matrixLink> аҿы аилазаара ачат<matrixLink>шәалал, <githubLink>GitHub</githubLink> ала асаит аҿы апроблемақәа шәрышьклаԥшла, мамзаргьы <discourseLink> ҳ-Discourse-форумқәа</discourseLink>шәырҭаа.
error-title-503 = Ҳара ҳҿы узқәымгәыӷуа имариоу
error-content-503 = Асаит хара имгакәа аусура еиҭалагоит. Аҵыхәтәантәи аинформациа аиуразы, шәалал<matrixLink>аилазаара ачат Matrix</matrixLink> аҿы, шәаҭаа <githubLink>GitHub</githubLink> мамзаргьы <discourseLink>ҳ-Discoure  форумқәа</discourseLink> азҵаарақәа рзы адырра аҭаразы, насгьы урҭ рыцклаԥшразы.
error-title-504 = Аҳәара азыԥшра аамҭа нҵәеит
error-content-504 = Аҳәара мыцхәы аамҭа агеит. Абжьааԥны ари аамҭалоуп. Даҽазнык шәҽазышәшәа. Ҳшәыҳәоит, ацхыраара аиуразы  Matrix аҿы</matrixLink> аилазаара <matrixLink> ачат шәалал, асаит аҿы апроблемақәа <githubLink>GitHub</githubLink> ала шәрышьклаԥшла, мамзаргьы <discourseLink>ҳ-Discourse-формуқәа</discourseLink> шәырҭаа.
error-code = Агха акод: { $code }
# Warning message shown when none of the clips could be uploaded
error-duplicate-clips-all =
    { $total ->
        [one] Шәыклип аҭагалара ҳалымшаӡеит. Уи уаанӡа иҭагалахьан. Шәааи анаҩстәи апачкала иацаҳҵап!
       *[other] Ҳара { $total } клипк рҭагалара ҳалымшаӡеит. Урҭ уаанӡа иҭагалахьан. Шәааи анаҩстәи апачкала иацаҳҵап!
    }
# Warning message shown when only some of the clips could be uploaded (uploaded count will be <5)
error-duplicate-clips-some = Ҳара шәыклипқәа { $uploaded } ҭаҳгеит — егьырҭ иҭагалоуп. Шәааи анаҩстәи апачкала иацаҳҵап!
