## Request Language Pages

request-language-heading = bɔhimi bal' palli yɛla
request-language-form-email =
    .label = A iimaili Taliŋ adaka
request-language-form-info =
    .label = Lahibali zaŋ jɛndi balli
request-language-form-info-explanation = Pahimi lahabali shɛli kam a ni mi zaŋ kpa balli maa polo. Dini n-nyɛ:
request-language-form-info-explanation-list-1 = A balli yuya
request-language-form-info-explanation-list-2 = <isoCodeLink>ISO Codes</isoCodeLink> di yi baŋ
request-language-form-info-explanation-list-3 = Wɛbsatinim linkinima din ni tooi chaŋ ka ti baŋ balli maa
request-language-google-recaptcha-required = reCAPTCHA nyɛla di ni bɔri shɛli di yi niŋ ka a bɔri ni a tirisi tooni
request-language-google-recaptcha-error = reCAPTCHA nyɛla din mali muɣisiɣu. Labi niŋ.
# Success page
request-language-success-heading = Ti paɣiya,a zaŋ bɔhisi zaŋ jɛndi balli palli ti na.
request-language-success-list-1 = Di tuya ni a deegi iimali din yina ti dundɔŋ ŋɔ manija sani n-wuhiri so' shɛŋa di ni tu ni a doli zaŋ a balli pahi Common Voice
request-language-success-list-3 = A yi bi wum ti lahabali dabisabihi sunsuuni,bɔmi ti shee na<emailLink>{ $email }</emailLink>ti ni labina
request-for-scripted-speech-toggle = Suhimi lahabali din sabidɔya
need-help-deciding-platform = A bɔri sɔŋsim zaŋchaŋ a ni yɛn pii shɛli?
# text wrapped in <strong></strong> will be shown as bold
need-help-deciding-platform-explanation-1 = <strong>lahabali din sabidɔya</strong>(ti ni mi shɛli common voice) n nyɛ luɣishɛli a ni tooi yihi karim yɛltɔɣa a balli puuni
need-help-deciding-platform-explanation-3 = A yi bi nya a balli dundɔna ayi maa ni, nyin piimi di zaa
