## General

yes-receive-emails = Da, šaljite mi emailove. Želim biti informiran o projektu Common Voice.
stayintouch = Mi u Mozilli gradimo zajednicu oko govorne tehnologije. Želimo ostati u kontaktu s ažuriranjima, novim izvorima podataka i saznati više o tome kako upotrebljavate te podatke.
privacy-info = Obećajemo da ćemo pažljivo postupati s vašim informacijama. Pročitajte više u <privacyLink> obavijesti o privatnosti </privacyLink>.
return-to-cv = Povratak na Common Voice
email-input =
    .label = Email
submit-form-action = Pošalji
loading = Učitavanje…
indicates-required = * obilježava obvezno polje

# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

af = Afrikanski
an = Aragonski
ar = Arapski
as = Asamski
ast = Asturijski
az = Azerbejdžanski
bg = Bugarski
bn = Bengalski
br = Bretonski
ca = Katalonski
cs = Češki
cv = Čuvaški
cy = Velški
da = Danski
de = Njemački
dv = Divehijski
el = Grčki
en = Engleski
eo = Esperanto
es = Španjolski
et = Estonski
eu = Baskijski
fa = Perzijski
fi = Finski
fo = Ferojski
fr = Francuski
fy-NL = Frizijski
ga-IE = Irski
he = Hebrejski
hu = Mađarski
ia = Interlingua
id = Indonezijski
is = Islandski
it = Talijanski
ja = Japanski
ka = Gruzijski
kab = Kabilijski
ko = Korejski
kw = Kornski
mk = Makedonski
mn = Mongolski
ne-NP = Nepalski
nl = Nizozemski
nn-NO = Novonorveški
oc = Okcitanski
pl = Poljski
pt-BR = Portugalski (Brazil)
ro = Rumunjski
ru = Ruski
sc = Sardinijski
sk = Slovački
sl = Slovenski
sq = Albanski
sr = Srpski
sv-SE = Švedski
ta = Tamilski
te = Telugu
th = Tajlandski
tr = Turski
tt = Tatarski
uk = Ukrajinski
ur = Urdski
uz = Uzbečki
vi = Vijetnamski
zh-CN = Kineski (Kina)
zh-HK = Kineski (Hong Kong)
zh-TW = Kineski (Tajvan)

# [/]


## Layout

speak-now = Pričajte sada
datasets = Skupovi podataka
languages = Jezici
profile = Profil
help = Pomoć
contact = Kontakt
privacy = Privatnost
terms = Uvjeti
cookies = Kolačići
faq = Često postavljana pitanja
content-license-text = Sadržaj dostupan pod licencom <licenseLink> Creative Commons </licenseLink>
share-title = Pomozite nam pronaći donatore govora.
share-text = Pomozite naučiti strojeve kako ljudi govore, donirajte svoj govor na { $link }
link-copied = Poveznica je kopirana
back-top = Povratak na vrh
logout = Odjava

## Home Page

home-title = Common Voice je Mozilla-ina inicijativa da pomogne naučiti strojeve kako ljudi govore.
home-cta = Govori, doprinesi ovdje!
wall-of-text-start =
    Govor je prirodan, govor je ljudski. Zato smo fascinirani stvaranjem tehnologije upotrebljivog govora
    za naše strojeve. Ali za stvaranje govornih sustava, potrebne su iznimno velike količine snimljenog govora.
show-wall-of-text = Pročitajte više
vote-yes = Da
vote-no = Ne
voices-online = Prisutnih govornika
todays-progress = Današnji napredak
all-locales = Sve
today = Danas

## Account Benefits


## What's public


## Speak & Listen Shortcuts


## Listen Shortcuts


## Speak Shortcuts


## ProjectStatus


## ProfileForm

profile-form-username =
    .label = Korisničko ime
profile-form-submit-save = Spremi
profile-form-submit-saved = Spremljeno
male = Muški
dashboard = Nadzorna ploča
avatar = Avatar
goals = Ciljevi
settings = Postavke
edit-profile = Uredi profil
profile-close = Zatvori

## FAQ


## Glossary


## NotFound


## Data

data-download-yes = Da
data-download-deny = Ne
data-download-license = Licenca: <licenseLink>CC-0</licenseLink>

## New Datasets Page

language = Jezik
# File size in gigabytes
size = Veličina
cv-license = Licenca

## Download Modal

download-no = Ne hvala

## Contact Modal

contact-form-message =
    .label = Poruka
contact-required = *obavezno

## Request Language Modal

request-language-form-language =
    .label = Jezik

## Languages Overview

languages-show-more = Pogledajte više
language-meter-in-progress = Napredak
language-total-progress = Ukupno
language-search-input =
    .placeholder = Traži

## New Contribution

record-three-more-instruction = Još tri!

## Goals


## Dashboard


## Profile Delete

keep = Zadrži
remove = Ukloni
