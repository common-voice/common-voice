## Report Modal

report-modal-title = Cyflwyno adroddiad
report-modal-subtitle = Pa anawsterau ydych chi'n eu profi gyda'r frawddeg hon?
offensive_speech = Cynnwys Tramgwyddus
offensive_speech-detail = Mae'r clip yn cynnwys iaith amharchus neu dramgwyddus.
empty_or_unintelligible = Gwag neu Annealladwy
empty_or_unintelligible-detail = Mae'r cynnwys yn wag, wedi'i lygru, yn cynnwys iaith wahanol, ac ati.
personally_identifiable_information = Gwybodaeth sy'n bersonol adnabyddadwy
personally_identifiable_information-detail = Mae'r cynnwys hwn yn cynnwys gwybodaeth bersonol adnabyddadwy sy'n ymwneud â hunaniaeth unigolyn, megis ei enw, cyfeiriad e-bost, lleoliad, rhif adnabod, neu rif ffôn.
personally_identifiable_information-answer-detail = Mae'r cynnwys hwn yn cynnwys manylion personol adnabyddadwy sy'n ymwneud â hunaniaeth unigolyn, fel ei enw, cyfeiriad e-bost, lleoliad, rhif adnabod, neu rif ffôn.
report-other-comment =
    .placeholder = Sylw
# Label for textarea in report modal
other = Arall
report-error = Digwyddodd gwall wrth gyflwyno'r adroddiad
# Label for Continue button in Report modal to indicate that the user wants to continue contributing after reporting
continue = Parhau
# Header for report modal after successful reporting
success = Llwyddiant
report-success-message = Pasiwyd yr adroddiad yn llwyddiannus
