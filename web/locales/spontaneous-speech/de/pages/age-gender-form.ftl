# Age and Gender form
age-gender-form-title = Vielen Dank, dass Sie zu spontaner Sprache beitragen!
age-gender-form-subtile = Möchten Sie Informationen darüber weitergeben, wie Sie sprechen?
add-information-button = Informationen hinzufügen
continue-speaking-button = Weiter sprechen
age-select-label = Wie alt sind Sie?
select-your-age = Wählen Sie Ihr Alter aus
gender-select-label = Was ist Ihr Geschlecht oder Ihre Geschlechteridentität?
select-your-gender = Wählen Sie Ihr Geschlecht oder ihre Geschlechteridentität aus
country-of-residence-label = Land/Region des Wohnsitzes
country-of-residence-placeholder =
    .placeholder = Geben Sie das Land/die Region Ihres Wohnsitzes ein
why-are-you-asking = Warum fragen Sie?
why-are-you-asking-explanation = Diese Informationen sind optional – Sie sollten sie nur hinzufügen, wenn Sie sich wohl fühlen. Wenn wir den Datensatz veröffentlichen, pseudonymisieren wir alle personenbezogenen Daten und stellen sicher, dass Informationen wie Ihr Name und Ihre E-Mail-Adresse nicht mit demografischen Informationen wie Alter oder Geschlecht verbunden sind. Damit Technologie für alle gut funktioniert, müssen die Daten, die Entwicklern bei der Entwicklung von Technologien helfen, eine Bandbreite von Menschen widerspiegeln. Erfahren Sie mehr darüber, warum Metadaten wichtig sind.
thank-you-for-your-information = Vielen Dank für Ihre Information

## Gender dropdown

male_masculine = Männlich
female_feminine = Weiblich
intersex = Intersexuell
transgender = Transgender
non-binary = Nicht-binär
do_not_wish_to_say = Möchte ich nicht angeben
