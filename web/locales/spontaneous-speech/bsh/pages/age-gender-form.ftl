# Age and Gender form
age-gender-form-title = اسپونٹینیئس تقریر تہ حصہ اوݣستہ کہ شکریہ
age-gender-form-subtile = تو بارا تہ ایمو ولہتے، جامع ٹکنالوجے ستہ تعمیر و ترقے تہ مدد کولاش
add-information-button = معلومات سنا
continue-speaking-button = ولّلہ ستہ جاری سوتہ
age-select-label = تو چسہ بیسہ؟
select-your-age = توستہ سہ منتخب کشی
gender-select-label = توستہ جنس کائ اسہ؟
select-your-gender = توستہ جنس منتخب کشی
country-of-residence-label = وطن/علاقہ وپسیں جائ
country-of-residence-placeholder =
    .placeholder = داخل کشی توستہ وطن/علاقہ تہ وپسیں جائ
why-are-you-asking = تو کا کوداچی؟
thank-you-for-your-information = توستہ معلومات دیو کہ شکریہ

## Gender dropdown

male_masculine = منچی/نہ
female_feminine = جوکوعں/شتری
intersex = نہ جوکوعں نہ منچی
transgender = نہ جوکوعں نہ منچی
non-binary = غیر ثنائی نظام
do_not_wish_to_say = گجی کوستہ نہ وگچی
