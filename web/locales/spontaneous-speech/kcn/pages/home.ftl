## Home page

welcome-to-spontaneous-speech = Fadhal fi wanasa ta sutfa
# <br></br> represents a newline break
wall-of-text = Munazama al ta sauiti al mushtarak gi fata mengeles ta tinein al gi husu limu kalama al ta kiliga, sutfa, maa makataba. Wede bi saadu ketir ta ummah fi limu malumaat milan gwam bumara, wu kaman kun muhimu na sabal malumaat ta ulum ma sabal amara
why-spontaneous-speech = Sababu sunu ina abinu tajriba ta wanasa ta sutfa
# text wrapped with <mark></mark> will have a light red background
why-card-1-text = Muhimu fi kalamaa ta kiliga ma istamil gediya mathalan kan ita aju kede arufu ita fi wanasa ta so kidima ma anas al gi wonusu wanasa al maa ma kataba wu al moyomoyo
# text wrapped with <mark></mark> will have a light red background
why-card-2-text = Fi seme ta shaja haki ta rutan ja targim rutanaa ma badulu beina ta rutan tinein aw ziada
# text wrapped with <mark></mark> will have a light red background
why-card-3-text = Fi seme ta shaja rutana al endis siya au maa makataba ta istimal mathalan lahja, al gi geeru geeru fi historiya ta rutan al maa ma kataba
how-spontaneous-speech-works = Teriga al wanasa ta sutfa gi so moo
# text wrapped in <mark></mark> will have a blue background
prompt-bubble-title = Matlaba
prompt-bubble-subtitle = Ina bi weri neita matlaba au sual
# text wrapped in <mark></mark> will have a blue background
respond-bubble-title = Juwab
respond-bubble-subtitle = Kede ita juwab au majibu fogo fi sikil tabiya al ita agder
# text wrapped in <mark></mark> will have a blue background
transcribe-bubble-title = Kutu kalam fi kataba
transcribe-bubble-subtitle = Katib falata kalam al ita gi asuma fi sauti al ma gata de
# text wrapped in <mark></mark> will have a blue background
check-bubble-title = Hakikisha
check-bubble-subtitle = Hakikisha kan wanasa al maktub de gi wafiki ma sauti
# text wrapped in <mark></mark> will have a blue background
data-bubble-title = Malumaat
data-bubble-subtitle = Maramara ina gi tala koum ta malumaat de
contact-us = Wonusu ma ina
contact-us-title = Kun ma haam fi sharaka fi mashru wede, au limu ida ma ina
contact-us-email = Anwan ta email
signup-form-title = Sei ita jahiz kun ma ina fi majmu taina al afdhal fi aburu ta mashru ta kalam al sutfa?
signup-terms = Ma nakas, rasulu rabit fi email tayi, manato ita wafiki ma hukum ma shurut taina, ma siyasa ta bi nafsi taina. wu kaman Mozilla agder rasulu naita email al gi amsuku ma teriga ta shariki fi mashruu de
signup-input-placeholder = Dakalu email taki
signup-button-value = Rasulu rabit fi email tayi
footer-text-privacy = Binafsi
footer-text-terms = Hukum
footer-text-cookies = Malaf ta taarif ta raabit
