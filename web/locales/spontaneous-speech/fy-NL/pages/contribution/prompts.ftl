## Prompts page

record-player-header = Jo antwurd opnimme
mic-access-error = Jo moatte tagong ta jo mikrofoan tastean.
# actionType will be Click (for desktop devices) or Tap (for mobile devices)
prompt-page-instruction = { $actionType } <micIcon></micIcon> en antwurdzje sa natuerlik mooglik
error-skipping = Der is in flater bard by it oerslaan fan dizze fraach
error-recording = Der is in flater bard wylst it opnimmen
error-fetching-prompts = Der is wat misgien by it opheljen fan fragen. Probearje it letter opnij.
error-uploading-response = Der is in flater bard by it opladen fan jo antwurd.
prompt-submitted-success = Antwurd mei sukses ferstjoerd
prompt-response-deleted-success = Antwurd mei sukses fuortsmiten
# For the button to delete a response to a prompt
delete = Fuortsmite
# For the disabled button on a prompt that indicates a prompt is already submitted
submitted = Yntsjinne
# For the button on the Respond to prompts page which can be used to submit all responses at once
submit-all-button = Alles yntsjinje

## Explainer text (Respond to Prompt)

learn-how-to-use-prompts = Dizze side leare te brûken
learn-how-to-use-prompts-explanation = Beäntwurdzje de fraach sa natuerlik mooglik yn in pear sinnen. Soargje derfoar dat jo taalgebrûk respektfol en net kwetsend is.
learn-how-to-use-prompts-guidance = Klik <guidanceLink>hjir</guidanceLink> foar ynformaasje oer it gebrûk fan dizze side
learn-how-to-use-transcribe = Leare transkribearje
learn-how-to-use-transcribe-explanation = Skriuw presys op wat de persoan sein hat, ynklusyf plakhâlderwurden as ‘um’ of ‘euhh’.
learn-how-to-use-transcribe-guidance = Klik <guidanceLink>hjir</guidanceLink> foar ynformaasje oer it gebrûk fan dizze side
learn-how-to-use-check-transcript = Lês hoe’t jo in transkripsje kontrolearje kinne
learn-how-to-use-check-transcript-explanation = In transkripsje moat elk wurd of net-leksikaal konversaasjelûd dat troch de persoan útsprutsen is fêstlizze, ynklusyf werhellingen.
learn-how-to-use-check-transcript-guidance = Klik <guidanceLink>hjir</guidanceLink> foar ynformaasje oer it gebrûk fan dizze side
