## Errors

cs-error-fetching-datasets = Der is wat misgien by it opheljen fan de list mei beskikbere gegevenssets. Probearje it letter opnij.
cs-error-fetching-questions = Der is wat misgien by it opheljen fan fragen. Probearje it letter opnij.
cs-error-fetching-transcriptions = Der is wat misgien by it opheljen fan transkripsjes. Probearje it letter opnij.

## Modals

cs-locale-change-modal-subtitle = Jo hawwe net-bewarre wizigingen op dizze side. As jo fan gegevensset wikselje, wurde jo wizigingen net bewarre. Binne jo wis dat jo wikselje wolle?

## Question Page Items

cs-question-page-instruction = Foegje maksimaal 25 fragen <icon></icon> ta oan de gegevensset foar { $currentLocale } ta dy’t oaren beäntwurdzje kinne
cs-question-dataset-name = Gegevensset: { $datasetName }

## Validate Page Items

cs-validate-page-instruction = Falidearje <validateIcon></validateIcon> dat dizze fraach oan de rjochtlinen foldocht.
cs-no-questions-continue-skipped = Der binne op dit stuit gjin fragen mear foar dizze gegevensset. As jo ien oerslein hawwe, kinne jo trochgean mei oersleine ynhâld of of audiotranskripsje útfiere en it letter opnij probearje.

## Tag Page Items

cs-tag-dataset-name = Gegevensset [{ $datasetName }]

## Guidelines

learn-how-to-use = Dizze side leare te brûken
read-full-guidelines-here = <guidanceLink>Lês de folsleine rjochtlinen hjir</guidanceLink>

## Answer Page Guidelines

cs-learn-how-to-use-answer-explanation = Beäntwurdzje de fraach sa natuerlik mooglik, fiel jo frij om te wikseljen tusken talen as it natuerlik fielt. Soargje derfoar dat jo taalgebrûk respektfol en net kwetsend is.

## Review Page Guidelines

cs-learn-how-to-use-check-transcript-explanation = In transkripsje moat elk wurd of net-leksikaal konversaasjelûd dat troch de persoan útsprutsen is fêstlizze, ynklusyf werhellingen.
