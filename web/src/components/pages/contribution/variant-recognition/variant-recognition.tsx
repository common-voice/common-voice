...
class ContributionPage extends React.Component<ContributionPageProps, State> {
  ...
  private handleKeyDown = (event: any) => {
    const { getString, isSubmitted, locale, onReset, onSubmit } = this.props
  ...
  render() {
    ...
    return (
      <div className="contribution-wrapper" data-testid="contribution-page">
        ...
        <div className={['contribution'].join(' ')}>
          <div className="top">
  ...
  renderContent() {
    ...
    if (isSubmitted && noUserAccount) {
      return (
        <Success onReset={onReset} type={'variant'} userVariant={userVariant} />
      )
    }
          ...
          <div>
            <Localized id="next-button">
              <PrimaryButton
                className={[
                  'next',
                  getTrackClass('fs', `submit-${'variant'}`),
                ].join(' ')}
                disabled={!isClicked}
                onClick={onNext}
                type="submit"
