...
describe('Review page', () => {
  ...
  it('renders sentences for review', () => {
    const spy = jest.spyOn(storeHooks, 'useSentences').mockImplementation(() => ({
      'mock-locale-1': {
        sentences: [],
  ...
  it('skips a sentence', async () => {
    ...
    await waitFor(async () => {
      expect(useActionMock).toHaveBeenCalledWith(mockPendingSentences[0].sentenceId);
    });
  ...
  it('redirects to login page for unauthenticated users', () => {
    const spy = jest.spyOn(storeHooks, 'useAccount').mockImplementation(() => null);
