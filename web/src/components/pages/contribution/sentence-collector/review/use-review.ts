...
import { useAction, useLanguages, useSentences } from '../../../../../hooks/store-hooks';
...
const useReview = ({ getString, showReportModal }: UseReviewParams) => {
  ...
  const fetchPendingSentences = useAction(Sentences.actions.refillPendingSentences);

  const handleFetch = () => {
    try {
      fetchPendingSentences(localeId);
    } catch (error) {
      dispatch(Notifications.actions.addPill(getString('sentences-fetch-error'), 'error'));
      console.error(error);
    }
  };
  ...
  const handleError = ({
    ...
    if (error.toString().includes(TOO_MANY_REQUESTS_ERROR)) {
      dispatch(
        Notifications.actions.addPill(getString('review-error-rate-limit-exceeded'), 'error')
      );
    } else {
  ...
  const handleVoteYes = async () => {
    const sentenceId = pendingSentencesSubmissions[activeSentenceIndex].sentenceId;
  ...
  const handleSkip = async () => {
    ...
    try {
      ...
      dispatch(Notifications.actions.addPill(getString('sc-review-form-button-skip'), 'success'));
  ...
  const handleKeyDown = (evt: KeyboardEvent) => {
    if (evt.ctrlKey || evt.altKey || evt.shiftKey || evt.metaKey || showReportModal) {
      return;
    }

    const shortcut = reviewShortCuts.find(({ key }) => getString(key).toLowerCase() === evt.key);
