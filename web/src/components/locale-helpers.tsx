...
import { actions as LanguagesActions } from '../stores/languages'
...
export function useSpontaneousSpeechLocales() {
  ...
  const spontaneousSpeechLanguages = useTypedSelector(
    ({ languages }) => languages.spontaneousSpeechLanguages
  )

  // Fetch on first call if not already loaded
  useEffect(() => {
    if (spontaneousSpeechLanguages === undefined) {
      dispatch(LanguagesActions.loadSpontaneousSpeechLanguages())
    }
  }, [dispatch, spontaneousSpeechLanguages])

  return spontaneousSpeechLanguages
}
