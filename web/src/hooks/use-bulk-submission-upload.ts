...
const useBulkSubmissionUpload = () => {
  ...
  const bulkSubmissionRequest = useAction(Sentences.actions.bulkSubmissionRequest);

  const abortBulkSubmissionRequest = useAction(Sentences.actions.abortBulkSubmissionRequest);
  ...
  const handleError = () => {
    dispatch(Sentences.actions.setBulkUploadStatus('error'));
    dispatch(Notifications.actions.addPill(l10n.getString('bulk-upload-failed-toast'), 'error'));
  };
  ...
  const startUpload = async () => {
    try {
      ...
      if (response.ok) {
        ...
        dispatch(
          Notifications.actions.addPill(l10n.getString('bulk-upload-success-toast'), 'success')
        );
